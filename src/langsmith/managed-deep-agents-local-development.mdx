...
3. Creates a local [Context Hub](/langsmith/managed-deep-agents-context-hub) mock for instructions, skills, and memory.
...
- [Schedules](/langsmith/managed-deep-agents-schedules) do not run. `mda dev` lists each schedule at startup but does not provision it, so no scheduled run fires locally.
