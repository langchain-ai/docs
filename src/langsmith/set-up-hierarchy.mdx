...
{/*
  DOC-1597. The two parentheticals below are verbatim from the model
  descriptions on workload-isolation.mdx, which owns the taxonomy: 'This model
  (single workspace per team)' and 'In this model (multiple teams per
  workspace)'. Change both pages together. Nothing enforces this. Grep for
  'workspace per team' to find both sites; it also matches alt text in
  byoc-usage.mdx, which describes a diagram and is not part of the pair.
*/}
