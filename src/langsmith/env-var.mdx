...
To configure configure OpenTelemetry APM tracing for your deployment, set `LS_OTEL_INSTRUMENT` to `true` and `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` or `OTEL_EXPORTER_OTLP_ENDPOINT` to the target trace ingestion endpoint. Note that both `LS_OTEL_INSTRUMENT` and one of the other two export endpoints are required to activate OpenTelemetry APM tracing in server versions later than `0.7.17`.
...
```shell
# If you set LS_OTEL_INSTRUMENT AND (OTEL_EXPORTER_OTLP_TRACES_ENDPOINT or OTEL_EXPORTER_OTLP_ENDPOINT),
# the server starts with OpenTelemetry instrumentation enabled.
LS_OTEL_INSTRUMENT=true
