...
<Note>
Polly calls model providers from LangSmith's egress IP addresses. If your model provider (or a proxy in front of it) restricts traffic by IP, allowlist the LangSmith egress IPs listed in [Allowlisting IP addresses](/langsmith/cloud#allowlisting-ip-addresses).
</Note>
