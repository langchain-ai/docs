...
import { IdentityScopeExplorer } from '/snippets/identity-scope-explorer.jsx';
...
Use the explorer to pick a preset or override each axis. It shows who can access threads, how memory remounts, whose credentials tools use, and the matching declaration:

<IdentityScopeExplorer />

Choose the declaration that matches your product shape:

| Product shape | Declaration | Threads | Memory | Credentials |
| --- | --- | --- | --- | --- |
| Private assistant or internal tool | `defineIdentity({ scope: "user" })` | `user` | `user` | `user` |
| Multi-tenant SaaS | `defineIdentity({ scope: "organization" })` | `user` | `organization` | `none` |
| Shared channel bot | `defineIdentity({ scope: { threads: "conversation", memory: "user", credentials: "agent" } })` | `conversation` | `user` | `agent` |
| Service (cron/webhook) or bare default | `defineIdentity()` or `defineIdentity({ scope: "agent" })` | `user` | `agent` | `agent` |

Bare `defineIdentity()` expands to the service-agent axes (`threads: "user"`, `memory: "agent"`, `credentials: "agent"`). Unset axes in a per-axis object also follow that agent base, so a bare `{ threads: "conversation" }` inherits shared agent memory unless you set `memory` and `credentials` explicitly.

All declarations default to `backend` auth (your backend asserts the caller) and optional organizations, except when any axis uses `"organization"`, which requires an organization on every request.

<Tip>
**How to choose quickly:**

- One human per conversation who must not see anyone else's data → `defineIdentity({ scope: "user" })`
- SaaS with customer orgs → `defineIdentity({ scope: "organization" })`
- Shared channel bot → set `threads: "conversation"` and keep `memory: "user"` (usually with `credentials: "agent"`)
- Timer, webhook, or shared deployment vault → `defineIdentity()` / `scope: "agent"`
</Tip>

## Add an identity declaration

Create `identity.py` or `identity.ts` next to your agent entry and export a named `identity`. For a private assistant, set `scope: "user"` so every caller gets private threads, memory, and credentials behind your backend's auth:
...
Bare `defineIdentity()` (no options) is the service-agent shape: private threads with shared agent memory and the shared deployment vault. Use the full form when you want every field visible, or when you are assembling a config beyond the common shapes. `scope` accepts either one boundary (`"user"`, `"organization"`, `"agent"`, `"none"`) or per-axis overrides such as `{ default: "user", threads: "conversation" }`.
