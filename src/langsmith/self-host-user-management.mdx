...
### Limiting trace usage for personal organizations

If you allow personal organizations, you can cap how many traces each one may send per month. This applies a single, instance-wide maximum to every personal organization, which is useful when you bill users for their own usage.

Set the maximum number of traces a personal organization may send per calendar month. Once an organization reaches the cap, further trace ingestion is rejected for the remainder of the month and the user sees a usage-limit banner. A value of `-1` (the default) disables the cap. Any lower per-workspace usage limit still takes precedence.
...
```yaml Helm
config:
  personalOrgMaxMonthlyTraces: 1000000
```

```bash Docker
# In your .env file
PERSONAL_ORG_MAX_MONTHLY_TRACES="1000000"
```
