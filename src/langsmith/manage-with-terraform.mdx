...
<Warning>
Organization-scoped operations—creating workspaces, inviting organization members, and managing access policies—require an **organization-scoped service key with Organization Admin permissions**. Set `workspace_id` (or `LANGSMITH_WORKSPACE_ID`) to target workspace-scoped resources such as workspace memberships, resource tags, evaluators, and run rules.
</Warning>
