...
You can pre-specify a run ID by passing `id` in the LangSmith config. Use `uuid7()` from the LangSmith SDK to generate a valid ID. See [Specify a custom run ID](/langsmith/custom-run-id) for details on when this is useful.

Pass `id` at the wrapper level to apply it to all calls from that wrapper:
...
Or use `providerOptions` to set it per call with [`createLangSmithProviderOptions`](https://reference.langchain.com/javascript/langsmith/wrappers_vercel/createLangSmithProviderOptions):
