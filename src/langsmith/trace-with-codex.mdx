...
Codex loads the plugin's hooks once the plugin is enabled. Grant trust to each hook with `/hooks`.
...
- Confirm the tracing plugin is enabled in `config.toml` and its hooks are trusted in `/hooks`.
