...
Plugin hooks follow plugin enablement, so there is no separate flag to set. `[features] plugin_hooks` was removed in [openai/codex#22552](https://github.com/openai/codex/pull/22552) and is ignored if it is still in your config. `[features] hooks` is stable and on by default.
