...
- `organization:manage-model-configs` - Create, edit, or delete [organization-level model configurations](/langsmith/model-configurations#organization-scoped-configurations)
- `organization:manage-model-secrets` - Create, update, or delete [organization secrets](/langsmith/model-configurations#organization-secrets)
...
- Manage [organization model configurations and secrets](/langsmith/model-configurations#organization-scoped-configurations)
...
- `organization:manage-model-configs` and `organization:manage-model-secrets` are the organization-scope counterparts, covering [organization model configurations](/langsmith/model-configurations#organization-scoped-configurations) and [organization secrets](/langsmith/model-configurations#organization-secrets). They are separate from `organization:manage`: every role that held `organization:manage` when the permissions were introduced received both, but a custom organization role created afterward needs them granted explicitly. A workspace permission never grants access to an organization-scoped configuration, and an organization permission never grants access to a workspace-scoped one.
