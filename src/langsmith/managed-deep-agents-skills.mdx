...
When you run `mda deploy`, MDA syncs every UTF-8 file under `skills/` to the agent's [Context Hub](/langsmith/managed-deep-agents-context-hub).
...
For what syncs, what does not, and how to open the repo from a deployment, see [Context Hub](/langsmith/managed-deep-agents-context-hub).
