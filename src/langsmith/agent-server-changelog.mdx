...
This minor release enables the redis queue. Previously, redis was used to wake up workers when runs were ready, but the full queue polling logic was through postgres. This change offloads the queue polling logic to redis, reducing the load on the database and improving performance. Runs are still stored in postgres and that is the source of truth for run state.

This is not a breaking change and no coding actions are required to upgrade. However, it is worth taking some precautions:
- Due to the switch in queue polling logic, there may be a brief window immediately after upgrading where threads are executed non-chronologically. Runs will still be guaranteed to execute one at a time on a thread and in the order they were created, but threads themselves may not be executed in the order they were created.
- **Self-hosted Only** Redis will now be receiving slightly more traffic than before. From internal testing, this is not drastically more, but is worth monitoring. (The flip side is you should see notably less postgres load)
</Update>
