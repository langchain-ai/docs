...
- `rc` (release candidate): Published every three weeks and patched as needed during the bake window for critical bug backfills. Recommended for users who want to test the next stable release. Semantic versioning uses an rc tag off the minor version, for example `0.9.0rc1`.
...
## Release notes by minor

<Accordion title="0.11.x">
0.11 focuses on DeltaChannel lifecycle safety, Event Streaming v2 reliability, and Prometheus/OpenTelemetry metrics support and migration.

### Minor summary
- Added DeltaChannel-aware pruning across Postgres, SQLite, DeferredDelete, in-memory, and MongoDB checkpointers.
- Added opt-in Prometheus scrape support, plus later migration of metrics to a dedicated OTLP/Prometheus endpoint.
- Added optional run-queue trace logging with `coreApi.runQueueTraceLog` (`LSD_RUN_QUEUE_TRACE_LOG`).
- Fixed Event Streaming v2 reliability for JS sidecar graphs and HTTP `input.respond` handling.
- Added dependency and security maintenance updates.

### Releases

#### v0.11.0rc8 (2026-06-30)
- Moved Agent Server metrics to the dedicated Prometheus scrape endpoint (`LSD_PROM_METRICS_PORT`, default `9464`) via OpenTelemetry/Prometheus; `LSD_PROM_METRICS_ENABLED=true` enables endpoint exposure.
- Added internal-metrics exposure flag `EXPOSE_INTERNAL_METRICS_PROMETHEUS=true`.
- Documented potentially breaking metric naming and unit changes for Prometheus collectors and dashboards.

#### v0.11.0rc5 (2026-06-25)
- Fixed DeltaChannel replay for channels migrated from non-delta reducers, preventing incorrect reconstruction on non-additive reducers.

#### v0.11.0rc4 (2026-06-18)
- Fixed Event Streaming v2 `input.respond` interrupt validation to read pending interrupts from durable thread state.
- Fixed `input.respond` forwarding so optional `update` and `goto` fields are applied in the same `Command` as `resume`.

#### v0.11.0rc3 (2026-06-18)
- Added `coreApi.runQueueTraceLog` config (`LSD_RUN_QUEUE_TRACE_LOG`, default `false`) for verbose Redis run-queue traces.

#### v0.11.0rc2 (2026-06-17)
- Loosened the `starlette` lower bound to `>=0.38.6` for broader install compatibility, while lockfile builds still resolve Starlette `1.0.1`.

#### v0.11.0rc1 (2026-06-11)
- Added DeltaChannel-aware pruning and storage-preserving delete behavior for delta-channel threads.
- Added opt-in Prometheus metrics scrape support and opt-in Go core rate limits with Redis-backed GCRA.
- Added HTTPS dev-server certificate/key file support.
- Fixed JS graph Event Streaming v2 silent-failure and remote sidecar streaming-path issues.
- Fixed Prometheus metrics export and OpenTelemetry exporter alignment.

#### Stable release
- No 0.11 stable release has been published yet.
</Accordion>
