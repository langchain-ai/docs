...
- Backport Python dependency security updates and prevent obsolete Python package files from surviving upgrades in RC server images.
- Backport Go and Hono security dependency updates to RC server images and patch the benchmark tooling's Multer dependency.
...
- Fix Docker image publication for older stable release lines so exact version tags remain available without replacing aliases for newer releases.
- Fix vulnerabilities reported in the embedded Datadog init binary by updating it to 1.10.3.
- Support A2UI v0.9 clients that negotiate with standard A2A extension headers and canonical MIME types.
- Fix the remaining grpc-go vulnerabilities reported in the embedded Datadog init binary by updating it to 1.10.4.
...
- Fix gRPC checkpointer failures when private runtime config contains non-JSON values.
- Refresh system packages when rebuilding Wolfi-based Python and Node Agent Server images.
- Release langgraph-api 0.14.1 with the gRPC historical subgraph replay fix.
...
- Release Agent Server 0.14.4 with the merged stable-0.14 dependency security and compatibility fixes.
...
#### Fixes
- Refresh system packages when rebuilding Wolfi-based Python and Node Agent Server images.
- Pin the optional OpenTelemetry tracer's Requests dependency to a patched version on stable-0.13.
- Update the Go toolchain to 1.26.8 and upgrade `golang.org/x/mod` to address checksum database verification vulnerabilities on the stable 0.13 release line.
- Backport patched Python and JavaScript dependencies to the 0.13 release line to address security advisories.
- Release langgraph-api 0.13.5 with the latest stable-0.13 dependency security fixes.
- Fix vulnerabilities reported in the embedded Datadog init binary by updating it to 1.10.3.
- Fix the remaining grpc-go vulnerabilities reported in the embedded Datadog init binary by updating it to 1.10.4.
- Fix Docker image publication for older stable release lines so exact version tags remain available without replacing aliases for newer releases.
- Release langgraph-api 0.13.4 with the latest stable-0.13 fixes.
- Fix grpc-go security vulnerabilities by upgrading to 1.83.2.
- Release langgraph-api 0.13.3 with grpc-go 1.83.2 security fixes.
...
- Fix the remaining grpc-go vulnerabilities reported in the embedded Datadog init binary by updating it to 1.10.4.
- Fix Docker image publication for older stable release lines so exact version tags remain available without replacing aliases for newer releases.
- Backport patched Hono and optional OpenTelemetry tracer Requests dependencies to stable-0.12.
- Backport security dependency updates to the 0.12 release line.
- Release the 0.12 security backports, including the PostgreSQL store namespace-boundary fix in langgraph-checkpoint-postgres 3.1.1.
- Restore support for `langgraph-sdk` versions starting at 0.3.5.
- Includes dependency and security maintenance updates.
- Bumping @langchain/langgraph to 1.4.12 and @langchain/langgraph-checkpoint to 1.1.5 to resolve [GHSA-j87f-x5h5-gr75](https://github.com/langchain-ai/langgraphjs/security/advisories/GHSA-j87f-x5h5-gr75).
...
#### Security
- Update bundled npm tooling to address dependency vulnerabilities in server images.
- Release Agent Server 0.12.12 with the merged stable-0.12 dependency security and compatibility fixes.
