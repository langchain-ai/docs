...
The in-cluster LangSmith services (frontend, backend, queue, platform backend, Playground, ACE, and optionally the LangSmith Deployment control/data plane) ship as a **single Helm release** from the [`langchain/langsmith` chart](https://github.com/langchain-ai/helm/tree/main/charts/langsmith). Azure managed services (AKS, PostgreSQL, Managed Redis, Blob Storage, Key Vault, and ingress) are provisioned separately as standard Azure resources using your own infrastructure tooling (Terraform, ARM templates, or the Azure portal) before you install LangSmith.
