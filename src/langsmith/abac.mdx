...
In an [agent-based workspace](/langsmith/agents), the projects this matches are [agent environments](/langsmith/agent-environments), which carry the default **Application** tag like any other tracing project. A policy written this way therefore scopes to environments rather than to whole agents. Agents are not an ABAC resource type, so a policy cannot name one directly.
