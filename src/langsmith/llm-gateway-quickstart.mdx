...
Open [LangSmith](https://smith.langchain.com) and go to the tracing project named `gateway` or `gateway-<short_api_key>-<api_key_id>` in your workspace. Your request appears there with its token counts, cost, and latency. In an [agent-based workspace](/langsmith/agents), that project is the **Production** [environment](/langsmith/agent-environments) of an agent with the same name, so select the agent first.
