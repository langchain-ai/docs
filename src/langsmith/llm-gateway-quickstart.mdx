...
Provider-specific base URLs take precedence over the gateway, so you can still route an individual provider elsewhere. For example, with the gateway enabled, setting the OpenAI base URL variable sends OpenAI to that URL while every other provider continues to use the gateway.

The following table shows how the base URL and key are resolved, using OpenAI as the example (other providers use their own base URL and API key variables). `GW default` is `https://gateway.smith.langchain.com/openai/v1`.
