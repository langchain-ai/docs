...
Set `LANGSMITH_GATEWAY` so Deep Agents routes through the gateway automatically.
...
Set `LANGSMITH_GATEWAY` so LangChain routes through the gateway automatically. Use the `langsmith:` provider prefix with `init_chat_model`.
