...
client = anthropic.Anthropic(
    base_url=os.environ["LANGSMITH_GATEWAY_BASE_URL"].replace("/v1", ""),
    api_key=os.environ["LANGSMITH_API_KEY"],
...
<Note>
This uses the gateway's [direct Anthropic endpoint](/langsmith/llm-gateway-direct-model-access). For the full format reference, see [API formats](/langsmith/llm-gateway-api-formats).
</Note>
...
const client = new Anthropic({
  baseURL: process.env.LANGSMITH_GATEWAY_BASE_URL!.replace("/v1", ""),
  apiKey: process.env.LANGSMITH_API_KEY,
...
Set the gateway env var so LangChain routes through the gateway automatically:
...
To use [Gateway Credits](/langsmith/llm-gateway-credits) models or the unified `/v1` endpoint, prefix the model with `langsmith:` and set the provider to `openai` (requires `langchain>=1.3.15`):
