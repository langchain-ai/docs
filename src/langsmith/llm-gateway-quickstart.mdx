...
To use [Gateway Credits](/langsmith/llm-gateway-credits) models, prefix the model with `langsmith:` (requires `deepagents>=0.1.0`):
...
Use the `langsmith:` provider prefix to route through the unified `/v1` endpoint. This also gives access to [Gateway Credits](/langsmith/llm-gateway-credits) models:
