...
- Added rate limiting check before query shadowing to improve backend efficiency.
- Improved feedback stats computation in CRUD feedback endpoints.
- Enforced run permission checks on experiment routes for better security.
- Added skeleton loading states in frontend for a smoother user experience.
- Introduced mcp_server and integration ABAC resource types for advanced access control.
- Fixed loading states when switching agent chat threads to prevent UI confusion.
- Added LLM auth proxy support in self-hosted environments.
- Enhanced annotation queue with assigned reviewers table and review stats integration.
- Fixed tool use parsing to enhance the front-end feedback system.
- Enabled filtering and sorting in sandbox API for better user interaction.
- Added Apollo tools in agent-builder for expanded integration capabilities.
- Improved frontend with dataset split picker in evaluator playground.
- Added support for service URL routing with JWT auth in sandbox environments.
- Exposed LLM auth proxy URL allowlist controls for improved security management.
- Added fleet agents and users usage API endpoints for better management and reporting.
- Introduced POST /v2/traces/messages batch endpoint for improved API interactions.
