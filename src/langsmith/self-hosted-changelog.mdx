...
- Improved UI for async sandbox creation with status polling.
- Added JSON/YAML syntax highlighting to experiment comparison UI.
- Fixed thread trace open behavior and removed expand button for improved user experience.
- Eliminated n+1 query in the backend for listing personal access tokens, improving performance.
- Added sandbox endpoint to smith-go for enhanced functionality.
- Fixed support for OpenAI compatible endpoint in smith-polly integration.
- Fixed bulk export timeout issue when exports were stuck in CREATED status.
- Updated UI projects and runs write permissions with ABAC to enhance backend authentication and authorization.
- Fixed large attachment image preview in the UI.
- Added support for GPT-5.4 and GPT-5.4-pro model in both pricing and playground.
- Made GPT-5.4 the default OpenAI playground model.
- Increased the maximum number of tags displayed in the RunTags component.
- Added models and prompts columns to experiments table for better data display.
- Fixed issue where agent builder runs were rejected when limit was changed.
- Fixed Redis cache handling to return fetched value when SET fails, improving reliability.
- Added AWS IAM role support for agent builder, polly, and insights for enhanced security.
- Redesigned custom chart CRUD on frontend to improve user interface.
- Added filtering and grouping by prompt in the experiments table for easier data management.
