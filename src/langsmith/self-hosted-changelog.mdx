...
- POST /v1/fleet/sandboxes/{sandbox_slug}/files wrote a file into a sandbox from a multipart/form-data body, completing the Fleet file API alongside the existing list and read endpoints, with the path query parameter naming the destination on its own, so the upload no longer depended on accessing a sandbox's dataplane URL directly; files were streamed rather than buffered, and one over 100 MB was refused with 413.  
