...
- N/A (internal dev tooling — a Claude skill; no user-facing product change).
...
- Fleet now showed a warning — inline above the failing tool call in chat and as a message in Slack — when a Google Docs, Sheets, Drive, or Slides tool hit a 403 or 404, explaining the agent could only access files it had created itself with its connected Google account.
...
- The Gateway Usage tab no longer showed an in-page workspace dropdown — the page was already scoped to your current workspace, and its subtitle now named that workspace directly.
...
- N/A — internal eval tooling (agent skill); no product, API, or user-facing change.
...
- Listing or counting examples with an unsupported `filter` expression — an unfilterable attribute, or a `has(...)` comparison whose value was not valid JSON — now returned a 422 naming the `filter` parameter instead of a 500.
