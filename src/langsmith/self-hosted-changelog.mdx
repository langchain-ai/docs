...
- **Self-hosted Engine** is available in AWS and GCP. For installation instructions, see [LangSmith Engine on self-hosted](/langsmith/deploy-self-hosted-full-platform#enable-engine). You may need to contact your account representative to enable this feature on your license.
    ...
    - Inference is charged in [LangChain Compute Units (LCUs)](/langsmith/pricing-page) with an optional monthly spend limit. On Self-hosted, Engine emits no LangSmith traces of its own, so no run content leaves your environment.
