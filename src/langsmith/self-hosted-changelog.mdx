...
- Added a forge frontend connection for enhanced user interaction.
- Improved the Agent Builder by showing a grayed-out other identity option in the AgentTypeBadge trigger.
- Fixed UI compatibility by wrapping InfoCircleIcon in a span for Radix tooltip.
- Made the tracing support email a mailto link for easier access.
- Added a GitHub repository connection option in the forge settings modal.
- Fixed a UI issue by removing a stray "Linked" fallback text.
...
- Aligned 'no attachments' text to the left for a consistent UI experience.
- Fixed the render of the polly shortcut with kbd styling like the theme selector for consistency.
- Addressed connectivity issues with the connect/disconnect button on Slack integrations.
- Redesigned the secrets settings page for better usability.
- Added migrations for prompt environments to support advanced configurations.
- Prevented long text from overflowing in the DeleteConfirmationModal for improved readability.
- Stopped accepting Inputs/Outputs/Events in Inline Multipart for streamlined data processing.
- Enabled inline editable display name on the members table for easier management of user details.
- Added migration for the 'is_system' hidden evaluators to ensure data integrity.
- Ensured tenant isolation for price map insertion for enhanced data privacy and isolation.
- Fixed the issue of the wrong tab being highlighted on click for accurate navigation.
- Resolved vacuous connected state issues on the tool badge in the Agent Builder.
- Added new authentication features with Auth split ABAC and hub repositories.
...
- Added forge frontend connection for improved UI integration.
- Improved Agent Builder UI by showing a grayed-out other identity option in AgentTypeBadge trigger.
- Fixed tooltip compatibility in the frontend by wrapping InfoCircleIcon in a span for Radix.
- Enabled the tracing support email as a mailto link for better user interaction.
- Fixed a UI bug by removing a stray "Linked" fallback.
- Added a new endpoint to support offline self-hosted agent builder usage.
- Aligned 'no attachments' text to the left in UI rows for consistency.
- Rendered the polly shortcut with keyboard styling similar to the theme selector for a consistent look.
- Fixed the connect/disconnect button functionality on Slack integrations.
- Redesigned the secrets settings page in the frontend.
- Added new migrations for prompt environments in the backend.
- Prevented long text from overflowing in the DeleteConfirmationModal for better UI handling.
- Stopped accepting Inputs/Outputs/Events in Inline Multipart to streamline data ingestion.
- Made display names inline editable on the members table in the frontend for user convenience.
- Improved the frontend by preventing the wrong tab from being highlighted on click.
- Fixed a vacuous connected state on tool badges in the Agent Builder.
- Added support for Auth split ABAC + hub repos under the bring-your-own-code (BYOC) policy.
