...
- **SmithDB** is available in public beta. LangChain does not support or recommend setting this up on your own. Express interest through the [Support Portal](https://support.langchain.com) and the team will reach out to set you up for success with SmithDB.
...
- **Self-hosted Engine** is available in AWS and GCP. For installation instructions, see [LangSmith Engine on self-hosted](/langsmith/deploy-self-hosted-full-platform#enable-engine). You may need to contact your account representative to enable this feature on your license.
    ...
    - Scans each connected project every 6 hours, clustering failures into categorized issues ranked by severity and diagnosed against both your traces and your connected source code.
    - Proposes fixes as pull requests for Deep Agents, LangChain, and LangGraph apps, ships each issue with a generated evaluator and ground truth examples to catch regressions, and reopens issues automatically if they recur.
    - Inference is charged in [LangChain Compute Units (LCUs)](/langsmith/pricing-plans) with an optional monthly spend limit. On Self-hosted, Engine emits no LangSmith traces of its own, so no run content leaves your environment.
- **LangSmith LLM Gateway** is available in public beta. For installation instructions, see [LLM Gateway](/langsmith/llm-gateway).
...
- **Platform and tooling**
    ...
    - **Redis resources**: Configuration added to the Create deployment and Create revision panes for LangSmith Deployment.
    - **`ENABLE_UNCAPPED_LGP_DEPLOYMENT_RESOURCES`**: New config (default `false`). When enabled, removes the CPU and memory ceiling on LangSmith Deployment specs.
...
- **Observability and evaluation**
    ...
    - **Large trace performance**: Resolved "Query timeout exceeded" errors when opening large traces.
    ...
    - **Alerting**: Alert rules for Run Count, Errors, Latency, and Cost now support `<`, `<=`, `>`, and `>=` (previously `>=` only), plus Slack as a native alert type with one-click setup.
    ...
    - **Reusable evaluators**: LLM-as-judge evaluators can include extended stats and map prompt variables directly from `run.*` fields.
    ...
    - **Annotation queues**: Add RUN and THREAD items via `POST /items`. Item APIs are now public and work from the SDKs. Additionally, there is minimum thread idle time and ABAC support for `annotation-queues:*` permissions.
...
- New [RBAC](/langsmith/rbac) permissions: `bulk-exports:read` and `bulk-exports:manage`, covering the [bulk export](/langsmith/data-export) endpoints.
...
- OIDC display name resolves from `name` or `given_name` and `family_name` claims, and [SSO Groups Sync](/langsmith/user-management#sso-groups-sync-alternative) no longer silently no-ops during login. For more information, see [Set up SSO with OAuth2.0 and OIDC](/langsmith/self-host-sso).
...
- [Organization admins](/langsmith/rbac#organization-admin) can manage other organization admins again.
- Filter [API keys](/langsmith/administration-overview#api-keys) by workspace.
- [Edit pending member invite roles](/langsmith/user-management#assign-a-role-to-a-user) from the UI.
- [Restrict roles](/langsmith/rbac#restrict-roles) directly from the Organization Roles table.
