...
- Fixed several security alerts and a code injection vulnerability to enhance UI security.
- Fixed an issue preventing XSS via unescaped URLs in the Rich Text Editor.
- Improved sandbox environment security by restricting execution permissions to the Deno binary.
- Fixed issues with the playground in self-hosted deployments.
...
- Fixed a bug in the insights agent that was not persisting categories.
- Improved frontend with asynchronous sandbox creation UI and added status polling.
- Enhanced experiment comparison with JSON/YAML syntax highlighting.
- Improved trace open behavior in the frontend and removed expand button.
- Eliminated n+1 query in personal access token listing for better backend performance.
- Added a new sandbox endpoint to smith-go.
- Supported OpenAI compatible endpoint in smith-polly.
- Timed out bulk exports stuck in CREATED status to prevent backend issues.
- Implemented auth improvements for /sessions shadow query.
- Added attribute-based access control (ABAC) for projects and runs write permissions in the UI.
- Introduced support for GPT-5.4 and GPT-5.4-pro models.
- Updated the playground to make GPT-5.4 the default OpenAI model.
- Increased the number of tags displayed in the RunTags component of the frontend.
- Fixed large attachment image preview issue.
- Enhanced the experiments table with models and prompts columns.
- Addressed a bug preventing agent builder runs when limits changed.
- Implemented support for AWS IAM roles in agent builder, polly, and insights.
- Redesigned custom chart CRUD operations in the frontend.
- Added filter and group by prompt in the experiments table.
- Improved inbox counts and thread fetching logic in the agent builder.
...
- Fixed multiple security alerts in the frontend, enhancing overall security.
- Prevented XSS via unescaped URL in RichTextEditor, improving user security.
- Restricted --allow-run to only the Deno binary in the Smith ACE component for enhanced security.
- Fixed an issue affecting playground functionality in self-hosted deployments.
...
- Patched 3 security alerts in the frontend to enhance system security.
- Patched 2 security alerts in the Smith Polly module for improved user safety.
- Fixed a code scanning alert related to code injection, enhancing application security.
- Restricted the `--allow-run` flag to only the Deno binary in the Smith ACE, improving security.
- Fixed a frontend issue to prevent cross-site scripting (XSS) via unescaped URLs in the RichTextEditor.
- Fixed the self-hosted playground functionality, ensuring smoother performance and usability.
...
- Patched multiple security alerts across frontend components to enhance security.
- Fixed a code scanning alert related to code injection.
- Restricted permissions for the 'deno' binary to improve security in Smith-ace.
- Prevented XSS vulnerabilities in the frontend by escaping URLs in the RichTextEditor.
- Fixed issues with the self-hosted playground component to improve functionality.
...
- Improved the async sandbox creation UI with status polling.
- Made the logo bigger in the frontend sidebar.
- Added JSON/YAML syntax highlighting to experiment comparison.
- Improved thread trace open behavior and removed the expand button in the frontend.
- Eliminated n+1 query in list_personal_access_tokens_for_org to improve backend performance.
- Added a sandbox endpoint to smith-go.
- Supported OpenAI compatible endpoint in smith-polly.
- Timeout implemented for bulk exports stuck in CREATED status in the backend.
- Allowed service identity access to create repo endpoint.
- Fixed authentication for /sessions shadow query.
- Added role-based access control (ABAC): Backend deployments v2.
- Improved projects and runs write permissions using ABAC in the UI.
- Added GPT-5.4 and GPT-5.4-pro model pricing and playground support.
- Made GPT-5.4 the default OpenAI playground model in the frontend.
- Increased the max tags displayed in the RunTags component.
- Fixed issue where evaluator backfills needed to skip Eppo feature flag check for self-hosted.
- Added models and prompts columns to the experiments table.
- Fixed agent builder issues where runs were rejected when limits changed.
- Fixed /sessions go endpoint float errors.
- Improved Redis cache handling in the backend to return fetched values when SET fails.
- Updated AWS IAM role support for agent builder, Polly, and insights.
- Added a sticky header on new messages in the UI.
- Redesigned custom chart CRUD in the frontend.
- Enabled filtering and grouping by prompt in the experiments table.
...
- Fixed several security vulnerabilities in the frontend related to high, medium, and low threat alerts.
- Prevented XSS attacks via unescaped URLs in the RichTextEditor component.
- Restricted execution permissions to only the Deno binary to enhance security.
- Fixed issues in the playground for self-hosted environments to improve user experience.
