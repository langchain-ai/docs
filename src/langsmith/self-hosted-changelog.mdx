...
- Fixed insecure default JWT secret rejection at startup in agent builder.
- Improved Eppo control panel rendering above Polly in the frontend.
- Standardized dropdown width and height in Bedrock and Azure model config in the playground.
- Fixed workspace invites issue with no default role ID when RBAC is not enabled.
- Updated prompt owners dialog to be fully backwards compatible in the frontend.
- Included per-annotator feedback detail in experiments CSV export.
- Added Slack disconnect feature to delete user integrations in agent builder.
- Added GET /me/slack_id endpoint in agent builder.
- Implemented Slack authorization for agent builder in "is_user_permitted_for_agent" feature.
- Added strict model schema setting from evaluator and prompt hub.
- Improved frontend styling with new neutral color palettes.
- Extended frontend ColumnActions with isAlphabetical and onFilter props.
- Redesign of feedback chip popover in the frontend.
- Added real-time sub-agent streaming UI in agent builder.
- Added forge configurations CRUD API and experimental forge settings page.
- Created new `trace_count_transactions_granular_rollup` table.
- Fixed raw error message display on home page for tracing projects.
- Added new security and OAuth bug fixes across multiple modules.
- Enhanced performance and fixed various UX bugs in playground, agent builder, and frontend.
...
- Rejected insecure default JWT secret at startup to enhance security.
- Ensured Eppo control panel renders correctly above Polly.
- Standardized dropdown width and height in Bedrock and Azure model config for improved UI consistency.
- Updated prompt owners dialog to be fully backwards compatible.
- Included per-annotator feedback detail in experiments CSV export.
- Added GET /me/slack_id endpoint to the Agent Builder.
- Implemented Slack authorization to check user permissions for agents.
- Fixed model schema synchronization in prebuilt evaluator schemas and on model switch.
- Added a new forge configurations CRUD API.
- Added a feature to sort inbox messages by recency and fixed message wrapping.
- Redesigned feedback chip popover for improved user interaction.
- Implemented real-time sub-agent streaming UI in the Agent Builder.
- Hid raw error messages on home page for tracing projects to improve user experience.
...
- Ensured Eppo control panel renders correctly in the frontend.
- Standardized dropdown width and height in the Playground for Bedrock and Azure model configurations.
- Improved workspace invites by addressing the absence of a default role id when RBAC is disabled.
- Updated the prompt owners dialog in the frontend for backward compatibility.
- Included per-annotator feedback details in experiments CSV exports.
- Added Slack integration management with new `GET /me/slack_id` endpoint and improved authorization.
- Fixed frontend bugs, including generator issues and custom Slack bot addition bugs.
- Enhanced forge page with experimental UI and CRUD API for forge configurations.
- Added authentication improvements, including Slack authorization and ABAC middleware.
- Fixed frontend member search issue in the owners dialog.
- Improved frontend appearance with feedback chip popover redesign, metadata fields style redesign, and new color palettes.
- Implemented real-time sub-agent streaming UI in the agent builder.
- Renamed "Use object in LangChain" to "Use Programmatically".
- Stopped channels identity modal in the agent builder from auto-saving.
...
- Rejected insecure default JWT secret at startup in the agent builder for enhanced security.
- Ensured Eppo control panel renders above Polly in the frontend for improved user experience.
- Standardized dropdown width and height in Bedrock and Azure model config in the playground for consistent UI.
- Fixed workspace invites to handle cases with no default role ID when RBAC is not enabled.
- Updated prompt owners dialog in the frontend, making it fully backwards compatible.
- Included per-annotator feedback detail in experiments CSV export for better insights.
- Added a GET /me/slack_id endpoint in the agent builder for easier Slack integration.
- Updated UI for agent builder triggers popover and identity management for better usability.
- Handled potential HTTP auth leak for enhanced security.
- Improved member search in the owners dialog in the frontend.
- Recorded and normalized tool references in experiment metadata for better data tracking.
- Consistently fixed UTF-8 validation in example multipart uploads for better data handling.
- Made GitHub integration fields nullable in the forge configuration settings.
- Implemented ABAC (Attribute-Based Access Control) authorization middleware in the Go backend.
- Redesigned feedback chip popover for better user interaction.
- Added real-time sub-agent streaming UI feature in the agent builder for live data handling.
- Renamed "Use object in LangChain" to "Use Programmatically" for clearer instructions in the tracing projects.
- Stopped channels identity modal from auto-saving in the agent builder for better user control.
...
- Fixed rejection of insecure default JWT secret at startup in the agent builder trigger server.
- Standardized dropdown width and height in Bedrock and Azure model configurations in the playground.
- Ensured proper rendering of the Eppo control panel above Polly in the frontend.
- Updated prompt owners dialog to be fully backwards compatible.
- Added per-annotator feedback detail in experiments CSV export.
- Added GET /me/slack_id endpoint and Slack integration enhancements in the agent builder.
- Fixed potential HTTP auth leak in message_received.py.
- Improved UI in the agent builder with popover and identity updates.
- Addressed a generator bug related to adding custom Slack bots in the frontend.
- Added Slack authorization with is_user_permitted_for_agent implementation.
- Improved model schema management from the evaluator/prompt hub with strict:true enforcement.
- Added UI enhancements to center elements on the no access page and fix alignment in the Polly chat tooltip.
- Introduced real-time sub-agent streaming UI in the agent builder.
- Fixed sorting in the agent builder inbox all tab by recency and addressed message wrapping.
- Enhanced metadata fields and feedback chip popover redesigns for better usability.
- Added CRUD API support for forge configurations.
- Improved rejection of invalid UTF-8 in experiment metadata multipart uploads.
- Supported new slack credentials design and improvements.
- Took measures to improve error handling and feedback saving in various backend components.
- Enhanced performance with database-level statement timeout settings.
- Allowed for fallback to the latest version if unable to resolve the langgraph-api version.
- Fixed member search functionality in the prompt owners dialog.
- Implemented new metadata fields style redesign to improve visual consistency.
...
- Fixed issue in Agent Builder where insecure default JWT secret was rejected at startup.
- Ensured Eppo control panel renders above Polly in the frontend.
- Standardized dropdown width and height in Playground's Bedrock and Azure model configuration settings.
- Addressed issue where workspace invites would not have a default role ID when RBAC is not enabled.
- Updated prompt owners dialog in frontend for full backward compatibility.
- Added per-annotator feedback detail to experiments CSV export.
- Implemented Slack disconnection procedure to delete user integrations in Agent Builder.
- Added new API endpoint for retrieving Slack ID in Agent Builder.
- Implemented Slack authorization functionality in Agent Builder using the is_user_permitted_for_agent feature.
- Modified Agent Builder to dynamically retrieve OAuth token and improve modal behavior.
- Fixed a potential HTTP auth leak in the message_received.py.
- Improved sorting and message wrapping in the Agent Builder's inbox.
- Fixed frontend member search in owners dialog.
- Fixed long message wrapping and improved tab sorting by recency in Agent Builder's inbox.
- Redesigned feedback chip popover in frontend.
- Added a real-time sub-agent streaming UI in Agent Builder.
- Enhanced security by enforcing a directory boundary for reading secret files in the proxy.
- Introduced new forge settings page with an experimental UI.
- Improved the styling of several UI elements within the frontend, including icon button colors and tooltip consolidation.
...
- Added frontend connection for Forge and GitHub repository to Forge settings modal.
- Improved Agent Builder with a grayed-out other identity option in the AgentTypeBadge trigger.
- Fixed wrapping of InfoCircleIcon in span for Radix tooltip compatibility.
- Made tracing support email into a clickable mailto link.
- Fixed stray "Linked" fallback UI issue.
- Added a new endpoint to support offline self-hosted Agent Builder usage.
- Fixed alignment of 'no attachments' text in rows.
- Rendered Polly shortcut with keyboard styling like the theme selector.
- Fixed connect/disconnect button functionality on Slack integrations.
- Added migrations for prompt environments to backend.
- Prevented long text from overflowing in DeleteConfirmationModal.
- Made display names inline editable on the members table.
- Fixed tenant isolation for price map insertion.
- Prevented wrong tab from being highlighted on the frontend when clicked.
- Fixed vacuous connected state on tool badge in Agent Builder.
- Added support for Auth with ABAC and hub repositories.
...
- Added forge frontend connection to enhance user interaction with forge settings.
- Improved Agent Builder UI by showing a grayed-out option for different identity types on the AgentTypeBadge trigger.
- Fixed Radix tooltip compatibility issue by wrapping InfoCircleIcon in a span.
- Made tracing support email a clickable mailto link for easier user support access.
- Added a GitHub repository connection option in the forge settings modal for additional integration.
- Fixed stray "Linked" fallback UI issue which enhances the overall user interface consistency.
- Added a new API endpoint to support offline usage of the Agent Builder in self-hosted deployments.
- Connected the v2 stats handler to the smithdb-stats-query service for improved performance.
- Aligned 'no attachments' text to the left in the row for better UI presentation.
- Rendered polly shortcuts with keyboard styling for a consistent user experience.
- Fixed connect/disconnect button issues on Slack integrations to improve user interaction.
- Redesigned the secrets settings page to enhance user experience.
- Added migrations for prompt environments to support backend configuration.
- Prevented long text from overflowing in the DeleteConfirmationModal to maintain UI integrity.
- Stopped Inline Multipart from accepting Inputs/Outputs/Events for better data handling.
- Enabled inline editable display names on the members table for enhanced user management.
- Prevented the wrong tab from being highlighted on click to improve navigation accuracy.
- Fixed vacuous connected state on tool badge in the Agent Builder to clarify connectivity status.
- Enhanced authentication by splitting ABAC and hub repositories for improved security management.
...
- Added forge frontend connection to enhance UI functionality.
- Improved Agent Builder by displaying a grayed-out other identity option in AgentTypeBadge trigger.
- Fixed compatibility of InfoCircleIcon with Radix tooltip by wrapping it in a span.
- Made tracing support email a clickable mailto link.
- Added a new endpoint to support offline self-hosted agent builder usage.
- Connected v2 stats handler to smithdb-stats-query service for better performance.
- Fixed UI issue by aligning 'no attachments' text to the left in the row.
- Improved UI by rendering polly shortcut with keyboard styling similar to theme selector.
- Fixed connect/disconnect button on Slack integrations for better user interaction.
- Added migrations for prompt environments in the backend.
- Prevented long text from overflowing DeleteConfirmationModal for better UI presentation.
- Made display name inline editable on the members table in the frontend.
- Fixed frontend issue to prevent wrong tab being highlighted on click.
- Enhanced Agent Builder by addressing vacuous connected state on tool badge.
- Split authentication into ABAC and hub repos for improved authorization.
...
- Fixed the rejection of insecure default JWT secret at startup in the Agent Builder trigger server.
- Improved the rendering of the Eppo control panel in the frontend to ensure it displays correctly above Polly.
- Standardized dropdown width and height in the Playground's Bedrock and Azure model configuration.
- Fixed an issue in workspace invites to ensure no default role ID is set when RBAC is not enabled.
- Updated prompt owners dialog in the frontend, adding full backward compatibility.
- Included per-annotator feedback detail in experiments CSV export.
- Added dynamic retrieval of OAuth tokens in Agent Builder.
- Ensured organization ID is correctly passed from context in Agent Builder.
- Added new API endpoint: GET /me/slack_id in Agent Builder for Slack user integration.
- Implemented Slack authorization with is_user_permitted_for_agent API in Agent Builder.
- Added authentication and authorization middleware in ABAC for Go services.
- Fixed the Slack integration to prevent error messages when custom Slack bots are added.
- Made the GitHub integration ID and GitHub repo fields nullable for better flexibility.
- Added real-time sub-agent streaming UI to the Agent Builder.
- Fixed z-index and viewport edge padding issues in Polly chat tooltips.
...
- Fixed insecure default JWT secret rejection at startup for the Agent Builder trigger server.
- Improved rendering order for Eppo and Polly control panels in the frontend.
- Standardized dropdown dimensions for Bedrock and Azure model configurations in the playground.
- Updated workspace invites to handle cases without a default role ID when RBAC is disabled.
- Added a Slack disconnect feature to delete user integrations in the Agent Builder.
- Added a GET /me/slack_id endpoint in the Agent Builder.
- Updated the prompt owners dialog for enhanced compatibility in the frontend.
- Included per-annotator feedback details in experiments CSV exports.
- Ensured alignment evaluators are properly filtered in Smith-go for Go endpoints.
- Implemented Slack authorization to check if the user is permitted for an agent.
- Added strict schema validation for prebuilt evaluator schemas in the frontend.
- Fixed a generator bug involving custom Slack bots in the frontend.
- Fixed a potential HTTP auth leak in the message_received.py module.
- Enhanced the feedback save functionality in the Smith-backend to prevent SAQ timeout cancellations.
- Fixed message display and sorting issues in the Agent Builder inbox.
- Improved database performance by setting a statement timeout.
- Added CRUD API for forge configurations.
- Implemented ABAC authorization middleware in Smith-go.
- Redesigned the feedback chip popover and metadata fields style for improved UI.
- Added real-time sub-agent streaming UI in the Agent Builder.
- Removed raw error messages from the home page for tracing projects.
- Updated terminologies for ease of understanding: "Use object in LangChain" renamed to "Use Programmatically".
- Stopped auto-saving in the channels identity modal in the Agent Builder.
...
- Fixed insecure default JWT secret rejection at startup in the agent builder.
- Improved control panel rendering in the frontend to ensure it renders above other elements.
- Standardized dropdown dimensions in the Playground for Bedrock and Azure model configurations.
- Fixed a bug in workspace invites to ensure no default role ID is used when RBAC is not enabled.
- Updated prompt owners dialog in the frontend with enhanced compatibility.
- Enhanced experiments CSV export to include per-annotator feedback detail.
- Added new `/me/slack_id` API endpoint for user integration management in the agent builder.
- Implemented Slack authorization feature `is_user_permitted_for_agent` in the agent builder.
- Added strict schema enforcement to prebuilt evaluator schemas and ensured synchronization on model switch in the frontend.
- Enhanced loading speed by implementing a statement timeout at the database level.
- Added new endpoint to replace thread detail with the smithdb endpoint.
- Resolved potential HTTP auth leaks in message handling.
- Redesigned Forge settings page with an experimental UI.
- Improved OAuth token retrieval dynamics in the agent builder.
- Enhanced error handling in the smith backend to shield feedback save processes from SAQ timeout cancellations.
- Extended frontend column actions with new `isAlphabetical` and `onFilter` props.
- Redesigned feedback chip popover for improved user interaction.
- Added real-time sub-agent streaming UI to the agent builder for live monitoring.
- Improved home page messaging by hiding raw error messages for tracing projects.
- Updated bullet and chevron styles for consistency across the platform.
- Fixed agent builder channels identity modal to stop unscheduled auto-saving actions.
