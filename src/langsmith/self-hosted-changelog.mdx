...
{/* TODO before publishing: replace the TBD release-image marker with the underlying image ref (e.g., `0.15.7-<sha>`) from the langchainplus GA release body. */}
<Update label="2026-05-26" tags={["self-hosted"]}>
## langsmith-0.15.0

LangSmith Self-Hosted v0.15 brings **reusable evaluators and a library of 30+ evaluator templates** that centralize evaluation across your workspace, ships **per-example assertions** alongside reference outputs in annotation queues, and turns LangSmith itself into a **remote MCP server** at `<hostname>/api/mcp` so MCP-compatible clients can connect to your deployment directly. **Insights reports** can now be downloaded as PDFs for offline analysis. On the [Fleet](/langsmith/fleet) side (formerly **Agent Builder**), v0.15 adds **GitHub App integration** for repo access, a new **X (Twitter)** BYO-OAuth integration with 17 tools, and graduates the **Salesforce tool integration** to general availability. Several breaking changes are worth reviewing before upgrade: the `agent-bootstrap` script is deprecated, the Agent Builder rename to Fleet may require workload-identity service-account updates, **JWKS with `kid` values** is now required for OAuth token signing, and the `projects:update-retention` permission splits into `projects:increase-trace-tier` and `projects:decrease-trace-tier`.
...
- JWKS is now required, and keys must have `kid` (Key ID) values set. These are used to sign tokens issued by the LangSmith OAuth server as well as for various other application functionalities. See [LLM Auth Proxy](/langsmith/llm-auth-proxy-self-hosted) for setup details.
...
- **SSRF protection on Playground** — Server-Side Request Forgery protection is now enabled on all Playground touchpoints. Disabled by default in self-hosted deployments; opt in via the [Playground environment settings](/langsmith/self-host-playground-environment-settings).
- **S3 addressing-style configuration for bulk exports** — bulk export destinations now accept `config_kwargs_s3` for configuring S3 addressing style, restoring compatibility with S3-compatible services that require virtual-hosted addressing (for example, Volcengine TOS).
...
- **Remote MCP server** — self-hosted LangSmith now exposes a remote [Model Context Protocol (MCP)](/langsmith/langsmith-mcp-server) endpoint at `<hostname>/api/mcp`, so MCP-compatible clients can connect to your deployment without running a standalone server.
...
- **GitHub App integration for Fleet** — connect GitHub accounts to [Fleet](/langsmith/fleet) via App installations for repo access, replacing the previous OAuth-key flow.
- **Fleet enhancements**
  - The [Salesforce tool integration](/langsmith/fleet/salesforce) is now enabled for all tenants.
  - [Cron schedule triggers](/langsmith/fleet/schedules) support attaching skills through the `/` menu.
  - **X (Twitter) integration with BYO OAuth** — 17 [Fleet tools](/langsmith/fleet/tools) for posting, searching, monitoring mentions, managing follows, and more. The existing API-key Twitter tools are preserved but deprecated.
  - Fleet HTML file previews offer a presentation mode with thumbnail navigation and fullscreen viewing.
...
- **Expanded ABAC coverage** — [ABAC](/langsmith/abac) now applies to `runs:create` on `POST /runs` and `POST /runs/batch`, plus the remaining `/sessions/{session_id}/` endpoints. Hardened built-in integrations access controls so ABAC-allowed tools stay available and unauthorized tool execution is blocked.
...
{/* langsmith-release-image: 0.15.0 TBD */}
</Update>
...
**Download the Helm chart:** [`langsmith-0.15.0.tgz`](https://github.com/langchain-ai/helm/releases/download/langsmith-0.15.0-rc.16/langsmith-0.15.0-rc.16.tgz)
