...
- **LangSmith LLM Gateway** is not included in v0.16.0 stable for self-hosted deployments. Self-hosted LLM Gateway will be available in a future stable release. In the meantime:
    - Submit the [LLM Gateway self-hosted access request](https://www.langchain.com/langsmith-llm-gateway-self-hosted-access-request) to express interest and get notified when it is available.
    - Try LLM Gateway on v17 RC versions or BYOC (bring your own cloud) ahead of the stable release.
