...
- Evaluator API responses included a `created_by` field showing who created each evaluator, and the evaluators table UI displayed a "Created By" column.
- Migrated to add a fleet-admin:read permission.
- The auth interrupt dialog showed two options: authenticate with all provider scopes (primary) or just the requested scope (secondary).
- Salesforce tool integration gated behind a feature flag and hidden by default until enabled per tenant.
- Evaluators table supported filtering by feedback key and resource (project/dataset).
...
- Polly now reads and respects the active filter on the runs page, merging new conditions rather than replacing user-set filters.
...
- ABAC: Added the remaining `/sessions/{session_id}/` endpoints.
...
- Added an instance admin endpoint (`POST /v1/platform/ops/backfills/restart`) to restart backfill jobs and introduced the `INSTANCE_ADMIN_EMAILS` env var (replaces deprecated `USAGE_EXPORT_ADMIN_EMAILS`).
...
- Enabled granular rollup and cleanup jobs showing daily granularity for granular trace usage.
...
- Added an integration with Twitter (X) supporting BYO OAuth with 17 tools for posting, searching, monitoring mentions, managing follows, and more, while preserving but deprecating existing API-key-based Twitter tools.
- Clicking a workspace agent card now opened the chat view instead of the editor.
- Fixed multiple-resource filter on the Evaluators page to use AND logic, only showing evaluators attached to all selected resources.
...
- Added a fleet-admin:read permission to gate the new Fleet Admin section.
- Fixed an error when creating custom code evaluators from a dataset that use `perform_eval(run, example)` with two arguments.
- CSV export for the experiment comparison view now routed through the Go backend when the `experiments_use_smithdb` flag was enabled, streaming up to 10,000 rows with per-session and feedback key columns.
- Fixed workspace-agent navigation so read-only users could reach the editor and clone shared agents, and added hover-panel action buttons for agent cards.
...
- The unified files sidebar was now enabled for all tenants in the Agent Builder chat.
- CSV export for experiments now included per-annotator feedback detail (score, value, comment, user) when the "include annotator detail" option was selected.
- Filters in progress on the tracing & threads pages were now preserved when clicking away from the filter popover and reopening it.
- Hardened built-in integrations access controls so ABAC-allowed tools stayed available and unauthorized tool execution was blocked.
- Fixed a CSV file upload issue when creating a dataset in self-hosted and split data plane deployments.
- Fleet usage dashboard now tracked agent names on run metadata for display alongside agent IDs.
- Fleet usage API now returned agent names alongside agent IDs.
- Issue board linked traces now showed a short summary of why each trace was evidence for the issue.
...
- Clicking the back arrow on evaluator details now returned to the tracing project or dataset you came from, instead of always going to the global evaluators list.
- Fixed a bug where hiding the Outputs column had no effect on single-experiment views without a saved column configuration.
- Context canceled errors on run query endpoints were now classified as 499 (client closed) instead of 500, reducing false alerts.
- Users could now view and assign resource tags to integrations and MCP servers in Settings.
- Issue board proposed fix now rendered diff blocks with GitHub-style line highlighting instead of plain text.
- Issues agent now required at least 2 example traces before creating an issue and looked further back in history to find corroborating evidence when only 1 trace was found.
- Issue board suggested evaluators now included both Python and JavaScript versions, and switching languages in the evaluator editor preserved the suggested code.
- New `GET /v1/platform/orgs/current/members` endpoint returned paginated org members (active and pending) with per-workspace roles and glob filtering.
- Code evaluators that returned `{"score": value}` or `{"value": value}` (with optional "comment") without a named key now displayed the evaluator name as their feedback key instead of "score" or "value".
...
- Fixed HTTP 500 on the grouped comparison view for datasets where run metadata values were complex objects (dicts/lists) rather than scalars.
- Audit logs now captured dataset download events (`download_dataset`) separately from generic dataset reads. Applied to all export formats (CSV, JSONL, OpenAI, OpenAI FT).
- Fixed duplicate tracing projects appearing in the projects list.
- The Salesforce tool integration was now available to all tenants (no longer gated behind a feature flag).
- Fixed dataset experiment LLM calls failing when the customer's auth proxy required a non-empty JWT `sub` claim.
...
- Traces flagged for issues agent review were now automatically added to a project-specific annotation queue for team review and test harness creation.
- GitHub links in the datasets table now supported right-click to open in a new tab or copy the link address.
- Issues now recorded an `open_pr` event when a user opened a PR, enabling full PR lifecycle tracking in the issue activity timeline.
- Members API (`GET /v1/platform/orgs/current/members`) now included `linked_login_methods` (auth provider info) and `scim_groups` (SCIM group memberships) on each member.
- ABAC for runs:create on the POST /runs endpoint.
- Skills Library could now be served from the template API behind the `fleet_template_api` feature flag. No user-visible behavior change on either side of the flag — the flag is OFF by default.
- Polly now used the built-in time range picker for time-based queries instead of embedding absolute timestamps in the filter string.
- Agent Templates page now loaded templates from the new template API and auto-installed any required skills when creating an agent from a template — both from the templates page directly and from agents generated through the GPA chat. Behind feature flag during rollout.
- Enforce directional trace tier permissions: users needed `projects:increase-trace-tier` to raise retention and `projects:decrease-trace-tier` to lower it.
- Dataset description tooltips in the table now only appeared when the text was actually truncated and showed formatted GitHub links when they did.
- Dataset detail page description tooltips now rendered GitHub links with a clickable icon, matching the table view.
- Bulk export destinations now supported `config_kwargs_s3` for configuring S3 addressing style, fixing compatibility with S3-compatible services that required virtual-hosted addressing (e.g., Volcengine TOS).
- Annotation queues list view now supported sorting by name, total runs, updated at, and created at columns.
- Polly used the built-in time range picker for time-based queries instead of embedding absolute timestamps in the filter string.
- Enterprise SSO users could now log in from the main login page by entering their email, without needing their organization's SSO URL.
- ABAC: runs:create for POST /runs/batch.
- The batch trace messages endpoint (`POST /v2/traces/messages`) now returned `feedback_stats` per trace, eliminating the need to also query the runs endpoint for feedback data.
- Improved log signal-to-noise ratio in the Deployments UI by filtering out logs from an internal server process.
- Improved playground lag on keystroke caused by per-keystroke re-renders of the entire prompt editor component tree.
- Agentify banner in agent builder chat could again be dismissed with an X button.
...
- Markdown tables in Fleet chat now rendered with rounded corners, styled headers, and copy/download-as-CSV buttons.
- "View source run" on evaluator traces worked for dataset-scoped evaluators (previously it produced an *Invalid trace ID* page).
- Replaced the pre-run Python fetch script with direct CLI calls driven by the agent, requiring langsmith-cli ≥ v0.2.17.
...
- Fixed evaluator detail page traces/runs toggle reverting to "Traces" when a row was clicked.
- Fleet agents could now generate HTML slide decks that rendered natively in the in-app presentation viewer, no setup required.
- Issues-board-ao prompt now guided the agent toward fewer, broader subagent calls — screening inline on small scans and consolidating investigation subagents across related categories — reducing cost on large scans.
- GitHub App integration for agent builder — connected GitHub accounts via App installations for repo access.
- Fixed the "Run a skill" browser in cron schedule dialogs now lets you actually pick a skill.
...
- Slide presentation mode for HTML files was now available for all Fleet users.
...
- Evaluator prompt handles no longer include the parent dataset or session name, so moving an evaluator between containers did not leave a stale reference in its prompt handle.
- Showed a clear message when a user without `workspaces:manage` clicked Install GitHub App, instead of failing silently.
- Issue board now supported BYOK Anthropic API key — configure your own key via the settings pane.
...
- Issues-board-ao Phase 1b screening now used the new `trace messages` trajectory (input/output preview + step summary) as the first read per trace, only opening the full conversation when trajectory-level signals were ambiguous and reducing screening cost without changing coverage.
...
- Fixed the layout of the few-shot corrections config on the evaluator page so the template description read as one flowing sentence instead of breaking into stacked columns.
...
- Corrected feedback wouldn't update the chip, just the popover. This fix updates both.
