...
A factory function that reads `configurable.model` keeps the graph model-agnostic, but you can also hardcode the model in the graph when it should always run the same one.
