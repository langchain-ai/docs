...
<Note>
**Cloud limit: 200 max active bulk exports per workspace**

On [LangSmith cloud](/langsmith/cloud), each workspace can have at most 200 active (non-terminal) bulk exports at a time. Exports in `CREATED` or `RUNNING` state count toward this limit. Exports in a terminal state (`COMPLETED`, `FAILED`, `CANCELLED`, or `TIMEDOUT`) do not count.

If your workspace reaches the limit, new export requests will be rejected until existing active exports finish or are [cancelled](/langsmith/data-export-monitor#stop-an-export). To increase this limit, contact support via [support.langchain.com](https://support.langchain.com).

[Self-hosted LangSmith](/langsmith/self-hosted) does not have this limit by default.
</Note>
