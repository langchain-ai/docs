...
SmithDB compaction workers can scale on queue depth through [KEDA](https://keda.sh/) rather than the CPU-based HPA. This suits bursty compaction backlogs, where queue depth leads CPU. LangSmith already uses KEDA to scale its deployment system, so many clusters have it installed.

Install KEDA if it is not already present:
...
Then enable the KEDA scaler for compaction workers:
