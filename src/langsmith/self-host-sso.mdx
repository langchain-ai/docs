...
* You should also register your LangSmith URL (e.g., `https://<host>`) as a **post-logout redirect URI** (sometimes called "Sign-out redirect URI") in your IdP. This allows LangSmith to fully terminate the IdP session on logout, so users must re-authenticate on their next login.
