...
* *Note*: Langsmith SSO is only supported over `https`.
...
<Warning>
SCIM is not compatible with this method of configuration. See **Via Okta Integration Network** above.
</Warning>
...
Users can sign in using the **Login via SSO** button in the LangSmith homepage.
...
This flow does *not* require a `Client Secret` - see the flow [above](#with-client-secret-recommended) for the alternative that does.
