...
* If you are **not** upgrading from basic auth or you only currently have personal orgs, you will need to provide an email to provision the initial org admin of the newly provisioned SSO org. If you are upgrading from basic auth, the existing org will be reused.
...
config:
  ...
  initialOrgAdminEmail: test@email.com # Set this if required (read above)
  oauth:
    enabled: true
