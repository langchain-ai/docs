...
If there are restrictions on the source attribute of the `sub` claim and/or the SCIM `externalId`, the `ISSUER_SUB_CLAIM_OVERRIDES` environment variable may be used to select which OIDC JWT claim is used as the `sub`.
