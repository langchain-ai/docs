...
If the groups claim is correct but LangSmith does not assign roles, check the token's `iss` claim. It must be `https://login.microsoftonline.com/<tenant-id>/v2.0`. An `iss` of `https://sts.windows.net/<tenant-id>/` means `oauthIssuerUrl` is missing the `/v2.0` suffix, and the `platform-backend` logs show `SSO groups gate skipped: no saml_providers row matched issuer`. Add the suffix as described in [Step 1. Register the application](#step-1-register-the-application).
