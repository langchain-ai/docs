...
Yes. With [Bring your own VPC (BYOVPC)](/langsmith/byoc-byovpc), you supply a VPC and separate application and database subnets across two or three matching availability zones. You can create the VPC with the reference Terraform module or use an existing VPC that meets the network requirements. Supply public subnets as well if you require public load balancers.

LangSmith validates the network before provisioning. You manage the base network, while LangChain provisions and operates the workload infrastructure inside it.
</Accordion>
...
<Accordion title="Can I remove VPC creation permissions from the BYOC role?">
Yes, when you use BYOVPC. Set `allow_vpc_creation_permissions = false` in the `langsmith-byoc-role` Terraform module and include your VPC ID in `vpc_ids`. This removes permissions to manage the base network while retaining the workload permissions LangChain needs. See [Create the reduced-permission IAM role](/langsmith/byoc-byovpc#create-the-reduced-permission-iam-role).
</Accordion>

<Accordion title="Can I move an existing data plane into my own VPC?">
No. Choose BYOVPC when creating the data plane. You cannot convert an existing data plane between a LangChain-managed VPC and BYOVPC, or change its VPC and private subnet configuration after creation.
</Accordion>
...
Apply the [`langsmith-byoc-role` Terraform module](https://github.com/langchain-ai/terraform/tree/main/modules/byoc/aws/langsmith-byoc-role) using its supported inputs, including the BYOVPC inputs when you supply your own VPC. Editing the module's policies directly is a common source of missing permissions.
...
A private RFC 1918 range between `/18` and `/16`. For a LangChain-managed VPC, LangChain provisions the VPC in that range. With BYOVPC, your VPC's primary CIDR must meet the same requirement, but you supply VPC and subnet IDs instead of a CIDR range.
...
With BYOVPC, keep `allow_vpc_creation_permissions = false`. Your customer-managed VPC and network resources remain after data plane deletion; clean them up separately when they are no longer needed.
</Accordion>
...
Yes. With BYOVPC, you manage your own firewall and egress controls. LangChain-managed Network Firewall is not supported in that mode. Your network must continue to meet the [BYOVPC network requirements](/langsmith/byoc-byovpc#meet-the-network-requirements).
