...
Unlike traditional software, building agents at scale involves non-trivial, usage-based costs. LangSmith automatically captures LLM token usage and costs for the most popular providers, and also lets you send custom cost data for anything else

This gives you one place for **unified cost tracking** across your entire application to help you monitor and debug spend.
...
There are several places in the UI where you can inspect usage and spend for LLM calls and other runs.
...
- **Output**: Tokens in the response from the model. Subtypes include: reasoning tokens, text tokens, image tokens, etc
- **Other**: Costs from tool calls, retrieval steps or any custom runs. These are further broken down by run name.
...
<AccordionGroup>
  <Accordion title="In the trace tree">
    ...
    Open any run inside a tracing project to view it's trace tree.
    <img
    className="block dark:hidden"
  ...
  <Accordion title="In dashboards">
    Dashboards help you explore cost and token usage trends over time. The [prebuilt dashboard](/langsmith/dashboards/#prebuilt-dashboards) for a tracing project shows total costs, tokens and breakdowns by cost/token type.
    **[SCREENSHOT]**

    You may also configure cost tracking charts in [custom dashboards](https://docs.langchain.com/langsmith/dashboards#custom-dashboards).
  </Accordion>
</AccordionGroup>
...
To automatically compute cost from token usage, you need to provide **token counts**, the **model and provider** and the **model price**.
...
Extract the usage information from the outputs of your traced function, and **set a `usage_metadata` field on the run's metadata**. The advantage of this approach is that you do not need to change your traced function’s runtime outputs
...
The cost for a run is computed greedily from most-to-least specific token type. Suppose we set a price of \$2 per 1M input tokens with a detailed price of \$1 per 1M `cache_read` input tokens, and \$3 per 1M output tokens. If we uploaded the following usage metadata:
...
When using a custom model, the following fileds need to be specified in a [run's metadata](/langsmith/add-metadata-tags) in order to associate token counts with costs. It's also helpful to provide these metadata fields to identify the model when viewing traces and when filtering.
...
If your model follows a non-linear pricing scheme, we reccomend calculating costs client side and sending them to LangSmith as `usage_metadata`.
...
You can also send cost information for any non-LLM run type, such as: tool calls, external APIs, etc. Similar to LLM runs, you can send the cost information in the `usage_metadata` field of the run.
