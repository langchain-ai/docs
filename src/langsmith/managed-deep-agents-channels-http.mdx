...
`provider` is a stable name for the external service, such as `orders`. It identifies the service for [identity](/langsmith/managed-deep-agents-identity) and reply routing, and it is separate from the channel name. Declaring the channel registers the provider, so any non-empty name works.
