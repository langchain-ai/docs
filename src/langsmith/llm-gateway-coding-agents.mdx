...
## Group calls into conversation threads

The gateway maps recognized coding-agent session headers to LangSmith `thread_id` metadata. Calls with the same ID belong to the same conversation thread within a tracing project.

Automatic detection depends on the incoming API format, not the destination provider. It works across all compatible providers, including custom provider routes. For fusion requests, the gateway uses the original incoming format, before translation:

| Incoming API format | Client | Headers checked in order |
| --- | --- | --- |
| Anthropic Messages | Claude Code | `X-Claude-Code-Session-Id` |
| OpenAI Chat Completions or Responses | Codex | `thread-id`, `thread_id`, `session-id`, `session_id` |

The gateway uses the first nonblank header value after trimming whitespace. For Codex, thread IDs take precedence over session IDs; underscore variants support older clients.

To set a thread ID explicitly, send `X-Gateway-Thread-Id` or set `thread_id` in the JSON object in `X-Gateway-Metadata`. A nonblank explicit value overrides automatic detection. If both metadata headers specify a thread ID, `X-Gateway-Metadata` takes precedence.

For other incoming API formats, set the thread ID explicitly. If no explicit ID or recognized session header is present, the gateway does not add `thread_id` metadata.
