...
Automatic detection depends on the destination provider, not the incoming API format:

| Destination provider | Client | Headers checked in order |
| --- | --- | --- |
| Built-in Anthropic provider | Claude Code | `X-Claude-Code-Session-Id` |
| Built-in OpenAI provider | Codex | `thread-id`, `thread_id`, `session-id`, `session_id` |
...
For other destination providers or custom provider routes, set the thread ID explicitly. Using an OpenAI- or Anthropic-compatible request format alone does not enable automatic detection. If no explicit ID or recognized session header is present, the gateway does not add `thread_id` metadata.
