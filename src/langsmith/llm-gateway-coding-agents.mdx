...
Replace `YOUR_LANGSMITH_KEY_HERE` with your LangSmith API key. Restrict the file's permissions so only your user account can read it:

```bash
chmod 600 ~/.claude/langsmith.settings.json
```
