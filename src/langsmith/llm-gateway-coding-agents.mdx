...
Claude Code supports two separate authentication methods. Use the standard setup with a workspace provider key, or use Claude subscription OAuth with a personal Claude Plus or Max subscription.

### Use a workspace provider key

Set `ANTHROPIC_API_KEY` to your LangSmith API key. Claude Code reads these environment variables automatically, so no code changes are required.
...
The model IDs are examples. You can map each tier to any model available through your gateway configuration.

<a id="use-claude-subscription-oauth"></a>
### Use Claude subscription OAuth (Claude Plus and Max only)

<Note>
This is a separate setup from the standard workspace provider key method above. Claude subscription OAuth is only available to users with an active Claude Code Plus or Max subscription. It lets Anthropic bill LLM calls to the user's personal subscription rather than the workspace's provider key. If you are using a workspace Anthropic API key, use the **Use a workspace provider key** method instead.
</Note>
