...
Automatic detection works across all providers, request paths, and API formats. The gateway checks these headers in order:
...
The gateway trims whitespace, skips blank values, and uses the first nonblank header value.
...
If no explicit ID or recognized session header is present, the gateway does not add `thread_id` metadata.
