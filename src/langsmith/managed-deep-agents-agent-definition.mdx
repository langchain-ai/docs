...
Set `response_format` in Python or `responseFormat` in TypeScript when the agent must return data that matches a schema instead of an unconstrained text response. See [Structured output](/oss/langchain/structured-output).
