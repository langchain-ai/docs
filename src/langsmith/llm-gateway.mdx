...
description: Call models across providers through unified APIs while enforcing spend limits, protecting data, and centrally managing provider credentials.
mode: wide
---
...
Use the [unified API](/langsmith/llm-gateway-unified-endpoint) as the standard way to access models through the gateway. Choose the OpenAI Chat Completions, Anthropic Messages, or OpenAI Responses format, then call models from any configured provider through one gateway URL:
...
1. Translates the request and response when the unified API format differs from the provider's native format.
...
<CardGroup cols={2}>
  <Card
    ...
    href="/langsmith/llm-gateway-unified-endpoint"
    arrow="true"
  >
    Use one standard API format to call models across configured providers. Recommended for most model calls.
  </Card>
...
If your LangSmith account is on a regional instance, use the corresponding regional gateway base URL. For the unified API, append `/v1`. For direct model access, append the provider path.
