...
curl https://gateway.smith.langchain.com/v1/chat/completions \
    ...
    -d '{"model":"anthropic/claude-sonnet-4-6","messages":[{"role":"user","content":"Explain what an LLM gateway does in one sentence."}]}'
```
...
To route a single model instead of the whole process, or to see which integrations are supported, see [Use the gateway with LangChain](/langsmith/llm-gateway-langchain).
...
Open [LangSmith](https://smith.langchain.com) and go to the tracing project named `gateway` or `gateway-<short_api_key>-<api_key_id>` in your workspace. Your request appears there with its token counts, cost, and latency.
...
These examples use the US gateway. For the EU, APAC, and AWS hostnames, see [Use a regional gateway](/langsmith/llm-gateway-api-formats#use-a-regional-gateway). For BYOC, see [Use a BYOC data plane](/langsmith/llm-gateway-api-formats#use-a-byoc-data-plane).
...
- [Use the gateway with LangChain](/langsmith/llm-gateway-langchain): route LangChain chat models and Deep Agents with an environment variable or a model prefix.
