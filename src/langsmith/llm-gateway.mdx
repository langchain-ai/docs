...
Set `model` to a provider-prefixed bring-your-own-key ID such as `openai/gpt-5.4-mini` or `anthropic/claude-opus-5`, or use a [Gateway Credits](/langsmith/llm-gateway-langchain-provider) model slug such as `moonshotai/kimi-k3`. The model ID determines the upstream route. When the selected provider uses a different native format, the gateway translates the request and response.
...
| [Gateway Credits](/langsmith/llm-gateway-langchain-provider) | LangChain owns the upstream credential. | No provider secret is required. Invocations are billed to your LangSmith account. |
