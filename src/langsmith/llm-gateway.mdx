...
On [BYOC](/langsmith/byoc), the gateway runs inside your data plane. Use the same paths behind the `/gateway` prefix on your data plane endpoint, such as `https://<data_plane_host>/gateway/v1/chat/completions`. For more information, see [Use a BYOC data plane](/langsmith/llm-gateway-api-formats#use-a-byoc-data-plane).
