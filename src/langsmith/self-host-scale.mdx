...
| **Frontend replicas**<br />(requests 500m CPU / 1Gi, limits 1 CPU / 2Gi per replica) | 1 (default) | 4 | 2 | 2 | 4 |
| **Platform backend replicas**<br />(requests 1 CPU / 2Gi, limits 2 CPU / 4Gi per replica) | 3 (default) | 20 | 3 (default) | 3 (default) | 20 |
| **Ingest queue replicas**<br />(requests 1 CPU / 2Gi, limits 2 CPU / 4Gi per replica) | 3 (default) | 24 | 3 (default) | 6 | 24 |
| **Backend replicas**<br />(requests 1 CPU / 2Gi, limits 2 CPU / 4Gi per replica) | 2 (default) | 5 | 40 | 16 | 50 |
...
<Note>
The per-replica figures above are the chart's **resource requests** and **limits**. Requests
are what the scheduler reserves; limits are the ceiling at which a container is OOM-killed.
If you override `resources` in your `values.yaml`, set the limit no lower than the value
shown, or pods may be OOM-killed under load. For the shipped defaults of every service, see
...
## Default resource requests and limits <a name="default-resource-requests-and-limits"></a>

These are the resources the Helm chart ships for each service. Replica counts for the
services not covered in the summary table above default to 1 and scale with your workload.

| Service | Requests (CPU / memory) | Limits (CPU / memory) | Default replicas |
| :--- | :--- | :--- | :--- |
| `frontend` | 500m / 1Gi | 1 CPU / 2Gi | 1 |
| `backend` | 1 CPU / 2Gi | 2 CPU / 4Gi | 2 |
| `platform-backend` | 1 CPU / 2Gi | 2 CPU / 4Gi | 3 |
| `ingest-queue` | 1 CPU / 2Gi | 2 CPU / 4Gi | 3 |
| `queue` | 1 CPU / 2Gi | 2 CPU / 4Gi | 1 |
| `playground` | 500m / 1Gi | 1 CPU / 8Gi | 1 |
| `ace-backend` | 1 CPU / 2000Mi | 2 CPU / 4000Mi | 1 |
| `host-backend` | 200m / 1000Mi | 1 CPU / 2Gi | 1 |
| `listener` | 1 CPU / 2Gi | 2 CPU / 4Gi | 1 |
| `operator` | 1 CPU / 2Gi | 2 CPU / 4Gi | 1 |

<Note>
`playground` has a memory limit of 8Gi against a 1Gi request. Size the nodes it schedules
onto accordingly.
</Note>
