...
Autoscaling is disabled by default on every service. Each one ships both an HPA block
(`autoscaling.hpa`) and a KEDA block (`autoscaling.keda`), with the same replica range in
both. A service can use HPA or KEDA, not both.
...
CPU and memory alone. See
[KEDA autoscaling for LangSmith queues](#keda-autoscaling-for-langsmith-queues).

<Warning>
**Enabling autoscaling overrides your replica count.** When either autoscaling block is
enabled for a service, the chart stops setting `replicas` on that deployment and the
autoscaler owns it. The `deployment.replicas` value you configured is no longer applied.

These maximums are generic starting points, not values matched to the load patterns in the
[summary table](#summary). High reads / high writes calls for 50 `backend` replicas, 24
`ingestQueue` and 20 `platformBackend`, against shipped maximums of 6, 10 and 10. Enabling
autoscaling without raising them scales a correctly sized deployment down.

Set `maxReplicas` at or above the replica count for your load pattern, and `minReplicas` to
the steady-state count you want to hold.
</Warning>
