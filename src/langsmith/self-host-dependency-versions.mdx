...
<Tip>
Running on a managed service (such as Amazon RDS, Google Cloud SQL, or Azure Database)? Pick a version at or above the minimum listed here. Refer to the linked "External services" pages for connection and authentication details.
</Tip>
...
| [ClickHouse](/langsmith/self-host-external-clickhouse) | 23.9 | Stores traces and feedback. ClickHouse versions >= 24.2 require LangSmith v0.6 or later. Downgrades are not supported. |
...
- PostgreSQL `>= 14` — see [Connect to an external PostgreSQL database](/langsmith/self-host-external-postgres#requirements).
- Redis `>= 5` and Valkey `8` — see [Connect to an external Redis or Valkey database](/langsmith/self-host-external-redis#requirements).
- ClickHouse `>= 23.9` — see [Connect to an external ClickHouse database](/langsmith/self-host-external-clickhouse#requirements).
- Kubernetes cluster prerequisites — see [Self-host LangSmith on Kubernetes](/langsmith/kubernetes#prerequisites).

If you are unsure which version is currently running, contact your database administrator or refer to your cloud provider's console. If you need to upgrade a dependency before upgrading LangSmith, refer to your vendor's documentation; LangChain does not manage upgrades for external services.
