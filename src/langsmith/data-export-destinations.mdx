...
<Note>**Available only on GCP SaaS deployments.**</Note>

Use `PATCH /api/v1/bulk-exports/destinations/{destination_id}` to switch an existing destination between authentication modes without recreating it. LangSmith supports two mutually exclusive modes:
...
To switch from one mode to the other, provide the new auth field in the PATCH body. Setting one mode automatically clears the other.

### Switch from static credentials to AWS IAM role

Provide `aws_role_arn` in the PATCH body. LangSmith will assume the specified role and discard any previously stored credentials.
...
Provide a `credentials` object in the PATCH body. LangSmith will use the new static credentials and clear the stored role ARN.
...
LangSmith validates the new auth configuration by performing a test write to the bucket before saving. The request fails with `400` if the new configuration does not have sufficient write permissions.
