...
5. You will need to enable egress to two control plane URLs. The listener polls these endpoints for deployments (use the pair that matches your LangSmith region—GCP EU: `eu.api.host` / `eu.api.smith`; GCP APAC: `apac.api.host` / `apac.api.smith`; AWS US: `aws.api.host` / `aws.api.smith`):
...
4. Configure your `langgraph-dataplane-values.yaml` file.
    ```bash
      config:
        ...
        hostBackendUrl: "https://api.host.langchain.com" # Override for GCP EU (`eu.api.host...`), GCP APAC (`apac.api.host...`), or AWS US (`aws.api.host...`)
        smithBackendUrl: "https://api.smith.langchain.com" # Override for GCP EU (`eu.api.smith...`), GCP APAC (`apac.api.smith...`), or AWS US (`aws.api.smith...`)
