...
<Frame caption="The Error Analysis prebuilt uses filter attributes to focus on traces where the agent made an error (like incorrect tool usage, hallucination, or planning failures) and generates insights on only these traces"><img src="/langsmith/images/insights-filter-by-attributes.png" /></Frame>
...
**How it works:**
- Add `"filter_by": true` to any boolean attribute in your attribute schema
...
- If multiple `filter_by` attributes are defined, traces must satisfy ALL conditions (AND logic)
