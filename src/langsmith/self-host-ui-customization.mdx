...
description: Replace the LangSmith logo and the support contact information in self-hosted deployments.
---

## Custom logo

Self-hosted deployments can show your own logo in place of the LangSmith logo. The logo appears in the navigation sidebar and on error screens, next to the LangSmith wordmark.
...
### Use a different logo for each color scheme

A single-color logo is often unreadable in one of the two themes. Set a separate logo for each:
...
Each name refers to the scheme the logo appears in, so `darkModeLogoUrl` is the logo drawn on a dark background. When you set only one of the three values, LangSmith uses it in both schemes. Requires Helm chart version 0.17.0-rc.38 or later.
...
<Note>
When your values already set `frontend.deployment.volumes` or `frontend.deployment.volumeMounts`, append to those lists. Replacing them removes mounts that other settings depend on, such as the writable volumes that a read-only root filesystem requires.
</Note>

A ConfigMap holds up to 1 MB, so SVG suits this best. When you serve LangSmith under a base path, include it in the value: `/<base-path>/logos/acme.svg`. For the full list of Helm values, see [Environment variables](/langsmith/self-host-environment-variables).
