...
<CodeGroup>
```bash pip
pip install -qU "langsmith[sandbox]"
```
...
with client.sandbox() as sandbox:
    sandbox.run("pip install -qU langsmith", timeout=120, env=tracing_env)
...
const sandbox = await client.createSandbox();
try {
  await sandbox.run("pip install -qU langsmith", { timeout: 120, env: tracingEnv });
