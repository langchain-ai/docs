...
<Tip>
**Prerequisites**
You have added your own [custom auth middleware](/langsmith/custom-auth) that populates the authenticated user, making it accessible via `runtime.server_info.user` for every node in your graph.
</Tip>
...
def mcp_tools_node(state, runtime: Runtime):
    user = runtime.server_info.user  # [!code highlight]
    # access user["github_token"], user["email"], etc.
