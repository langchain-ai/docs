...
In both the [Python](#python) and [TypeScript](#typescript) SDKs, attachments can be added to your traces by specifying the MIME type and binary content of each file. This guide explains how to define and trace attachments using the `Attachment` type in Python and `Uint8Array` / `ArrayBuffer` in TypeScript.
...
Decorate a function with `@traceable` and include your `Attachment` instances as arguments. The following code block demonstrates loading file bytes manually and wrapping them in `Attachment`and passing a `Path` object with `dangerously_allow_filesystem=True`:
...
In the T[ypeScript SDK](/langsmith/smith-js-ts-sdk), you can add attachments to traces by using `Uint8Array` or `ArrayBuffer` as data types. Each attachment's MIME type is specified within `extractAttachments`:
