...
The **Build** tab appears only in an agent-based workspace. To build an agent from files on your own machine instead, see [Managed Deep Agents](/langsmith/managed-deep-agents-overview).
...
Two other controls reach the same form. **+ Agent** above the agent list opens the **New Agent** panel, whose **Builder** option, "Build with AI inside LangSmith", opens this tab on a blank form. For the panel's other three options, see [Create an agent](/langsmith/navigate-agents#create-an-agent).
...
Two things are gated. Without edit access on the agent, the form opens **View only**. Renaming needs permission to update the agent's tracing projects, and the **Name** field says so when you lack it.
...
An agent built in the UI has no deployment, because it runs on a Studio runtime instead of one you deployed. It has no **Deployments** section on its Overview, and no deployment can be created for it. The workspace's other deployments are hidden while that agent is selected.
