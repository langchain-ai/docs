...
This page covers how to set up and configure Engine, control costs, and work through the fix and evaluation loop.
...
You can change any of these choices later from the [**Engine Settings**](#configure-engine) panel.
...
Engine can notify a Slack channel or an HTTP webhook endpoint when it opens a new issue, links a new trace to an existing issue, or fails to complete a run. Add destinations under **Notifications** in the [**Engine Settings**](#configure-engine) panel. For Slack setup, webhook delivery and signing, and the event payload reference, see [Engine notifications](/langsmith/engine-notifications).
