...
You can mark the issue resolved directly from here once you're done, a shortcut for [resolving from Close](#close-resolve-mark-as-incorrectly-flagged-or-reopen-an-issue), or discard the fix to abandon it without resolving the issue. Discarding also stops watching the issue if it was being watched.

<Note>
Fixing is only available for open issues: [reopen](#close-resolve-mark-as-incorrectly-flagged-or-reopen-an-issue) a resolved or incorrectly flagged issue first.
</Note>
...
Watching keeps an issue open for monitoring without resolving it or marking it as incorrectly flagged, useful when you're not ready to fix it yet but still want to know if it keeps happening. Engine confirms it's watching, and from there you can set up a Slack alert for the issue via **Alert me via Slack**, which jumps directly to the **Notifications** section of [Engine settings](#configure-engine).

If new traces link to a watched issue, Engine surfaces an alert with how many new traces arrived and moves the issue to the top of your list, so you can act on the recurrence: pick up the fix, or keep watching.
...
### Close: resolve, mark as incorrectly flagged, or reopen an issue

Closing records the outcome of your review: **Resolve** marks the issue as fixed, **Incorrectly Flagged** dismisses it as not real or not worth fixing. You can optionally provide a reason for either, which feeds back into Engine's analysis.

A closed issue can be reopened at any time, which clears any fix in progress and stops watching the issue if it was being watched. Engine also reopens an issue automatically if it detects the same problem recurring in a later trace.
