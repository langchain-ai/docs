...
Setting up Engine is a two-step process: an [Organization Admin](/langsmith/rbac#organization-admin) first enables Engine for the [workspace](/langsmith/administration-overview#workspaces), then any user can turn on Engine for each tracing project or agent environment.
...
Once Engine is enabled, any team member in your organization can set it up for their tracing projects or agent environments.
...
- **Per-project limit**: Open the **Engine** tab in a tracing project or agent environment, click the **Engine Settings** <Icon icon="settings"/> icon, and set a limit under **Monthly LCU spend limit**.
