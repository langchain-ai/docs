...
<Steps>
  ...
  <Step title="Choose an analysis level">
    Under **Analysis level**, choose how many of your project's traces Engine analyzes. The default, **Standard**, analyzes more of your eligible traces for fuller coverage. **Reduced** monitors fewer traces at a lower cost. **Expanded** gives maximum coverage and is available only for projects with enough tracing volume. The dialog's estimated monthly cost range updates to match the level you choose.
  </Step>
  <Step title="Focus on specific traces (optional)">
    Under **Focus on specific traces**, narrow Engine's attention to a subset of runs by run name or metadata. Leave it empty to analyze all traces. See [Focus on specific traces](#focus-on-specific-traces).
  </Step>
...
- **Engine spend**: View the month-to-date Engine LCU spend for this project. Click **Set limit** to cap monthly spend. See [Set spend limits and monitor usage](#set-spend-limits-and-monitor-usage).
- **Analysis level**: Choose **Reduced**, **Standard**, or **Expanded**. **Reduced** reviews fewer traces at a lower cost. **Expanded** gives maximum coverage and is available only for projects with enough tracing volume.
- **Focus on specific traces**: Narrow Engine's attention to a subset of runs by run name or metadata. See [Focus on specific traces](#focus-on-specific-traces).
- **Notifications**: Add Slack channel or webhook destinations, each with its own event types and minimum priority. See [Engine notifications](/langsmith/engine-notifications).
...
Click any issue in the list to open its detail panel. At the top, a diagnosis describes the problem and its impact. Each issue has a toolbar for setting its priority, closing or reopening it, opening a pull request, creating a Linear issue, and watching it.
