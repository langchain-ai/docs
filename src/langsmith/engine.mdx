...
## Manage issues via the CLI

You can fetch and update issues programmatically using the [LangSmith CLI](/langsmith/cli).

```bash
# List issues for a project
langsmith project issues list --project <project-name>

# Update an issue's proposed fix (use the issue ID returned by list)
langsmith project issues update <issue-id> --proposed-fix "Root cause: ..."
```
