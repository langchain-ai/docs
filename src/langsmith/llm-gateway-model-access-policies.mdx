...
A model access policy defines which providers and models are permitted through the [LLM Gateway](/langsmith/llm-gateway). Requests to providers or models not included in the policy are blocked with a `403` response. If no policy applies, all providers and models are available.

Model access policies apply to direct provider integrations only. Custom providers and saved model configurations are not supported and are always blocked when a model access policy is active.

## Policy configuration

A model access policy lists one or more providers, each with an access mode:

- **All models**—every model offered by the provider is permitted.
...
| Tier | Scope |
| --- | --- |
| Organization | Applies to all users and workspaces in the organization. |
| Workspace | Applies to all users in a workspace. |
| User | Applies to a single user. |
| API key | Applies to a single API key. |

### Policy overrides

Policy overrides let you grant a more specific subject different access than the broader default. A common case is giving a particular API key access to a premium model that is not available to the rest of the organization.

When a request matches policies at multiple tiers, only the most specific tier applies:

```
API key > User > Workspace > Organization
```

The more specific policy replaces the broader one entirely. For example, if the organization policy permits OpenAI and Anthropic, and an API key policy permits only OpenAI and Gemini, requests using that key may access OpenAI and Gemini only. If multiple policies match at the same tier, a model must be permitted by all of them to be accessible.

## No policy

If no model access policy applies to a request, there are no restrictions. All configured providers and models are available.
