...
| **Analytics:**<br/>• [Charts and dashboards](#organization-charts-and-dashboards): Org-level visualizations<br/>• [Usage and analytics](#usage-and-analytics): Usage tracking and TTL settings<br/>• [Model configurations](#organization-model-configurations): Org-level model configurations<br/>• [Organization secrets](#organization-secrets): Provider keys for org configurations | **Workspace management:**<br/>• [Model price map](#model-price-map): Per-token model prices<br/>• [Workspace settings](#workspace-settings-and-management): Members, settings<br/>• [Tags](#tags): Metadata tagging system<br/>• [Bulk Exports](#bulk-exports): Data export operations |
...
### Organization model configurations

[Model configurations](/langsmith/model-configurations#organization-scoped-configurations) defined once at the organization level and available in every workspace.

| Operation | Org Admin | Org Operator | Org User | Org Viewer | Required Permission |
|-----------|:---------:|:------------:|:--------:|:----------:|---------------------|
| Create an organization model configuration | ✓ | ✓ | ✗ | ✗ | `organization:manage-model-configs` |
| Update an organization model configuration | ✓ | ✓ | ✗ | ✗ | `organization:manage-model-configs` |
| Delete an organization model configuration | ✓ | ✓ | ✗ | ✗ | `organization:manage-model-configs` |
| Change feature availability for an organization model configuration | ✓ | ✓ | ✗ | ✗ | `organization:manage-model-configs` |

<Note>
Viewing an organization configuration happens through a workspace, and the list a workspace returns includes both scopes, so reads require the workspace `prompts:read` permission rather than an organization permission. The organization settings pages that manage these configurations also require `organization:manage`.
</Note>

### Organization secrets

Provider API keys that [organization model configurations](/langsmith/model-configurations#organization-secrets) reference. A workspace configuration never reads them, and an organization configuration never reads workspace secrets.

| Operation | Org Admin | Org Operator | Org User | Org Viewer | Required Permission |
|-----------|:---------:|:------------:|:--------:|:----------:|---------------------|
| List organization secret names | ✓ | ✓ | ✓ | ✓ | `organization:read` |
| Add or update an organization secret | ✓ | ✓ | ✗ | ✗ | `organization:manage-model-secrets` |
| Delete an organization secret | ✓ | ✓ | ✗ | ✗ | `organization:manage-model-secrets` |

<Note>
Secret values are write-only. Listing returns names only, and no endpoint returns a stored value.
</Note>
