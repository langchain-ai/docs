...
The SmithDB-backed endpoints carry dedicated rate limits that are higher than the limits on the endpoints they replace, so migrating raises the request throughput available to your application.

| Endpoint | SDK method | Limit |
|---|---|---|
| `POST /api/v2/runs/query` | `client.runs.query()` | 300 requests per 10 seconds |
| `GET /api/v2/runs/{run_id}` | `client.runs.retrieve()` | 300 requests per 10 seconds |

These limits apply in Cloud, per API key for requests authenticated with an API key and per user for requests authenticated as a user.

The deprecated `POST /api/v1/runs/query` endpoint behind `list_runs` is limited to 15 requests per 10 seconds per API key, and is additionally subject to per-tenant limits that vary by query shape. The SmithDB-backed query method is not subject to those per-tenant limits. For the deprecated endpoint limits, see [Query traces using the SDK](/langsmith/export-traces#rate-limits).
