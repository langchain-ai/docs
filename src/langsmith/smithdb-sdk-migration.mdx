...
The following methods are discontinued. They call the retired `/feedback/formulas` endpoints, which return `410 Gone` on composite-feedback v2 and are scheduled for complete removal. Composite scores are now managed as code evaluators with run rules. There is no SDK replacement.
