...
Data planes are provisioned for high availability by default. The deployment spans multiple availability zones in your region, and RDS and ElastiCache run Multi-AZ with automatic failover. With [BYOVPC](/langsmith/byoc-byovpc), supply subnets across two or three matching availability zones.
...
- **VPC flow logs**: Recorded in an S3 bucket in your account. With BYOVPC, you configure and maintain flow logs, either through the VPC reference module or your own logging configuration.
