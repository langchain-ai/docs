...
Agent Server runs the TCK on every CI build as a required check. Every known failure is listed,
with a reason and an owner, in
[`a2a-conformance-baseline.yml`](https://github.com/langchain-ai/langgraph-api/blob/main/a2a-conformance-baseline.yml).
CI fails if a new failure appears, and also if a listed requirement starts passing, so the file
cannot drift from reality.

Read it before you build against a capability. The largest current group is the response wire
shape: Agent Server declares v1.0 but still emits the v0.3 `kind` discriminator and part shape.
