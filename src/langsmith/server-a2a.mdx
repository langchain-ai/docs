...
- Streaming does not support `historyScope: "task"`. `SendStreamingMessage` rejects it with `-32602`, because a streamed response cannot provide a final task-scoped snapshot.
...
- Option names are case-sensitive. `historyscope` returns `-32602` with `Unknown history option 'historyscope'; use 'historyScope'`.
