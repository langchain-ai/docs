...
BYOC uses a split responsibility model: the control plane runs in LangChain's cloud, and the data plane runs entirely in your cloud environment. For the full breakdown of what each side manages, see the [BYOC shared responsibility model](/langsmith/byoc-shared-responsibility).
