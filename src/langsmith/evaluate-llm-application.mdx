...
In this guide we'll go over how to evaluate an application using the LangSmith SDK. For API reference, see the [Python](https://reference.langchain.com/python/langsmith/evaluation), [JavaScript/TypeScript](https://reference.langchain.com/javascript/langsmith/evaluation), or [Java](https://javadoc.io/doc/com.langchain.smith/langsmith-java/0.1.0-beta.11/com/langchain/smith/evaluation/package-summary.html) evaluation APIs.
...
In Java use `Evaluation.evaluateAsync()` for non-blocking evaluation.
...
// Evaluation.evaluate() traces this automatically when uploadResults is true (the default).
Function<Map<String, Object>, Map<String, Object>> toxicityClassifier =
    inputs -> Map.of("class", "Not toxic");
```

</CodeGroup>

We've optionally enabled tracing to capture the inputs and outputs of each step in the pipeline. In Java, `Evaluation.evaluate()` does this for the target automatically — use [`Tracing.traceFunction`](/langsmith/annotate-code) only if you need a custom run name or nested traces inside your app. For Python and TypeScript, see [Custom instrumentation](/langsmith/annotate-code).
...
Dataset dataset =
    client.datasets().create(
        DatasetCreateParams.builder().name("Toxic Queries").build());
...
for (String[] row : labeledTexts) {
  client.examples().create(
      ExampleCreateParams.builder()
          .datasetId(dataset.id())
          .inputs(
              ExampleCreateParams.Inputs.builder()
                  .putAdditionalProperty("text", JsonValue.from(row[0]))
                  .build())
          .outputs(
              ExampleCreateParams.Outputs.builder()
                  .putAdditionalProperty("label", JsonValue.from(row[1]))
                  .build())
          .build());
}
```
...
RunEvaluator correct =
    DynamicRunEvaluatorKt.runEvaluatorFromOutputsReference(
        (outputs, reference) ->
            Objects.equals(outputs.get("class"), reference.get("label")) ? 1.0 : 0.0);
```

</CodeGroup>

<Note>
If you build a custom `RunEvaluator` that returns `EvaluationResult` directly (for example via `DynamicRunEvaluatorKt.runEvaluator(...)`), pass `Map.of()` for `evaluatorInfo` when constructing the result from Java. Kotlin default arguments are not applied from Java callers.
</Note>
...
EvaluateParams params =
    EvaluateParams.builder()
        .data(dataset.name())
        .experimentPrefix("gpt-5.4-mini, baseline")
        .description("Testing the baseline system.")
        .maxConcurrency(4)
        .metadata(
            Map.of(
                "models", List.of("openai:gpt-5.4-mini"),
                "prompts", List.of("my-org/my-eval-prompt:abc12345")))
        .addEvaluator(correct)
        .build();

ExperimentResults results = Evaluation.evaluate(client, toxicityClassifier, params);
```
...
The Java SDK uses `EvaluateParams.builder()` to configure an evaluation. Common options:

| Option | What it does |
| ------ | ------------ |
| `data(...)` | **Required.** Dataset name, dataset id, or a list of in-memory examples. |
| `addEvaluator(...)` | Score each row. You can add more than one. |
| `addSummaryEvaluator(...)` | Score the whole experiment after all rows finish. |
| `experimentPrefix(...)` | Short label for the experiment name. LangSmith adds a unique suffix. |
| `experimentName(...)` | Set the full experiment name. Do not use with `experimentId`. |
| `experimentId(...)` | Add more runs to an existing experiment. |
| `maxConcurrency(...)` | Rows to test at once. `0` runs one at a time. |
| `numRepetitions(...)` | Run each test case multiple times. |
| `uploadResults(false)` | Run locally without saving to LangSmith. |
| `errorHandling(...)` | What to do when your app fails on a row (`LOG` or `IGNORE`). |

<Note>
To re-score runs from a prior experiment, prefer `Evaluation.evaluateExisting()` (see below). The `Evaluation.evaluate(client, experimentId, params)` overload also exists, but `EvaluateParams` still requires `.data(...)` even when you are not re-running your app.
</Note>

### Run in the background

Use `Evaluation.evaluateAsync()` when you do not want to wait for every row to finish:
...
CompletableFuture<ExperimentResults> future =
    Evaluation.evaluateAsync(client, toxicityClassifier, params);
ExperimentResults results = future.join();
```

### Re-score an existing experiment

Add new scores without re-running your app:
...
Evaluation.evaluateExisting(
    client,
    "your-experiment-id",
    EvaluateExistingParams.builder().addEvaluator(correct).build());
```

See [Evaluate an existing experiment](/langsmith/evaluate-existing-experiment) for more.

### Compare two experiments

Pick a winner between two versions side by side:

```java
import com.langchain.smith.evaluation.EvaluateComparativeParams;
import java.util.List;

Evaluation.evaluateComparative(
    client,
    List.of(experimentAId, experimentBId),
    EvaluateComparativeParams.builder().addEvaluator(myComparativeEvaluator).build());
```

See [Run a pairwise evaluation](/langsmith/evaluate-pairwise) for more.

### Test without uploading

Set `uploadResults(false)` for quick local smoke tests. Nothing is saved to LangSmith.
...
EvaluateParams params =
    EvaluateParams.builder()
        .data(dataset.name())
        .experimentPrefix("gpt-5.4-mini, simple")
        .description("Testing the baseline system.")
        .maxConcurrency(4)
        .addEvaluator(correct)
        .build();
