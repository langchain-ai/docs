...
As you specify more filters, you can click each filter individually to update attributes your searching on.
...
You can remove filters are needed from filter path, which will widen the search to the remaining filters.
...
To filter based on key-value pairs, Ffor example, to match the following input:
