...
<Steps>
  ...
  <Step title="Verify your hostname is externally reachable" id="verify-your-hostname-is-externally-reachable">
    ...
    Serve that hostname through your ingress with TLS, as described in [Set up an ingress](/langsmith/self-host-ingress). Engine does not require you to expose anything beyond the address your own users already reach. Sandbox egress is restricted to a fixed allowlist: your LangSmith hostname, `github.com`, `api.github.com`, and the PyPI and npm registries. The allowlist is not configurable, so Engine installs dependencies from those public registries rather than an internal mirror. Go module proxies are excluded, and fix runs use `vendor/` when it is present. Per-run credentials are injected by a proxy outside the sandbox rather than being readable inside it.
  </Step>
...
Connecting a GitHub repository is optional and improves Engine's diagnosis and fixes. Without one, Engine detects and diagnoses issues and proposes prompt fixes, but it cannot read your source code or open pull requests.

In LangSmith Cloud, Engine uses a LangChain-managed GitHub App. Self-hosted deployments have no managed app, so create and configure your own, then reference it from `hostBackend.deployment.extraEnv`.
