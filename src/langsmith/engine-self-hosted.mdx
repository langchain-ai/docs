...
1. Any user turns on Engine for a tracing project, or an agent environment from its **Engine** tab. For more information, see [Turn on Engine for a tracing project](/langsmith/engine#set-up-engine-for-a-tracing-project).
