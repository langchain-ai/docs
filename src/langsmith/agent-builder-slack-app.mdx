...
1. Verify that **Linked Agent: \<Agent Name\>** appears next to the bot name.
...
<Accordion title="Use the default LangSmith Agent Builder Slack bot">
  You can also use the default LangSmith Agent Builder Slack bot instead of a custom app:
  ...
  1. Send any message in the channel to trigger a run.
</Accordion>
...
1. Click **+ Add**.
1. Search for "Slack" and add the tools you need:
...
- Check the thread in Agent Builder for any approvals that need human input.
