...
Point an OpenAI-compatible client at `https://gateway.smith.langchain.com/v1`:
...
Point an Anthropic client at `https://gateway.smith.langchain.com`:
...
Call `GET /v1/models` to list models available across providers configured for the workspace:
...
A provider without a configured secret is omitted. Use the returned provider-prefixed ID as `model`.
...
For setup-specific resolutions, see [Troubleshoot your first request](/langsmith/llm-gateway-quickstart#troubleshoot-your-first-request).
