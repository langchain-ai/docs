...
### Use environment variables with the Anthropic SDK

To route Anthropic SDK calls through the LLM Gateway without code changes, set `ANTHROPIC_BASE_URL` and set `ANTHROPIC_API_KEY` to your LangSmith API key. The SDK reads these environment variables automatically, so you do not need to change the client constructor.

<Note>
This standard API-key setup is separate from [Claude Code subscription OAuth](/langsmith/llm-gateway-coding-agents#use-claude-subscription-oauth). Subscription OAuth uses your saved Claude Plus or Max login, requires different environment variables, and bills Anthropic usage to your Claude subscription.
</Note>

**Simplest setup (single-provider, Anthropic models only):**

Set `ANTHROPIC_BASE_URL` to the Anthropic-format gateway endpoint. Model names are inferred as Anthropic, so no provider prefix is required:
...
**Multi-provider setup with per-tier model routing:**

Set `ANTHROPIC_BASE_URL` to the standard gateway root and map each Anthropic model tier to a gateway model ID. This lets you route Opus, Sonnet, and Haiku calls to any provider:

```bash
export ANTHROPIC_BASE_URL="https://gateway.smith.langchain.com"
export ANTHROPIC_API_KEY="$LANGSMITH_API_KEY"

# Map Anthropic model tiers to any gateway-available model
...
With these variables set, existing code that calls `anthropic.claude-opus-5` (or any Anthropic tier alias) is redirected through the gateway using the model you specified.

<Note>
**This is not the Claude Code subscription OAuth setup.** The environment variable approach above uses a workspace provider key (`ANTHROPIC_API_KEY`) and is designed for any Anthropic SDK application — not specifically for Claude Code's OAuth-based subscription billing. If you are using Claude Code with a Claude Plus or Max subscription and want to route those calls through the gateway without a provider API key, see [Use Claude subscription OAuth](/langsmith/llm-gateway-coding-agents#use-claude-subscription-oauth) instead.
</Note>
