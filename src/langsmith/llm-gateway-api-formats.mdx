...
For bring-your-own-key models, set `model` to `<provider>/<model>`, such as `openai/gpt-5.4-mini` or `anthropic/claude-sonnet-4-6`. For Gateway Credits, use a hosted model slug such as `moonshotai/kimi-k3`. The request format does not limit which available model you can call.
...
Call `GET /v1/models` to list models available from providers configured for the workspace and from [Gateway Credits](/langsmith/llm-gateway-langchain-provider). The gateway returns a single OpenAI-compatible list:
