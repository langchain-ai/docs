...
If you want a zero-code-change drop-in that routes all Anthropic SDK calls through the LLM Gateway, set `ANTHROPIC_BASE_URL` and `ANTHROPIC_API_KEY` as environment variables. The SDK picks them up automatically — no client constructor changes are needed.
