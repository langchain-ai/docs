...
Each model configuration in a chain can point to a different provider and model identifier, and the gateway calls every candidate with that configuration's own model name in place of the one the client sent—on the first attempt as well as on any fallback. The request's `model` serves only to pick a chain among those matching the path's [wire format](#wire-formats); if it matches none of them, the gateway uses that format's first (default) chain.
...
A single fallback configuration can hold chains of both formats, so one route can serve both OpenAI-compatible and Anthropic-compatible clients. It doesn't have to: a configuration with only OpenAI-compatible chains returns `502` for a `/v1/messages` call, and one with only Anthropic-compatible chains returns `502` for a `/v1/chat/completions` call.
...
1. Under **Model fallback chains**, click **Add chain**, then add two to five model configurations to it in the order they should be tried. A chain's first model fixes its [wire format](#wire-formats), and only configurations of that format can be added after it. Chains are grouped by format, and the first chain in each group is that format's **default** chain, used whenever a request's model doesn't select a different one.
1. Click **Create configuration**.
...
An alias is optional and set per chain when you create the configuration. It's a caller-facing name, so a client can ask for `"model": "heavy"` without knowing which model backs it.

Each model configuration in a chain can point to a different host. So a chain can fail over across separate deployments of the same model (for example, a primary endpoint to a backup endpoint) without a single host being a point of failure.
...
- A `/v1/messages` request uses chain 3 whatever its `model` is, since it's the only Anthropic-compatible chain—chains 1 and 2 are never eligible on that path.
