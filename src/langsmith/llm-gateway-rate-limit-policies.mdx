...
Rate limit policies and [spend cap policies](/langsmith/llm-gateway-spend-policies) are complementary and can be applied together—spend caps for cost control, rate limits for throughput and abuse control.

## Policy dimensions

Rate limit policies are evaluated for every incoming request. You can set a policy as a default (applying a blanket rate limit to all users, workspaces, or API keys) or as a specific policy (an individual limit or a limit on a group of subjects).
...
2. **Specific policies** target a named subject and override the default for that subject only. Example: "The on-call engineer gets 500 requests per minute." Editing the default updates everyone still on it.
...
The following example enforces 100 requests per minute and 1,000,000 tokens per hour for a specific user:

```json
{
  "name": "100 req/min + 1M tokens/hour per user",
  "policy_type": "rate_limit",
  "subject_matchers": [{"key": "user_id", "value": "<user-id>"}],
  "config": {
    "limits": [
...
## What callers see when a limit is exceeded

When a rate limit is exceeded, the gateway returns **HTTP 429 (Too Many Requests)**:

```
API Error: 429 request blocked by gateway policies: <policy-name>
```

The response includes a **`Retry-After`** header indicating how many seconds remain until the current window resets. Clients should read this header and wait before retrying.

<Note>
For comparison, a spend cap violation returns **HTTP 402 (Payment Required)** instead of 429. See [Spend policies](/langsmith/llm-gateway-spend-policies).
</Note>

## Integration with LangSmith Engine

When a rate limit policy blocks a request, the violation is recorded as metadata on the trace. These violations surface as issues in [LangSmith Engine](/langsmith/engine), where you can click through from the issue to the trace to understand what the agent was doing when it hit the limit.

This is useful for diagnosing whether a blocked request represents a genuine abuse pattern (a user hammering the gateway in a loop) or a policy that needs adjustment (a legitimate workload that grew beyond its cap).
