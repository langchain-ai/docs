...
[LangChain](/oss/langchain/overview) chat models and [Deep Agents](/oss/deepagents/overview) can route directly through the gateway using the `LANGSMITH_GATEWAY` environment variable, without changing your model configuration. See [Use the gateway with LangChain](/langsmith/llm-gateway-langchain) for setup, supported chat models, and configuration precedence.
