...
Each supported integration appends its own provider path, so `ChatAnthropic` calls `https://gateway.smith.langchain.com/anthropic` and `ChatOpenAI` calls `https://gateway.smith.langchain.com/openai/v1`. For supported integrations, override precedence, and how to route a single model instead, see [Use the gateway with LangChain](/langsmith/llm-gateway).
