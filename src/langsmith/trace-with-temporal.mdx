...
<Note>
The OpenTelemetry exporters in the examples below derive their endpoint from `LANGSMITH_ENDPOINT` (defaults to `https://api.smith.langchain.com`).

If your account is in a region other than US (the default), set `LANGSMITH_ENDPOINT` to the API URL for your region. Without this, your API key won't be recognized and requests will fail to authenticate.
...
If you self-host LangSmith, set `LANGSMITH_ENDPOINT` to your LangSmith API URL.
</Note>
...
def init_tracer_provider() -> TracerProvider:
    ...
    # Create OTLP exporter for LangSmith. The endpoint defaults to the US region;
    # set LANGSMITH_ENDPOINT (for example, https://eu.api.smith.langchain.com)
    # so EU, APAC, and AWS US accounts export to the correct region.
...
export function initTracerProvider(): NodeTracerProvider {
  ...
  // set LANGSMITH_ENDPOINT (for example, https://eu.api.smith.langchain.com)
  // so EU, APAC, and AWS US accounts export to the correct region.
  const apiUrl = (process.env.LANGSMITH_ENDPOINT || 'https://api.smith.langchain.com').replace(/\/+$/, '');
...
2. **Check endpoint**: Confirm you're using `https://api.smith.langchain.com/otel/v1/traces`, or your regional equivalent (see the region table in [Environment variables](#environment-variables)) for non-US accounts
