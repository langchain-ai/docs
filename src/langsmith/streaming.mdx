...
<Tip>
For new applications targeting `langgraph-api>=0.9.0rc1`, we recommend [event streaming](/langsmith/event-streaming)—the typed-projection API exposed by `client.threads.stream(...)`. Event streaming gives application code separate iterators per projection (messages, values, tool calls, subgraphs, output) over a single thread subscription, instead of branching on `stream_mode` chunks. This page documents the legacy `stream_mode`-based API and remains supported for existing applications and older servers.
</Tip>
