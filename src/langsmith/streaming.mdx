...
Thread streaming opens a long-lived SSE connection to a thread and streams output from **every run** executed on that thread. The connection stays open indefinitely until the client closes it. This is different from [run streaming](#basic-usage), which creates a single run and streams its output until that run completes.

Thread streaming is useful when you want to monitor all activity on a thread from a single connection, for example in a chat UI where multiple runs may be triggered over time (follow-up messages, [human-in-the-loop](/langsmith/add-human-in-the-loop) resumptions, or [background runs](/langsmith/background-run)).
...
| Mode | Description |
|------|-------------|
| `run_modes` (default) | Streams all run events, equivalent to the output you would get from `client.runs.stream()`. |
...
### Thread streaming vs. run streaming
