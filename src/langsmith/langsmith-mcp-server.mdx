...
LangSmith Cloud now hosts an OAuth-authenticated remote MCP server at `https://api.smith.langchain.com/mcp`, with regional endpoints for GCP EU (`https://eu.api.smith.langchain.com/mcp`), GCP APAC (`https://apac.api.smith.langchain.com/mcp`), and AWS US (`https://aws.api.smith.langchain.com/mcp`). It exposes the same tool surface as the standalone server documented on this page, but authenticates via OAuth 2.1 with dynamic client registration—no API key, no separate deployment, no header configuration.
