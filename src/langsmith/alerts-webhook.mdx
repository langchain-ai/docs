...
This guide details the process for setting up webhook notifications for [LangSmith alerts](/langsmith/alerts). Webhooks enable integration with custom services and third-party platforms by sending HTTP POST requests when alert conditions are triggered. Use webhooks to forward alert data to ticketing systems, chat applications, or custom monitoring solutions.

<Callout type="info">
Before following this guide, complete the [Alerts initial setup](/langsmith/alerts). This page covers the notification configuration step of alert setup.
</Callout>

## Prerequisites

- Completed the [Alerts initial setup](/langsmith/alerts).
