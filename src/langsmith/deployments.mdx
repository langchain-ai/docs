...
<Steps>
  ...
  <Step title={<a href="/langsmith/platform-setup">Choose hosting</a>}>
    Select Cloud, Hybrid, or Self-hosted.
  </Step>
...
- **Pause and resume.** An agent calls [`interrupt()`](/langsmith/add-human-in-the-loop) and the runtime checkpoints its state, frees resources, and waits — for a human to approve something, for a condition to be satisfied, or for new input to arrive. When [`Command(resume=...)`](/langsmith/add-human-in-the-loop) is called hours or days later, execution picks up exactly where it stopped. This is the same primitive underneath [human-in-the-loop](/langsmith/add-human-in-the-loop) workflows and [time-travel debugging](/langsmith/human-in-the-loop-time-travel).
...
Agents need to show their work while they're working. The runtime provides [resumable streaming](/langsmith/streaming) that survives network interruptions — clients reconnect and pick up without losing progress. Multiple streaming modes (`values`, `updates`, `messages`) control what surfaces to the user, from full state snapshots to individual token output.
...
- [RemoteGraph](/langsmith/use-remote-graph) — Compose agents across deployments
...
- [MCP](/langsmith/server-mcp) and [A2A](/langsmith/server-a2a) protocol support
