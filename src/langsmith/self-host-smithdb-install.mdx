...
<Note>
SmithDB is opt-in. The minimum LangSmith version depends on your cloud, and what ClickHouse requires depends on your LangSmith version. See [Check cloud support](/langsmith/self-host-smithdb#check-cloud-support) and [Storage requirements](/langsmith/self-host-smithdb#storage-requirements).
</Note>
...
<Warning>
After query cutover, LangSmith trace reads no longer go to ClickHouse. On LangSmith 0.16, keep the ClickHouse datastore in place regardless.
</Warning>
...
Skip migration if historical ClickHouse data does not need to be available through SmithDB-backed queries. Existing data stays in ClickHouse and is reachable only while ClickHouse is enabled.
...
<Warning>
On LangSmith 0.16, do not disable or remove ClickHouse after query cutover. This applies to both in-cluster and external ClickHouse deployments. Preserve the existing ClickHouse configuration throughout and after this process. On LangSmith 0.17, you can disable ClickHouse once cutover is complete, but any history you did not migrate becomes unreachable when you do.
</Warning>
