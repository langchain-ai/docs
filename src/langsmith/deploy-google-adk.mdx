...
- Forwards ADK token events through LangGraph's streaming pipeline, so partial tokens show up in [`stream_mode="messages"`](/langsmith/streaming) and in [LangGraph Studio](/langsmith/studio).
...
- A LangSmith API key — see [Create an account and API key](https://docs.langchain.com/langsmith/create-account-api-key)
- A Google AI API key if you use Gemini models — see [Google AI Studio](https://aistudio.google.com/api-keys)

## Installation

Install the package with the `google-adk` extra. The extra pulls in `google-adk` itself and other dependencies needed for the wrapper:
...
This minimal example builds an "echo" agent that does not require any model API key — it short-circuits the LLM call so you can verify the deployment plumbing end-to-end before wiring in a real model.
...
For a real agent, drop the `before_model_callback` and configure a model normally — for example, Gemini via `model="gemini-2.0-flash"` with `GOOGLE_API_KEY` set, or Claude/OpenAI via ADK's LiteLLM adapter (`google.adk.models.lite_llm.LiteLlm`, available through `google-adk[extensions]`).
...
This serves the agent at `http://127.0.0.1:2024` and opens [LangGraph Studio](/langsmith/studio) so you can chat with the agent. Send a request directly with `curl`:
...
See [Deploy to cloud](/langsmith/deploy-to-cloud) for environment configuration, deployment types, and revision management. For self-hosted setups, see [Self-hosted deployments](/langsmith/self-hosted).
...
Traces show agent invocations, tool calls, and LLM interactions in the [LangSmith UI](/langsmith/observability). For more on the underlying tracing integration, see [Trace Google ADK applications](/langsmith/trace-with-google-adk).
