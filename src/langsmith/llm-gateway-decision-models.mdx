...
Decision models classify or score text and return structured answers through the [LLM Gateway](/langsmith/llm-gateway).

## Quickstart

SemIf (`semif-qwen3.5-4b`) is free through September 28, 2026. It does not require a provider key or [Gateway Credits](/langsmith/llm-gateway-credits).

Before you start:

- Use a paid, non-Enterprise [plan](/langsmith/pricing-plans) with SemIf enabled. Availability depends on your organization and region.
- Create a workspace-scoped [LangSmith API key](/langsmith/create-account-api-key) with `gateway:invoke` and `workspaces:read` [permissions](/langsmith/organization-workspace-operations).
