...
Decision models classify text and score it against a rubric through the [LLM Gateway](/langsmith/llm-gateway). Try hosted SemIf in your browser, without installing an SDK.
...
### Try SemIf in your browser

You need `gateway:invoke` and `workspaces:read` [permissions](/langsmith/organization-workspace-operations). See [Admin setup](/langsmith/llm-gateway-admin-setup) if you need access.

To send your first request:

1. Open [LangSmith](https://smith.langchain.com), go to **LLM Gateway**, and select **SemIf** from the hosted models.
2. Select **Run request**. If prompted, create a [LangSmith API key](/langsmith/create-account-api-key) to run the example.

To call from your application, use the [Python, JavaScript, or cURL examples](#call-semif) below. For your own TypeSafe provider key, see [TypeSafe](#typesafe).
...
For regional base URLs, see [Regional gateways](/langsmith/llm-gateway-direct-model-access#use-a-regional-gateway).
