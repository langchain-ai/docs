...
The gateway also supports TypeSafe decision models with bring-your-own-key (BYOK). Use your TypeSafe API key as a [provider secret](/langsmith/llm-gateway-admin-setup#1-add-provider-secrets). See [TypeSafe setup](/oss/integrations/providers/typesafe#setup) to create a key and install the LangChain integration.
