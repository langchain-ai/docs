...
An app runs in a sandbox with no network access of its own. Its requests reach the LangSmith API through the host, using the viewer's credentials. Each person sees only the data their [permissions](/langsmith/organization-workspace-operations) allow. The host forwards only LangSmith API requests, and blocks the endpoints for API keys, members, users, identities, roles, permissions, SCIM, and service accounts.
...
- **Sandboxes**: Must be enabled on your deployment, because each session runs in a [sandbox](/langsmith/sandboxes).
...
Type what the app should do into the composer, and select the model to build with from the menu beside it. The composer accepts a prompt while the sandbox behind it is still starting. When Chat starts writing files, LangSmith saves a draft and opens the **Preview** and **Code** tabs beside the chat.
</Step>
