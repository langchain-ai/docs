...
The CLI creates an `agent.py` or `agent.ts` entry, an `instructions.md` system prompt, and supporting project files.

  </Step>

  <Step title="Edit the agent and instructions">

Open `agent.py` or `agent.ts` and configure the model, tools, middleware, and interrupts. Open `instructions.md` and write the system prompt for your agent.

Add any additional authored tools, middleware, schedules, skills, connectors, messaging channels, optional identity, and an optional sandbox to the project directory.

  </Step>

  <Step title="Optionally compile and run evals">

(Optional) Compile Harbor-style [evals](/langsmith/managed-deep-agents-evals) and run them with Harbor:

```bash
mda evals compile .
```

`mda evals compile` prints a Harbor command you can run to execute the trials. For task layout, verifiers, and Harbor configuration, see [Evals](/langsmith/managed-deep-agents-evals).
...
This runs your agent in LangSmith Studio so you can test it before deploying.
...
On success, the CLI prints a URL to your deployment dashboard in LangSmith.

  </Step>

  <Step title="Inspect the deployment">

Open the deployment dashboard URL from the previous step. Inspect build status, traces, and runtime state in LangSmith.
