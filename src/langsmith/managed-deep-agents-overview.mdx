...
Managed Deep Agents runs the open source [Deep Agents](/oss/deepagents/overview) harness rather than a second framework. The agent loop, filesystem tools, subagents, and skills are the same, so there is no second API to learn and an agent you already built keeps working.

Managed Deep Agents adds the layer around the agent. The sandbox, durable memory, channels, and schedules above are declarations in a project file rather than services you build and operate. [Connections](/langsmith/managed-deep-agents-connections) run OAuth, so end users authorize their own accounts instead of you provisioning credentials for each one. LangSmith hosts the result on [Agent Server](/langsmith/agent-server-overview). In the code, the difference is narrower: who compiles the agent.

:::python
- @[create_deep_agent] returns a compiled agent. You run it, and you own the backend, store, checkpointer, and server around it.
- `define_deep_agent` returns a definition. The `mda` CLI hands it to the managed runtime, which supplies those pieces along with memory, skills, and the system prompt, then compiles the agent with @[create_deep_agent].

So `define_deep_agent` takes the same parameters minus the managed ones, plus a required static `name`. Every `define_*` function works this way: declare what the agent needs, and leave the lifecycle to the runtime.
:::

:::js
- @[createDeepAgent] returns a compiled agent. You run it, and you own the backend, store, checkpointer, and server around it.
- `defineDeepAgent` returns a definition. The `mda` CLI hands it to the managed runtime, which supplies those pieces along with memory, skills, and the system prompt, then compiles the agent with @[createDeepAgent].

So `defineDeepAgent` takes the same options minus the managed ones, plus a required static `name`. Every `define*` function works this way: declare what the agent needs, and leave the lifecycle to the runtime.
:::

Stay on Deep Agents when you run the agent yourself or need a [backend](/oss/deepagents/backends), store, or checkpointer of your own. For a side-by-side comparison of the two, and the steps to convert an agent you already built, see [Move from Deep Agents](/langsmith/managed-deep-agents-migrate).
