...
- [Context Hub](/langsmith/use-the-context-hub)-backed instructions, skills, and optional memory
- Traces
...
|------|----------|------------|-------------------|
| **Managed Deep Agents** | You want a code-first Deep Agent deployed quickly on managed infrastructure. | Agent code, tools, middleware, instructions, schedules, optional identity, and optional memory. | Backend, store, checkpointer, memory storage, skills, sandbox, hosted deployment, and identity auth when declared. |
...
You organize a Managed Deep Agent as a local project directory. A file's location determines its role: the CLI reads the directory to find the agent entry, managed instructions, skills, schedules, optional identity, sandbox configuration, and local eval tasks, then packages the deploy-owned pieces into a hosted deployment.

For the full directory layout and packaging rules, see the [CLI project file reference](/langsmith/managed-deep-agents-cli#project-file-reference).
...
4. Add authored tools, middleware, schedules, skills, optional identity, and an optional sandbox.
