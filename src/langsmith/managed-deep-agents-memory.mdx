...
Each layer uses its [default access policy](#control-access-to-a-layer) unless you supply your own.
  ...
  <Step title="Declare an identity for user memory" id="declare-an-identity">

User memory requires a caller the deployment can authenticate as a person. A LangSmith API key authenticates a service, not a person, so a deployment that uses the default identity provider never mounts user memory. To authenticate signed-in end users, configure [Supabase](/langsmith/managed-deep-agents-identity#configure-identity-with-supabase).

Runs that arrive through a connected Slack workspace resolve to the linked person and satisfy this requirement.

Agent memory has no identity requirement.
