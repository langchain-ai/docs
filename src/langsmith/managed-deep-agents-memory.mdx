...
When enabled, durable memory is backed by the [Context Hub](/langsmith/managed-deep-agents-context-hub). The deployment gets one read/write tree at `/memories/agent/`, shared by every caller.
