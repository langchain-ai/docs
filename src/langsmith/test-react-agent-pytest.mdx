...
<CodeGroup>
```bash pip
pip install -qU langgraph langchain[openai] langchain-community e2b-code-interpreter
```

```bash uv
uv add langgraph langchain[openai] langchain-community e2b-code-interpreter
```
</CodeGroup>
Or with TypeScript:
```bash
...
Next, choose the testing framework from the following.

Using Python with pytest:
<CodeGroup>
```bash pip
# Make sure you have langsmith>=0.3.1
pip install -qU "langsmith[pytest]"
```
...
Or using TypeScript (yarn) with vitest:
```bash
