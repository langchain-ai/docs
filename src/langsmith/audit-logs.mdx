...
| `start_time` | ISO 8601 datetime | Yes | Start of the time range (inclusive). |
| `end_time` | ISO 8601 datetime | Yes | End of the time range (inclusive). |
| `workspace_id` | UUID | No | Filter events to a specific workspace. |
| `operations` | list[string] | No | Filter by operation names (see [tracked operations](#tracked-operations-reference)). Pass multiple values by repeating the parameter. If omitted, all operations are returned. |
...
curl -G \
  ...
  -d 'operations=create_api_key' \
  -d 'operations=delete_api_key'
```
