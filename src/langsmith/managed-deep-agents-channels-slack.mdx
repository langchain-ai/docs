...
Each Slack conversation maps to its own thread, and every thread gets its own sandbox. A file uploaded in a direct message does not appear in a channel run. Within a conversation the sandbox is shared, so a run started by anyone there can reach files that others uploaded. Downloads and the history scan use the bot token, not the permissions of the person who started the run.
