...
Finish writing the file before calling the tool. `attach_file` returns `File attached.` on success and a short reason on failure. When Slack accepts an upload without confirming delivery, the tool reports that and tells the agent not to retry.
