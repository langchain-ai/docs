...
A Slack channel moves files in both directions. Uploads land in the agent's sandbox, and the agent sends a file back by calling `attach_file`. Managed Deep Agents enables both when the project declares a Slack channel and a [sandbox](/langsmith/managed-deep-agents-sandboxes). No extra tool, middleware, or Slack token is needed.
...
Managed Deep Agents downloads the files on the incoming message into `/workspace/attachments/` before the agent runs. Each file is saved as `<file ID>__<filename>`. Characters outside letters, digits, `.`, `_`, and `-` become underscores.

The agent then reads an attachment status message, one line per file:
...
Paths come from that message, and the agent opens the files with its sandbox tools. The status and the file contents are labeled as data, not instructions.

A scan of the thread also picks up files shared before the message that started the run. It reads up to five pages of history, 100 messages per page, and needs the bot's history scope for the conversation. When the scan falls short, the status reports that earlier attachments may be missing and the run continues.

Each Slack thread maps to its own agent thread, and every agent thread gets its own sandbox. The thread key combines the conversation and the root message. A reply thread, a standalone channel message, and a direct message each stage files into a different sandbox. Within one Slack thread the sandbox is shared, so a run started by anyone in that thread can reach files the others uploaded. Downloads and the history scan use the bot token, not the permissions of the person who started the run.

Each file ID is downloaded once per run. External and deleted Slack files are skipped.

### Attach files to a reply

The agent calls `attach_file` with a path under `/workspace`. The file goes to the conversation that started the run, and the destination cannot be changed.

Attachment is never automatic, so state in the agent's [instructions](/langsmith/managed-deep-agents-instructions) when a file belongs in the reply:
...
`attach_file` returns `File attached.` on success and a short reason on failure. When Slack accepts an upload without confirming delivery, the tool reports that and tells the agent not to retry.

The tool stays available after a human-in-the-loop approval resumes the run.
...
- **Slack scopes**: `files:read` for downloads, `files:write` for uploads, and the bot's history scope for the thread scan. A missing scope fails the transfer and reports that you need to reconnect Slack.
