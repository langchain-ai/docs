...
The tool sends the bytes it reads at that moment, so finish the file first. It returns `File attached.` on success and a short reason on failure. When Slack accepts an upload without confirming delivery, the tool reports that and tells the agent not to retry.
