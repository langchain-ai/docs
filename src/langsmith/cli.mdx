...
<Tabs>
    ...
    * `ttl`: Includes `strategy`, `sweep_interval_minutes`, `default_ttl`, and `sweep_limit` (Agent server 0.8+), which collectively set how checkpoints expire.
    * `sweep_limit` _(Agent server 0.8+)_ : How many threads the sweeper processes in a single iteration. Defaults to `10000` (Agent server 0.12+) or `1000` (Agent server 0.8-0.11).
