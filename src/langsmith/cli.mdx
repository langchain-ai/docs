...
<Tabs>
    ...
    **Prerequisites**
    ...
    <Note>On LangSmith Cloud the CLI pushes to a managed registry. Self-hosted and hybrid installations push to a registry you manage with `--push-to`. See [Deploy to a self-hosted or hybrid LangSmith](#deploy-to-a-self-hosted-or-hybrid-langsmith).</Note>
    ...
    **Options**
    ...
    | `--listener-id TEXT`     |                      | Listener that will run the deployment, for workspaces that deploy through a listener in your own cluster. Used only when creating a deployment with `--push-to`. Defaults to the workspace's only listener. |
    ...
    **Example**
    ...
    #### Deploy to a self-hosted or hybrid LangSmith

    Self-hosted LangSmith and hybrid installations (a LangSmith Cloud control plane with a self-hosted data plane) run images from a registry you manage. Pass `--push-to` with a repository in that registry. The CLI builds the image, pushes it with your existing Docker credentials, and creates or updates the deployment from the pushed image.
    ...
    # Hybrid: the control plane stays on LangSmith Cloud, only the image location changes
    ...
    The control plane URL is derived from `LANGSMITH_ENDPOINT`, read from the environment or the project `.env` file: `https://<host>/api-host` for a self-hosted instance, and the matching LangSmith Cloud control plane for cloud endpoints. Set `LANGGRAPH_HOST_URL` to override it.

    #### Workspaces that deploy through a listener

    Hybrid workspaces run deployments through a listener in your own cluster. When you create a deployment, the control plane needs to know which listener runs it and which Kubernetes namespace it deploys into.

    You do not have to pass either when there is only one possible answer. If your workspace has a single listener that serves a single namespace, the CLI uses it:
    ...
    The listener and the namespace are fixed when the deployment is created and cannot be changed afterwards, so both options are refused on an existing deployment. Later revisions need only `--push-to`.
