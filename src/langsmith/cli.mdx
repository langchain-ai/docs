...
<Tabs>
    ...
    **Options**
    ...
    | `--agent-id TEXT`        |                      | Agent to deploy to in an agent-based workspace, by identifier. See [Deploy to an agent environment](/langsmith/deploy-to-agent-environment). Available in `langgraph-cli>=0.4.32`. |
    | `--agent-environment TEXT` |                    | Environment the deployment reports into: `local`, `development`, `staging`, or `production`. Available in `langgraph-cli>=0.4.32`. |
