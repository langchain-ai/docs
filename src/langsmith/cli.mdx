...
<Tabs>
    ...
    Build and deploy a LangGraph image directly to [LangSmith Deployments](/langsmith/deployment). This command builds a Docker image locally, pushes it to a managed registry, and creates or updates a deployment—all in a single step. If Docker / Docker Build is not installed, it triggers a remote build. 
    ...
    **Prerequisites**
    ...
    - (Optional) **Docker Buildx** is required for local builds on non-x86_64 machines (e.g., Apple Silicon) to cross-compile for linux/amd64. Not required for remote builds [Learn more about Buildx](https://docs.docker.com/build/install-buildx/).

    <Note>Works only with LangSmith Cloud. Support for Self-hosted LangSmith coming soon.</Note>
