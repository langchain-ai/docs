...
<Note>
Spaces within role name tokens (e.g., `Organization Admins`, `Organization User`) are optional and may be omitted. This is useful for identity providers that do not support spaces in group names. For example, `OrganizationAdmins` and `OrganizationUser` are accepted equivalently to `Organization Admins` and `Organization User`. Note that this flexibility does **not** apply to workspace names — `Workspace 1` and `Workspace1` are distinct workspaces and cannot be used interchangeably.
</Note>
