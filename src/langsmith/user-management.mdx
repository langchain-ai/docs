...
<Note>
SCIM works with a custom app integration, but the [Okta Integration Network app](#via-okta-integration-network) is recommended. See [Using a custom SAML app](#scim-custom-saml-app).
</Note>
...
<div id="scim-custom-saml-app">
    <b>Using a custom SAML app</b>
</div>

If you already sign in through a custom SAML app, you have two options.

**Add the OIN app for SCIM only (recommended).** Add the LangSmith app from the Okta Integration Network and follow Steps 2 through 4 above, skipping the SAML configuration. Under **Application Visibility**, check **Do not display application icon to users** so nobody is shown a tile that can't sign them in. Your custom SAML app continues to handle login.
...
Then complete Steps 2 through 4 above, starting from **To App**: enable Create Users, Update Users, and Deactivate Users, assign your users and groups, set the attribute mappings, and configure group push.

<Note>
If **Provisioning** isn't available in your app's settings, first check that your organization has Okta Lifecycle Management, which is required for SCIM on Okta. If you have it and the option still isn't there, Okta may need to enable it for your organization.
</Note>
