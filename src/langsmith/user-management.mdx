...
<Note>
If you use Azure Entra ID (formerly Azure AD) or Okta, there are specific instructions below for identity provider setup ([Azure Entra ID](#azure-entra-id-configuration-steps), [Okta](#okta)). The requirements and steps above are applicable for all identity providers.
</Note>
...
<Note>
In self-hosted installations, the `oid` JWT claim is used as the `sub` rather than the `sub` claim.
