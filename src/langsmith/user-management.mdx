...
<Note>
SCIM works with a custom app integration, but the [Okta Integration Network app](#via-okta-integration-network) is recommended — its SCIM connection and attribute mappings are preconfigured. See [Using a custom SAML app](#scim-custom-saml-app).
</Note>
...
**Add the OIN app for SCIM only (recommended).** Add the LangSmith app from the Okta Integration Network, configure only its provisioning settings, and leave the SAML configuration blank. Under **Application Visibility**, hide the app icon so users aren't shown a tile that can't sign them in. Your custom SAML app continues to handle login, and the SCIM connection and attribute mappings come preconfigured.
...
Then follow Steps 3 and 4 above to set the attribute mappings and configure group push.

<Note>
If **Provisioning** isn't available in your app's settings, Okta may need to enable it for your organization. Contact Okta support.
</Note>
