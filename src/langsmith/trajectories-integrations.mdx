---
title: Trajectory integrations
description: How each LangSmith SDK integration sets the metadata that trajectories require, with examples for setting it manually.
---

<Note>
[Trajectories](/langsmith/trajectories) and the [Messages view](/langsmith/view-traces#messages-view) are in **beta**.
</Note>
...
When you write your own guardrail, policy check, or middleware function around an LLM or tool call, wrap it in `@traceable` and set `ls_agent_type: "middleware"` on the metadata. The trajectory endpoint and the Messages view filter these runs out of the main conversation.
