...
2. Setup environment variables for the endpoint, substitute your specific values:
   ...
   <Note>
   `OTEL_EXPORTER_OTLP_ENDPOINT` is a base URL: OTLP/HTTP exporters append the signal path (`/v1/traces`) themselves, so don't include it. Add the full path only when you set the per-signal `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` variable, or a collector exporter's `traces_endpoint`, which are used exactly as given. Setting the base endpoint to `https://api.smith.langchain.com/otel/v1/traces` sends traces to `/otel/v1/traces/v1/traces`, which returns `404`.
   </Note>
