...
1. Install the LangSmith package with OpenTelemetry support:
   ...
   <CodeGroup>
   ```bash pip
   pip install -qU "langsmith[otel]" langchain
   ```

   ```bash uv
   uv add "langsmith[otel]" langchain
   ```
   </CodeGroup>
...
1. Install the OpenTelemetry SDK, OpenTelemetry exporter packages, as well as the OpenAI package:
   ...
   <CodeGroup>
   ```bash pip
   pip install -qU openai opentelemetry-exporter-otlp opentelemetry-sdk
   ```
   
   ```bash uv
   uv add openai opentelemetry-exporter-otlp opentelemetry-sdk
   ```
   </CodeGroup>
