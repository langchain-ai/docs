...
- Optionally declare [identity](/langsmith/managed-deep-agents-identity) in `identity.ts` or `identity.py` to authenticate callers and scope threads and store access.
...
For declarations, scope, provisioning, and lifecycle, see [Sandboxes](/langsmith/managed-deep-agents-sandboxes).
...
<CardGroup cols={2}>
  <Card title="Identity" icon="fingerprint" href="/langsmith/managed-deep-agents-identity">
    Authenticate callers and scope threads and store access.
  </Card>
