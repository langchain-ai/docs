...
The CLI deploys a local project directory. The SDKs and REST API deploy the same agent shape directly from request payloads. Use SDK fields for structured configuration and `files` for extra managed files that do not have a typed field.

| CLI project file | SDK or API field |
| --- | --- |
| `agent.json` `name`, `description`, `model`, `backend` | `name`, `description`, `model`, `backend` |
