...
The CLI deploys a local project directory. The SDKs and REST API deploy the same agent shape directly from request payloads. The SDKs accept CLI-style `model` and backend shorthands and normalize them to the API shape. Use SDK fields for structured configuration and `files` for extra managed files that do not have a typed field.
