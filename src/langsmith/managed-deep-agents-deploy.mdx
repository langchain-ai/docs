...
Conflict detection starts after the CLI records a baseline in Context Hub. Durable memory under `memories/` is not deploy-owned, so neither strategy overwrites it. If the Context Hub repo changes while the deploy is running, the deploy stops; rerun `mda deploy` against the latest version.
