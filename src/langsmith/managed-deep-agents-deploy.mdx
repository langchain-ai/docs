...
LangChain hosts Managed Deep Agents, so you can deploy an agent without running your own infrastructure. Deploying creates or updates the hosted agent resource and syncs the managed file tree that contains instructions, skills, subagents, and tool configuration. A deploy also creates a separate LangSmith tracing project and a [Context Hub](/langsmith/use-the-context-hub) repo for the file tree, but it does not create a LangSmith Deployment.

Use the CLI for project-based workflows. Use the REST API when you need a custom client, automation that cannot invoke the CLI directly, or direct control over request payloads.

<Note>
Managed Deep Agents is in **private preview**, available on LangSmith Cloud in the US region only. If you do not have access yet, [join the waitlist](https://www.langchain.com/langsmith-managed-deep-agents-waitlist) to request it before you start.
</Note>

For a faster path, see the [quickstart](/langsmith/managed-deep-agents-quickstart). For all commands and project file rules, see the [CLI reference](/langsmith/managed-deep-agents-cli).
...
`deepagents init` scaffolds an empty `tools.json`, one example skill, and one example subagent so the initial deploy succeeds before you register an MCP server. Edit or remove the examples to fit your agent. Deploy reads every file in the project and syncs the tree to the Context Hub agent repo. For the complete field reference, see the [CLI reference](/langsmith/managed-deep-agents-cli#project-file-reference).
...
Run `deepagents deploy --dry-run` to inspect the assembled payload and managed file tree before deploying.

## Create or update an agent (REST API)
