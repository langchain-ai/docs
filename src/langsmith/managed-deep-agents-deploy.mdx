...
For the full file layout and packaging rules, see the [CLI project file reference](/langsmith/managed-deep-agents-cli#project-file-reference). To define the agent entry, tools, middleware, and interrupts, see the [quickstart](/langsmith/managed-deep-agents-quickstart#configure-the-agent).
...
Durable memory is optional through a project-root `memory.py` or `memory.ts` declaration and is shared across the deployment when enabled. Shared memory is visible to all callers, so do not store personal data or secrets. Redeploying preserves existing durable memory. See [Memory](/langsmith/managed-deep-agents-memory).

## Add tools and middleware
...
- Optionally declare [identity](/langsmith/managed-deep-agents-identity) in `identity.ts` or `identity.py` to authenticate callers and give them private threads.
...
For declarations, scope, and lifecycle, see [Sandboxes](/langsmith/managed-deep-agents-sandboxes).
