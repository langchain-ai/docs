...
Read that chain in either direction. A workspace holds your agents, each agent has the same four environments, and every trace belongs to one environment of one agent. Given a trace, the application that produced it and the deployment it ran on are both already known, so neither has to be recovered from a naming convention or a filter.

## Environments

An _environment_ records which deployment of an application produced a trace. Each agent has four of them: Production, Staging, Development, and Local.
