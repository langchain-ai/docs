...
- **Identifier**: Set when the agent is created, and not editable afterwards. How you supply it depends on the path: the **Create a new agent** dialog takes it in an **Agent ID** field beside the display name, and tracing to an identifier that does not exist yet creates the agent under the value you sent. LangSmith uses it to address traces, so it stays stable even when the display name changes. The tracing project behind each environment is the identifier, a hyphen, and the environment name, so an agent whose identifier is `checkout` has its production traces in `checkout-production`.
