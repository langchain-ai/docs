...
- **Model provider key**: Managed Fleet does not require a model-provider API key. The Fast, Pro, and Max tiers use models managed by LangChain. An OpenAI or Anthropic API key is required only when you use [custom models](/langsmith/fleet/essentials#custom-models). When set, the agent graphs load this key from workspace secrets for inference.
