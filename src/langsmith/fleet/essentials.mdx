...
Managed Fleet includes models out of the box through three tiers: **Fast**, **Pro**, and **Max**. You do not need to choose a provider, configure a model, or supply a provider API key. For how the tiers work and how usage is billed, see [Models and billing](/langsmith/fleet/models-and-billing).

Custom models are not available alongside Fast, Pro, and Max in the managed Fleet model picker. If custom models are a requirement for an enterprise deployment, contact your LangChain account team or [reach out to sales](https://www.langchain.com/contact-sales).
