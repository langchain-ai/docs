...
Fleet manages models for you. It selects and maintains a strong model for each task, so you get good results without choosing a provider, configuring a model, or supplying an API key. Usage is billed in [LangChain Compute Units (LCUs)](#langchain-compute-units-lcus).
