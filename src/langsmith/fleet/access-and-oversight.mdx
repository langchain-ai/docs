...
<Note>
A role with `mcp-servers:read` and `mcp-servers:invoke` can see and use all MCP servers and integrations in the workspace. A role without these permissions has no tool access.
</Note>
...
<Note>
The Access control page is only visible to admin users (requires `workspace:manage` permission).
</Note>
