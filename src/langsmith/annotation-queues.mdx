...
- **From the Threads tab**: In a tracing project, open the **Threads** tab, select one or more threads (use the row checkboxes), then click **Add to Annotation Queue**. Selected threads are added as thread items.
    ...
    ![Tracing project Threads tab listing threads with selection checkboxes.](/langsmith/images/threads-tab-light.png)
...
1. Review the current item:
    ...
    - **Thread items**: The center pane shows the conversation transcript for that [thread](/langsmith/observability-concepts#threads). Add **Reviewer Notes** and score the same rubric **Feedback** keys. Use **View** to open the thread in the conversation peek for more context.
