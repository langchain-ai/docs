...
Content is only redacted as it gets sent to the LLM. Once the response returns, that content gets rehydrated. The gateway trace still reflects what the provider saw (redacted, if content capture is on). However, this applies only to the gateway trace; the original agent can receive (and therefore trace) the restored sensitive values.
