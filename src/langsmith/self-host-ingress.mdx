...
config:
  ...
  basePath: "" # If you want to serve langsmith on a subdomain
ingress:
  enabled: true
...
If your cluster uses the [Kubernetes Gateway API](https://gateway-api.sigs.k8s.io/), you can configure LangSmith to provision HTTPRoute resources. This will create an HTTPRoute for LangSmith and an HTTPRoute for each LangGraph platform agent deployment.
...
If your cluster uses [Istio](https://istio.io/), you can configure LangSmith to provision VirtualService resources. This will create a VirtualService for LangSmith and a VirtualService for each LangGraph platform agent deployment.
