...
- Filtering for examples that contain specific `input` / `output`
- Filtering to see all the runs with status `success` or `error`
- Filtering on runs that take more than x seconds in `latency`
- Filtering on specific `metadata`, `tag`, or `feedback`
