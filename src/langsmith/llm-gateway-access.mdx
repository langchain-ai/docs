...
Turn off tracing across the whole organization or within specific policy. This lives under Data Protection controls. When settinga  data protection policy, toggle tracing on or off for policies which cover: organization, workspace, user and API key. 

When tracing is turned off, content of the trace will not be retained. Metadata will still be retained. This currently still counts against trace usage counts. 
