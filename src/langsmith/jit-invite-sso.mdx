...
Response includes current organization configuration:
...
- These settings only apply to the default organization (identified by `default_sso_provision=true`). Other organizations must use different authentication modes or SCIM provisioning.
...
1. Check if invites are enabled **and** a pending invite exists:
    ```
   ├─ YES → Claim the invite (use invite's workspace/role configuration)
...
<Note>
When both JIT provisioning and invites are enabled, **invites take precedence**. If a user has a pending invitation, they will be added with the invite's workspace and role configuration, not the default SSO settings.
</Note>
...
- Users must be provisioned through SCIM or directly by an administrator.
