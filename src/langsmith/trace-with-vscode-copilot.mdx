---
title: Trace VS Code Copilot Chat sessions
...
[VS Code Copilot Chat](https://code.visualstudio.com/docs/copilot/overview) can export traces, metrics, and events over [OpenTelemetry](https://opentelemetry.io/) (OTel). Because LangSmith ingests OTLP directly, you can point Copilot Chat at LangSmith and inspect agent turns, model metadata, tool calls, and token usage alongside the rest of your LLM traces.
...
Start a Copilot Chat session in agent mode and send a request. Open your LangSmith project to view the resulting traces. Each agent interaction produces a hierarchical span tree following the [OTel GenAI Semantic Conventions](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/gen-ai/):
