...
- `strategy`: Specifies the action taken on expiration.
    - `"delete"`: Removes the entire thread and all associated data when the TTL expires, including:
        - All checkpoint data
        - Associated runs
        - Thread metadata
    - `"keep_latest"`: Deletes historical checkpoints when the TTL expires but preserves:
        - The thread itself
