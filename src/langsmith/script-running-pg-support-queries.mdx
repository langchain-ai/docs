...
All export methods produce the same data across all workspaces and organizations, covering usage for:

- LangSmith traces
- LangSmith Deployments (LGP) nodes
- Fleet (Agent Builder) runs
- Usage snapshots (daily entity counts)
- Sandboxes
- LangSmith Engine token usage (Smith Intelligence and issues agent)
- LLM Gateway
...
If you have direct database access, you can run `run_all_full_exports_pg.sh` to export every usage dataset in a single command. This requires database access credentials — no application-level permissions apply. It runs all read-only full-export queries, writes one CSV per dataset, and bundles them into a single `.tar.gz`. Run it from the chart's `scripts/` directory; it locates `run_support_query_pg.sh` and `support_queries/postgres/` relative to itself (requires Helm chart version 0.17.x or later).
...
This writes one CSV per dataset into `./langsmith-usage-export/` and bundles `./langsmith-usage-export.tar.gz`. Pass `--output-dir <dir>` to change the location, `--debug` for verbose psql output, or `--help` for full usage.
