...
If you have direct database access, run `run_all_full_exports_pg.sh` from the chart's `scripts/` directory to export every usage dataset in one command (requires Helm chart version 0.17.x or later).
...
#### Export individual datasets on older releases

On releases that predate the combined export script, run each query individually with `run_support_query_pg.sh`.
