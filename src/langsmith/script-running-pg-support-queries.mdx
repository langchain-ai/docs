...
#### Export all datasets at once (recommended)

To export every usage dataset in a single command — instead of running each script individually — use `run_all_full_exports_pg.sh`. It runs all read-only full-export queries, writes one CSV per dataset, and bundles them into a single `.tar.gz`. Run it from the chart's `scripts/` directory; it locates `run_support_query_pg.sh` and `support_queries/postgres/` relative to itself.
