...
The SDKs can register MCP servers and complete auth sessions. To discover a registered server's tool schemas, use `deepagents mcp-servers tools` or [`GET /v1/deepagents/mcp/tools`](/langsmith/managed-deep-agents-api/mcp-tools/list-mcp-tools), then pass the selected tool entries to `client.agents.create(...)` or `client.agents.update(...)`.
