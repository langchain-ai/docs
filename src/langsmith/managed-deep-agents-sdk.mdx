...
Requests to create an agent can use the same top-level `model` and `backend` fields as the [Managed Deep Agents CLI](/langsmith/managed-deep-agents-cli). Pass `model` as `{provider}:{model_id}`; see [supported providers and models](/oss/langchain/models#supported-providers-and-models).
