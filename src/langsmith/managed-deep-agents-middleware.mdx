...
Middleware adds behavior around model calls, tool calls, and the agent lifecycle. Like [custom tools](/langsmith/managed-deep-agents-tools), MDA does not discover middleware automatically. Import it and pass it to the agent definition.
...
To read or write files in the thread's sandbox from a middleware hook, use `runtime.backend`. See [Read and write sandbox files from code](/langsmith/managed-deep-agents-sandboxes#read-and-write-sandbox-files-from-code).
