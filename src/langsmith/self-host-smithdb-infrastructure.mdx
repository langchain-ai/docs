...
A common AWS mapping is EKS, RDS for PostgreSQL, S3, IRSA, and EC2 instance store.
...
A common GCP mapping is GKE Standard, AlloyDB or another compatible PostgreSQL service, Cloud Storage, Workload Identity, and Local SSD-backed ephemeral storage.
...
A common Azure mapping is AKS, Azure Database for PostgreSQL, Azure Blob Storage, Workload Identity, and the VM temporary disk as Kubernetes ephemeral storage.
...
On LangSmith 0.16, set the migration label under `smithdb.migration.deployment.labels` instead.
