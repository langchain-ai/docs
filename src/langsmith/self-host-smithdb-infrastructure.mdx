...
Configure access so SmithDB can list the bucket and read, write, and delete objects. Prefer workload identity over static credentials. Block public access and require encrypted transport.
...
Upgrading to 0.17 moves the default cache from `emptyDir` to a per-pod PersistentVolumeClaim. To stay on local SSD, add the [0.17 local SSD values](#local-ssd) to the same upgrade. If you carry custom cache volumes forward, rename them from `local-ssd-storage` to `cache`, and drop any `SMITHDB_QUERY__VORTEX_CACHE__DISK__LIMIT` entry from `extraEnv`, because the chart now sets it.
...
Set `storageClassName`, or leave it empty to use the cluster default. The tier sets the volume size, CPU, and memory. The chart sets `fsGroup: 1001` and the query disk cache limit, so no `extraEnv` is needed.
...
Apply the query overrides to any enabled `mutations`, `runRules`, and `statsQuery` services too.
...
Place query, ingestion, compaction worker, any enabled separate query services, and migration on the local SSD pool. Place compaction and cluster manager on the general compute pool. Node-pool labels and taints must match the Helm selectors and tolerations. `metastoreMigration` can run on any node and does not need a selector.
