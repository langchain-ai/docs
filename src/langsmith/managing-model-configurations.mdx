...
<Note>
OAuth client credential fields appear in this popover only after the configuration is saved as a preset. To attach OAuth to a one-off configuration, save it as a preset first. See [OAuth client credentials](/langsmith/model-configurations#oauth-client-credentials).
</Note>
