...
Feedback is used to continuously improve the implementation of your agent. To learn more about how feedback works in LangSmith, see [LangSmith Feedback](/langsmith/observability-concepts#feedback).
...
2. The `feedback` object from the response contains a pre-signed URL for each key. For example, the `feedback` object is...
    ...
    ```
3. Request the returned URL (e.g. `POST /api/v1/feedback/tokens/{token_id}`) to associate the feedback key with the trace generated from the Agent Server run. See the [LangSmith API reference](https://api.smith.langchain.com/redoc) for more details.
