...
<Note>
Creating feedback for a run requires `session_id` / `sessionId`, the UUID of the tracing project (session) that owns the run. Resolve it once with `read_project()` / `readProject()` and reuse the value for every run in that project. Passing the run's `start_time` / `startTime` is optional but recommended, for faster processing.
</Note>
...
# The run tree carries the project name, not its UUID, so resolve the UUID once
session_id = client.read_project(project_name=root_run.session_name).id
...
client.create_feedback(
    ...
    # start_time= is optional but recommended for faster processing
    start_time=root_run.start_time,
    comment="the user said that ..."
)

# Provide feedback for a child run
foo_run = [run for run in child_runs if run.name == "foo"][0]
client.create_feedback(
    key="correctness",
    score=0,
    run_id=foo_run.id,
...
// Resolve the UUID of the tracing project that owns the run
const { id: sessionId } = await client.readProject({ projectName: "default" });
