...
- To forward the end user's IdP access token, self-hosted LangSmith with OIDC authentication. See [IdP token forwarding](#idp-token-forwarding)
...
3. **Allowed URLs:** control which destination URLs the proxy is permitted to forward JWTs to. This prevents credential forwarding to unintended hosts. Choose one of three options:
    ...
    With [IdP token forwarding](#idp-token-forwarding) enabled, this setting also bounds where a live end-user credential can travel. **Block all** sends no JWT at all, so your `ext_authz` service is never called.
...
By default the JWT tells your service who the user is but carries no credential to act as them. With IdP token forwarding enabled, LangSmith adds an `idp_token` claim holding the signed-in user's IdP access token, so your `ext_authz` service can exchange it for a credential your gateway accepts — typically an [RFC 8693 token exchange](https://datatracker.ietf.org/doc/html/rfc8693), with `idp_token` as the subject token. LLM calls are then authorized as the end user rather than as LangSmith.

LangSmith passes the token through unmodified. It does not validate, inspect, cache, or refresh it, and the token may be opaque rather than a JWT. The exchange happens entirely in your `ext_authz` service.

<Note>
Requires self-hosted LangSmith with OIDC authentication. Supported for [Fleet](/langsmith/fleet), the [Playground](/langsmith/prompt-engineering-concepts), and [evaluations](/langsmith/evaluation) started from the LangSmith UI. Not supported for [Chat](/langsmith/chat), [Insights](/langsmith/insights), or scheduled runs.
</Note>
...
```yaml
commonEnv:
  - name: LLM_AUTH_PROXY_FORWARD_IDP_TOKEN
    value: "true"
```

Forwarding is installation-wide and off by default. Turning it on sends a live end-user credential to every destination your [Allowed URLs](#3-configure-organization-settings-in-langsmith) setting permits, so set that to **Custom** and list only the hosts that should receive one.

The claim also requires a [custom model configuration](/langsmith/model-configurations). Built-in models do not route through the auth proxy, so no JWT is minted and the claim never appears. This is the normal setup for the auth proxy, which exists so requests reach your own gateway.

<Note>
Installations that set `OAUTH_OVERRIDE_TOKEN_EXPIRY` never forward the token, because the access token's real expiry is unknown there.
</Note>

### Handle a missing claim

`idp_token` is omitted rather than fatal — the JWT still mints and your service is still called. Absence is expected, and which requests carry the claim changes as LangSmith adds surfaces. Common reasons include no signed-in user behind the request, forwarding being disabled, and the token no longer being usable.

Treat a missing claim as its own outcome and label it distinctly. A generic denial makes an absent claim indistinguishable from token forwarding being broken.

The same applies to identity: use `ls_user_id` to attribute a call to a person, and decide explicitly what your policy does when it is absent. See the [JWT claims reference](#jwt-claims-reference).

### Token freshness

Two independent credentials expire on this path, and they fail in different places:

| Credential | Lifetime | Where it fails |
|---|---|---|
| The JWT LangSmith signs | Currently 5 minutes, about 6 including Envoy's default 60-second clock skew allowance | Envoy's `jwt_authn` filter, before `ext_authz` runs. Your service is never called, so nothing appears in its logs |
| The IdP access token in `idp_token` | The IdP's own lifetime | Inside your `ext_authz` service, when the exchange is attempted |

LangSmith reads the access token when it mints the JWT, or up to an hour earlier, and never refreshes it. Assume on every surface that the token may be close to expiry, or already past it, by the time your service receives it. Fail distinguishably on an expired subject token so it can be told apart from an absent claim.
...
<Warning>
This sample authenticates as LangSmith, not as the end user, so it is not a starting point for [IdP token forwarding](#idp-token-forwarding). For that, read the `idp_token` claim from the JWT and exchange it.
</Warning>
...
| `ls_user_id` | LangSmith user ID of the person the request is attributed to. Not present on every request — see the warning below |
| `idp_token` | The signed-in user's IdP access token, passed through unmodified, for use as the subject token in a token exchange. Not present on every request — see [IdP token forwarding](#idp-token-forwarding) |

<Warning>
`ls_user_id` is the only claim that identifies the person behind a request. Do not infer identity from `sub` or `actor_type` — `sub` identifies whatever triggered the request, which on agent runs is the assistant, not the person.

`ls_user_id` is not present on every request, and which requests carry it changes as LangSmith adds surfaces. Treat its absence as "no attributable user" and decide explicitly what your policy does then.
</Warning>
...
With [IdP token forwarding](#idp-token-forwarding) enabled the JWT carries a live end-user credential, so do not accept HTTP outside local development.
</Accordion>
...
<Accordion title="My ext_authz service is never called for a long-running request.">
The JWT LangSmith signs is short-lived, so a request reaching the proxy after it expires is rejected by Envoy before `ext_authz` runs. See [Token freshness](#token-freshness).
</Accordion>
