...
For OAuth2 `client_credentials` specifically, [OAuth client credentials on a model configuration](/langsmith/model-configurations#oauth-client-credentials) offers a self-service alternative that workspace admins can configure without deploying an `ext_authz` service.
