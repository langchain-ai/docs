...
<Note>
The auth proxy currently supports the Playground, Evals, Fleet, Polly, and Insights features.
Playground and Evals are available in 0.13.33+. Polly and Insights are available in version 0.13.39+.
</Note>
...
To enable HTTP traffic to the auth proxy for Polly and/or Insights, set this environment variable in the respective `extraEnv` sections: `config.polly.agent.extraEnv` and `config.insights.agent.extraEnv`.
</Accordion>
