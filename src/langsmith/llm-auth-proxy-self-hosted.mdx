...
- To forward the end user's IdP access token, self-hosted LangSmith with OIDC authentication. See [Forward IdP access tokens](#forward-idp-access-tokens)
...
3. **Allowed URLs:** control which destination URLs the proxy is permitted to forward JWTs to. This prevents credential forwarding to unintended hosts. Choose one of three options:
    ...
    With [forwarding IdP access tokens](#forward-idp-access-tokens) enabled, this setting also bounds where a live end-user credential can travel. **Block all** sends no JWT at all, so your `ext_authz` service is never called.
...
## Forward IdP access tokens

LangSmith can add an `idp_token` claim carrying the signed-in user's IdP access token. Your `ext_authz` service exchanges it, typically through [RFC 8693 token exchange](https://datatracker.ietf.org/doc/html/rfc8693), for a credential your gateway accepts, so LLM calls are authorized as the end user rather than as LangSmith.

LangSmith passes the token through unmodified and does not validate, inspect, cache, or refresh it. The token may be opaque rather than a JWT. The exchange happens entirely in your service.
...
To turn on forwarding, add the following to `commonEnv` in your [LangSmith `values.yaml`](https://github.com/langchain-ai/helm/blob/main/charts/langsmith/values.yaml):
...
Forwarding is installation-wide and off by default. It sends a live end-user credential to every destination that [Allowed URLs](#3-configure-organization-settings-in-langsmith) permits, so set that control to **Custom** and list only the hosts that should receive one. Installations that set `OAUTH_OVERRIDE_TOKEN_EXPIRY` never forward the token. The claim also requires a [custom model configuration](/langsmith/model-configurations), because built-in models do not route through the auth proxy.

Handle two cases in your `ext_authz` service:

- **A missing claim**: `idp_token` is omitted rather than fatal, and the JWT still mints. Return a distinct, labeled failure so an absent claim is not mistaken for forwarding being broken. Common causes are no signed-in user behind the request, forwarding disabled, and an unusable token.
- **An expired token**: LangSmith reads the access token up to an hour before your service receives it and never refreshes it. Treat a failed exchange as its own outcome.

To attribute a call to a person, use the `ls_user_id` claim. See the [JWT claims reference](#jwt-claims-reference).
...
<Warning>
This sample authenticates as LangSmith, not as the end user, so it is not a starting point for [forwarding IdP access tokens](#forward-idp-access-tokens). For that, read the `idp_token` claim from the JWT and exchange it.
</Warning>
...
| `idp_token` | The signed-in user's IdP access token, passed through unmodified, for use as the subject token in a token exchange. Not present on every request. See [Forward IdP access tokens](#forward-idp-access-tokens) |
...
With [forwarding IdP access tokens](#forward-idp-access-tokens) enabled the JWT carries a live end-user credential, so do not accept HTTP outside local development.
</Accordion>
...
The JWT LangSmith signs is short-lived, so a request that reaches the proxy after it expires is rejected by Envoy before `ext_authz` runs.
</Accordion>
