...
- **`LANGSMITH_OLD_API_KEY`**: An API key from your current deployment with permission to read all resources you want to migrate. For LangSmith Cloud at [smith.langchain.com](https://smith.langchain.com), create it under **Settings > API Keys**. For another Cloud region or a self-hosted instance, use that deployment's UI.
