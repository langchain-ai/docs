...
<Note>
  OpenTelemetry support is only available in `langsmith` JS SDK version `>=0.3.37`. If you are on an older version of `langsmith`, see [this page](/observability/how_to_guides/trace_with_vercel_ai_sdk_legacy).
...
The [default OTEL setup](https://nextjs.org/docs/app/api-reference/file-conventions/instrumentation) for Next.js will trace all routes, including those that do not contain LLM traces. We instead suggest manually instrumenting specific routes by creating and passing a tracer as shown below:
