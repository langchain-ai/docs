...
{/* TODO: Add screenshot of the "Open service" widget on the sandbox detail page */}
...
| Scenario | How |
|----------|-----|
| Preview a Streamlit app | `sb.service(port=8501)` then open `browser_url` |
| Test a FastAPI server | `svc = sb.service(port=8000)` then `svc.get("/docs")` |
| Share a demo with a teammate | Click **Share Link** in the UI, send the URL |
| Hit an API from a notebook | `svc.post("/predict", json={"input": "..."})` |
| Serve Jupyter | Start Jupyter on port 8888, open via `browser_url` |
| Health-check from CI | `curl -H "X-Langsmith-Sandbox-Service-Token: $TOKEN" $SERVICE_URL/health` |
...
{/* TODO: Add screenshot of the sandbox detail page showing the "Open service" widget with the port input, Open button, and Share Link button */}
...
{/* TODO: Add screenshot of the FastAPI /docs page opened via a sandbox service URL in a browser */}

## Service URLs vs TCP tunnels

| | Service URLs | TCP tunnels |
|---|---|---|
| **Protocol** | HTTP and WebSocket | Any TCP (databases, Redis, SSH, HTTP) |
...
| **"Service timed out"** | The server didn't respond within 5 minutes | Check if the service is overloaded or stuck |
