...
**Custom apps:**
- [Custom Apps](#custom-apps): Build and edit UIs that run inside LangSmith.
...
In **Custom Apps**, Chat builds and edits [custom apps](/langsmith/custom-apps): UIs that run inside LangSmith and call the LangSmith API. Chat writes the app's source in a sandbox and renders a live preview beside the conversation. Unlike the other locations, Chat opens as the main pane rather than in the corner.

For requirements and the full workflow, see [Build with chat](/langsmith/custom-apps#build-with-chat).
