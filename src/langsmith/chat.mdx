...
**Custom Apps:**
- [Custom app editing](#custom-apps): Build and edit a custom app by describing the change. Here Chat opens in the app's editing workspace rather than the corner.
...
In [Custom Apps](/langsmith/custom-apps), Chat writes the app. Select **App** to start one, or open an existing app and select **Edit**, and Chat opens in the editing workspace with a preview of the app beside it. Chat edits the app's files, and **Deploy** publishes the result to the workspace.

**Example requests:**
- "Build a queue that shows one run at a time with a pass or fail rubric"
- "Add a filter for runs from the last 24 hours"
- "Chart token cost per day for this project"
- "The table is unreadable on narrow screens, fix the layout"

Editing an app in the browser requires the Enterprise plan and [sandboxes](/langsmith/sandboxes) enabled on your deployment. For the permissions involved and the local alternative, see [Custom Apps](/langsmith/custom-apps).
