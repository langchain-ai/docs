...
This guide covers running the historical ClickHouse-to-SmithDB migration.
...
- **`smithdb.migration.job.resources`**: The CPU, memory, and ephemeral storage allocated to each migration pod.
...
Start from the chart default of 8 vCPU and 32 GiB per migration pod, and treat 32 GiB as a practical minimum rather than a function of CPU. Migrating 100 million runs in one day therefore suggests about 20 allocated vCPUs across the Job pods. This estimates total CPU, not pod count or per-pod resources. Actual requirements vary by data and environment.
...
Finished migration Jobs remain for seven days by default so you can inspect their logs. The logs are diagnostic only; the `Complete` condition is the signal that migration finished. Retain TaskDB only if needed for diagnosis.
