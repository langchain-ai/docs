...
```
Self-hosted OAuth login requires Helm chart `0.16.0` or later with a signing JWKS. For setup steps, see [Configure a signing JWKS](/langsmith/self-host-signing-jwks). Otherwise create an API-key profile.
