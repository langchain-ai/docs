...
Muting lets you suppress input/output content and unsafe metadata from traces on a per-thread basis, without disabling tracing entirely. Muting is off by default unless you [configure a default](#setting-a-default-mute-configuration).

With tracing enabled, run these argument-free plugin commands inside Claude Code:

- `/langsmith-tracing:mute` — omit this thread's input/output content and unsafe metadata from all subsequent traces.
- `/langsmith-tracing:unmute` — restore full tracing for subsequent turns in this thread.

Muted runs retain their normal nesting, names, timing, status, model/tool identity, and token usage. Inputs and outputs are replaced with a system notice. Both commands apply **from the next turn**; the current turn is unchanged.

### Setting a default mute configuration

To start all threads in metadata-only mode without running a command in each thread, set the `CC_LANGSMITH_DEFAULT_MUTED` environment variable:
...
Save this to your project `.claude/langsmith.json` or user `~/.claude/langsmith.json`.
...
Thread mute preferences are sticky and stored in a `langsmith_state.privacy.json` file. A saved per-thread override always wins over the configured default. Deleting the privacy file removes all thread overrides and resets to the configured default (unmuted when unset).
