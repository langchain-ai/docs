...
def run_claude(prompt: str):
    ...
    # Build the subprocess environment explicitly to avoid variable shadowing.
    # Start from the current environment, then add Claude Code plugin variables.
    # Note: LANGSMITH_* SDK variables are not forwarded to the subprocess because
    # the plugin uses its own CC_LANGSMITH_* variables for tracing configuration.
    claude_env = {
