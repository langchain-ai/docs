...
import CodingAgentSecretRedaction from '/snippets/langsmith/coding-agent-secret-redaction.mdx';
...
That refreshes the catalog only. To move the install to the new version, run the following in your terminal, then restart Claude Code:

```bash
claude plugin update langsmith-tracing@langsmith-claude-code-plugins
```

`/reload-plugins` does not change the installed version, so the hooks keep running the old bundle until you update and restart.

<Note> Enabling auto-update on the marketplace refreshes the catalog and the install together. Third-party marketplaces have auto-update off by default. </Note>
