...
Tools that need credentials — custom MCP servers, private endpoints, or any bearer-authenticated API — are registered once per workspace via `POST /v1/deepagents/mcp-servers`. The platform stores the server's URL and any credential headers; when an agent's `tools[].mcp_server_url` matches a registered server, the stored headers are attached automatically each time the tool is invoked.

The bundled LangChain Fleet MCP server (`https://tools.langchain.com`) does **not** require a separate registration — its credentials are wired up via your LangSmith API key. Register your own MCP servers only when you need to attach custom credentials.
...
<Tabs>
  <Tab title="Python (httpx)">
    ...
    response = httpx.patch(
        ...
        json={
            "headers": [
                {"key": "Authorization", "value": "Bearer sk-rotated-..."},
            ],
        },
    )
    response.raise_for_status()
...
<Note>
    OAuth-backed MCP servers (the `POST /mcp-servers/{id}/oauth-provider` endpoint on `/v1/fleet`) are intentionally not exposed under `/v1/deepagents`. Managed Deep Agents pass static credentials through `headers`; OAuth per-user flows are configured at the LangSmith workspace level, not per agent.
</Note>
...
Each entry in `tools[].mcp_server_url` must either point to the bundled Fleet MCP server (`https://tools.langchain.com`) or match a server you registered in [Configure MCP servers](#configure-mcp-servers). The platform looks up the URL per workspace and attaches the stored headers at invocation time.
