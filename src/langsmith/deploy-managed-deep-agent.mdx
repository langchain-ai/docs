...
| `tools.json` | Advanced tool configuration for compatibility with API-shaped tool payloads. |
...
Reference MCP servers from `agent.json` by URL:
...
If your project uses API-shaped tool configuration, you can keep it in `tools.json`:
