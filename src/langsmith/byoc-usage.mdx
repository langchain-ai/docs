...
- **Workspace**: Belongs to exactly one data plane, which you select when you create the workspace. Traces, datasets, experiments, and other application data live in a workspace, the same as on Cloud or self-hosted. Within a workspace, [applications](/langsmith/administration-overview#applications) group the resources that support the same agent.
...
Use data planes for physical separation of data, and workspaces for logical separation within a data plane.

Common ways to divide data planes are:
...
Below the data plane, workspaces work the same as on Cloud or self-hosted. Create one workspace per team, then use applications to group the resources for each agent. For more information, see [Workload isolation](/langsmith/workload-isolation).
