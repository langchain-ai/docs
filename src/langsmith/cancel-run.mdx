...
<Tabs>
    ...
    run = await client.runs.create(
        thread["thread_id"],
        assistant_id,
        input={"messages": [{"role": "user", "content": "Summarize the docs"}]},
    )
...
<Tabs>
    ...
    await client.runs.cancel(thread["thread_id"], run["run_id"], action="rollback")
...
By default, the cancel request returns after the cancellation is requested and the run is cancelled asynchronously. `wait=True` makes the cancel request block until the run has been fully cancelled. This is useful when you want to know the final state of the run after it has been cancelled (e.g. what checkpoints were created, what the final output was, etc.).
...
<Tabs>
    ...
    run_after = await client.threads.get_state(thread["thread_id"])
    ```
    </Tab>
    <Tab title="Javascript">
    ```js
    ...
    // Get the final state of the thread
    const runAfter = await client.threads.get_state(thread["thread_id"]);
    ```
    </Tab>
    <Tab title="CURL">
    ```bash
    # Create a run, then cancel with wait=true (request blocks until cancelled)
    curl --request POST \
      --url <DEPLOYMENT_URL>/threads/<THREAD_ID>/runs \
      --header 'Content-Type: application/json' \
      --data '{"assistant_id": "agent", "input": {"messages": [{"role": "user", "content": "Summarize the docs"}]}}'
...
The bulk cancel endpoint cancels multiple runs in one request. Both the interrupt and rollback actions are supported.
...
<Tabs>
    ...
    for r in runs_after:
        if r["run_id"] in (run1["run_id"], run2["run_id"]):
            print(r["run_id"], r["status"])  # "interrupted"
    ```
    </Tab>
    <Tab title="Javascript">
    ```js
    ...
    await client.runs.cancelRuns({
        threadId: thread["thread_id"],
        runIds: [run1["run_id"], run2["run_id"]],
        action: "interrupt",
    });

    const runsAfter = await client.runs.list(thread["thread_id"]);
    for (const r of runsAfter) {
        if ([run1["run_id"], run2["run_id"]].includes(r["run_id"])) {
            console.log(r["run_id"], r["status"]);  // "interrupted"
        }
    }
    ```
    </Tab>
...
<Tabs>
    ...
    runs_after = await client.runs.list(thread["thread_id"])
    print(runs_after)  # running run is now "interrupted"
    runs_after2 = await client.runs.list(thread2["thread_id"])
    print(runs_after2)  # runs are cancelled across all threads
    ```
    </Tab>
    <Tab title="Javascript">
    ```js
    ...
    const runsAfter = await client.runs.list(thread["thread_id"]);
    console.log(runsAfter);  // running run is now "interrupted"
    const runsAfter2 = await client.runs.list(thread2["thread_id"]);
    console.log(runsAfter2);  // runs are cancelled across all threads
    ```
    </Tab>
    <Tab title="CURL">
    ```bash
...
When starting a run with streaming, you can set `cancel_on_disconnect` so that the run is cancelled if the client disconnects. This avoids leaving runs in progress when a user closes the app or loses connection. See the stream/connect options in the API you use (for example, `client.runs.stream` or `client.runs.wait`) for the exact parameter name and behavior.
