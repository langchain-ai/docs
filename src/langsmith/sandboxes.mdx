...
- **Deep Agents** — [LangSmith sandbox integration](/oss/integrations/sandboxes/langsmith): step-by-step guide to using `LangSmithSandbox` as a Deep Agents backend, including installation, backend creation, and cleanup.
- **Sandboxes as agent backends** — [Sandboxes overview](/oss/deepagents/sandboxes): configure any sandbox as the execution backend so your agent gets `execute` and filesystem tools automatically.
