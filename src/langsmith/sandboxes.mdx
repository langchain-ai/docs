...
Your organization's network policy determines which external services sandbox code can reach. With restricted egress enabled, sandboxes use a managed allowlist for common package registries, source repositories, and model APIs. LangSmith blocks destinations outside the allowlist, even if you add them to a sandbox's proxy configuration.
