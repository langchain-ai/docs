...
Every trace lands in a container, and which one depends on how your workspace is organized. An agent-based workspace groups traces by [agent and environment](/langsmith/agents). A project-based workspace groups them by [tracing project](/langsmith/observability-concepts#tracing-projects). The sidebar tells you which one you are on: an agent-based workspace has **Overview** as the first item under **Search**, and an agent picker in the top bar. This quickstart configures a tracing project, and the steps below note where the two differ.
