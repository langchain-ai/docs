...
const assistant = traceable(async function assistant(question: string) { // capture the full function as a trace
...
In the [LangSmith UI](https://smith.langchain.com), go to **Tracing** and select your **default** project. Click the `assistant` row to open the **Trace** details panel, which shows the `assistant` function with the OpenAI call nested inside it.
