...
[_Observability_](/langsmith/observability-concepts) is a critical requirement for large language model (LLM) applications. LLMs are non-deterministic, which means that the same input can produce different outputs across runs. This behavior makes debugging and monitoring more challenging than with traditional software.
...
<Tip>
If you prefer to watch a video on getting started with tracing, refer to:

- The quickstart [Video guide](#video-guide).
- [Tracing Basics video](https://academy.langchain.com/courses/intro-to-langsmith) included in the Introduction to LangSmith LangChain Academy course.
</Tip>
...
## 1. Create a project and dependencies

Create a project for the app and install the dependencies in your environment:
...
1. Include the highlighted lines in your app file:
    ...
    from openai import OpenAI
    from langsmith.wrappers import wrap_openai  # traces SDK calls

    def retriever(query: str):
        return ["Harrison worked at Kensho"]

    client = wrap_openai(OpenAI())  # child run for the model call
    ...
    import "dotenv/config";
    import OpenAI from "openai";
    import { wrapOpenAI } from "langsmith/wrappers"; // traces SDK calls

    function retriever(query: string): string[] {
    return ["Harrison worked at Kensho"];
    }

    const client = wrapOpenAI(new OpenAI()); // child run for the model call
...
1. In the [platform UI](smith.langchain.com), select the **default** workspace (or the workspace you specified in [Step 2](#2-set-up-environment-variables)). Select the tracing project and then the run under the **Runs** tab. You'll find a trace of just the OpenAI call.
...
1. Include the highlighted code in your app file:
    ...
    from langsmith import traceable  # parent run for your function

    def retriever(query: str):
        return ["Harrison worked at Kensho"]

    client = wrap_openai(OpenAI())  # keep this so the LLM call nests as a child run
    ...
    import "dotenv/config";
    import OpenAI from "openai";
    import { wrapOpenAI, traceable } from "langsmith/wrappers"; // parent + child

    function retriever(query: string): string[] {
    return ["Harrison worked at Kensho"];
    }

    const client = wrapOpenAI(new OpenAI()); // keep this to nest the LLM call
...
1. Return to the [platform UI](smith.langchain.com), select the **default** workspace, or the workspace you specified in [Step 2](#2-set-up-environment-variables). Select the tracing project and then the most recent run under the **Runs** tab. You'll find a trace of the entire app pipeline.

<div style={{ textAlign: 'center' }}>
<img
    className="block dark:hidden"
    src="/langsmith/images/trace-quickstart-app.png"
    alt="LangSmith UI showing a trace of the entired application called rag with an input followed by an output."
/>
...
Tracing projects can contain a significant amount of data as you continue to trace your app. To filter traces in the UI, refer to the [Filter traces](/langsmith/filter-traces-in-application) page.
...
- [Trace a RAG application](/langsmith/observability-llm-tutorial) full tutorial, which adds observability to an application from development through to production.
