...
When tracing is enabled for the LLM Gateway, requests are recorded as LangSmith traces. The tracing configuration lets you control how long that data is retained.

To configure tracing:

1. Go to **Settings → Gateway → LLM Gateway**.
1. Open the **Data Policy** tab.
1. Under **Tracing**, set the data retention period.
1. Save.

Retention changes take effect for new traces. Existing traces are not affected retroactively.
