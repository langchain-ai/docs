---
title: Data policy
description: Configure tracing and data protection settings for LLM Gateway requests.
---
...
The **Data Policy** tab in LLM Gateway settings lets you control two areas:

- **Tracing**—configure data retention for requests flowing through the gateway.
- **Data protection**—scan and redact secrets and PII from requests before they reach LLM providers.

## Tracing

Every call through the LLM Gateway is traced to LangSmith. By default, trace content (request and response bodies) is **off** for all organizations. When enabled, gateway-proxied calls are written to:

- A project named `gateway` in the workspace associated with the caller's API key.
- A per-API-key project named `gateway-<short_api_key>-<api_key_id>` for UI isolation.

For more on where traces land, what metadata they carry, and how to restrict trace visibility, see [Traces and access control](/langsmith/llm-gateway-access).

### Data retention

The tracing configuration lets you control whether request and response bodies are captured and, if so, how long they are retained.

To configure tracing:

1. Go to **Settings → Gateway → LLM Gateway**.
1. Open the **Data Policy** tab.
1. Under **Tracing**, enable or disable trace content capture and set the data retention period.
1. Save.

Disabling trace content means request and response bodies are not written to the trace. Metadata (cost, policy evaluation results, token counts) is still recorded. Retention changes take effect for new traces; existing traces are not affected retroactively.
...
<Note>
**Private beta:** PII detection is not yet available. Contact your account team if you are interested in early access.
</Note>
...
1. Select **Secrets redaction** as the policy type. (PII redaction is not yet available.)
1. Configure which categories to detect (or enable all).
1. Save.

Redaction policies apply to all requests that pass through the gateway in the scope where they are configured. They take effect immediately.

### How redacted content appears

When a secret is detected, the content is replaced with a placeholder in both the request sent to the provider and the LangSmith trace. For example:
