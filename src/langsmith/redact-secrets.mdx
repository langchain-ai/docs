...
When the preset does not fit, `create_anonymizer` takes a list of regex patterns and replacement strings and applies only those. Pass the resulting anonymizer to the @[Client] constructor, and it will automatically apply to all run inputs and outputs before they reach LangSmith.
