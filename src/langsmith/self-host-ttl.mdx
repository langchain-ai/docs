...
When multiple TTL settings apply, LangSmith resolves them in the following order (highest precedence first):

1. **Project-scoped TTL** — set per project in the UI or API; overrides all other settings for that project.
2. **Workspace-scoped TTL** — the default retention applied to all projects in a workspace, unless a project-level value is set.
3. **Organization-scoped TTL** — the organization-wide default, applied when no workspace or project value is set.
4. **Helm-configured TTL** — the system-wide retention periods set via Helm (see [Requirements](#requirements) below); applies only when no UI-managed value is configured at the project, workspace, or organization level.
5. **System-defined defaults** — 14 days (short-lived) and 400 days (long-lived), used when no other configuration is present.
