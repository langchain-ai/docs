...
What that grouping is called, and which control selects it, depends on which information architecture your workspace uses. The sidebar tells you which one you are on:

- **Agent-based workspaces**, which are in [beta](/langsmith/release-stages) for a private group of customers, call it an [agent](/langsmith/agents), and an agent's traces divide further into four [environments](/langsmith/agent-environments). Select one from the agent picker in the top bar, or select **All agents** to see every resource in the workspace. For more information, see [Navigate agents](/langsmith/navigate-agents).
...
Any resource can be created without being tagged to either one. Those resources are visible when **All agents** or **All applications** is selected.
...
- **Agent-based workspaces** have environments built in. An [agent](/langsmith/agents) divides its traces across four of them, and every trace lands in one environment of one agent, so no tagging convention is required. For more information, see [Agent environments](/langsmith/agent-environments).
