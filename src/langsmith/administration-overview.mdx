...
- **Agent-based workspaces** have environments built in. Each [agent](/langsmith/agents) has four of them, and every trace lands in one environment of one agent, so no tagging convention is required. For more information, see [Agent environments](/langsmith/agent-environments).
