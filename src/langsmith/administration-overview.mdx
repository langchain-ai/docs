...
Each trace is limited to a maximum of **25,000 runs**. Once a trace reaches this limit, any additional runs sent for that trace are rejected.

This limit exists to maintain platform performance and typically indicates an application issue such as an infinite loop or unintended recursive tracing. If your traces are approaching this limit, see [Troubleshoot large traces](/langsmith/troubleshoot-large-traces) for guidance on identifying and resolving the root cause.
