...
- **Agent-based workspaces**, which are in [beta](/langsmith/release-stages) for a private group of customers, call it an [agent](/langsmith/agents), and an agent's traces divide further into [environments](/langsmith/agent-environments) drawn from a fixed set of four. Select one from the agent picker in the top bar, or select **All agents** to see every resource in the workspace. For more information, see [Navigate agents](/langsmith/navigate-agents).
