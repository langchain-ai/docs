...
Any resource can be created without being tagged to a single application. These resources will be visible when the `Show all applications` option is selected, as will any resource tagged to an application.

### Additional info

The following diagram explains the relationship between organizations, workspaces, applications, and resources: ![Resource Hierarchy](/langsmith/images/resource-hierarchy.png) TODO UPDATE THIS IMAGE
