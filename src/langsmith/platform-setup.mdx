...
<div class="home-page mx-auto max-w-8xl px-0 lg:px-5" style={{ paddingBottom: "8rem" }}>
    <div class="mdx-content prose prose-gray dark:prose-invert mx-4 pt-10">
        ...
        This section covers how to host and manage LangSmith infrastructure. Setting up LangSmith involves two independent choices: where the **LangSmith platform** ([observability](/langsmith/observability), [evaluation](/langsmith/evaluation), and [prompt engineering](/langsmith/prompt-engineering)) runs, and, if you use [LangSmith Deployment](/langsmith/deployment) for agent workloads, which topology it uses. For details on LangSmith Deployment for agent workloads, refer to [Deploy](/langsmith/deployment).

        <h2 class="flex whitespace-pre-wrap group font-semibold">Where LangSmith runs</h2>

        Set up LangSmith in one of two hosting models:
        ...
        **(Enterprise)** Full control with observability, evaluation, and prompt engineering in your infrastructure.
        ...
        - **Fastest to start, managed everything.** [LangSmith Cloud](/langsmith/cloud) paired with [LangSmith Deployment](/langsmith/deployment) on Cloud. LangChain hosts the platform, and, when you use LangSmith Deployment, also hosts your [Agent Servers](/langsmith/agent-servers).
