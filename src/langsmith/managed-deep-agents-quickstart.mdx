...
AZURE_AI_OPENAI_ENDPOINT=https://<resource>.services.ai.azure.com/openai/v1
...
This quickstart uses OpenAI by default. For Google, Anthropic, or Microsoft Foundry, set the matching values instead. `mda deploy` adds the provider key to the deployment. You can also use any [other chat provider](/oss/integrations/chat/).
...
When you deploy, Managed Deep Agents syncs these instructions to [LangSmith Context Hub](/langsmith/managed-deep-agents-context-hub), where you can update them without redeploying the agent.

  </Step>

  <Step title="Configure your model and search" id="configure-model-and-search">

Now set the model and a built-in web search tool. Google, OpenAI, Anthropic, and Microsoft Foundry offer server-side search without a separate search API key.

For Microsoft Foundry, use a deployment that supports `web_search` through the Responses API. Install the chat model integration:
...
Pass the provider tool dict that matches your model:
...
<CardGroup cols={2}>
  ...
  <Card title="Connections" icon="key" href="/langsmith/managed-deep-agents-connections">
