...
Now set the model and a built-in web search tool. Google, OpenAI, Anthropic, and Microsoft Foundry offer server-side search without a separate search API key. Pass the provider tool dict that matches your model:
