...
:::js
Create a project with the `mda` CLI, then open its directory. `npx --package managed-deepagents` runs the CLI without a global install:

```bash
npx --package managed-deepagents mda init research-assistant
cd research-assistant
```
:::
