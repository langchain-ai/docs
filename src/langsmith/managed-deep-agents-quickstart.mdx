...
`mda dev` loads the project `.env` file from the compiled local build so model provider keys and other runtime credentials are available during local development.
