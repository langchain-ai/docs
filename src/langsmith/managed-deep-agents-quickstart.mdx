...
To create a LangSmith API key, [sign up](https://smith.langchain.com), open [Settings](https://smith.langchain.com/settings), go to **API Keys**, and click **Create API Key**. For more details, see [Create an account and API key](/langsmith/create-account-api-key).

This quickstart uses OpenAI by default. If you choose Google or Anthropic in the next step, set that provider's API key instead. `mda deploy` uses the LangSmith API key to deploy the agent and adds the provider key to the deployment. You can also use any [other chat provider](/oss/integrations/chat/).
