...
A Managed Deep Agent is a [project directory](/langsmith/managed-deep-agents-overview#structure-your-agent-project) with a required agent entry (`agent.py`, `agent.ts`, or `agent.tsx`). To scaffold a project and chat with the agent locally:
...
For the full list of author-set and managed fields, see the [agent definition reference](/langsmith/managed-deep-agents-cli#agent-definition-reference).
