...
Scaffold a project and open its directory:
...
You now have all the scaffolding for your agent. The project declares `managed-deepagents` as a dependency, so you run the `mda` CLI from the project rather than installing it globally. Every command after this one runs from the project directory.
