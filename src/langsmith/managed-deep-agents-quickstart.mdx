...
LangChain hosts [Managed Deep Agents](/langsmith/managed-deep-agents-overview), so you can deploy your first agent and stream a response quickly without standing up infrastructure. This quickstart uses the CLI to deploy and the REST API to run.

For the full deploy workflow and all backend options, see [Deploy an agent](/langsmith/managed-deep-agents-deploy).

<Note>
Managed Deep Agents is in **private preview**. If you do not have access yet, [join the waitlist](https://www.langchain.com/langsmith-managed-deep-agents-waitlist) to request it before you start.
</Note>
...
Save the `agent_id` for the [run step](#api-run-the-agent). To confirm the agent deployed, list the agents in your workspace:
...
Run a Managed Deep Agent with the REST API. Reuse the `LANGSMITH_API_KEY` from earlier, or set it again if you opened a new shell, then set the remaining request defaults:
