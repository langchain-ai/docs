...
description: Use Gateway Credits to access hosted generative models without a provider key.
---
...
Gateway Credits let you call LangChain-hosted generative models through the standard LLM Gateway API without setting up a provider account or key. Authenticate with only your [LangSmith API key](/langsmith/create-account-api-key). No [provider secret](/langsmith/llm-gateway-admin-setup#1-add-provider-secrets) is required.

For typed classification, routing, and scoring, see [Decision models](/langsmith/llm-gateway-decision-models).
...
Hosted generative models use the same standard API formats as bring-your-own-key models:
