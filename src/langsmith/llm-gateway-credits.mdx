...
LangChain also hosts [SemIf](#use-semif-for-free), which does not require a Gateway Credits purchase when enabled for your organization on a paid, non-Enterprise plan.
...
| `semif-qwen3.5-4b` | SemIf for structured classification and scoring through the System One API. Free through September 28, 2026, when enabled for your organization on a paid, non-Enterprise plan. |
...
SemIf is a LangChain-hosted model for structured classification and scoring. It is free through September 28, 2026, with no provider key or Gateway Credits purchase required.

Your organization must be on a paid, non-Enterprise plan with SemIf enabled. You also need a workspace-scoped LangSmith API key with `gateway:invoke` and `workspaces:read`. In the gateway home page, select SemIf from the hosted models to get a request example. Availability depends on your organization and region.
