...
<Note>
Configuring workspace-level extended retention requires the **Workspace Admin** role (`workspaces:manage` permission). Organization-level retention defaults require the **Organization Admin** or **Organization Operator** role (`organization:manage` permission). For a full permissions reference, see [Organization and workspace operations](/langsmith/organization-workspace-operations).
</Note>
