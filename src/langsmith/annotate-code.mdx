...
<Note>
When you wrap a sync function with `traceable` (e.g., `formatPrompt` in the following example), use the `await` keyword when calling it to ensure the trace is logged correctly.
</Note>
