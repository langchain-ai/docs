...
```ts
const stream = client.threads.stream(threadId, { agentId });
...
The TypeScript SDK exposes a LangGraph projection stream with helpers such as `stream.messages`, `stream.values`, and `stream.output`. For route-level streaming details, use the Python SDK event iterator or the cURL example.
