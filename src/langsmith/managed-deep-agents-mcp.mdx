...
List the tools exposed by a registered MCP server before you reference them in an agent. The current SDKs register and connect MCP servers, but do not yet wrap the tool-listing route, so use the CLI or REST API for this discovery step.
...
Use `force_refresh=true` to bypass the cached tool definitions and fetch from the MCP server again. After you choose tool names from the response, [reference them](#reference-tools).
