---
title: Managed Deep Agents MCP servers
sidebarTitle: MCP servers
description: Register, connect, and reference MCP servers for Managed Deep Agents.
icon: "plug"
tag: "PREVIEW"
---
...
For the full command reference, see [Managed Deep Agents CLI reference](/langsmith/managed-deep-agents-cli). For route-level details, see the [Managed Deep Agents API reference](/langsmith/managed-deep-agents-api).
...
Use `interrupt_config` to require human approval before a tool runs. Key each entry by `"{mcp_server_url}::{tool_name}::{mcp_server_name}"` and set it to `true`.
