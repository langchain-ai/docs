...
MCP servers are workspace-level resources. Register or connect a server before you deploy an agent that references it. View all of your MCP servers in [LangSmith > Settings > MCP Servers](https://smith.langchain.com/settings/workspaces/mcp-servers).
...
- For CLI workflows, `deepagents-cli>=0.2.0`. Confirm the installed version with `deepagents --version`. See the [CLI reference](/langsmith/managed-deep-agents-cli) for installation.
...
For an OAuth server, run `deepagents mcp-servers connect <id|name|url>` after step 1. The sections below cover each step in detail.

Use the CLI for most setups. Use the REST API when you need a custom client, automation that cannot shell out to the CLI, or direct control over request payloads.

## Connect tools with the CLI

In the CLI, `add` registers a server and `connect` completes OAuth for a registered server.
...
The command prints each tool name with its first description line, then prints a `tools.json` snippet to copy into your project. Use this command to add or refresh tool entries after registering a server. Then [reference them](#reference-tools).
...
- **Deploy fails because a server URL is not registered:** Add the server with `deepagents mcp-servers add`, then redeploy.
- **Deploy fails because an OAuth server cannot be invoked:** Run `deepagents mcp-servers connect <id|name|url>`, then redeploy.
- **`mcp-servers tools` returns no tools:** Confirm the server URL is reachable and, for OAuth servers, that you completed `connect`.
- **CLI commands are missing or behave unexpectedly:** Confirm `deepagents --version` reports `0.2.0` or later. An older `deepagents` on your `PATH` can shadow the current release.
