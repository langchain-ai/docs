...
<Warning>
Treat the marker as an untrusted hint that can only reduce what a run may do. Anything that can reach your deployment can set it, so never use it to grant access, skip authentication, or widen permissions.
</Warning>
