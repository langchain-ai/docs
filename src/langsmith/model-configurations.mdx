...
<Note>
Providers disabled at the organization level appear locked here and can't be re-enabled per workspace. See [Disable providers organization-wide](#disable-providers-organization-wide).
</Note>

## Disable providers organization-wide

Organization admins can disable a model provider for the **entire organization**—across every [workspace](/langsmith/administration-overview#workspaces) and every feature—from a single setting. This organization-level control sits above the per-workspace [Feature Access](#feature-access) table: a provider disabled here is unavailable everywhere and can't be re-enabled by workspace admins.

This requires the [Organization Admin](/langsmith/rbac#organization-admin) role (the `organization:manage` permission) and is available on LangSmith [Cloud](/langsmith/cloud) and [Self-hosted](/langsmith/self-hosted) deployments. It applies to organizations, not personal accounts.

### Disable a provider for the organization

To configure organization-wide provider access in the [UI](https://smith.langchain.com):
...
Providers you disable here appear **locked** in every workspace's [Feature Access](#feature-access) table—workspace admins can see them but can't turn them back on. When a provider is enabled at the organization level, workspace admins keep full per-feature control over it.
