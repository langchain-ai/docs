...
<Note>
Organization-scoped model configurations are available on LangSmith [Self-hosted](/langsmith/self-hosted) and [Cloud Enterprise](/langsmith/pricing-plans). They are not available on Cloud Plus or personal accounts.

Self-hosted deployments must set the following Helm environment variable to enable this feature:

```yaml
frontend:
  deployment:
    extraEnv:
      - name: VITE_SELF_HOSTED_ORG_MODEL_CONFIGURATIONS_ENABLED
        value: "true"
```
