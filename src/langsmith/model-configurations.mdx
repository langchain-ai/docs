...
- **Secret round-trip**: the server returns the secret as `********`. The input renders empty with a *"Secret is set. Type to replace."* hint. Submitting without retyping leaves the stored secret unchanged.
    ...
    <img
        className="block dark:hidden"
        src="/langsmith/images/model-config-oauth-edit-masked-light.png"
        alt="Model configuration Edit modal re-opened on an OAuth-enabled preset, showing the Client Secret input empty with a Secret is set, type to replace hint and other OAuth fields preserved."
    />
    <img
        className="hidden dark:block"
        src="/langsmith/images/model-config-oauth-edit-masked-dark.png"
        alt="Model configuration Edit modal re-opened on an OAuth-enabled preset, showing the Client Secret input empty with a Secret is set, type to replace hint and other OAuth fields preserved."
    />
...
If the broker cannot mint a bearer (IdP unreachable, credentials invalid, configuration deleted between request preparation and execution), the request falls back to the static workspace API key for the provider. Check the broker's `langsmith_oauth_bearer_failures_total` counter and the `oauth_config_rejected` audit log for the cause.
</Accordion>
