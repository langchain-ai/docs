...
<Tabs>
  ...
  <Tab title="Dynatrace">
    ...
    **Prerequisites**
    ...
    2. Navigate to **Access Tokens** (under Manage)
    ...
    1. In the notification section of your alert set-up in LangSmith, select **Dynatrace**
    2. Enter your Dynatrace environment URL
    3. Click the key icon to save the API token as a Workspace secret or select an existing Workspace secret. As a best practice, save the API token as a Workspace Secret rather than adding it directly. This allows you to re-use the same token across alerts for a workspace.
    4. Configure additional notification options:
       - **Event Type**: Select the Dynatrace event type (e.g., `CUSTOM_ALERT`, `ERROR_EVENT`)
    5. Send a test alert by clicking **Send Test Alert**
