...
Alerts in LangSmith are scoped to one trace container, requiring separate configuration for each one you monitor. Which container that is depends on how your workspace is organized, and the sidebar tells you which one you are on.
...
In the [UI](https://smith.langchain.com), navigate to the tracing project you would like to configure alerts for, or to the agent environment in an agent-based workspace. Click the **Alerts** icon on the top right-hand corner of the page to view existing alerts for it and set up a new alert.
