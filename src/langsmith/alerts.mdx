  ...
  <Tab title="Webhook">
    ...
    **Optional fields**
    ...
    <Accordion title="Configure Microsoft Teams notifications via webhook">
      Here is an example for configuring LangSmith alerts to send notifications to a Microsoft Teams channel using an [Incoming Webhook connector](https://learn.microsoft.com/en-us/microsoftteams/platform/webhooks-and-connectors/how-to/add-incoming-webhook).

      **Prerequisites**

      - Access to a Microsoft Teams workspace with permissions to add connectors
      - A LangSmith project to set up alerts
      ...
      **Step 1: Create an Incoming Webhook in Teams**
      ...
      5. Provide a name for the webhook (e.g., "LangSmith Alerts") and optionally upload an image
      6. Click **Create**
      7. Copy the webhook URL that is generated — you will use this in LangSmith
      ...
      **Webhook URL**
      ...
      Paste the Incoming Webhook URL you copied from Teams:
      ...
      **Request Body Template**
      ...
      <Note>
      Replace `{project_url}` with your LangSmith project URL. The fields `alert_name`, `project_name`, `triggered_metric_value`, and `triggered_threshold` are populated automatically by LangSmith at alert time.
      </Note>
      ...
      ```json
      {
        ...
        "themeColor": "FF0000",
        "summary": "{alert_name} triggered",
        "sections": [
          {
            "activityTitle": "🚨 LangSmith Alert: {alert_name}",
            "activitySubtitle": "Project: {project_name}",
            "facts": [
              {
                "name": "Metric value",
                "value": "{triggered_metric_value}"
              },
              {
                "name": "Threshold",
      ...
      **Step 3: Test the integration**
      ...
      <Note>
      Microsoft's legacy Office 365 Incoming Webhook connectors are being retired. If your organization uses the newer **Workflows** app in Teams (Power Automate), create a workflow with the "Post to a channel when a webhook request is received" template instead, and use the workflow's HTTP trigger URL as your webhook endpoint in LangSmith.
      </Note>
    </Accordion>
    ...
    <Accordion title="Configure email notifications via webhook">
      ...
      **Request Body Template**
      ...
      <Note>
      Replace `alerts@your-company.com` with your verified sender address and `oncall@your-company.com` with the recipient address. The fields `alert_name`, `project_name`, `triggered_metric_value`, `triggered_threshold`, and `timestamp` are populated automatically by LangSmith at alert time.
      </Note>
      ...
      ```json
      {
        "personalizations": [
          {
            ...
            "subject": "🚨 LangSmith Alert: {alert_name}"
          }
        ],
        "from": {
        ...
        "content": [
          {
            "type": "text/plain",
            "value": "Alert '{alert_name}' was triggered for project '{project_name}'.\n\nMetric value: {triggered_metric_value}\nThreshold: {triggered_threshold}\nTimestamp: {timestamp}"
          },
          {
            "type": "text/html",
            "value": "<h2>🚨 LangSmith Alert: {alert_name}</h2><p><strong>Project:</strong> {project_name}</p><ul><li><strong>Metric value:</strong> {triggered_metric_value}</li><li><strong>Threshold:</strong> {triggered_threshold}</li><li><strong>Timestamp:</strong> {timestamp}</li></ul>"
          }
        ]
      }
      ```
      ...
      **Using other email providers**
      ...
      The same pattern works with other transactional email APIs. Change the **Webhook URL** and **Headers** to match your provider:
      ...
      | Amazon SES (SMTP) | Use an SMTP relay or SES HTTP endpoint | See [AWS SES docs](https://docs.aws.amazon.com/ses/latest/dg/send-email-api.html) |

      Adjust the **Request Body Template** to match each provider's expected payload format.
    </Accordion>
    ...
    - [Microsoft Teams Workflows app](https://learn.microsoft.com/en-us/microsoftteams/platform/webhooks-and-connectors/how-to/add-incoming-webhook?tabs=newteams%2Cdotnet)
