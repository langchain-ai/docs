...
A connection is a named credential in the LangSmith workspace. Tools and MCP connectors resolve it at runtime. Create connections in one of three modes: opaque secret (fixed API key), general OAuth (BYOT app from the catalog or custom endpoints), or MCP OAuth (discover and register from an MCP server URL). Use `mda connections` to manage these credentials in the current workspace.
