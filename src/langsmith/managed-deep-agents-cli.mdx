...
For the end-to-end deploy workflow, see [Managed Deep Agents](/langsmith/deploy-managed-deep-agent). For MCP setup details, see [MCP servers](/langsmith/managed-deep-agents-mcp).
...
Bare `deepagents` invocations no longer start an interactive REPL. Install and run [Deep Agents Code](/oss/deepagents/code/overview) for interactive coding sessions.
...
For a practical setup guide, see [MCP servers](/langsmith/managed-deep-agents-mcp).
...
Supported backend `type` values are `default`, `thread_scoped_sandbox`, and `agent_scoped_sandbox`. The legacy `sandbox` value is normalized to `thread_scoped_sandbox`.

Sandbox backend settings can include:
...
Each tool requires `name` and `mcp_server_url`. `mcp_server_name` and `display_name` are optional. `interrupt_config` is optional and must be an object when present.
