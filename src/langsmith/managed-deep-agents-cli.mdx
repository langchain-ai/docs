...
The `managed-deepagents` package exposes the `mda` binary, and the language you author in is set by the package you run it from: the PyPI package builds Python projects, the npm package builds TypeScript ones.

Scaffold the first project with a one-off run of the package, then run every later command from inside the project, where `mda init` has already declared `managed-deepagents` as a dependency. Using the project-local package keeps the CLI, the runtime, and the authoring API on one version.
...
`uv sync` installs the project dependencies, including the `mda` console script; `uv run mda …` runs it. The package provides agent, identity, schedule, and sandbox authoring APIs with snake-case names.
:::
...
`npm install` installs the project dependencies, including the `mda` binary; `npx mda …` runs it from `node_modules`. The package provides agent, identity, schedule, and sandbox authoring APIs.

<Note>
Use `npx managed-deepagents` — the package name — to bootstrap. `npx mda` resolves the `mda` binary only from inside a project that already depends on `managed-deepagents`.
</Note>
:::
...
The tables name the commands. Run them from the project root through the local package: `uv run mda <command>` in Python, `npx mda <command>` in TypeScript.
...
Use `mda init` to create a new project directory. There is no project to run it from yet, so run the published package directly:
...
The scaffold language comes from the package you run, not from the current directory: the PyPI package writes a Python project, the npm package writes a TypeScript one. A CLI from one package refuses a project authored for the other.
