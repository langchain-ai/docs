...
| `tools.json` | Empty tool configuration. Add MCP-backed tools after registering a server. |
| `skills/example-skill/` | Example skill directory. Edit, replace, or delete it. |
| `subagents/researcher/` | Example subagent directory. Edit, replace, or delete it. |
...
The legacy `model_id` key is still accepted in local subagent files, but new projects should use `model`. The REST API `SubagentSpec` still uses `model_id`.
