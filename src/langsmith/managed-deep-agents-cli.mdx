...
The CLI reads `LANGSMITH_API_KEY`, or `LANGCHAIN_API_KEY` as a fallback. To create a key, see [Create an API key](/langsmith/create-account-api-key). To override the default endpoint, set `LANGSMITH_ENDPOINT` (or `LANGCHAIN_ENDPOINT` as a fallback).
