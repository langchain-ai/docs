...
For credential owners, create modes, caller identity, and runtime examples, see [Manage connections](/langsmith/managed-deep-agents-connections).
...
| `--context-strategy overwrite\|keep-hub` | Resolve Context Hub conflicts when `instructions.md` or `skills/` changed in Hub since the last sync. Required in non-interactive shells. See [Context Hub](/langsmith/managed-deep-agents-context-hub#source-of-truth). |
