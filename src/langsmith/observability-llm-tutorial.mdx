...
if __name__ == "__main__":
    ...
    session_id = ls_client.read_project(project_name=project_name).id  # [!code highlight]
...
(async () => {
    ...
    // Feedback requires the UUID of the tracing project that owns the run // [!code highlight]
    const { id: sessionId } = await lsClient.readProject({ // [!code highlight]
        projectName: capturedProjectName, // [!code highlight]
    }); // [!code highlight]
    await lsClient.createFeedback(capturedRunId, "user-score", { // [!code highlight]
