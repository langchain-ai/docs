...
The sandbox proxy accepts [Agent Auth connections](/langsmith/managed-deep-agents-connections) as header values to authenticate outbound requests.

:::python
Set `proxy_config` in `define_sandbox` to configure proxy rules.
:::

:::js
Set `proxyConfig` in `defineSandbox` to configure proxy rules.
:::

Use a connection reference as the header value, or format it with `bearer(ref)` or `basic(username, ref)`. Omit the header's `type`; Managed Deep Agents sets it to `opaque`.

Managed Deep Agents resolves credentials before each command and updates changed values. Long commands and background processes can fail if their tokens expire. Later runs in the same thread replace the proxy credentials used by background processes.
