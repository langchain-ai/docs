...
Store `OPENAI_API_KEY` in your LangSmith workspace secrets. This rule adds it to requests from the sandbox to the OpenAI API:
...
Use a user-owned connection to authenticate sandbox requests with the caller's credentials. If the caller has not authorized the connection, Managed Deep Agents interrupts the run before the first model call to request authorization. For setup and authorization details, see [Manage connections](/langsmith/managed-deep-agents-connections).
