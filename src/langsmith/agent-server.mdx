...
3. As the graph executes, the worker writes checkpoints to the persistence layer (the frequency depends on the [durability mode](/oss/langgraph/checkpointers#durability-modes)) and broadcasts streaming events over the configured pubsub provider.
