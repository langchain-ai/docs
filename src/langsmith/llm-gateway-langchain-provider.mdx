...
Gateway Credits are denominated in **LangChain Credit Units (LCUs)** at **$1.50 per LCU**. Each model call consumes LCUs based on token usage. For current rates and plan details, refer to [langchain.com/pricing](https://www.langchain.com/pricing).
...
Like all gateway traffic, Gateway Credit calls are traced to LangSmith. For where traces land and how to control access to them, refer to [Traces, Engine, and access control](/langsmith/llm-gateway-access).
