...
<Note>
Gateway routing for LangChain chat models requires Python and a supported `langchain-*` integration package at the minimum version listed in the [gateway quickstart](/langsmith/llm-gateway-langchain). If the integration package is below the minimum version, the call will bypass the gateway and go directly to the provider.
</Note>

For full configuration options, provider support, and regional endpoints, see the [LLM Gateway quickstart](/langsmith/llm-gateway).
