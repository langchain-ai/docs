...
**Setup**: Configure your MCP server connection once in workspace settings.

```mermaid
sequenceDiagram
    participant User
    participant Workspace as Workspace Settings/Secrets

    User->>Workspace: Add MCP server URL
    User->>Workspace: Add headers (e.g., Authorization: Bearer)
    User->>Workspace: Add secret values (e.g., MCP_TOKEN)
```
