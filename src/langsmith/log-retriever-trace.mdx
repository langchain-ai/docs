...
Pass [`run_type="retriever"`](/langsmith/run-data-format#run_type) to the @[traceable] decorator (Python) or `traceable` wrapper (TypeScript). This tells LangSmith to treat the step as a retrieval run and apply retriever-specific rendering in the LangSmith UI.
...
@traceable(run_type="retriever")
def retrieve_docs(query):
    # Returning hardcoded dummy documents.
