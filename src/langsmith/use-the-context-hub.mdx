...
Each save creates a new **commit** in the history panel on the left. Select a commit to browse its files. Context Hub does not provide a file comparison view or an action to restore a prior version as the latest commit.
...
- [Mount a Context Hub repo in a sandbox](/langsmith/sandbox-mounts#mount-a-context-hub-repo): give sandbox code read-only filesystem access to a repo that stays in sync as it changes.
