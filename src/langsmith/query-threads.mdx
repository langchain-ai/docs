...
for (const thread of threads) {
  ...
  for (const run of thread.runs) {
    console.log(`    - ${run.name} (${run.run_type})`);
  }
}
```
...
Each entry represents one conversation, with threads sorted by most recent activity. The `runs` list within each thread is ordered chronologically, which provides a ready-made timeline of the conversation:
...
[
    {
        ...
        "count": 3,
        "min_start_time": "2026-02-25T10:00:00.123456+00:00",
        "max_start_time": "2026-02-25T10:05:42.000000+00:00",
        "runs": [
    ...
    {
        "thread_id": "conv-def456",
        "count": 1,
        "min_start_time": "2026-02-25T09:30:00.000000+00:00",
        "max_start_time": "2026-02-25T09:30:00.000000+00:00",
        "runs": [
            Run(id=UUID("g7h8..."), name="my_agent", run_type="chain", status="success", start_time=datetime(2026, 2, 25, 9, 30, 0, tzinfo=utc), ...),
        ],
    },
]
```
