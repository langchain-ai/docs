...
In addition to traces, LangSmith charges for deployed agents via LangSmith Deployment (formerly LangGraph Platform). Deployments are billed on the resources they consume:

- **Compute**: The vCPU and memory a deployment uses while running, measured in LangChain Compute Units (LCU). Because a [`Serverless`](/langsmith/cloud-platform-features#serverless) deployment scales to zero when idle, it consumes compute only while handling requests, so you pay little to nothing while it is idle. A [`Dedicated`](/langsmith/cloud-platform-features#dedicated) deployment is always-on and consumes compute continuously.
...
<Note>
This usage-based model replaces the previous per-run and uptime pricing. Existing deployments remain on their current pricing during a transition period before moving to the new model. For the transition timeline that applies to your account, see the [pricing page](https://www.langchain.com/pricing) or contact your account team.
</Note>
