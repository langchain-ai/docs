...
1. Choose one of the following:
   ...
   - **Create from a template**: Start from a ready-made evaluator for common evaluation patterns. Templates are organized by the following categories:
     ...
     |----------|-------------|
     | Recommended | A curated selection of commonly used templates across categories |
     | Security | Detect security vulnerabilities such as PII leakage, prompt injection, and code injection |
     | Safety | Assess outputs for harmful content, including toxicity and bias |
     | Quality | Measure response quality, including correctness, conciseness, hallucination, and code validity |
     | Categorization | Classify conversation properties such as perceived errors, support intent, and language |
     | User Experience | Evaluate user satisfaction and task completion across conversations |
     | Image Evaluations | Detect explicit or sensitive content in image outputs |
     | Voice Evaluation | Assess voice interaction quality, including transcription accuracy and audio properties |
     | Trajectory | Assess agentic traces for plan adherence, tool selection, and trajectory accuracy |
