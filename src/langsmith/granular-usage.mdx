...
For self-hosted instances, data collection begins when the feature is enabled via environment variables or after upgrading to a version with it enabled by default. Set the following in `commonEnv`:
