...
<Note>
Granular billable usage data collection started on January 5, 2026. Data is not available for usage before this date.
</Note>
...
- Export usage data for internal reporting or chargeback purposes

## Prerequisites

- You must have `ORGANIZATION_READ` permission to access granular usage data
...
This endpoint accepts the same query parameters as the granular usage endpoint and returns a CSV file with the following columns:
...
3. Use the controls to:
   - Select a time range (Last 7 days, 30 days, 3 months, 6 months, or custom)
