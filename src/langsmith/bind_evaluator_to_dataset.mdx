...
* **Programmatically**, by specifying evaluators in your code (see [this guide](/evaluation/how_to_guides/evaluate_llm_application) for details)
...
* An `Example` ([reference](/reference/data_formats/example_data_format)). This represents the reference example in your dataset that the chain or model you are testing uses. The `inputs` to the Run and Example should be the same. If your Example has a reference `outputs`, then you can use this to compare to the run's output for scoring.
