...
The default identity declaration verifies a LangSmith API key. That key authenticates the calling client, not an individual person, so every caller who presents it resolves to the same identity. To give each signed-in person their own credentials, declare [Supabase identity](/langsmith/managed-deep-agents-identity#configure-identity-with-supabase).
...
OIDC principal authentication binds an end user's verified identity to their user-owned connections. It lets a custom application identify callers without sending a user ID that the application chose.
...
For more information, see [Connect to MCP servers](/langsmith/managed-deep-agents-mcp-connectors).
