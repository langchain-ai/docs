...
Agent-owned opaque secrets belong to the project's Managed Deep Agents deployment, so create them after at least one successful [`mda deploy`](/langsmith/managed-deep-agents-deploy).
...
Add the server in `connectors/` and reference the connection slug. For more information, see [Connect to MCP servers](/langsmith/managed-deep-agents-mcp-connectors).
...
With [`useStream`](/oss/langchain/frontend/human-in-the-loop#setting-up-usestream) (`@langchain/react`, `@langchain/vue`, `@langchain/svelte`) or `injectStream` (`@langchain/angular`), the pending interrupt is on `stream.interrupt`. The credential gate payload is `stream.interrupt.value`.
...
For the general interrupt lifecycle (`stream.interrupt`, resume, checkpoints), see [Human-in-the-loop](/oss/langchain/frontend/human-in-the-loop).
...
- [Human-in-the-loop](/oss/langchain/frontend/human-in-the-loop)
