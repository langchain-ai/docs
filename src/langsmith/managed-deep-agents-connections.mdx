...
A connection is a named credential in your LangSmith workspace. Tools and MCP connectors resolve it at runtime with `connections.get(...)`, so secrets stay out of source code and each caller can authorize their own account when needed.
