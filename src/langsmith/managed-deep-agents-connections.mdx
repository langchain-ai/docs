...
- **Reuse one credential across agents**: Connections belong to the workspace, so several deployments can resolve the same slug. Rotating the value takes effect on the next run of each, with no redeploy. One developer can configure a Connection which other developers can use in their agents.
...
A connection is a container, and it belongs to the workspace rather than to an owner. The credentials inside it have owners, and `mda connections create` sets the owner by which flags you pass. There is no `--owner` flag:

| Create with | Credential owner | Code asks for |
| --- | --- | --- |
| `--secret-from-env`, `--secret-from-file`, or stdin | The agent | `{"type": "agent"}` |
| `--oauth` or custom endpoints, without `--authorize` | Each caller, granted at runtime | `{"type": "user"}` |
| `--oauth` or `--mcp` with `--authorize` | The agent | `{"type": "agent"}` |

`connections.get(...)` selects an owner. It does not create one. Asking for an owner with no stored credential fails the run for `agent`, and raises a [`credential_authorization_required` interrupt](#handle-the-authorization-interrupt) for `user`.
