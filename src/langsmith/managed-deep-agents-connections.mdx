...
A connection links a managed deep agent to an external service such as GitHub, Notion, or Tavily. The credential lives in your LangSmith workspace, and tools and MCP connectors resolve it at runtime with `connections.get(...)`.

Connections let you:

- **Rotate a credential without redeploying**: Each run resolves the current value from the workspace, so a rotated key takes effect on the next run.
...
| --- | --- | --- |
| `agent` | One credential that belongs to the deployment. Every caller uses it. | The capability is the same for everyone, such as web search with Tavily or read-only access to a company knowledge base. |
| `user` | The calling person's own credential. Each person authorizes their own account. | The agent acts on someone's behalf, such as searching Notion pages they have access to or sending email as them. |

:::python
```python
# The same slug can resolve either owner. The call site decides.
agent_value = await connections.get("organization-tavily", {"type": "agent"})
user_value = await connections.get("engineering-notion", {"type": "user"})
```
:::
...
```typescript
// The same slug can resolve either owner. The call site decides.
const agentValue = await connections.get("organization-tavily", { type: "agent" });
const userValue = await connections.get("engineering-notion", { type: "user" });
```
:::

Ownership is a property of the call, not of the stored connection. `connections.get(...)` resolves to the credential value as a string.

An agent-owned connection holds either an opaque secret or an OAuth grant that you [authorize once for the deployment](#authorize-an-agent-owned-oauth-account). A user-owned connection holds one OAuth grant per caller. When the caller has not authorized a user-owned connection yet, the run pauses with a [`credential_authorization_required` interrupt](#handle-the-authorization-interrupt) instead of failing.

Agent-owned credentials belong to the project's deployment, so create them after at least one successful [`mda deploy`](/langsmith/managed-deep-agents-deploy).
...
| --- | --- |
| [Slack channel](/langsmith/managed-deep-agents-channels-slack) | The Slack user who sent the message. |
| LangSmith Studio | The signed-in LangSmith user. |
| SDK client or custom frontend | The identity that the project's [identity declaration](/langsmith/managed-deep-agents-identity) authenticates. |

The default identity declaration verifies a LangSmith API key. That key authenticates the calling client, not an individual person, so every caller who presents it resolves to the same identity. To give each signed-in person their own credentials, declare [Supabase identity](/langsmith/managed-deep-agents-identity#authenticate-end-users-with-supabase).
...
| **MCP OAuth** | An MCP server advertises OAuth and can register a client automatically. | `--mcp <url>`, or the connection slug alone when the project already declares that user-owned MCP server |

Do not mix modes in one command. For example, `--mcp` cannot combine with `--oauth`, custom endpoints, `--client-id`, or a secret value.

### Name a connection

The first argument to `mda connections create` is a slug: your name for the connection, and the name your code passes to `connections.get(...)`. It is not the provider name, which goes to `--oauth`. In `mda connections create engineering-notion --oauth notion-api`, `engineering-notion` is the slug and `notion-api` is the catalog provider.
...
Every OAuth provider needs the same five settings: an authorization URL, a token URL, a token endpoint authentication method, authorization parameters, and default scopes. The catalog stores those for common services, so you supply only your own client ID and client secret.

The catalog is a convenience, not the list of services you can connect to. Any OAuth provider works if you [pass the endpoints yourself](#register-a-provider-with-custom-endpoints).
...
The command prints each service's default scopes and the page where you register an app. It does not call LangSmith, so it needs no workspace ID or API key. The catalog covers common providers, including `github`, `google`, `linear`, `slack`, `atlassian`, and `notion-api`.
...
- [Add identity to Managed Deep Agents](/langsmith/managed-deep-agents-identity)
