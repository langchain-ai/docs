...
<Note>
This example requires the `requests` and `requests_toolbelt` libraries. Install these packages with the following:
<CodeGroup>
```bash pip
pip install -qU requests requests_toolbelt
```

```bash uv
uv add requests requests_toolbelt
```
</CodeGroup>
</Note>
