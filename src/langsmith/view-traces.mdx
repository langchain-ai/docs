...
- **Add to an annotation queue:** Send the run or its entire [thread](/langsmith/observability-concepts#threads) to a queue for human review and feedback. Refer to [Annotation queues](/langsmith/annotation-queues#assign-runs-to-a-single-run-queue).
