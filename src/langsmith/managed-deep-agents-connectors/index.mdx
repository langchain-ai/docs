...
To scaffold a connector and complete its workspace OAuth from the terminal, use [`mda connect`](/langsmith/managed-deep-agents-cli#connect-integrations) (for example `mda connect gmail`). Run `mda connect --list` to see every connector and its connection status.
