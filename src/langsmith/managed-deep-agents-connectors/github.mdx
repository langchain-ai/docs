...
Each repository entry accepts `repo`, `path`, `ref`, `depth`, and `on_reuse` / `onReuse`, plus an optional `write` field. Repository paths must be relative and unique. Set `write` to `true` on a checkout that needs write credentials.
...
The connector runs only when the project declares a managed sandbox. After startup, ask the agent to inspect the configured path or run `gh auth status`. For deploy symptoms and fixes, see [Troubleshooting](/langsmith/managed-deep-agents-cli#troubleshooting).
