...
Instead of connecting the integration in the LangSmith UI, you can scaffold the connector and complete its OAuth from the terminal with [`mda connect`](/langsmith/managed-deep-agents-cli#connect-integrations):

```bash
mda connect --list   # see every connector and its connection status
mda connect gmail    # scaffold connectors/gmail and complete OAuth in the browser
```

`mda connect <name>` creates the connector file if it does not exist, opens the authorization URL in your browser, and waits for the workspace connection to complete. Use `mda connect <name> --check` to verify a connection and `mda connect <name> --revoke` to disconnect. OAuth integrations (Google, X, and others) connect this way; secret-based integrations (for example Exa or Tavily) take a workspace API key in LangSmith instead.
