...
| Trusted backend | `X-MDA-Ingress-Secret`, `X-MDA-User-Id`, and `X-MDA-Organization-Id` when multi-tenant |
...
curl -X POST \
  ...
  -H "X-MDA-Organization-Id: $ORGANIZATION_ID" \
  -d '{
    "action": "create",
    "runId": "<langsmith-run-id>",
    "key": "user_score",
...
Send `X-MDA-Organization-Id` only for multi-tenant deployments. For how the runtime resolves these headers, see [identity ingress](/langsmith/managed-deep-agents-identity#auth-identify-the-caller).
