...
Ask a user to run the [verification cURL from the quickstart](/langsmith/llm-gateway-quickstart#make-a-call). A `200` response confirms the gateway, the API key, provider secrets, and role permissions are all configured correctly. The call will appear as a trace in the **gateway** tracing project in the workspace.
