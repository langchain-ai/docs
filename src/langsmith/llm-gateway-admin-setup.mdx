...
Gateway calls read workspace Provider Secrets in every case except one: when a request resolves an [organization model configuration](/langsmith/model-configurations#organization-scoped-configurations), the gateway reads that configuration's key from [organization secrets](/langsmith/model-configurations#organization-secrets) instead.
