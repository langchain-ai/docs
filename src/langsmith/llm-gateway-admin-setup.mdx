...
Skip this step for LangChain-hosted models: they use your LangSmith API key, not provider secrets. Paid hosted models use [Gateway Credits](/langsmith/llm-gateway-credits). SemIf (`semif-qwen3.5-4b`) is free through September 28, 2026, when enabled for your organization on a paid, non-Enterprise plan. It requires no Gateway Credits purchase. Continue with step 2 to grant gateway access.
...
For SemIf, ask a user to select it from the hosted models on the gateway home page, or run the [SemIf cURL example](/langsmith/llm-gateway-credits#use-semif-for-free). It uses `POST /v1/systemone` for structured classification and scoring, not a chat endpoint. A `200` response confirms model access, the API key, and role permissions without requiring provider secrets or consuming Gateway Credits. If SemIf is not enabled for your organization, adding provider secrets does not enable it.
