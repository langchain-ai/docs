...
<Note>
These limits apply to the deprecated `POST /api/v1/runs/query` endpoint. The SmithDB-backed replacement, `client.runs.query()`, has a dedicated limit of 300 requests per 10 seconds and is not subject to the per-tenant tiers above. See [Migrate to SmithDB-backed SDK methods](/langsmith/smithdb-sdk-migration#rate-limits).
</Note>
