...
On a later deploy, the CLI detects instructions that changed in Context Hub since the previous sync. An interactive deploy asks whether to overwrite the Hub edit with the local `instructions.md`; an unattended deploy requires an explicit conflict strategy. See [Resolve Context Hub edits](/langsmith/managed-deep-agents-deploy#resolve-context-hub-edits).
