...
On [self-hosted LangSmith](/langsmith/self-hosted), the concurrency limits above are the defaults. If your workers are running out of memory (OOM) during bulk exports, you can lower these values using the following environment variables on the `langsmith-backend` service:

| Environment variable | Default | Description |
|---|---|---|
| `BULK_EXPORT_MAX_CONCURRENT_RUNS` | `45` | Maximum number of export runs that can execute in parallel within a single export job. Reduce this to limit peak memory usage when processing large date partitions. |
| `DATA_EXPORT_RUN_LIMIT` | `15` | Maximum number of export jobs that can run concurrently across the workspace. Reduce this if multiple simultaneous exports are causing memory pressure. |
| `DATA_EXPORT_MAX_BATCH_PAYLOAD_SIZE_KB` | (system default) | Maximum size (in KB) of a single batch payload fetched from the database per export run. Reduce this to lower the memory footprint of each individual batch read. |
...
Lowering these values reduces parallelism and may increase total export time, but prevents OOM errors on nodes with limited memory.
</Note>
