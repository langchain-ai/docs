...
Custom tools are application code the agent can call for fetching real-time data, querying databases, executing code, and taking actions. Unlike [instructions](/langsmith/managed-deep-agents-instructions) and [skills](/langsmith/managed-deep-agents-skills), MDA does not discover them automatically.

To load tools from a remote MCP server, see [Connect to MCP servers](/langsmith/managed-deep-agents-mcp-connectors).
...
To pause on a form your own tool posts in Slack instead, see [Agent-owned interrupts](/langsmith/managed-deep-agents-agent-owned-interrupts).
...
LangSmith Managed Tools provides Parallel Search through an MCP server. Set `LANGSMITH_WORKSPACE_ID` in `.env`. To test in Studio, also set `LANGSMITH_API_KEY` to your personal LangSmith API key.

:::python
In `tools/mcp.py`, add `Parallel` to the `servers` map in your existing `mcp` declaration.
:::

:::js
In `tools/mcp.ts`, add `Parallel` to the `servers` map in your existing `mcp` declaration.
:::

Set its `transport` to `http` and its `url` to `https://api.smith.langchain.com/v1/managed-tools/servers/parallel/mcp`. Keep all servers in one `mcp` declaration.
...
| **[MCP connectors](/langsmith/managed-deep-agents-mcp-connectors)** | Managed configuration | Declared in the MCP module under `tools/`; no import into the agent entry |
...
If a tool requires an API key or OAuth token, use a connection to resolve the credential at runtime. See [Manage connections](/langsmith/managed-deep-agents-connections).

## Access runtime context

For per-run values such as request metadata or feature flags, use the normal LangChain runtime context patterns for tools. See [how to access context from within your tools](/oss/langchain/tools#access-context).

To read or write files in the thread's sandbox from a tool, use `runtime.backend`. See [Read and write sandbox files from code](/langsmith/managed-deep-agents-sandboxes#read-and-write-sandbox-files-from-code).
