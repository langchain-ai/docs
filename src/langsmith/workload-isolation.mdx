...
An agent-based workspace groups resources by [agent](/langsmith/agents), and each agent's traces divide across four [environments](/langsmith/agent-environments). A project-based workspace groups them by application instead, with no environment tier. The models below apply to both, and the differences are called out where they matter.
