...
1. Select an action to trigger when the rule is applied. There are four actions you can take with an automation rule:
    ...
    - **Add to annotation queue**: Add the matching run/trace to an [annotation queue](/langsmith/annotation-queues) as a **run** item. Automation rules do not enqueue entire [threads](/langsmith/observability-concepts#threads) as thread items; to review a full conversation, add the thread from the UI ([Assign runs and threads](/langsmith/annotation-queues#assign-runs-to-a-single-run-queue)).
