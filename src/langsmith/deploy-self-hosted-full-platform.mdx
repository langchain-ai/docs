...
Slack setup is documented with the rest of the Slack integration, because the same provider powers both Slack tools and the Slack apps you add to individual agents.

For the full walkthrough, covering the Slack app, bot scopes, provider registration, redirect URI, and Helm values, see [Set up Slack on Self-hosted](/langsmith/fleet/slack-app#set-up-slack-on-self-hosted).
