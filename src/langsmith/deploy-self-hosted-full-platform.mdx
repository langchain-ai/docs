...
4. Optionally configure access for the control plane to read Agent Server deployments logs from the other namespaces. See [instructions](#optional-configure-access-for-reading-agent-server-deployments-logs-from-other-namespaces) below for more details.
...
1. Create a `Role` with the required permissions in the Kubernetes namespace where the Agent Server is deployed. Replace `<data_plane_namespace>` in the command below.
        ```
        ...
        EOF
        ```
2. Get the control plane `ServiceAccount`. Replace `<control_plane_namespace>` in the command below.
        ```
        kubectl get serviceaccounts -n <control_plane_namespace> | grep host-backend
        ```
2. Bind that `Role` to the control plane `ServiceAccount`. Replace `<data_plane_namespace>`, `<control_plane_namespace>`, and `<control_plane_service_account>` in the command below.
        ```
        kubectl apply -n <data_plane_namespace> -f - <<EOF
        apiVersion: rbac.authorization.k8s.io/v1
        kind: RoleBinding
...
In this example, the `Role` and `RoleBinding` are created in the Kubernetes namespace where the Agent Server deployment exists. The name of the `Role` and `RoleBinding` are arbitrary and can be customized to whatever you want.
