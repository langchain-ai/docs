...
<Steps>
  ...
  <Step title="Generate the Engine encryption key">
    ...
    To rotate the key later, copy the current value to `engine_encryption_key_previous` and set the new key as `engine_encryption_key`. The previous key is accepted for decryption only, so runs encrypted just before the swap still completes.
  </Step>
</Steps>
