...
This guide walks you through enabling LangSmith Deployment, Fleet, Insights, and Polly on your [self-hosted LangSmith](/langsmith/self-hosted) instance.

- **[LangSmith Deployment](/langsmith/deployment)** adds a [control plane](/langsmith/control-plane) and [data plane](/langsmith/data-plane) that let you deploy, scale, and manage agents and applications directly through the LangSmith UI. After enabling it, you'll have access to LangSmith [Observability](/langsmith/observability), [Evaluation](/langsmith/evaluation), and [Deployment](/langsmith/deployment).
...
<Steps>
  <Step title="Complete the Kubernetes installation guide">
    Follow the [Kubernetes installation guide](/langsmith/kubernetes) before continuing.
  </Step>
  ...
  <Step title="Configure an ingress">
    Configure an ingress, gateway, or Istio for your LangSmith instance. All agents will be deployed as Kubernetes services behind this ingress. See [Set up an ingress](/langsmith/self-host-ingress). You must provide a `hostname` in your `values.yaml`.
  </Step>
...
Fleet, Insights, and Polly all require LangSmith Deployment to be enabled first. Enabling it provisions the following resources in your cluster:
...
To enable LangSmith Deployment, update your Helm chart:
...
<Steps>
  ...
  <Step title="Apply during Helm installation or upgrade">
    When you deploy or upgrade your LangSmith instance using Helm, this configuration is applied automatically. All user deployments created through the LangSmith UI inherit these registry credentials.
  </Step>
</Steps>
...
[Fleet](/langsmith/fleet/index), [Insights](/langsmith/insights), and [Polly](/langsmith/polly) each require the `agentBootstrap` job and a Fernet encryption key. You can enable all three in a single Helm configuration.

### Generate encryption keys

Each feature uses its own encryption key. Generate one key per feature using Python:

```bash
python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"
```

Run this command three times to generate separate keys for Fleet, Insights, and Polly.

<Note>
You can store each key in a predefined Kubernetes secret. See [Use an existing secret](/langsmith/self-host-using-an-existing-secret#parameters) for the relevant parameters: `fleet_encryption_key`, `insights_encryption_key`, and `polly_encryption_key`.
</Note>
...
<Note>
If you are using an Istio Gateway, you must use LangSmith version 0.13.38 or later and set the following on the agent bootstrap job:
...
To disable Fleet, set the following in your `values.yaml`:
