...
- **[Engine](/langsmith/engine-overview)** finds recurring issues in a tracing project, diagnoses them against your source code, and proposes fixes. Engine requires Sandboxes, and is installed from [Engine on Self-hosted](/langsmith/engine-self-hosted).
...
### (Optional) Forward caller headers to MCP servers

Fleet can forward caller-supplied headers through to your agents' **custom MCP servers** on each tool call — useful for passing the invoking user's ID, an agent ID, or a downstream token. This is disabled by default. To enable it, set `FLEET_MCP_FORWARD_CALLER_HEADERS=true` on the Fleet components and redeploy:
...
fleet:
  apiServer:
    deployment:
      extraEnv:
        - name: FLEET_MCP_FORWARD_CALLER_HEADERS
          value: "true"
  queue:
    deployment:
      extraEnv:
...
Once enabled, callers pass headers prefixed with `X-Fleet-Forward-` on the run request. See [Forward caller headers to MCP servers](/langsmith/fleet/code#forward-caller-headers-to-mcp-servers) for usage, endpoints, and constraints.
...
For that reason, Engine's installation, egress, and data handling are documented together on one page. See [Engine on Self-hosted](/langsmith/engine-self-hosted) for regional availability, prerequisites, Helm values, and verification steps.
