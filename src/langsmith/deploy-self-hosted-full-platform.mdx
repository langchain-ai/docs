...
To enable Slack OAuth for Fleet, create a Slack app and configure it with the required scopes and credentials.
...
<Steps>
  ...
  <Step title="Copy credentials from Slack">
    Copy the **Client ID** and **Client Secret** from the Slack app settings.
  </Step>
...
Agents can now use Slack tools. To give an agent its own Slack bot, add a custom Slack app to it. See [Integrate Slack with an agent](/langsmith/fleet/slack-app#set-up-slack-on-self-hosted).
