...
Filtering is how you get from every [run](/langsmith/observability-concepts#runs) in a tracing project to the few you need: the conversations that errored, the tool calls that ran slowly, the runs a low feedback score points at. Write a query in the search bar, and choose which runs in the [trace](/langsmith/observability-concepts#traces) or [thread](/langsmith/observability-concepts#threads) hierarchy that query is tested against.
...
Click the search bar and start typing to build one:
...
<Note>
`*` means two different things depending on where it appears. On its own after the colon it tests for existence, so `name:*` matches every run that has a name. Inside a value it is a wildcard, so `name:docs*` matches names beginning with `docs`.
</Note>
