...
You can connect LangSmith to model providers that require custom TLS certificates (for example, when using a self-signed certificate, a custom certificate authority (CA), or mutual TLS authentication). These are the supported methods:

- [Use the system truststore](#use-the-system-truststore)
- [Provide explicit custom certificates](#provide-explicit-custom-certificates)
...
### Provide explicit custom certificates
