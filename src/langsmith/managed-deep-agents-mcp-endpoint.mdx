...
Authentication failures return one of two statuses:

| Status | Body | Cause |
| --- | --- | --- |
| 401 | `{"detail":"missing x-api-key"}` | No credential header on the request. |
| 403 | `{"detail":"API key is forbidden"}` | Header present, but the key is invalid or not authorized for the deployment's workspace. |
...
Each `/mcp` request is independent. The endpoint does not support sessions, so separate calls do not share thread state.

To retain knowledge across calls, [opt in to durable memory](/langsmith/managed-deep-agents-memory). Durable memory is optional and shared by every caller of the deployment. Do not enable it when MCP clients should not influence one another.
...
| **[MCP connectors](/langsmith/managed-deep-agents-mcp-connectors)** | Managed configuration | Add tools hosted by remote MCP servers to the agent |
| **[Channels](/langsmith/managed-deep-agents-channels)** | Managed configuration | Receive messages from an external messaging service that start agent runs and deliver responses |
