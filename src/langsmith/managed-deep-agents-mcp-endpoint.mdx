...
The endpoint is the outbound direction. An [MCP connector](/langsmith/managed-deep-agents-mcp-connectors) points the other way: it adds tools from a remote MCP server to your agent. A deployment can use both.
...
The local server started by [`mda dev`](/langsmith/managed-deep-agents-local-development) serves the same endpoint. Append `/mcp` to the local server URL the CLI prints.

## Authenticate requests

The MCP endpoint uses the deployment's [identity](/langsmith/managed-deep-agents-identity) configuration, the same as every other route on the deployment. Send the credential that matches the configured mode:
...
- **Tool name**: The agent `name` set in the [agent definition](/langsmith/managed-deep-agents-agent-definition#name).
...
Each `/mcp` request is stateless and independent. The endpoint does not support sessions, so separate calls do not share thread state. To retain knowledge across calls, enable [durable memory](/langsmith/managed-deep-agents-memory).

## Distinguish the endpoint from other capabilities

- **MCP endpoint** exposes the deployed agent as a tool to MCP clients.
- **[MCP connectors](/langsmith/managed-deep-agents-mcp-connectors)** add tools hosted by remote MCP servers to the agent.
- **[Channels](/langsmith/managed-deep-agents-channels)** receive messages from an external messaging service that start agent runs and deliver responses.

## Next steps

<CardGroup cols={2}>
  <Card title="MCP connectors" icon="plug" href="/langsmith/managed-deep-agents-mcp-connectors">
