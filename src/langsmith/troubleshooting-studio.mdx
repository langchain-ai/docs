...
<Tabs>
    ...
    <CodeGroup>
    ```bash pip
    pip install -qU langgraph-cli>=0.2.6
    langgraph dev --tunnel
    ```

    ```bash uv
    uv add langgraph-cli>=0.2.6
    langgraph dev --tunnel
    ```
    </CodeGroup>
    </Tab>
...
<CodeGroup>
```bash pip
pip install -qU "langgraph-cli[inmem]"
```
...
- **Local development server**: If you are running your application locally, make sure you have upgraded to the latest version of the `langgraph-cli` (`pip install -qU langgraph-cli` or `uv add langgraph-cli`). Additionally, ensure you have tracing enabled by setting the `LANGSMITH_API_KEY` in your project's `.env` file.
