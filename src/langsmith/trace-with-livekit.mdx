...
<Note>
This setup requires `langsmith[livekit]>=0.10.14` and `livekit-agents>=1.3`.
</Note>
...
Call `configure_livekit` once before creating your `AgentServer`. No session-end callback is required.
...
`configure_livekit()` uses LiveKit session recordings by default. The `record={"audio": True}` option tells LiveKit to create the recording. If LiveKit does not create a recording, the trace and transcript are still sent to LangSmith.
...
No session-end callback is required with this setup either.
...
Turn on LiveKit's session recording. No session-end callback is required:
...
Keep the default `configure_livekit()` mode even if some sessions do not produce a recording. The integration exports the trace and transcript whether or not the session contains audio.
...
Use [LiveKit Egress](https://docs.livekit.io/home/egress/overview/) when you want the recording in your own object storage or need video. Configure the integration for Egress, then call `complete_recording` after the Egress file is available:
...
If only some conversations use Egress, keep the default recording mode and call `processor.expect_recording(thread_id)` before that conversation starts. Then call `complete_recording` with the same thread ID when its recording is ready.
