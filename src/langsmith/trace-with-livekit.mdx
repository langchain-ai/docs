...
Call `configure_livekit` once before creating your `AgentServer`. After each session ends, pass its report to `attach_session_report`:
...
`configure_livekit()` uses session reports as its recording source by default. The `record={"audio": True}` option tells LiveKit to create the recording. If LiveKit does not create a recording, `attach_session_report` still sends the trace and transcript to LangSmith.
...
Use `processor.attach_session_report(...)` after each session, as shown in the preceding example.

## Group a conversation into a thread

To group a conversation's runs into a LangSmith [thread](/langsmith/threads), call `set_thread_id` inside the session handler. Use a unique ID for each active session, and pass the same ID to `attach_session_report`:
...
For a speech-to-speech (realtime) model, call `instrument_session` after creating the `AgentSession` to capture the user's transcript. Pass the same thread ID to `set_thread_id`, `instrument_session`, and `attach_session_report`:
...
By default, the integration attaches audio from the LiveKit session report. Use Egress mode when you record to external storage instead.

### Record with LiveKit's session recording

Turn on LiveKit's session recording, then pass the finished `SessionReport` to the integration from an `on_session_end` callback. This is the recommended path:
...
Keep the default `configure_livekit()` mode even if some sessions do not produce a recording. The integration attaches the transcript and exports the trace whether or not the report contains audio.
...
Use [LiveKit Egress](https://docs.livekit.io/home/egress/overview/) when you want the recording in your own object storage or need video. Configure the integration for Egress, call `attach_session_report` with the finished report, then call `complete_recording` after the Egress file is available:
...
<Note>
Always call `complete_recording`, including on failure with `data=None`. Otherwise, the integration waits for `recording_timeout_seconds` (30 seconds by default) before exporting the trace without audio.
</Note>
