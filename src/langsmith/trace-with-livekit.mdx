...
<Note>
This setup requires `langsmith[livekit]>=0.10.14` and `livekit-agents>=1.6`.
</Note>
...
`configure_livekit()` uses LiveKit session recordings by default. The `record={"audio": True}` option tells LiveKit to create the recording. The integration correlates the report and recording by the session span's trace ID, so you do not need to set a thread ID. If LiveKit does not create a recording, the trace and transcript are still sent to LangSmith.
