...
- Reviewers could be permanently unable to submit an annotation queue item whose required rubric field was answered, because feedback written without a resolved user identity was invisible to the required-field check. Feedback is now attributed by whichever identity the request resolves, so a reviewer's own answers count toward required fields.
