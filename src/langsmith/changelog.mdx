...
<Update label="August 24-28, 2026" rss={{ title: "2026-08-24 - LangSmith Cloud update" }}>
...
- Split the read datasets into a new download datasets permission.  Enforce this new permission in both the application and in APIs.  The donwload button is disabled for those users without the download permission. [Learn more](/langsmith/organization-workspace-operations#datasets).
...
- When a runs query times out, the runs table now shows an  banner for better responsiveness.
...
- The recurrence chart on an issue now labels its x-axis with a day-and-month axis instead of repeating the month on every tick, runs its window up to the present, and states when the issue was last seen; so an issue that stopped recurring months ago no longer looks like one that recurred this morning.
...
- Editing a series name in the dashboard chart editor now commits on blur rather than on every keystroke, so bar colors and legend items stop flickering while you type. Updated tooltips and button labels for filter groups, fix incorrct tooltip
...
- Gateway traces now carry a `gateway.credential_source` metadata field naming the credential that paid for each upstream LLM call; `credits`, `byok`, `subscription_oauth`, or `platform_managed`; so you can filter and chart Gateway Credits usage separately from bring-your-own-key usage, plus a `gateway.provider` field naming the provider route that served the call.
