...
| `githubOAuthProvider` | GitHub | - | [See below](#other-oauth-providers)
| `linkedinOAuthProvider` | LinkedIn | - | [See below](#other-oauth-providers)
| `linearOAuthProvider` | Linear | - | [See below](#other-oauth-providers)
...
<Accordion title="Other OAuth providers (GitHub, Linear, LinkedIn)" id="other-oauth-providers">

To enable GitHub, Linear, or LinkedIn OAuth for Agent Builder, follow these general steps.

<Steps>
  <Step title="Create an OAuth app">
    Create an OAuth application in the provider's developer console:
    - **GitHub**: [github.com/settings/developers](https://github.com/settings/developers)
    - **Linear**: [linear.app/settings/api/applications](https://linear.app/settings/api/applications)
    - **LinkedIn**: [linkedin.com/developers](https://www.linkedin.com/developers/)
  </Step>

  <Step title="Add redirect URL">
    Add the following redirect URL to your OAuth app, replacing `<hostname>` with your LangSmith hostname and `<provider-id>` with your chosen provider ID (e.g., `github`, `linear`, `linkedin`):
  ...
  <Step title="Configure OAuth provider in LangSmith">
    In LangSmith, go to **Settings > OAuth Providers** and add a new provider with:
    - **Client ID**: from the OAuth app
    - **Client Secret**: from the OAuth app
    - **Authorization URL**: found in the provider's OAuth documentation
    - **Token URL**: found in the provider's OAuth documentation
    - **Provider ID**: your chosen ID (e.g., `github`, `linear`, `linkedin`)
  </Step>

  <Step title="Deploy">
    Add the provider ID to your `values.yaml` and deploy:

    ```yaml
    agentBuilder:
      oauthProviderOrgId: "<your-org-id>"
      oauth:
        githubOAuthProvider: "<provider-id>"      # if using GitHub
        linearOAuthProvider: "<provider-id>"      # if using Linear
        linkedinOAuthProvider: "<provider-id>"    # if using LinkedIn
    ```
  </Step>
</Steps>
