...
- **Planning Tool**: Built-in planning capabilities to break down complex tasks
...
- **Detailed System Prompt**: Heavily inspired by Claude Code's effective prompting
...
Here's how to create a research agent that can conduct thorough investigations:
...
### Sub Agents
Built-in support for calling specialized sub agents with:
...
Traditional agent loops often fail on complex tasks because they lack:
- **Long-term planning**: No way to break down and track multi-step tasks
- **Persistent state**: No memory between tool calls beyond conversation history
- **Specialized handling**: Same approach for all types of tasks
- **Effective prompting**: Generic prompts that don't leverage proven patterns

Deep Agents solves these issues by implementing the architectural patterns that make sophisticated agents like Claude Code effective.
...
This project was primarily inspired by Claude Code and initially was largely an attempt to understand what made Claude Code general purpose and make it even more so.