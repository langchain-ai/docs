---
title: "Transwarp"
---

> [Transwarp](https://www.transwarp.cn/en/introduction) aims to build enterprise-level big data and AI infrastructure software, to shape the future of data world. It provides enterprises with infrastructure software and services around the whole data lifecycle, including integration, storage, governance, modeling, analysis, mining and circulation.
>
> `Transwarp` focuses on technology research and development and has accumulated core technologies in these aspects: distributed computing, SQL compilations, database technology, unification for multi-model data management, container-based cloud computing, and big data analytics and intelligence.

## Installation

You have to install several python packages:

```
pip install -U tiktoken hippo-api
```

and get the connection configuration.

## Vector stores

### Hippo

See [a usage example and installation instructions](/src/oss/integrations/vectorstores/hippo).

```
from langchain_community.vectorstores.hippo import Hippo
```

**API Reference:**[Hippo](https://python.langchain.com/api_reference/community/vectorstores/langchain_community.vectorstores.hippo.Hippo.html)
