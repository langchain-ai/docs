...
From here, see the [SQL Chain](/src/oss/how_to/#qa-over-sql--csv) documentation on how to use.
...
```
import sqlalchemyfrom langchain.globals import set_llm_cacheeng = sqlalchemy.create_engine(conn_str)set_llm_cache(SQLAlchemyCache(engine=eng))
```

From here, see the [LLM Caching](/src/oss/integrations/llm_caching) documentation on how to use.
