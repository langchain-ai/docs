...
See a [usage example](/src/oss/integrations/text_embedding/optimum_intel). We also offer a full tutorial notebook "rag\_with\_quantized\_embeddings.ipynb" for using the embedder in a RAG pipeline in the cookbook dir.

```
from langchain_community.embeddings import QuantizedBiEncoderEmbeddings
```

**API Reference:**[QuantizedBiEncoderEmbeddings](https://python.langchain.com/api_reference/community/embeddings/langchain_community.embeddings.optimum_intel.QuantizedBiEncoderEmbeddings.html)
...
As large language models (LLMs) become more prevalent, there is a growing need for new and improved quantization methods that can meet the computational demands of these modern architectures while maintaining the accuracy. Compared to [normal quantization](https://github.com/intel/intel-extension-for-transformers/blob/main/oss/quantization.md) like W8A8, weight only quantization is probably a better trade-off to balance the performance and the accuracy, since we will see below that the bottleneck of deploying LLMs is the memory bandwidth and normally weight only quantization could lead to better accuracy.
...
Install intel-extension-for-transformers. For system requirements and other installation tips, please refer to [Installation Guide](https://github.com/intel/intel-extension-for-transformers/blob/main/oss/installation.md)
...
See a [usage example](/src/oss/integrations/llms/weight_only_quantization).

## Detail of Configuration Parameters

Here is the detail of the `WeightOnlyQuantConfig` class.

#### weight\_dtype (string): Weight Data Type, default is "nf4".: Weight Data Type, default is \"nf4\".")
...
#### compute\_dtype (string): Computing Data Type, Default is "fp32".: Computing Data Type, Default is \"fp32\".")
...
#### llm\_int8\_skip\_modules (list of module's name): Modules to Skip Quantization, Default is None.: Modules to Skip Quantization, Default is None.")

It is a list of modules to be skipped quantization.

#### scale\_dtype (string): The Scale Data Type, Default is "fp32".: The Scale Data Type, Default is \"fp32\".")

Now only support "fp32"(float32).

#### mse\_range (boolean): Whether to Search for The Best Clip Range from Range \[0.805, 1.0, 0.005], default is False.: Whether to Search for The Best Clip Range from Range \[0.805, 1.0, 0.005], default is False.")

#### use\_double\_quant (boolean): Whether to Quantize Scale, Default is False.: Whether to Quantize Scale, Default is False.")

Not support yet.

#### double\_quant\_dtype (string): Reserve for Double Quantization.: Reserve for Double Quantization.")

#### double\_quant\_scale\_dtype (string): Reserve for Double Quantization.: Reserve for Double Quantization.")

#### group\_size (int): Group Size When Auantization.: Group Size When Auantization.")

#### scheme (string): Which Format Weight Be Quantize to. Default is "sym".: Which Format Weight Be Quantize to. Default is \"sym\".")

* **sym**: Symmetric.
* **asym**: Asymmetric.

#### algorithm (string): Which Algorithm to Improve the Accuracy . Default is "RTN": Which Algorithm to Improve the Accuracy . Default is \"RTN\"")
