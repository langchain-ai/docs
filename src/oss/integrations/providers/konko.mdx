...
Please see [the Konko docs](https://docs.konko.ai/oss/getting-started) for more details.

## LLM

**Explore Available Models:** Start by browsing through the [available models](https://docs.konko.ai/oss/list-of-models) on Konko. Each model caters to different use cases and capabilities.
...
See a usage [example](/src/oss/integrations/llms/konko).
...
* **Completion with mistralai/Mistral-7B-v0.1:**
  ...
  ```
  from langchain_community.llms import Konkollm = Konko(max_tokens=800, model='mistralai/Mistral-7B-v0.1')prompt = "Generate a Product Description for Apple Iphone 15"response = llm.invoke(prompt)
  ```

## Chat Models

See a usage [example](/src/oss/integrations/chat/konko).
