---
title: "Runhouse"
---

This page covers how to use the [Runhouse](https://github.com/run-house/runhouse) ecosystem within LangChain. It is broken into three parts: installation and setup, LLMs, and Embeddings.

## Installation and Setup

* Install the Python SDK with `pip install runhouse`
* If you'd like to use on-demand cluster, check your cloud credentials with `sky check`

## Self-hosted LLMs

For a basic self-hosted LLM, you can use the `SelfHostedHuggingFaceLLM` class. For more custom LLMs, you can use the `SelfHostedPipeline` parent class.

```
from langchain_community.llms import SelfHostedPipeline, SelfHostedHuggingFaceLLM
```

**API Reference:**[SelfHostedPipeline](https://python.langchain.com/api_reference/community/llms/langchain_community.llms.self_hosted.SelfHostedPipeline.html) | [SelfHostedHuggingFaceLLM](https://python.langchain.com/api_reference/community/llms/langchain_community.llms.self_hosted_hugging_face.SelfHostedHuggingFaceLLM.html)

For a more detailed walkthrough of the Self-hosted LLMs, see [this notebook](/src/oss/integrations/llms/runhouse)

## Self-hosted Embeddings

There are several ways to use self-hosted embeddings with LangChain via Runhouse.

For a basic self-hosted embedding from a Hugging Face Transformers model, you can use the `SelfHostedEmbedding` class.

```
from langchain_community.llms import SelfHostedPipeline, SelfHostedHuggingFaceLLM
```

**API Reference:**[SelfHostedPipeline](https://python.langchain.com/api_reference/community/llms/langchain_community.llms.self_hosted.SelfHostedPipeline.html) | [SelfHostedHuggingFaceLLM](https://python.langchain.com/api_reference/community/llms/langchain_community.llms.self_hosted_hugging_face.SelfHostedHuggingFaceLLM.html)

For a more detailed walkthrough of the Self-hosted Embeddings, see [this notebook](/src/oss/integrations/text_embedding/self-hosted)
