...
Please refer to [NCP User Guide](https://guide.ncloud-docs.com/oss/clovastudio-overview) for more detailed instructions (also in Korean).

## Installation and Setup

* Get a CLOVA Studio API Key by [issuing it](https://api.ncloud-docs.com/oss/ai-naver-clovastudio-summary#API%ED%82%A4) and set it as an environment variable (`CLOVASTUDIO_API_KEY`).
...
See a [usage example](/src/oss/integrations/text_embedding/naver).
...
```
from langchain_naver_community.tool import NaverSearchResultsfrom langchain_naver_community.utils import NaverSearchAPIWrapper# Set up the search wrappersearch = NaverSearchAPIWrapper()# Create a tooltool = NaverSearchResults(api_wrapper=search)
```

See a [usage example](/src/oss/integrations/tools/naver_search) for more details.
...
```
from langchain_naver_community.tool import NaverNewsSearch  # For news articlesfrom langchain_naver_community.tool import NaverBlogSearch  # For blog postsfrom langchain_naver_community.tool import NaverImageSearch  # For images
```
