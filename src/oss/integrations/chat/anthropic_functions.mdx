...
<Warning>
  The Anthropic API officially supports tool-calling so this workaround is no longer needed. Please use [ChatAnthropic](/src/oss/integrations/chat/anthropic) with `langchain-anthropic>=0.1.15`.
</Warning>
...
`ChatAnthropicTools` also implements the [`with_structured_output` spec](/src/oss/how_to/structured_output) for extracting values. Note: this may not be as stable as with models that explicitly offer tool calling.
