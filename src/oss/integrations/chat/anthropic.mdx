...
This notebook provides a quick overview for getting started with Anthropic [chat models](/src/oss/concepts/chat_models). For detailed documentation of all ChatAnthropic features and configurations head to the [API reference](https://python.langchain.com/api_reference/anthropic/chat_models/langchain_anthropic.chat_models.ChatAnthropic.html).
...
<Info>
  Note that certain Anthropic models can also be accessed via AWS Bedrock and Google VertexAI. See the [ChatBedrock](/src/oss/integrations/chat/bedrock) and [ChatVertexAI](/src/oss/integrations/chat/google_vertex_ai_palm) integrations to use Anthropic models via these services.
</Info>
...
<Accordion title="End to end example with LangGraph">
  Here we demonstrate an end-to-end example in which we populate a LangChain [vector store](/src/oss/concepts/vectorstores) with sample documents and equip Claude with a tool that queries those documents. The tool here takes a search query and a `category` string literal, but any valid tool signature can be used.
...
Anthropic also lets you specify your own splits using [custom document](https://docs.anthropic.com/en/oss/build-with-claude/citations#custom-content-documents) types. LangChain [text splitters](/src/oss/concepts/text_splitters) can be used to generate meaningful splits for this purpose. See the below example, where we split the LangChain README (a markdown document) and pass it to Claude as context:
...
Anthropic supports a variety of [built-in tools](https://docs.anthropic.com/en/oss/build-with-claude/tool-use/text-editor-tool), which can be bound to the model in the [usual way](/src/oss/how_to/tool_calling). Claude will generate tool calls adhering to its internal schema for the tool:
