...
[![Open In Colab](/images/oss/integrations/memory/google_el_carro/assets/colab-badge.svg)](https://colab.research.google.com/github/googleapis/langchain-google-el-carro-python/blob/main/oss/chat_message_history.ipynb)
...
```
from langchain_google_el_carro import ElCarroChatMessageHistoryhistory = ElCarroChatMessageHistory(    elcarro_engine=elcarro_engine, session_id="test_session", table_name=TABLE_NAME)history.add_user_message("hi!")history.add_ai_message("whats up?")
```
...
```
chain_with_history = RunnableWithMessageHistory(    chain,    lambda session_id: ElCarroChatMessageHistory(        elcarro_engine,        session_id=session_id,        table_name=TABLE_NAME,    ),    input_messages_key="question",    history_messages_key="history",)
```
