...
After initializing a TimescaleVector store, you can use it as a [retriever](/oss/how_to/#retrievers).
...
Next, we'll define a metadata function to extract the relevant metadata from the JSON record. We'll pass this function to the JSONLoader. See the [JSON document loader docs](/oss/how_to/document_loader_json) for more details.
...
For more on self-querying, [see the docs](/oss/how_to/self_query).
