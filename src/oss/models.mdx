...
<Warning>
  **Tool calling support**
  ...
  language model supports [tool calling](./tools). Compatible models can be found in the [LangChain integrations directory](https://python.langchain.com/docs/integrations/chat/).
</Warning>
...
1. **Direct invocation with custom streaming**: Use your model directly by [adding custom streaming logic](./streaming#use-with-any-llm) with `StreamWriter`.
  Refer to the [custom streaming documentation](./streaming#use-with-any-llm) for guidance. This approach suits custom workflows where prebuilt agent integration is not necessary.
