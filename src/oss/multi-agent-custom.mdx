...
1. Access the [state](./low_level#state) of the agent that is calling the handoff tool using the @[InjectedState] annotation. 
...
See the multi-agent [supervisor](./multi_agent/agent_supervisor#4-create-delegation-tasks) example for a full example of using @[`Send()`][Send] in handoffs.
