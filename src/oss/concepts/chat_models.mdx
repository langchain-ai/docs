...
Modern LLMs are typically accessed through a chat model interface that takes a list of [messages](/oss/concepts/messages) as input and returns a [message](/oss/concepts/messages) as output.

The newest generation of chat models offer additional capabilities:

* [Tool calling](/oss/concepts/tool_calling): Many popular chat models offer a native [tool calling](/oss/concepts/tool_calling) API. This API allows developers to build rich applications that enable LLMs to interact with external services, APIs, and databases. Tool calling can also be used to extract structured information from unstructured data and perform various other tasks.
...
* [Multimodality](/oss/concepts/multimodality): The ability to work with data other than text; for example, images, audio, and video.

## Features

LangChain provides a consistent interface for working with chat models from different providers while offering additional features for monitoring, debugging, and optimizing the performance of applications that use LLMs.

* Integrations with many chat model providers (e.g., Anthropic, OpenAI, Ollama, Microsoft Azure, Google Vertex, Amazon Bedrock, Hugging Face, Cohere, Groq). Please see [chat model integrations](/oss/integrations/chat) for an up-to-date list of supported models.
* Use either LangChain's [messages](/oss/concepts/messages) format or OpenAI format.
* Standard [tool calling API](/oss/concepts/tool_calling): standard interface for binding tools to models, accessing tool call requests made by models, and sending tool results back to the model.
* Standard API for [structuring outputs](/oss/concepts/structured_outputs/#structured-output-method) via the `with_structured_output` method.
* Provides support for [async programming](/oss/concepts/async), [efficient batching](/oss/concepts/runnables/#optimized-parallel-execution-batch), [a rich streaming API](/oss/concepts/streaming).
...
* Additional features like standardized [token usage](/oss/concepts/messages/#aimessage), [rate limiting](#rate-limiting), [caching](#caching) and more.
...
Please review the [chat model integrations](/oss/integrations/chat) for a list of supported models.
...
LangChain chat models implement the [BaseChatModel](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html) interface. Because `BaseChatModel` also implements the [Runnable Interface](/oss/concepts/runnables), chat models support a [standard streaming interface](/oss/concepts/streaming), [async programming](/oss/concepts/async), optimized [batching](/oss/concepts/runnables/#optimized-parallel-execution-batch), and more. Please see the [Runnable Interface](/oss/concepts/runnables) for more details.

Many of the key methods of chat models operate on [messages](/oss/concepts/messages) as input and return messages as output.
...
1. **invoke**: The primary method for interacting with a chat model. It takes a list of [messages](/oss/concepts/messages) as input and returns a list of messages as output.
...
5. **with\_structured\_output**: A wrapper around the `invoke` method for models that natively support [structured output](/oss/concepts/structured_outputs).
...
Chat models can call [tools](/oss/concepts/tools) to perform tasks such as fetching data from a database, making API requests, or running custom code. Please see the [tool calling](/oss/concepts/tool_calling) guide for more information.

## Structured outputs

Chat models can be requested to respond in a particular format (e.g., JSON or matching a particular schema). This feature is extremely useful for information extraction tasks. Please read more about the technique in the [structured outputs](/oss/concepts/structured_outputs) guide.

## Multimodality

Large Language Models (LLMs) are not limited to processing text. They can also be used to process other types of data, such as images, audio, and video. This is known as [multimodality](/oss/concepts/multimodality).
...
The size of the input is measured in [tokens](/oss/concepts/tokens) which are the unit of processing that the model uses.
...
A semantic cache introduces a dependency on another model on the critical path of your application (e.g., the semantic cache may rely on an [embedding model](/oss/concepts/embedding_models) to convert text to a vector representation), and it's not guaranteed to capture the meaning of the input accurately.
...
Please see the [how to cache chat model responses](/oss/how_to/chat_model_caching) guide for more details.

## Related resources

* How-to guides on using chat models: [how-to guides](/oss/how_to/#chat-models).
* List of supported chat models: [chat model integrations](/oss/integrations/chat).

### Conceptual guides

* [Messages](/oss/concepts/messages)
* [Tool calling](/oss/concepts/tool_calling)
* [Multimodality](/oss/concepts/multimodality)
* [Structured outputs](/oss/concepts/structured_outputs)
