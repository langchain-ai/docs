...
2. Adhering to a cross-provider standard (see [how-to guides](/src/oss/how_to/#multimodal) for detail)

### How to use multimodal models

* Use the [chat model integration table](/src/oss/integrations/chat) to identify which models support multimodality.
* Reference the [relevant how-to guides](/src/oss/how_to/#multimodal) for specific examples of how to use multimodal models.

### What kind of multimodality is supported?

#### Inputs

Some models can accept multimodal inputs, such as images, audio, video, or files. The types of multimodal inputs supported depend on the model provider. For instance, [OpenAI](/src/oss/integrations/chat/openai), [Anthropic](/src/oss/integrations/chat/anthropic), and [Google Gemini](/src/oss/integrations/chat/google_generative_ai) support documents like PDFs as inputs.
...
See the [how-to guides](/src/oss/how_to/#multimodal) for more detail.
...
Otherwise, chat models will typically accept the native, provider-specific content block format. See [chat model integrations](/src/oss/integrations/chat) for detail on specific providers.

#### Outputs

Some chat models support multimodal outputs, such as images and audio. Multimodal outputs will appear as part of the [AIMessage](/src/oss/concepts/messages/#aimessage) response object. See for example:

* Generating [audio outputs](/src/oss/integrations/chat/openai/#audio-generation-preview) with OpenAI;
* Generating [image outputs](/src/oss/integrations/chat/google_generative_ai/#multimodal-usage) with Google Gemini.

#### Tools

Currently, no chat model is designed to work **directly** with multimodal data in a [tool call request](/src/oss/concepts/tool_calling) or [ToolMessage](/src/oss/concepts/tool_calling) result.

However, a chat model can easily interact with multimodal data by invoking tools with references (e.g., a URL) to the multimodal data, rather than the data itself. For example, any model capable of [tool calling](/src/oss/concepts/tool_calling) can be equipped with tools to download and process images, audio, or video.

## Multimodality in embedding models

<Info>
  * [Embedding Models](/src/oss/concepts/embedding_models)
</Info>
