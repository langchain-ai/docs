...
Many AI applications interact directly with humans. In these cases, it is appropriate for models to respond in natural language. But what about cases where we want a model to also interact *directly* with systems, such as databases or an API? These systems often have a particular input schema; for example, APIs frequently have a required payload structure. This need motivates the concept of *tool calling*. You can use [tool calling](https://platform.openai.com/oss/guides/function-calling/example-use-cases) to request model responses that match a particular schema.

<Info>
  You will sometimes hear the term `function calling`. We use this term interchangeably with `tool calling`.
</Info>

![Conceptual overview of tool calling](/images/oss/concepts/tool_calling/assets/images/tool_calling_concept-552a73031228ff9144c7d59f26dedbbf.png)

## Key concepts

1. **Tool Creation:** Use the [@tool](https://python.langchain.com/api_reference/core/tools/langchain_core.tools.convert.tool.html) decorator to create a [tool](/src/oss/concepts/tools). A tool is an association between a function and its schema.
...
![Conceptual parts of tool calling](/images/oss/concepts/tool_calling/assets/images/tool_calling_components-bef9d2bcb9d3706c2fe58b57bf8ccb60.png)
...
<Info>
  * See our conceptual guide on [tools](/src/oss/concepts/tools) for more details.
  * See our [model integrations](/src/oss/integrations/chat) that support tool calling.
  * See our [how-to guide](/src/oss/how_to/tool_calling) on tool calling.
</Info>

## Tool binding

[Many](https://platform.openai.com/oss/guides/function-calling) [model providers](https://platform.openai.com/oss/guides/function-calling) support tool calling.

<Check>
  See our [model integration page](/src/oss/integrations/chat) for a list of providers that support tool calling.
</Check>
...
![Diagram of a tool call by a model](/images/oss/concepts/tool_calling/assets/images/tool_call_example-2348b869f9a5d0d2a45dfbe614c177a4.png)
...
For more details on usage, see our [how-to guides](/src/oss/how_to/#tools)!

## Tool execution

[Tools](/src/oss/concepts/tools) implement the [Runnable](/src/oss/concepts/runnables) interface, which means that they can be invoked (e.g., `tool.invoke(args)`) directly.
...
<Info>
  * See our [how-to guide](/src/oss/how_to/tool_choice) on forcing tool use.
</Info>

## Best practices

When designing [tools](/src/oss/concepts/tools) to be used by a model, it is important to keep in mind that:

* Models that have explicit [tool-calling APIs](/src/oss/concepts/tool_calling) will be better at tool calling than non-fine-tuned models.
