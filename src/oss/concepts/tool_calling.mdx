...
1. **Tool Creation:** Use the [@tool](https://python.langchain.com/api_reference/core/tools/langchain_core.tools.convert.tool.html) decorator to create a [tool](/oss/concepts/tools). A tool is an association between a function and its schema.
...
<Info>
  * See our conceptual guide on [tools](/oss/concepts/tools) for more details.
  * See our [model integrations](/oss/integrations/chat) that support tool calling.
  * See our [how-to guide](/oss/how_to/tool_calling) on tool calling.
</Info>
...
<Check>
  See our [model integration page](/oss/integrations/chat) for a list of providers that support tool calling.
</Check>
...
For more details on usage, see our [how-to guides](/oss/how_to/#tools)!

## Tool execution

[Tools](/oss/concepts/tools) implement the [Runnable](/oss/concepts/runnables) interface, which means that they can be invoked (e.g., `tool.invoke(args)`) directly.
...
<Info>
  * See our [how-to guide](/oss/how_to/tool_choice) on forcing tool use.
</Info>

## Best practices

When designing [tools](/oss/concepts/tools) to be used by a model, it is important to keep in mind that:

* Models that have explicit [tool-calling APIs](/oss/concepts/tool_calling) will be better at tool calling than non-fine-tuned models.
