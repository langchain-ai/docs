...
<Info>
  * [Retrievers](/src/oss/concepts/retrievers)
...
![Retrieval](/images/oss/concepts/retrieval/assets/images/retrieval_concept-2bcff1b2518f194b34eaf472ac748ffa.png)
...
| --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Multi-query](/src/oss/how_to/MultiQueryRetriever)                                                          | When you want to ensure high recall in retrieval by providing multiple phrasings of a question. | Rewrite the user question with multiple phrasings, retrieve documents for each rewritten question, return the unique documents for all queries.                                                                                                                                        |
...
| Name                                     | When to Use                                                                                                                          | Description                                                                                                                                                                                                                                          |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Self Query](/src/oss/how_to/self_query)   | If users are asking questions that are better answered by fetching documents based on metadata rather than similarity with the text. | This uses an LLM to transform user input into two things: (1) a string to look up semantically, (2) a metadata filter to go along with it. This is useful because oftentimes questions are about the METADATA of documents (not the content itself). |
| [Text to SQL](/src/oss/tutorials/sql_qa)   | If users are asking questions that require information housed in a relational database, accessible via SQL.                          | This uses an LLM to transform user input into a SQL query.                                                                                                                                                                                           |
| [Text-to-Cypher](/src/oss/tutorials/graph) | If users are asking questions that require information housed in a graph database, accessible via Cypher.                            | This uses an LLM to transform user input into a Cypher query.                                                                                                                                                                                        |
...
<Info>
  * See our tutorials on [text-to-SQL](/src/oss/tutorials/sql_qa), [text-to-Cypher](/src/oss/tutorials/graph), and [query analysis for metadata filters](/src/oss/tutorials/rag/#query-analysis).
...
Vector indexes are an alternative way to index and store unstructured data. See our conceptual guide on [vectorstores](/src/oss/concepts/vectorstores) for a detailed overview. In short, rather than using word frequencies, vectorstores use an [embedding model](/src/oss/concepts/embedding_models) to compress documents into high-dimensional vector representation. This allows for efficient similarity search over embedding vectors using simple mathematical operations like cosine similarity.

<Info>
  * See our [how-to guide](/src/oss/how_to/vectorstore_retriever) for more details on working with vectorstores.
  * See our [list of vectorstore integrations](/src/oss/integrations/vectorstores).
...
<Info>
  * See our [tutorial](/src/oss/tutorials/sql_qa) for working with SQL databases.
  * See our [SQL database toolkit](/src/oss/integrations/tools/sql_database).
</Info>
...
<Info>
  * See our [tutorial](/src/oss/tutorials/graph) for working with graph databases.
  * See our [list of graph database integrations](/src/oss/integrations/graphs).
  * See Neo4j's [starter kit for LangChain](https://neo4j.com/developer-blog/langchain-neo4j-starter-kit/).
</Info>

### Retriever

LangChain provides a unified interface for interacting with various retrieval systems through the [retriever](/src/oss/concepts/retrievers) concept. The interface is straightforward:
...
<Info>
  * See our [conceptual guide on retrievers](/src/oss/concepts/retrievers).
  * See our [how-to guide](/src/oss/how_to/#retrievers) on working with retrievers.
</Info>
