...
Many components of LangChain implement the [Runnable Interface](/oss/concepts/runnables), which includes support for asynchronous execution. This means that you can run Runnables asynchronously using the `await` keyword in Python.

```
await some_runnable.ainvoke(some_input)
```

Other components like [Embedding Models](/oss/concepts/embedding_models) and [VectorStore](/oss/concepts/vectorstores) that do not implement the [Runnable Interface](/oss/concepts/runnables) usually still follow the same rule and include the asynchronous version of method in the same class with an "a" prefix.
...
Runnables created using the [LangChain Expression Language (LCEL)](/oss/concepts/lcel) can also be run asynchronously as they implement the full [Runnable Interface](/oss/concepts/runnables).
...
2. In [LCEL](/oss/concepts/lcel) any "cheap functions" that appear as part of the chain will be either scheduled as tasks on the event loop (if they are async) or run in a separate thread (if they are sync), rather than just be run inline.
