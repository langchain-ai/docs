...
Messages are the unit of communication in [chat models](/src/oss/concepts/chat_models). They are used to represent the input and output of a chat model, as well as any additional context or metadata that may be associated with a conversation.
...
* Additional metadata: id, name, [token usage](/src/oss/concepts/tokens) and other model-specific metadata.
...
| **tool**              | A message used to pass the results of a tool invocation back to the model after external data or processing has been retrieved. Used with chat models that support [tool calling](/src/oss/concepts/tool_calling). |
| **function** (legacy) | This is a legacy role, corresponding to OpenAI's legacy function-calling API. **tool** role should be used instead.                                                                                              |

### Content

The content of a message text or a list of dictionaries representing [multimodal data](/src/oss/concepts/multimodality) (e.g., images, audio, video). The exact format of the content can vary between different chat model providers.
...
* [Multimodality](/src/oss/concepts/multimodality) -- for more information on multimodal content.
...
* **Tool Calls**: A request made by the model to call one or more tools> See [tool calling](/src/oss/concepts/tool_calling) for more information.
...
Please read the [chat history](/src/oss/concepts/chat_history) guide for more information on managing chat history and ensuring that the conversation structure is correct.
...
* [AIMessageChunk](#aimessagechunk): corresponds to **assistant** role, used for [streaming](/src/oss/concepts/streaming) responses
* [ToolMessage](#toolmessage): corresponds to **tool** role

Other important messages include:

* [RemoveMessage](#removemessage) -- does not correspond to any role. This is an abstraction, mostly used in [LangGraph](/src/oss/concepts/architecture/#langgraph) to manage chat history.
...
Please see the [multimodality](/src/oss/concepts/multimodality) guide for more information.
...
| `tool_calls`         | Standardized     | Tool calls associated with the message. See [tool calling](/src/oss/concepts/tool_calling) for details.                                                                                                                     |
| `invalid_tool_calls` | Standardized     | Tool calls with parsing errors associated with the message. See [tool calling](/src/oss/concepts/tool_calling) for details.                                                                                                 |
| `usage_metadata`     | Standardized     | Usage metadata for a message, such as [token counts](/src/oss/concepts/tokens). See [Usage Metadata API Reference](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.ai.UsageMetadata.html). |
...
* A **list of dictionaries** -- Each dictionary represents a content block and is associated with a `type`.
  ...
  * Used by Anthropic for surfacing agent thought process when doing [tool calling](/src/oss/concepts/tool_calling).
  * Used by OpenAI for audio outputs. Please see [multi-modal content](/src/oss/concepts/multimodality) for more information.
...
It is common to [stream](/src/oss/concepts/streaming) responses for the chat model as they are being generated, so the user can see the response in real-time instead of waiting for the entire response to be generated before displaying it.
...
This represents a message with role "tool", which contains the result of [calling a tool](/src/oss/concepts/tool_calling). In addition to `role` and `content`, this message has:

* a `tool_call_id` field which conveys the id of the call to the tool that was called to produce this result.
* an `artifact` field which can be used to pass along arbitrary artifacts of the tool execution which are useful to track but which should not be sent to the model.

Please see [tool calling](/src/oss/concepts/tool_calling) for more information.

### RemoveMessage

This is a special message type that does not correspond to any roles. It is used for managing chat history in [LangGraph](/src/oss/concepts/architecture/#langgraph).
...
### (Legacy) FunctionMessage FunctionMessage")
