...
The Runnable interface is the foundation for working with LangChain components, and it's implemented across many of them, such as [language models](/src/oss/concepts/chat_models), [output parsers](/src/oss/concepts/output_parsers), [retrievers](/src/oss/concepts/retrievers), [compiled LangGraph graphs](https://langchain-ai.github.io/langgraph/concepts/low_level/#compiling-your-graph) and more.
...
<Info>
  ...
  * A list of built-in `Runnables` can be found in the [LangChain Core API Reference](https://python.langchain.com/api_reference/core/runnables.html). Many of these Runnables are useful when composing custom "chains" in LangChain using the [LangChain Expression Language (LCEL)](/src/oss/concepts/lcel).
</Info>
...
* [Invoked](/src/oss/how_to/lcel_cheatsheet/#invoke-a-runnable): A single input is transformed into an output.
* [Batched](/src/oss/how_to/lcel_cheatsheet/#batch-a-runnable): Multiple inputs are efficiently transformed into outputs.
* [Streamed](/src/oss/how_to/lcel_cheatsheet/#stream-a-runnable): Outputs are streamed as they are produced.
* Inspected: Schematic information about Runnable's input, output, and configuration can be accessed.
* Composed: Multiple Runnables can be composed to work together using [the LangChain Expression Language (LCEL)](/src/oss/concepts/lcel) to create complex pipelines.

Please review the [LCEL Cheatsheet](/src/oss/how_to/lcel_cheatsheet) for some common patterns that involve the Runnable interface and LCEL expressions.
...
<Note>
  The async versions of `abatch` and `abatch_as_completed` relies on asyncio's [gather](https://docs.python.org/3/library/asyncio-task.html#asyncio.gather) and [as\_completed](https://docs.python.org/3/library/asyncio-task.html#asyncio.as_completed) functions to run the `ainvoke` method in parallel.
</Note>

<Check>
  When processing a large number of inputs using `batch` or `batch_as_completed`, users may want to control the maximum number of parallel calls. This can be done by setting the `max_concurrency` attribute in the `RunnableConfig` dictionary. See the [RunnableConfig](/src/oss/concepts/runnables/#runnableconfig) for more information.

  Chat Models also have a built-in [rate limiter](/src/oss/concepts/chat_models/#rate-limiting) that can be used to control the rate at which requests are made.
</Check>
...
Please refer to the [Async Programming with LangChain](/src/oss/concepts/async) guide for more details.
...
Please refer to the [Streaming Conceptual Guide](/src/oss/concepts/streaming) for more details on how to stream in LangChain.
...
These APIs are mostly used internally for unit-testing and by [LangServe](/src/oss/concepts/architecture/#langserve) which uses the APIs for input validation and generation of [OpenAPI documentation](https://www.openapis.org/).
...
Currently, this inference does not work well for more complex Runnables that are built using [LCEL](/src/oss/concepts/lcel) composition, and the inferred input and / or output types may be incorrect. In these cases, we recommend that users override the inferred input and output types using the `with_types` method ([API Reference](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.base.Runnable.html#langchain_core.runnables.base.Runnable.with_types)).
...
| ---------------- | ------------------------------------------------------------------------------------------ |
| run\_name        | Name used for the given Runnable (not inherited).                                          |
| run\_id          | Unique identifier for this call. sub-calls will get their own unique run ids.              |
...
| max\_concurrency | Maximum number of parallel calls to make (e.g., used by batch).                            |
| recursion\_limit | Maximum number of times a call can recurse (e.g., used by Runnables that return Runnables) |
...
If this were not the case, it would be impossible to set and propagate [callbacks](/src/oss/concepts/callbacks) or other configuration values like `tags` and `metadata` which are expected to be inherited by all sub-calls.

There are two main patterns by which new `Runnables` are created:

1. Declaratively using [LangChain Expression Language (LCEL)](/src/oss/concepts/lcel):
...
Using these attributes can be useful for tracking and debugging runs, as they will be surfaced in [LangSmith](https://docs.smith.langchain.com/) as trace attributes that you can filter and search on.

The attributes will also be propagated to [callbacks](/src/oss/concepts/callbacks), and will appear in streaming APIs like [astream\_events](/src/oss/concepts/streaming) as part of each event in the stream.
...
<Check>
  If you're trying to rate limit the number of requests made by a **Chat Model**, you can use the built-in [rate limiter](/src/oss/concepts/chat_models/#rate-limiting) instead of setting `max_concurrency`, which will be more effective.

  See the [How to handle rate limits](/src/oss/how_to/chat_model_rate_limiting) guide for more information.
</Check>
...
It is used frequently in [LangGraph](/src/oss/concepts/architecture/#langgraph) with [LangGraph Persistence](https://langchain-ai.github.io/langgraph/concepts/persistence/) and [memory](https://langchain-ai.github.io/langgraph/concepts/memory/).
...
Use this option to configure [callbacks](/src/oss/concepts/callbacks) for the runnable at runtime. The callbacks will be passed to all sub-calls made by the runnable.

```
some_runnable.invoke(   some_input,   {      "callbacks": [         SomeCallbackHandler(),         AnotherCallbackHandler(),      ]   })
```

Please read the [Callbacks Conceptual Guide](/src/oss/concepts/callbacks) for more information on how to use callbacks in LangChain.

<Info>
  If you're using Python 3.9 or 3.10 in an async environment, you must propagate the `RunnableConfig` manually to sub-calls in some cases. Please see the [Propagating RunnableConfig](#propagation-of-runnableconfig) section for more information.
</Info>

## Creating a runnable from a function

You may need to create a custom Runnable that runs arbitrary logic. This is especially useful if using [LangChain Expression Language (LCEL)](/src/oss/concepts/lcel) to compose multiple Runnables and you need to add custom processing logic in one of the steps.
...
See the [How to run custom functions](/src/oss/how_to/functions) guide for more information on how to use `RunnableLambda` and `RunnableGenerator`.
...
<Note>
  ...
  It helps with configuration of large "chains" created using the [LangChain Expression Language (LCEL)](/src/oss/concepts/lcel) and is leveraged by [LangServe](/src/oss/concepts/architecture/#langserve) for deployed Runnables.
</Note>
...
See the [How to configure runtime chain internals](/src/oss/how_to/configure) guide for more information on how to configure runtime chain internals.
