...
LangChain has a number of [`ExampleSelectors`](/oss/concepts/example_selectors) which make it easy to use any of these techniques.
...
If we insert our examples as messages, where each example is represented as a sequence of Human, AI messages, we might want to also assign [names](/oss/concepts/messages) to our messages like `"example_user"` and `"example_assistant"` to make it clear that these messages correspond to different actors than the latest input message.
