        ...
        HumanInTheLoopMiddleware(
            interrupt_on={
                "write_file": {
                    ...
                    # Only pause writes outside the workspace directory.
                    "when": lambda req: not req.tool_call["args"]
                    .get("path", "")
                    .startswith("/workspace/"),
                },
                "execute_sql": {
                    "allowed_decisions": ["approve", "reject"],
                    # Auto-approve read-only queries; pause on anything else.
                    "when": lambda req: not req.tool_call["args"]
                    .get("query", "")
                    .lstrip()
                    .upper()
...
When `when` returns `False`, the call runs without interrupting. When it returns `True`, or when you omit `when`, the call pauses as usual. Calls that evaluate to `False` are never added to the interrupt batch, so a reviewer only sees the actions that need a decision.
:::
