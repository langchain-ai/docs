...
agent = create_agent(
    ...
    # In production, use a persistent checkpointer like AsyncPostgressSaver.
...
<Important>
    ...
    In production, use a persistent checkpointer like @[AsyncPostgressSaver]. For testing or prototyping, use @[InMemorySaver].

    When invoking the agent, pass a `config` that includes the **thread ID** to associate execution with a conversation thread.
    See the [LangGraph human-in-the-loop documentation](/oss/langgraph/human-in-the-loop) for details.
</Important>
...
// If an interrupt is triggered, the result will include an `__interrupt__` field, which will contain interrupts for human review.
const requests = state.tasks?.[0]?.interrupts?.[0]?.value;

// Resume with approval decision
await agent.invoke(
    new Command({ // [!code highlight]
        resume: [{ type: "accept" }], // or "edit", "response" [!code highlight]
    }), // [!code highlight]
    config
);
```
    ...
    agent.invoke(
        Command(
            ...
            resume=[
                {
                    "type": "edit",
                    "action": "new_tool_name",          # updated tool to call
                    "args": {"modified_arg": "value"},  # updated arguments
                }
            ]
        ),
        config=config  # Same thread ID to resume the paused conversation
    ...
    agent.invoke(
        Command(
            ...
            resume=[
                {
                    "type": "respond",
                    "tool_message": "Manual response text",
                }
            ]
        ),
        config=config  # Same thread ID to resume the paused conversation
