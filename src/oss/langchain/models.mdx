...
`ChatAnthropic` also accepts `effort`, and `ChatGoogleGenerativeAI` also accepts `thinking_level`, as aliases for `reasoning_effort`, at both construction and call time. If both the alias and `reasoning_effort` are set, the alias wins. See the [Anthropic effort docs](/oss/integrations/chat/anthropic#effort) and [Gemini thinking docs](/oss/integrations/chat/google_generative_ai#thinking-support) for detail.
:::
