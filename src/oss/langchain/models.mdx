...
<Tabs>
    <Tab title="Pydantic">
        ...
        <Note>
            `Field(description="...")` marks a field as required with no default value. To make a field optional, provide a default: `Field(None, description="...")` for nullable fields or `Field("default", description="...")` for a specific default. The `description` is passed to the LLM as a hint about what the field should contain.
        </Note>
    ...
    <Tab title="TypedDict">
        Python's `TypedDict` provides a simpler alternative to Pydantic models, ideal when you don't need runtime validation. In `Annotated[type, ..., "description"]`, the middle `...` marks the field as required (no default value) and the final string is the field description passed to the LLM.
