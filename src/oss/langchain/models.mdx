...
:::python
<Note>
    `reasoning_effort` as a standard parameter requires `langchain-core>=1.5.2`, plus the corresponding partner package version: `langchain-anthropic>=1.5.3`, `langchain-openai>=1.4.1`, `langchain-fireworks>=1.5.2`, or `langchain-xai>=1.3.0`.
</Note>

@[`ChatOpenAI`], @[`ChatAnthropic`], @[`ChatFireworks`], and @[`ChatXAI`] support a standard `reasoning_effort` parameter. Like `temperature`, it can be set at model construction or per invocation, and each provider translates it into its own API format:
...
<Info>
    `ChatAnthropic` also accepts `effort` as an alias for `reasoning_effort`, at both construction and call time. If both are set, `effort` wins. See the [Anthropic effort docs](/oss/integrations/chat/anthropic#effort) for detail.
</Info>
:::
