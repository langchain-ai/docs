...
<Tabs>
  <Tab title="State">
    Trim messages transiently (only for this model call) based on conversation length:
    ...
    @wrap_model_call
    def trim_messages_from_state(
        request: ModelRequest,
        handler: Callable[[ModelRequest], ModelResponse]
    ) -> ModelResponse:
        """Transiently trim messages - only affects this model call, not state."""

        if len(messages) > 10:
            # Trim messages transiently for this call only
            request.messages = [
                *request.messages[-8:]  # Recent messages
            ]
      ...
      wrapModelCall: (request, handler) => {
        ...
        if (messages.length > 10) {
          ...
          request.messages = [
            ...
            ...request.messages.slice(-8)  // Recent messages
          ];
        }
  ...
  <Tab title="Store">
    Transiently inject user preferences from Store into messages:
    ...
    @wrap_model_call
    def inject_preferences_from_store(
        request: ModelRequest,
        handler: Callable[[ModelRequest], ModelResponse]
    ) -> ModelResponse:
        """Transiently inject user preferences from Store as context."""
        user_id = request.runtime.context.user_id

        # Read from Store: get user preferences (persistent)
        store = request.runtime.store
        user_prefs = store.get(("preferences",), user_id)

        if user_prefs:
            # Build context message from preferences
            ...
            if context_parts:
                # Transiently inject preferences as a human message
                request.messages = [
                    *request.messages,
                    {"role": "user", "content": f"My preferences: {', '.join(context_parts)}"}
                ]
    ...
    agent = create_agent(
        model="openai:gpt-4o",
        tools=[...],
        middleware=[inject_preferences_from_store],
        context_schema=Context,
        store=InMemoryStore()
    )
    ```
    ...
    const injectPreferencesFromStore = createMiddleware({
      name: "InjectPreferencesFromStore",
      contextSchema,
      wrapModelCall: async (request, handler) => {
        ...
        if (userPrefs) {
          ...
          const contextParts = [];
          if (prefs.language) {
            contextParts.push(`preferred language: ${prefs.language}`);
          }
          if (prefs.timezone) {
            contextParts.push(`timezone: ${prefs.timezone}`);
          }
          if (prefs.interests) {
            contextParts.push(`interests: ${prefs.interests.join(", ")}`);
          }
          ...
          if (contextParts.length > 0) {
            ...
            request.messages = [
              ...request.messages,
              { role: "user", content: `My preferences: ${contextParts.join(", ")}` }
            ];
          }
        }
  ...
  <Tab title="Runtime Context">
    Transiently inject session context from Runtime Context:
    ...
    def inject_session_context(
        ...
        """Transiently inject session info from Runtime Context."""
        # Read from Runtime Context: get session information
        user_id = request.runtime.context.user_id
        location = request.runtime.context.user_location
        flags = request.runtime.context.feature_flags

        # Build context message from runtime information
        context_parts = [f"user ID: {user_id}"]
        if location:
            context_parts.append(f"location: {location}")
        if flags:
            context_parts.append(f"enabled features: {', '.join(flags)}")

        # Transiently inject session context as a human message
        request.messages = [
            *request.messages,
            {"role": "user", "content": f"Session context: {', '.join(context_parts)}"}
        ]
    ...
    agent = create_agent(
        model="openai:gpt-4o",
        tools=[...],
        middleware=[inject_session_context],
        context_schema=Context
    )
    ```
    :::
    ...
    const injectSessionContext = createMiddleware<Context>({
      name: "InjectSessionContext",
      contextSchema,
      wrapModelCall: (request, handler) => {
        // Read from Runtime Context: get session information
        const { userId, userLocation, featureFlags } = request.runtime.context;

        // Build context message from runtime information
        const contextParts = [`user ID: ${userId}`];
        if (userLocation) {
          contextParts.push(`location: ${userLocation}`);
        }
        if (featureFlags.length > 0) {
          contextParts.push(`enabled features: ${featureFlags.join(", ")}`);
        }

        // Transiently inject session context as a human message
        request.messages = [
          ...request.messages,
          { role: "user", content: `Session context: ${contextParts.join(", ")}` }
        ];
