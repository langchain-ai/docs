...
- Return @[`ExtendedModelResponse`] from `wrap_model_call` with a @[`Command`] to inject state updates from the model call layer. See [State updates from wrap_model_call](/oss/langchain/middleware/custom#state-updates-from-wrap_model_call).
