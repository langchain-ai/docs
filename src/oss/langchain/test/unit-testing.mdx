...
Unit tests exercise small, deterministic pieces of your agent in isolation. By replacing the real LLM with an in-memory fake, you can script exact responses (text, tool calls, and errors) so tests are fast, free, and repeatable without API keys.
...
`.respond()` also accepts a factory function that computes the response based on the input messages. The function receives the full message array and returns either a `BaseMessage` or an `Error`:
...
<Note>
    Each factory function is a single queue entry, consumed once. To reuse the same dynamic logic for multiple turns, queue multiple factory entries.
</Note>
...
See the [reference docs](https://reference.langchain.com/javascript/langchain-core/testing) for the full API.

| Method | Description |
| --- | --- |
| `fakeModel()` | Create a new fake chat model. Returns a chainable builder. |
...
| `.withStructuredOutput(schema)` | Return a runnable that produces the `.structuredResponse()` value. |
...
Learn how to test your agent with real LLM APIs in [Integration testing](/oss/langchain/test/integration-testing).
