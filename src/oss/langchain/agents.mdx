...
<Tip>
**Want a batteries-included agent?** [Deep Agents](/oss/deepagents/overview) is a higher-level SDK built on top of this framework. It pre-assembles the full harness — filesystem, summarization, subagents, memory, and prompt caching — so you can skip the boilerplate and start building immediately. `create_agent` is the right choice when you need fine-grained control; Deep Agents is the right choice when you want a production-ready default.
</Tip>
