...
`AgentState` is also the type signature for every node-style middleware hook (`before_model`, `after_model`, etc.) — hooks receive the current state and can return a dict of updates to merge back into it.
...
`AgentState` is also the type passed to every node-style middleware hook (`beforeModel`, `afterModel`, etc.) — hooks receive the current state and can return an object of updates to merge back into it.
