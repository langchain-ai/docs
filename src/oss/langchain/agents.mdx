...
To use a dynamic model, create middleware with the `@modify_model_request` decorator that modifies the model in the request:

```python wrap
from langchain_openai import ChatOpenAI
from langchain.agents import create_agent
from langchain.agents.middleware import modify_model_request, ModelRequest, AgentState
...
def dynamic_model_selection(request: ModelRequest, state: AgentState, runtime: Runtime) -> ModelRequest:
    """Choose model based on conversation complexity."""
    message_count = len(request.system_prompt)
...
agent = create_agent(
    ...
    # tools=tools,
    system_prompt="You are an intelligent assistant and your name is Cute Bear",
