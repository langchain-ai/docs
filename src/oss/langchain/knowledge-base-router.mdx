---
title: Build a multi-source knowledge router
sidebarTitle: Multi-source router
---
...
When a user asks "How do I authenticate API requests?", the router decomposes the query, determines which sources to consult, queries them in parallel, and synthesizes results into a coherent answer.
...
First, define the state schema that will flow through your router workflow. This state tracks the query, routing decisions, results from each agent, and the final synthesized answer:
...
class RouterState(TypedDict):
    query: str
    routes: list[str]  # Which knowledge bases to query  # [!code highlight]
...
const RouterState = Annotation.Root({
  ...
  routes: Annotation<string[]>(),  // Which knowledge bases to query  // [!code highlight]
...
The `routes` field is key—it stores the routing decisions made by the decomposition step, determining which agents will be invoked in parallel.
...
def search_code(query: str, repo: str = "main") -> str:
    ...
    # Stub: In practice, this would call the GitHub API
...
def search_notion(query: str) -> str:
    ...
    # Stub: In practice, this would call the Notion API
...
def search_slack(query: str) -> str:
    ...
    # Stub: In practice, this would call the Slack API
...
github_agent = create_agent(  # [!code highlight]
    model,
...
notion_agent = create_agent(  # [!code highlight]
    model,
...
slack_agent = create_agent(  # [!code highlight]
    model,
...
const githubAgent = createAgent({  // [!code highlight]
  model: llm,
...
const notionAgent = createAgent({  // [!code highlight]
  model: llm,
...
const slackAgent = createAgent({  // [!code highlight]
  model: llm,
...
1. **Decompose**: Analyze the query and determine which sources to consult
2. **Route**: Fan out to selected agents in parallel using `Send`
3. **Query agents**: Each selected agent searches its domain
...
# Use a smaller model for routing decisions
...
Return a JSON object with a "sources" array containing one or more of: "github", "notion", "slack".
Only include sources that are likely to have relevant information.

Example: {"sources": ["github", "notion"]}"""
        },
        {"role": "user", "content": state["query"]}
    ])

    # Parse the response to extract routes
    import json
    try:
        result = json.loads(response.content)
        routes = result.get("sources", ["github", "notion"])
    except json.JSONDecodeError:
        # Default to all sources if parsing fails
        routes = ["github", "notion", "slack"]

    return {"routes": routes}


def route_to_agents(state: RouterState) -> list[Send]:  # [!code highlight]
    """Fan out to selected agents in parallel using Send."""  # [!code highlight]
...
def synthesize_results(state: RouterState) -> dict:  # [!code highlight]
    ...
    # Use LLM to synthesize results into a coherent response
    synthesis_response = router_llm.invoke([
        {
            "role": "system",
            "content": """Synthesize these search results into a coherent, helpful answer.
...
Example: {"sources": ["github", "notion"]}`
  ...
  try {
    ...
    routes = result.sources || ["github", "notion"];
  } catch {
    // Default to all sources if parsing fails
    routes = ["github", "notion", "slack"];
  }
...
:::python
```python
# Query that requires multiple sources
result = workflow.invoke({
    "query": "How do I authenticate API requests?"
})

print("Routes selected:", result["routes"])
print("\n" + "="*60 + "\n")
print("Final Answer:")
...
console.log("Routes selected:", result.routes);
console.log("\n" + "=".repeat(60) + "\n");
...
The router analyzed the query, determined that GitHub and Notion were relevant sources (but not Slack for this technical question), queried both in parallel, and synthesized the results into a coherent answer.
...
The `decompose_query` function analyzes the user's query and determines which knowledge sources are relevant. This is where the "routing" intelligence lives. You can make this as simple or sophisticated as needed:

- **Simple**: Keyword matching or rule-based routing
- **LLM-based**: Use an LLM to analyze query intent (as shown in this tutorial)
- **Classifier**: Train a classifier to predict relevant sources

### Parallel execution

The `route_to_agents` function returns a list of `Send` objects, one for each selected route. LangGraph executes these in parallel, significantly reducing latency compared to sequential approaches.

```python
# If routes = ["github", "notion"], this returns:
...
After all agents complete, the `synthesize_results` function combines their outputs. The synthesis step waits for all parallel branches to complete before executing.
...
A router decomposes queries, routes to specialized agents in parallel,
...
# ============================================================================
# Step 2: Define tools for each vertical
# ============================================================================
...
# ============================================================================
# Step 4: Build the router workflow
# ============================================================================
...
def route_to_agents(state: RouterState) -> list[Send]:
    """Fan out to selected agents in parallel using Send."""
...
if __name__ == "__main__":
    ...
    print("Query:", "How do I authenticate API requests?")
    print("Routes selected:", result["routes"])
    print("\n" + "="*60 + "\n")
    print("Final Answer:")
    print(result["final_answer"])
...
function routeToAgents(state: typeof RouterState.State): Send[] {
  return state.routes.map(route => new Send(route, state));
}
...
console.log("Query:", "How do I authenticate API requests?");
console.log("Routes selected:", result.routes);
console.log("\n" + "=".repeat(60) + "\n");
console.log("Final Answer:");
console.log(result.finalAnswer);
...
@tool
def search_knowledge_base(query: str) -> str:  # [!code highlight]
...
# Conversational agent wraps the router as a tool
conversational_agent = create_agent(  # [!code highlight]
    model,
    ...
    checkpointer=InMemorySaver(),  # Enables conversation memory  # [!code highlight]
)
```
:::
...
const searchKnowledgeBase = tool(  // [!code highlight]
  async ({ query }) => {
...
// Conversational agent wraps the router as a tool
const conversationalAgent = createAgent({  // [!code highlight]
  model: llm,
...
documentation, and team discussions.
  `.trim(),
  checkpointer: new MemorySaver(),  // Enables conversation memory  // [!code highlight]
});
```
:::
...
The pattern has three phases: **decompose** (determine which sources to query), **route** (execute queries in parallel), and **synthesize** (combine results).
...
- Explore the [supervisor pattern](/oss/langchain/supervisor) for centralized orchestration
