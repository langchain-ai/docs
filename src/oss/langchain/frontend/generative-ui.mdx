...
Declarative generative UI is the middle position on the
...
components you registered ahead of time. Instead of rendering text responses in chat
...
The catalog is the guardrail that makes this position safe: the agent can arrange
and combine your components freely, but cannot step outside the set you approved.
...
control. This page covers LangChain's approach with
[json-render](https://json-render.dev), the generative UI framework
that defines component catalogs, generates specs with AI, and renders them safely
across React, Vue, Svelte, and Angular. For CopilotKit's declarative approach, see
[A2UI](#a2ui-an-alternative-declarative-spec) below.
...
Reach for declarative generative UI across the long tail of your product, where the
agent should compose layouts you did not fully anticipate while staying inside a set
of components you approved: secondary interactions, internal tools, and dashboards.
...
you want interfaces invented outside your application, move toward
...
json-render is one way to describe an interface declaratively. A2UI is another:
Google's declarative, streaming-first generative UI specification, which integrates
easily with CopilotKit. Like json-render, it composes interfaces from components you
register, so the agent stays inside a guardrail you define. A2UI comes in two
variants:
