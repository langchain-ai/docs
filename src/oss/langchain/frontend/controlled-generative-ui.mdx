...
Controlled generative UI is the author-controlled end of the
...
This position gives the highest predictability of any generative UI approach.
...
capability needs a component you write in advance. Controlled generative UI is the
workhorse of the pattern and the right fit for high-traffic, brand-critical surfaces
where a layout must be exact, such as flight tickets and booking confirmations. Your
component library is the boundary: the agent can only render what you shipped.
...
anticipate, across the long tail of secondary interactions, move one step along the
spectrum to [declarative generative UI](/oss/langchain/frontend/generative-ui).

Controlled generative UI covers four techniques, ordered from the agent picking a
whole interface to the frontend reacting to agent internals.

## Components as tools

You expose UI components to the agent the way you expose tools. Each component has a
name, a description, and a typed set of props, and the agent selects a component and
supplies its data as part of its response. The frontend maps the agent's choice to
your real implementation. This keeps the agent's job small, deciding which
...
See the [Structured output](/oss/langchain/frontend/structured-output) pattern.
...
<Info>
CopilotKit documents the same controlled techniques, including
[components as tools](https://docs.copilotkit.ai/generative-ui/tool-based), tool-call rendering,
state rendering, and reasoning, within its [generative UI overview](https://docs.copilotkit.ai/concepts/generative-ui-overview).
</Info>

## See also

- [Generative UI overview](/oss/langchain/frontend/generative-ui-overview)
- [Declarative generative UI](/oss/langchain/frontend/generative-ui)
