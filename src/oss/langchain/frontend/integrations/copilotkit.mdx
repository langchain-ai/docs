...
The same agent that powers your in-app copilot can also run as a bot in Slack and other messaging platforms. [CopilotKit Channels](https://docs.copilotkit.ai/channels) wraps any LangChain agent with a platform adapter, so you point a channel at any LangChain deployment you already run and reach the agent from where your team already works.
