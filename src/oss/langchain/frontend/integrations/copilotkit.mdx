...
1. **Deploy the graph as usual** using LangGraph.
...
In `langgraph.json`, point `http.app` at your Hono entrypoint:
...
This is the important extension point: `registerCopilotKit(app)` mounts a provider-specific runtime without replacing the underlying LangGraph deployment.
