...
import ChannelsChannel from "/snippets/code-samples/copilotkit-channels-channel-js.mdx"
...
The same agent that powers your in-app copilot can also run as a bot in messaging platforms. [CopilotKit Channels](https://docs.copilotkit.ai/channels) connects your LangChain agent to Slack and other messaging platforms, with threads, tool calls, rich interactive messages, and human-in-the-loop approvals handled natively in the channel.

<Info>
  Full platform setup lives in the [CopilotKit Channels documentation](https://docs.copilotkit.ai/channels). This page shows how Channels fit together with a LangChain agent.
</Info>

### How it fits together

Your LangChain deployment stays where it is, exposed over [AG-UI](https://docs.ag-ui.com/) (see [the custom endpoint above](#extend-the-langgraph-deployment-with-a-custom-endpoint)). A channel is a separate long-running Node service built with `@copilotkit/channels`: you point it at your agent and attach one or more platform adapters. `createChannel` takes an array of adapters, so a single process can serve several platforms at once.

Install the Channels SDK (batteries-included, every adapter) alongside the runtime that provides the LangGraph agent:
...
Build the agent as a per-thread factory so each conversation gets its own session. LangGraph Server speaks the LangGraph API, so use `LangGraphAgent` from `@copilotkit/runtime/langgraph`:
...
The agent receives ordinary AG-UI input and emits ordinary AG-UI events; the platform mechanics stay behind the adapter, so the same LangChain agent runs unchanged across every channel. Rich messages are written as JSX and rendered to each platform's native format (Block Kit on Slack, for example), so an interactive card degrades gracefully where a platform has no equivalent.

### Slack

Slack runs over Socket Mode, which opens an outbound WebSocket to Slack, so no public URL or tunnel is required during development. Set the Slack credentials in your environment:

- **`SLACK_BOT_TOKEN`**: Bot User OAuth token (`xoxb-...`).
- **`SLACK_APP_TOKEN`**: App-level token with the `connections:write` scope (`xapp-...`).

For the full Slack app walkthrough, see the [CopilotKit Channels documentation](https://docs.copilotkit.ai/slack).

### Other platforms

The Channels SDK is not limited to Slack. Platforms like Microsoft Teams, Discord, Telegram, and WhatsApp run through the same `@copilotkit/channels` API: add the matching adapter to the `adapters` array and the rest of your agent code stays the same. See the [CopilotKit Channels documentation](https://docs.copilotkit.ai/channels) for the current platform list and per-platform setup.

### Message pipeline and architecture

Channels can also run as managed connections through CopilotKit Intelligence, which keeps a deliberate credential split, useful when your LangChain agent holds its own model keys and tools:
