...
<Info>
  This page shows how Channels fit together with a LangChain agent. For the full platform walkthrough and other agent backends, see the [CopilotKit Channels documentation](https://docs.copilotkit.ai/channels).
</Info>
...
Declare the Channel with the `Code` from Intelligence and forward each message to the agent. Creating the listener is what connects the Channel: there is no `channel.start()`.
...
One Intelligence Channel can serve Slack and Microsoft Teams from the same `Code` and listener. See the [CopilotKit Channels documentation](https://docs.copilotkit.ai/channels) for the current platform list and per-platform setup.
