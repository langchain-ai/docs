...
import ChannelsAgent from "/snippets/code-samples/copilotkit-channels-agent-js.mdx"
import ChannelsListener from "/snippets/code-samples/copilotkit-channels-listener-js.mdx"
...
The same agent that powers your in-app copilot can also run as a bot in Slack and other messaging platforms. [CopilotKit Channels](https://docs.copilotkit.ai/channels) connects any LangChain agent to a messaging platform through a managed CopilotKit Intelligence connection, so you reach the agent from where your team already works without moving platform credentials into your agent process.

<Note>
  Channels require `@copilotkit/channels>=0.6.1` and `@copilotkit/runtime>=1.65.0`, installed together. The Channel listener is a small long-running Node service (Node.js 22 or later); the LangChain deployment it connects to can be Python or TypeScript.
</Note>
...
4. The listener runs your LangChain agent over [AG-UI](https://docs.ag-ui.com/), executes channel tools, and renders the reply.
...
Create a managed Channel in CopilotKit Intelligence: open your project, add a Channel, connect Slack from the generated app manifest, and create it. Intelligence stores the Slack credentials and gives you two values:

- **Channel code**: the Channel's unique `Code` (for example `support-slack`). The SDK declaration must use this exact value.
- **Intelligence API key**: a project-scoped key the listener uses to authenticate to the Intelligence gateway.

For the full Slack app walkthrough, see [Configure the Channel in Intelligence](https://docs.copilotkit.ai/slack/intelligence) in the CopilotKit docs.

### Install

Install the SDK pair together, then add the TypeScript tooling:
...
```bash bun
bun add --exact @copilotkit/channels@0.6.1 @copilotkit/runtime@1.65.0
...
The listener returns a fresh agent for each conversation, keyed by thread, so no state leaks across Slack threads. Point the native LangGraph adapter at the [deployment you already run](#extend-the-langgraph-deployment-with-a-custom-endpoint). LangGraph Server speaks the LangGraph API, so use `LangGraphAgent` from `@copilotkit/runtime/langgraph`, not `HttpAgent`:

<ChannelsAgent />

### Run the Channel listener

Declare the managed Channel with the `Code` from Intelligence, forward each incoming message to the agent, and start a long-running Node listener:

<ChannelsListener />

Creating the listener activates the Channel: it connects because it was declared. `ready()` is optional, resolving once activation settles; inspect `status()` before reporting the Channel online.

Set the runtime secrets the listener reads:
...
Post rich replies from inside the message handler by writing JSX, which the Slack adapter renders to Block Kit:

<ChannelsRichMessage />

### Other platforms

One Intelligence Channel can serve Slack and Microsoft Teams from the same `Code` and listener. Managed Discord and WhatsApp are coming soon; the Channels SDK already ships developer-operated direct adapters for Discord, Telegram, and WhatsApp. Each maps the shared JSX vocabulary to that platform's native message format, so one agent serves every surface.
