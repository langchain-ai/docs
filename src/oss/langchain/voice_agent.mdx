...
This guide demonstrates how to interact with LangChain [agents](/oss/langchain/agents) through voice channels. It follows the reference architecture implemented in the LangChain repo [here](https://github.com/langchain-ai/voice-sandwich-demo).
...
- Access to latest reasoning capabilities from state-of-the-art text models
- Transparent behavior with clear boundaries between components
- Flexible integration with existing LangChain tools and agents
- Lower total latency with optimized providers (~700ms achievable)
...
- Potentially lower latency for simple interactions

**Cons:**
- Limited model options and provider lock-in
- Reasoning capabilities lag behind state-of-the-art text models
- Less transparency in how audio is processed
- Reduced controllability and customization options
- More potential failure modes hidden behind the API
...
This guide walks through building a voice-based agent using the sandwich architecture. We will build a sandwich shop ordering assistant that demonstrates all three components of the sandwich architecture. We will use [AssemblyAI](https://www.assemblyai.com/) for STT and [ElevenLabs](https://elevenlabs.io/) for TTS, but adapters can be built for most providers.
...
- Orchestrates the three-step pipeline:
  1. [STT step](#1-speech-to-text): Forwards audio to the STT provider (e.g., AssemblyAI), receives transcript events
  2. [Agent step](#2-langchain-agent): Processes transcripts with LangChain agent, streams response tokens
  3. [TTS step](#3-text-to-speech): Sends agent responses to the TTS provider (e.g., ElevenLabs), receives audio chunks
