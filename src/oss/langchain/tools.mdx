...
- **Multiple parallel tool calls:** When the model calls several tools in one step, all of them execute first — `return_direct` does not short-circuit individual tool execution. After all tools finish, if **any** tool in the batch has `return_direct=True`, the agent routes to `END` instead of calling the model again. The final response includes the `ToolMessage` outputs of every tool that was called in that step.
...
<Warning>
    **Mixed parallel calls:** If the model calls a `return_direct=True` tool alongside tools that do not have `return_direct=True`, the agent still exits after that step. The non-`return_direct` tool results are included in the message history but are not processed further by the model. Design your tool sets carefully — avoid mixing `return_direct=True` and `return_direct=False` tools if you expect the model to reason over all results.
</Warning>
