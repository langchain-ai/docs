...
Tools are not limited to plain text. When the model supports multimodal tool results, the tool can return a list of [standard content blocks](/oss/langchain/messages#standard-content-blocks) so the model receives text, images, and other media in one tool result.

Supported patterns include:

- **Text only**: return a string (converted to a text block).
- **Text plus media**: return an ordered list with a text block followed by an image, file, audio, or video block.
- **Interleaved blocks**: return any ordered mix of block types the model accepts.
...
- The model must support the modalities you return. Check your [model's capabilities](/oss/langchain/models#multimodal) before returning images, audio, or video.
