...
<Note>
`context` is distinct from `thread_id`. `thread_id` (passed via `config={"configurable": {"thread_id": ...}}`) scopes the *conversation*: message history and checkpoints. `context` carries *per-run* data your tools and middleware read at invocation time. In production you typically pass both together: a stable `thread_id` per conversation, and a `context` object on every invoke.
</Note>
...
result = agent.invoke(
    ...
    context=UserContext(user_id="user123"),  # [!code highlight]
)
```
...
const result = await agent.invoke(
  ...
  {
    configurable: { thread_id: uuidv4() },  // [!code highlight]
