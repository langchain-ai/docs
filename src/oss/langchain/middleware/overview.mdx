...
:::python
The key is the Python function name — `@tool`-decorated functions take their name from the function itself (so the key below is `"send_email"`).
...
:::js
The key matches the `name` you pass to `tool({...}, { name })` when registering each tool (so the key below is `"send_email"`).
