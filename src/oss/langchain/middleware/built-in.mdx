...
<ParamField body="trigger" type="tuple | list[tuple]">
    Conditions for triggering summarization. Can be:

    - A single condition tuple (all properties must be met - AND logic)
    - A list of condition tuples (any condition must be met - OR logic)
...
<ParamField body="keep" type="tuple" default="("messages", 20)">
