...
<ParamField body="trigger" type="ContextSize | list[ContextSize] | None">
    ...
    - **Single condition** — a single @[`ContextSize`] tuple. Summarization triggers when that one condition is met.
    ...
    - **Multiple conditions (OR logic)** — a list of @[`ContextSize`] tuples. Summarization triggers when **any** condition in the list is met.
    ...
    <Note>
        Python's `ContextSize` is a single `("key", value)` tuple and holds exactly one threshold at a time. Unlike the JavaScript API (which accepts a multi-property object for AND logic), Python does **not** support requiring multiple different conditions to be true simultaneously. A list of tuples always evaluates as **OR logic** — summarization triggers when **any one** of the listed conditions is met.
    </Note>

    Each condition must be one of the following:
...
:::python
- **Single condition** — pass a single tuple `("key", value)`. Triggers when that one condition is met.
- **Multiple conditions (OR logic)** — pass a list of tuples `[("key1", v1), ("key2", v2)]`. Triggers when **any** condition in the list is met.
- Each condition uses `fraction` (of model's context size), `tokens` (absolute count), or `messages` (message count).
- **No AND logic** — Python's `ContextSize` tuple holds exactly one threshold. A list of tuples is always OR logic; there is no Python equivalent of JavaScript's multi-property AND object.
:::

:::js
- **Single condition (AND logic)** — pass a single object `{ key: value, ... }`. All properties in the object must be met simultaneously.
- **Multiple conditions (OR logic)** — pass an array of objects `[{ key1: v1 }, { key2: v2 }]`. Triggers when **any** object's conditions are met.
- Each object can include `fraction` (of model's context size), `tokens` (absolute count), or `messages` (message count).
:::

**Keep condition** controls how much context to preserve after summarization (specify exactly one):
...
// Each object in the array is an independent condition set; any match triggers summarization.
