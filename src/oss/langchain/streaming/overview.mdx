---
title: Overview
description: Stream real-time updates from agent runs including LLM tokens, thinking and reasoning tokens, and custom events
---
...
Some models (such as OpenAI `o`-series, Anthropic Claude with extended thinking, and others) perform internal reasoning before producing a final answer. You can stream these thinking / reasoning tokens as they're generated by filtering [standard content blocks](/oss/langchain/messages#standard-content-blocks) for `type == "reasoning"`.

<Note>
Reasoning output must be enabled on the model. See the [reasoning section](/oss/langchain/models#reasoning) and your [provider's integration page](/oss/integrations/providers/overview) for configuration details.
</Note>

To stream thinking tokens from an agent, use `stream_mode="messages"` and filter for reasoning content blocks:

```python title="Streaming thinking / reasoning tokens"
from langchain.agents import create_agent
...
for token, metadata in agent.stream(
    ...
    if not isinstance(token, AIMessageChunk):
        continue
    reasoning = [b for b in token.content_blocks if b["type"] == "reasoning"]  # [!code highlight]
