...
import McpAuthOauthPy from '/snippets/code-samples/mcp-auth-oauth-py.mdx';
import McpAuthPerServerPy from '/snippets/code-samples/mcp-auth-per-server-py.mdx';
...
Most remote MCP servers require authentication. @[`MCPAdapter`] delegates auth to FastMCP, so any credential a `fastmcp.Client` accepts works: a static bearer token, a full OAuth 2.1 flow, or any [`httpx.Auth`](https://www.python-httpx.org/advanced/authentication/). Pass the credential on a prebuilt client and hand that client to the adapter.

## Bearer token

The simplest case: the server verifies a token you provisioned out of band and present in the `Authorization: Bearer <token>` header. There is no discovery, browser, or refresh. Pass the token as the client's `auth`:
...
client = Client("https://example.com/mcp", auth=OAuth(mcp_url="https://example.com/mcp", token_storage=...))
```
...
This differs from an `MCPConfig` fleet, which shares one aggregate connection. Reach for a `ClientGroup` whenever servers authenticate separately.
...
2. **Mint or exchange a credential for that user.** Inside the [graph factory](/oss/langchain/mcp/connections#long-lived-connections), read the user's identity and build the MCP client with a per-user token, so the connection carries that user's authorization.
