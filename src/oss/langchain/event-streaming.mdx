...
for subagent in run.subgraphs:
    if subagent.graph_name not in ("weather_agent", "research_agent"):  # [!code highlight]
        continue
...
for await (const subagent of run.subgraphs) {
  if (!["weather_agent", "research_agent"].includes(subagent.name)) continue; // [!code highlight]
