...
<Info>
LangChain v1.3 introduced **Event streaming**, a typed-projection API on top of `.stream()`. Each projection (`run.messages`, `run.values`, `run.tool_calls`, `run.subgraphs`) is its own iterator, so you can read them independently instead of branching on `stream_mode` chunks. For docs on the existing `.stream()` API, see the [`.stream()` API guide](/oss/langchain/streaming).
</Info>
