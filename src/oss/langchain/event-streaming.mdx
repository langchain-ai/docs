...
| Projection | Use |
| ---------- | --- |
| `for event in run` | Raw protocol events with full envelope and access to every channel. |
...
| `run.subgraphs` | Nested graph runs (sub-agents and plain subgraphs). |
...
When a `create_agent` call invokes another `create_agent` (via a wrapping tool, typically), the inner agent's events flow at a nested namespace and surface as a handle on `run.subgraphs`. Each handle exposes the inner agent's own `.messages`, `.values`, `.tool_calls`, and `.output` projections. The `name=` you pass to `create_agent` becomes `subagent.graph_name` (Python) / `subagent.name` (JS), which lets you filter and label per agent.

Every nested `CompiledStateGraph` shows up on `run.subgraphs` — `create_agent` instances are one specific kind. Filter on the name to act only on the ones you care about.
...
supervisor = create_agent(
    model=init_chat_model("openai:gpt-5.4"),
    tools=[call_weather, call_research],
    name="supervisor",
)

run = supervisor.stream_events(
    {"messages": [{"role": "user", "content": "Weather in Boston, and any recent papers on it?"}]},
    version="v3",
)

for subagent in run.subgraphs:
    if subagent.graph_name == "weather_agent":  # [!code highlight]
        print("weather: ", end="")
    elif subagent.graph_name == "research_agent":  # [!code highlight]
        print("research: ", end="")
    else:  # [!code highlight]
...
const searchPapers = tool(
  async ({ topic }) => `Found 3 papers on ${topic}.`,
  { name: "search_papers", schema: z.object({ topic: z.string() }) }
);
...
const callResearch = tool(
  ...
  { name: "call_research", schema: z.object({ query: z.string() }) }
);

const supervisor = createAgent({
  model: "openai:gpt-5.4",
  tools: [callWeather, callResearch],
  name: "supervisor",
});
...
for await (const subagent of run.subgraphs) {
  let label: string;
  if (subagent.name === "weather_agent") label = "weather"; // [!code highlight]
  else if (subagent.name === "research_agent") label = "research"; // [!code highlight]
  else continue; // [!code highlight]
...
See [Build your own projection](/oss/langgraph/streaming/event-streaming#build-your-own-projection) for the transformer contract.

## Related

- [Pregel Streaming](/oss/langchain/streaming) covers low-level Pregel stream modes.
- [Build your own projection](/oss/langgraph/streaming/event-streaming#build-your-own-projection) covers writing application-specific projections.
