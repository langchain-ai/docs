...
<Info>
LangChain v1.3 introduced **event streaming**, a typed-projection API on top of `.stream()`. Each projection is its own iterator, so you can consume them independently instead of branching on stream-mode chunks. See the [Streaming](/oss/langchain/streaming) docs for the existing API.
</Info>
