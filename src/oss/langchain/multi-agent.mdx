...
def call_subagent1(state: Annotated[CustomState, InjectedState]):
    result = subagent1.invoke({
        "messages": [
            {
                "role": "user", 
                "content": some_logic(state.messages)  # Apply any logic needed to transform the messages
            }
        ]
    })
...
const callSubagent1 = tool(
  async () => {
    ...
    // Apply any logic needed to transform the messages
    const subAgentInput = someLogic(state.messages);
    const result = await subagent1.invoke({
