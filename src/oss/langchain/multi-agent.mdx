...
This example shows a workflow that combines three different types of nodes:

- **Model node** (Rewrite): An LLM call that rewrites the user query for better retrieval. Uses structured output to produce a clean search query.
- **Deterministic node** (Retrieve): A standard function that performs vector similarity search. No LLM involved — just embedding lookup and ranking.
- **Agent node** (Agent): A full agent with tools that can reason over the retrieved context and optionally fetch additional information.
