...
sequenceDiagram
    ...
    User->>Agent: "My device is broken"
    Note over Agent,Workflow State: warranty_collector<br/>Available: record_warranty_status
    Agent->>Workflow State: record_warranty_status("in_warranty")
    Note over Workflow State: → issue_classifier
    Agent-->>User: "Can you describe the issue?"

    User->>Agent: "The screen is cracked"
    Note over Agent,Workflow State: issue_classifier<br/>Available: record_issue_type
    Agent->>Workflow State: record_issue_type("hardware")
    Note over Workflow State: → resolution_specialist
    Agent-->>User: "I can help with that"

    User->>Agent: "What should I do?"
    Note over Agent,Workflow State: resolution_specialist<br/>Available: provide_solution, escalate_to_human
