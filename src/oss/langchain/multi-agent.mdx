...
There are three main approaches to implementing supervisor coordination:
...
### One tool for all agents (spawning)
...
A supervisor can manage other agents in a non-blocking way by kicking off agent invocations as background jobs. This is particularly valuable because agents can be long-running—analyzing complex documents, multi-step research, or extensive code reviews can take minutes or longer. By running agents as background jobs, the supervisor remains responsive while specialized work proceeds asynchronously.
...
The pattern uses three tools: one to kick off an agent as a background job (returning a job ID), one to check status, and one to retrieve results when complete. While this pattern is especially useful for managing long-running agents (legal review, financial analysis, code audits), the same approach works for any asynchronous computation (database queries, API calls, file processing, ML model inference, etc.).

When a job completes, you'll need to decide how your application handles that event. For example, you could surface a notification to the user that a job is complete. When the user clicks the notification, your application could send a `HumanMessage` like "Please check on the status of job_123 and summarize the results" — this prompts the agent to use the status and result tools, then present the findings to the user.
...
There are two main approaches to implementing routing:
