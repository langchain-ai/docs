...
Context engineering determines **what information** is passed to agents at each step.
LangChain enables fine-grained control over:
...
One of the simplest ways to compose agents is to let the main agent **treat subagents like tools**. In this setup, the main (or controller) agent routes work to specialized subagents, gathers their responses, and decides the next step.
...
3. [**Input to the subagent**](#control-subagent-input): In this example, we pass the agent-generated `query` directly. You can customize this input to better shape how the subagent interprets tasks.
4. **Output from the subagent**: Here, we return the `.text` property of the final message. This is the **response** passed back to the main agent. You can adjust what is returned to control how the main agent interprets results.
...
# When passing the full conversation, you often do NOT want to pass in a query.
