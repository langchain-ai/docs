...
| Decision | Options |
|----------|---------|
| [**Execution mode**](#sync-vs-async) | Sync (blocking) vs. async (background) |
| [**Tool design**](#tool-patterns) | Tool per agent vs. single dispatch tool |
...
| Input approach | What subagent receives | Use when |
|----------------|------------------------|----------|
| **Query only** | Just the task string | Subagent is self-contained and doesn't need conversation context |
| **Query + history** | Task string + relevant messages | Subagent needs to understand prior conversation |
| **Query + state** | Task string + custom state keys | Subagent needs structured data (user preferences, prior results) |
...
Customize what the main agent receives back so it can make good decisions.

| Output approach | What main agent receives | Use when |
|-----------------|--------------------------|----------|
| **Text only** | Final message content as string | Simple cases where text response is sufficient |
| **Text + state** | Text plus structured state updates via `Command` | Main agent needs structured data to continue (e.g., extracted entities, status flags) |

Two strategies for controlling outputs:
