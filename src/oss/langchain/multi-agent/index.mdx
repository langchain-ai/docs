...
| [**Router**](/oss/langchain/multi-agent/router) | A routing step classifies input and directs it to specialized agents. Two variations: **classifier** (single agent, direct response) or **synthesis** (multiple agents in parallel, combined response). |
...
- **Distributed development**: Can different teams maintain components independently? Subagents, Skills, and Router all support this—specialized agents or skills can be owned by different teams.
...
- **Direct user interaction**: Can specialized agents converse directly with the user while keeping multi-turn flow manageable? Handoffs and Skills excel here. Subagents and Router route through a central coordinator or synthesis step (no direct user interaction). A Router can pass through to a specialized agent for direct response, but multi-turn coordination becomes complex in that setup.
...
**Same-domain vs. cross-domain:** Handoffs and Skills save calls when conversation stays within a single domain (e.g., repeated coffee orders). When turns span multiple domains (e.g., "Buy coffee" then "What's the Python syntax for list comprehensions?"), Subagents and Router excel: each turn routes to the right specialist without carrying state from unrelated prior turns, and both support parallel execution when a single turn touches multiple domains.
