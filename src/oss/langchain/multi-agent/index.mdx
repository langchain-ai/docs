...
<Tip>
**Looking for a batteries-included option?** [Deep Agents](/oss/deepagents/overview) is a higher-level harness built on LangChain that ships with [subagents](/oss/deepagents/subagents), [skills](/oss/deepagents/skills), planning, a virtual filesystem, and context management out of the box—so you don't have to wire up these patterns yourself. Use the patterns below when you want full control over your multi-agent architecture; reach for Deep Agents when you'd rather start from a prebuilt harness and customize from there.
</Tip>
