  ...
  <Tab title="Handoffs">
    **3 model calls:**
    ...
    sequenceDiagram
        ...
        participant CA as Coffee Agent
        participant BT as buy_coffee tool

        U->>MA: "Buy coffee"
        Note over MA: Call 1
        MA->>CA: transfer_to_coffee_agent()
        Note over CA: Call 2
        CA->>BT: buy_coffee()
        BT-->>CA: Done
        Note over CA: Call 3
        CA-->>U: "I bought coffee for you"
    ```
  </Tab>
  ...
  <Tab title="Skills">
    **3 model calls:**
    ...
    sequenceDiagram
        ...
        U->>A: "Buy coffee"
        Note over A: Call 1
        A->>LS: load_skill("coffee")
        LS-->>A: Coffee skill context
        Note over A: Call 2
        A->>BT: buy_coffee()
        BT-->>A: Done
        Note over A: Call 3
        A-->>U: "I bought coffee for you"
    ```
  </Tab>
  ...
  <Tab title="Router">
    **3 model calls:**
    ...
    sequenceDiagram
        ...
        U->>R: "Buy coffee"
        Note over R: Call 1: Route to coffee agent
        R->>CA: Invoke with query
        Note over CA: Call 2
        CA->>BT: buy_coffee()
        BT-->>CA: Done
        Note over CA: Call 3
  ...
  <Tab title="Subagents">
    **5 calls, ~9K tokens**
    ...
    sequenceDiagram
        participant U as User
        participant MA as Main Agent
        participant PS as Python Subagent
        participant JS as JS Subagent
        participant RS as Rust Subagent

        U->>MA: "Compare Python, JS, Rust"
        Note over MA: Call 1 (1K)
        par Parallel execution
            MA->>PS: python_subagent()
            Note over PS: Call 2 (2K)
            PS-->>MA: Python analysis
        and
            MA->>JS: js_subagent()
            Note over JS: Call 3 (2K)
            JS-->>MA: JS analysis
        and
            MA->>RS: rust_subagent()
        ...
        Note over MA: Call 5 (2K)
        MA-->>U: Synthesized comparison
    ```
  ...
  <Tab title="Handoffs">
    **7+ calls, ~14K+ tokens**
    ...
    sequenceDiagram
        ...
        participant JA as JS Agent
        participant RA as Rust Agent

        U->>MA: "Compare Python, JS, Rust"
        Note over MA: Call 1 (1K)
        MA->>PA: transfer_to_python()
        Note over PA: Calls 2-3 (2K each)
        PA->>JA: transfer_to_js()
        Note over JA: Calls 4-5 (2K each)
        JA->>RA: transfer_to_rust()
        Note over RA: Calls 6-7 (2K each)
        RA-->>U: Combined comparison
    ```
  ...
  <Tab title="Skills">
    **3 calls, ~15K tokens**
    ...
    sequenceDiagram
        ...
        U->>A: "Compare Python, JS, Rust"
        Note over A: Call 1 (1K)
        A->>LS: load_skill("python", "js", "rust")
        LS-->>A: +6K context (2K each)
        Note over A: Call 2 (7K: 1K base + 6K skills)
        Note over A: Call 3 (7K: 1K base + 6K skills)
  ...
  <Tab title="Router">
    **5 calls, ~9K tokens**
    ...
    sequenceDiagram
        ...
        U->>R: "Compare Python, JS, Rust"
        Note over R: Call 1 (1K)
        par Parallel execution
            R->>PA: Route to Python
            Note over PA: Call 2 (2K)
            PA-->>S: Python analysis
        and
            R->>JA: Route to JS
            Note over JA: Call 3 (2K)
            JA-->>S: JS analysis
        and
            R->>RA: Route to Rust
            Note over RA: Call 4 (2K)
            RA-->>S: Rust analysis
        end
        Note over S: Call 5 (2K)
        S-->>U: Combined comparison
    ```
