...
**Example:** A warranty support flow. User says "My phone is broken" → agent collects warranty status → once recorded, transitions to specialist step with different tools and prompt → agent provides resolution. The transition is driven by *state* (we've collected the data), not by classifying the user's next message.

<Note>
**Handoffs vs. classifier (Router):** Handoffs route based on *conversation state*—what step we're in, what we've collected, what preconditions are met. The [classifier variation of the Router pattern](/oss/langchain/multi-agent/router#two-variations) routes based on *user intent*—which agent handles this question. Use Handoffs for sequential workflows (collect X, then do Y). Use the classifier for "which specialist answers this?"
</Note>
