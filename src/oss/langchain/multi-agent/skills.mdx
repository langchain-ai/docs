...
**Example:** A coding assistant with skills for SQL, Python, and API design. User asks "Write a query to find top customers" → agent loads the SQL skill on-demand → uses its specialized prompt and context → responds. Skills load only when needed, keeping the default context lean.
