...
:::python
<Note>
Requires `deepagents>=0.5.0` (or the corresponding `langchain>=1.1.0` / `langgraph>=1.1.3` releases) for `runtime.execution_info` and `runtime.server_info`.
</Note>
:::
...
:::python
<Note>
Requires `deepagents>=0.5.0` (or the corresponding `langchain>=1.1.0` / `langgraph>=1.1.3` releases).
</Note>
:::

:::js
<Note>
Requires `deepagents>=1.9.0` (or the corresponding `@langchain/langgraph` release).
</Note>
:::
