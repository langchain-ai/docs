...
:::python
Use the `Runtime` parameter to access the @[`Runtime`] object inside node-style (before/after) hooks.
The `Runtime` object is available inside the @[`ModelRequest`] parameter passed to wrap-style (wrap) hooks.
