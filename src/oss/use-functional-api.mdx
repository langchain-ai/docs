...
---
The [**Functional API**](./functional_api) allows you to add LangGraph's key features — [persistence](./persistence), [memory](./memory/add-memory), [human-in-the-loop](./human_in_the_loop), and [streaming](./streaming) — to your applications with minimal changes to your existing code.

<Tip>
  For conceptual information on the functional API, see [Functional API](./functional_api).
</Tip>
...
read the [**streaming guide**](./streaming) section for more details.
...
We will create three [tasks](./functional_api#task):
...
We can now compose these tasks in an [entrypoint](./functional_api#entrypoint):
...
Short-term memory allows storing information across different **invocations** of the same **thread id**. See [short-term memory](./functional_api#short-term-memory) for more details.
...
* [Workflows and agent](./workflows) guide for more examples of how to build workflows using the Functional API.
