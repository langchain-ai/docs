...
After a successful update, OpenWiki records metadata in `openwiki/.last-update.json` only when the wiki content snapshot changed. A no-op update that leaves docs untouched does not rewrite that metadata, which helps keep scheduled workflows from opening empty PRs in a loop.
...
You can uncomment the corresponding line in the example workflow. For what is collected and how to opt out locally, see [Customize OpenWiki](/oss/openwiki/customize#telemetry).
