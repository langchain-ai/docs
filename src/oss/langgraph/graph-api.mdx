...
Alternatively, wrap the update with [`Overwrite`](https://reference.langchain.com/javascript/langchain-langgraph/index/Overwrite) to bypass the reducer for a single update, while keeping the field's normal merging behavior for every other update:
