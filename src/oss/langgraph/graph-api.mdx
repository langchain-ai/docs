...
`Command` is used in two contexts:

- **[Return from node functions](#combine-state-updates-and-control-flow)**: Use `update`, `goto`, and `graph` to combine state updates with control flow.
- **[Pass as input](#resume-after-an-interrupt)** to `invoke()`/`stream()`: Use `resume` to continue execution after an interrupt.
