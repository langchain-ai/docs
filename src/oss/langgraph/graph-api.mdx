...
import LanggraphGraphApiReducersOverwriteJs from '/snippets/code-samples/langgraph-graph-api-reducers-overwrite-js.mdx';
import LanggraphGraphApiReducersOverwritePy from '/snippets/code-samples/langgraph-graph-api-reducers-overwrite-py.mdx';
...
A common source of confusion with reducers: with a merging reducer such as `operator.add`, returning an empty value does **not** clear the field. Because the reducer merges the right argument into the left one, an empty update is merged in and previously accumulated values are kept:
...
<LanggraphGraphApiReducersOverwritePy />

:::

:::js

<LanggraphGraphApiReducersOverwriteJs />
