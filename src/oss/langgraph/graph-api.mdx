...
When you compile with a [checkpointer](/oss/langgraph/persistence), LangGraph saves checkpoints at [super-step](#graphs) boundaries, not mid-function inside a node. If execution stops and later resumes (for example after an [interrupt](/oss/langgraph/interrupts) or a [retry](/oss/langgraph/fault-tolerance#retries)), the affected **node** can run again from the start of its function, so statements before the pause execute again.

To ensure that your workflow is deterministic and can be consistently replayed, follow these guidelines:

* **Avoid repeating work**: Prefer several smaller **nodes** (or subgraphs) so each side effect or network call sits behind its own checkpoint boundary. If you need finer-grained checkpointing inside one node, call [**tasks**](/oss/langgraph/functional-api#task) from the node body; they are part of the [Functional API](/oss/langgraph/functional-api) and may only be invoked from an entrypoint, another task, or a graph node—see [Execution](/oss/langgraph/functional-api#execution) and [Using tasks in nodes](#using-tasks-in-nodes).
* **Encapsulate non-deterministic operations**: Avoid wall-clock reads, randomness, or branching on volatile values in the middle of a large node if that logic runs again on resume. Move that work behind a **node** boundary, or run it inside a [**task**](/oss/langgraph/functional-api#task) you call from the node so the recorded result drives replay.
* **Use idempotent operations**: Design side effects so a **node** that [retries](/oss/langgraph/fault-tolerance#retries) or re-executes after an interrupt does not corrupt data. Use idempotency keys or read-before-write checks. For how partial **task** failures interact with checkpoints, see [Idempotency](/oss/langgraph/functional-api#idempotency).

:::

:::js

When you compile with a [checkpointer](/oss/langgraph/persistence), LangGraph saves checkpoints at [super-step](#graphs) boundaries, not mid-function inside a node. If execution stops and later resumes (for example after an [interrupt](/oss/langgraph/interrupts) or a retry), the affected **node** can run again from the start of its function, so statements before the pause execute again.

To ensure that your workflow is deterministic and can be consistently replayed, follow these guidelines:

* **Avoid repeating work**: Prefer several smaller **nodes** (or subgraphs) so each side effect or network call sits behind its own checkpoint boundary. If you need finer-grained checkpointing inside one node, call [**tasks**](/oss/langgraph/functional-api#task) from the node body; they are part of the [Functional API](/oss/langgraph/functional-api) and may only be invoked from an entrypoint, another task, or a graph node—see [Execution](/oss/langgraph/functional-api#execution) and [Using tasks in nodes](#using-tasks-in-nodes).
* **Encapsulate non-deterministic operations**: Avoid wall-clock reads, randomness, or branching on volatile values in the middle of a large node if that logic runs again on resume. Move that work behind a **node** boundary, or run it inside a [**task**](/oss/langgraph/functional-api#task) you call from the node so the recorded result drives replay.
* **Use idempotent operations**: Design side effects so a **node** that retries or re-executes after an interrupt does not corrupt data. Use idempotency keys or read-before-write checks. For how partial **task** failures interact with checkpoints, see [Idempotency](/oss/langgraph/functional-api#idempotency).

:::

For effects around `interrupt()`, see [Side effects called before `interrupt` must be idempotent](/oss/langgraph/interrupts#side-effects-called-before-interrupt-must-be-idempotent).
