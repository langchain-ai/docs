...
Not sure which to pick? See [Choosing between Graph and Functional APIs](/oss/langgraph/choosing-apis). For conceptual information, see [Graph API overview](/oss/langgraph/graph-api) and [Functional API overview](/oss/langgraph/functional-api).

<Prompt
    description="Build the LangGraph calculator quickstart"
    icon="sparkles"
    actions={["copy"]}
>
...
Define tools and model, then the Graph API nodes/edges or Functional API tasks and agent.
2. Optionally enable LangSmith tracing by asking the user to set `LANGSMITH_TRACING=true` and `LANGSMITH_API_KEY` themselves.
3. Invoke the agent on a sample arithmetic query from the guide and print the final result.
...
See [chat model integrations](/oss/integrations/chat) for all available providers. If you use [LangSmith Gateway](/langsmith/llm-gateway), you can [bring your own provider keys](/langsmith/llm-gateway-quickstart) or use [Gateway Credits](/langsmith/llm-gateway-credits) to access models without a provider key.
...
:::js
Install `@langchain/langgraph`, `@langchain/core`, and `langchain`:
...
Build the agent with @[`StateGraph`] and compile it with @[`compile`][StateGraph.compile]:
...
## 7. Set up LangSmith tracing

[LangSmith](https://smith.langchain.com) shows tool calls and model responses as the agent runs.
...
# Step 7: Set up LangSmith tracing
# export LANGSMITH_TRACING=true
# export LANGSMITH_API_KEY="your-langsmith-api-key"
...
Define the agent with @[`@entrypoint`].
...
You now have a working calculator agent. Continue with one of the following subjects:

- **Learn the APIs**: [Choosing between Graph and Functional APIs](/oss/langgraph/choosing-apis), then [Build with the Graph API](/oss/langgraph/use-graph-api) for state and control-flow decisions, or the [Functional API overview](/oss/langgraph/functional-api) for the imperative path.
- **Add persistence**: Use a [checkpointer](/oss/langgraph/persistence) so runs can pause and resume across sessions.
- **Add human-in-the-loop**: Pause with [interrupts](/oss/langgraph/interrupts) for approval or edits.
- **Stream results**: Follow [streaming](/oss/langgraph/streaming) for tokens and state updates.
- **Go deeper**: [Thinking in LangGraph](/oss/langgraph/thinking-in-langgraph) and [Workflows and agents](/oss/langgraph/workflows-agents).
- **Deploy**: Host the agent with [LangSmith Deployment](/langsmith/deployment).
- **Evaluate**: Test behavior with [LangSmith evaluation](/langsmith/evaluation-quickstart).
