...
### Streaming with Human-in-the-Loop (HITL) Interrupts
...
To handle multiple interrupts in a single workflow, wrap the streaming logic in a loop:
```python
while True:
    has_interrupt = False
    
    async for metadata, mode, chunk in graph.astream(...):
        # ... handle streaming and interrupts
        
        if "__interrupt__" in chunk:
            has_interrupt = True
            # Handle interrupt and update initial_input
            break
    
    if not has_interrupt:
        # Graph completed without interrupts
        break
```
...
- Content review and editing pipelines
- Interactive research or writing assistants
- Any scenario requiring human validation before proceeding
