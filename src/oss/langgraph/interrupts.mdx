...
- **Interrupt payloads surface via `stream.interrupts`:** when using [event streaming](/oss/langgraph/event-streaming) (`graph.stream_events(..., version="v3")`), the values you pass to `interrupt()` appear on `stream.interrupts`, and `stream.interrupted` is `True` when the run paused for input.
:::
:::js
- **Checkpointing keeps your place:** the checkpointer writes the exact graph state so you can resume later, even when in an error state.
...
Use the typed projections returned by `graph.stream_events(..., version="v3")` to:
...
- Resume execution by calling `stream_events` again with `Command(resume=...)`

:::python
<LanggraphInterruptsHitlStreamPy />

- **`stream.messages`**: Chat-model output as content blocks; iterate each `message.text` for token deltas (nested subgraph messages flow through the same projection)
- **`stream.values`**: Full state snapshots after each step
- **`stream.interrupted` / `stream.interrupts`**: Check for and read pending interrupts after the stream is drained
