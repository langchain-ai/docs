...
@[`NodeTimeoutError`] is retryable by default. There is no exported equivalent of Python's `default_retry_on`—provide your own `retryOn` callback to extend or replace this behavior.
:::
...
The `timeout` parameter on @[`add_node`] / [`addNode`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#addnode) caps how long a single node attempt may run.

:::python
Pass a number (seconds), a `timedelta`, or a @[`TimeoutPolicy`] for separate run and idle limits:
...
:::js
Pass `errorHandler` to [`addNode`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#addnode) on @[`StateGraph`] only (not the base `Graph` class):
