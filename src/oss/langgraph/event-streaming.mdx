...
Built-in projections (`run.messages`, `run.values`, `run.subgraphs`, `run.output`) and product-specific projections (LangChain's `run.tool_calls`, Deep Agents' `run.subagents`) are themselves transformers using this same contract. User transformers stack on top via compile-time or call-time registration, and their projections appear under `run.extensions`.
