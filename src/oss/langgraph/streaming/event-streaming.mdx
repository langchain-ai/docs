...
A transformer that only processes events from specific Pregel stream modes should declare them via `required_stream_modes`. The runtime takes the union of every registered transformer's `required_stream_modes` and asks the graph for exactly those modes — modes that no transformer requests aren't emitted at all, which keeps the event stream tight.
...
class CustomTransformer(StreamTransformer):
    ...
    def process(self, event: ProtocolEvent) -> bool:
        # process() will only see "custom" events.
        ...
```
:::

Set the tuple to the modes your `process()` actually filters on (`"messages"`, `"tools"`, `"custom"`, `"values"`, `"updates"`, `"checkpoints"`, `"tasks"`, `"debug"`). The default is an empty tuple, which means the transformer consumes only synthetic events or is purely passive.
