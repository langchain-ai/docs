...
<Info>
LangGraph v1.2 introduced **Event streaming**, a typed-projection API on top of `.stream()`. Each projection is its own iterator, so you can consume them independently instead of branching on stream-mode chunks. See the [`.stream()` guide](/oss/langgraph/streaming) for docs on the existing API.
</Info>
