...
`process()` receives every event the graph emits (filtered by your own `event["method"]` checks), so the declaration is about turning on the upstream emission, not about narrowing what `process()` sees. Valid values are the Pregel stream modes: `"messages"`, `"tools"`, `"custom"`, `"values"`, `"updates"`, `"checkpoints"`, `"tasks"`, `"debug"`. The default is an empty tuple, which is correct for transformers that derive their projection from synthetic events emitted by other transformers (or are purely passive).
