...
See [StreamChannel](/oss/langgraph/streaming/streamchannel) for choosing whether pushed values stay on `run.extensions` only or also flow into the main event stream as `custom:<name>` events.
