...
The convention is to define a `setup()` method on the checkpointer or store instance to run the required migrations. However, you should check with your specific implementation of @[`BaseCheckpointSaver`] or @[`BaseStore`] to confirm the exact method name and usage.
