...
<Accordion title="Example: using MongoDB store">
  ...
  <Tip>
  `MongoDBStore.fromConnString` calls `start()` automatically, which creates the required indexes. For vector search, allow a moment for the Atlas Search index to become `READY` before querying.
  </Tip>
  ...
  const model = new ChatAnthropic({ model: "claude-sonnet-4-5-20251001" });
  ...
  // Manual embedding: supply an EmbeddingsInterface and dims to enable vector search.
  // Omit embeddings and indexConfig to use field-based search only.
  const store = await MongoDBStore.fromConnString(MONGODB_URI, {
    dbName: "langgraph",
    collectionName: "store",
...
:::js
<Tip>
`InMemoryStore` is suitable for development. For production, use a persistent store like `MongoDBStore`, `PostgresStore`, or `RedisStore`.
</Tip>
:::
