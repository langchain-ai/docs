...
* `langchain-cockroachdb`: A checkpointer that uses CockroachDB (`CockroachDBSaver` / `AsyncCockroachDBSaver`). Ideal for production workloads requiring distributed SQL, SERIALIZABLE isolation, and horizontal scalability. Needs to be installed separately via `pip install langchain-cockroachdb`.
:::
