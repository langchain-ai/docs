...
<Tip>
You can also access the store using the keyword argument injection pattern: `def update_memory(state, config: RunnableConfig, *, store: BaseStore)`. See the [memory how-to](/oss/langgraph/add-memory#access-the-store-inside-nodes) for more details.
</Tip>
