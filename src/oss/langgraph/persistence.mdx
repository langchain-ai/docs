...
items = await store.asearch(("alice", "memories"), limit=100)
```
:::
...
offset = 0
while True:
    page = await store.asearch(("alice", "memories"), limit=page_size, offset=offset)
    if not page:
        break
    for item in page:
        ...
...
namespaces = await store.alist_namespaces(prefix=("alice",), max_depth=2)
```
:::
