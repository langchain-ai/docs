...
#### Version compatibility and channel changes

<Warning>
Changing a persisted channel from `DeltaChannel` to a non-delta channel, or from a non-delta channel to `DeltaChannel`, is not recommended. Checkpoints encode these channel types differently, so changing the type for an existing thread can cause incomplete or incorrect state reconstruction. Keep channel definitions stable for the lifetime of a thread. Before changing a channel type, migrate affected threads to the new representation, or discard them and start new threads.
