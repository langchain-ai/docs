...
<Note>
Requires `deepagents>=0.5.0` (or the corresponding `langgraph>=1.1.3` release) for `runtime.execution_info` and `runtime.server_info`.
</Note>
...
| `nodeFirstAttemptTime` | `number \| undefined` | Unix timestamp (seconds) of when the first attempt started. |
...
const myNode = async (state: typeof State.State, runtime: any) => {
...
<Note>
Requires `deepagents>=0.5.0` (or the corresponding `@langchain/langgraph` release) for `runtime.executionInfo` and `runtime.serverInfo`.
</Note>
