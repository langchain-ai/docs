...
field](/oss/langgraph/graph-api#resetting-a-reducer-field)).
...
If multiple nodes overwrite the same field in the same super-step, an
`InvalidUpdateError` is raised.
