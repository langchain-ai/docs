...
`add_edge` also accepts a list of start nodes. This is not shorthand for separate `add_edge` calls — the two forms behave differently:

```python
builder.add_edge(["b_2", "c"], "d")  # d runs once, after both b_2 and c complete
```

- **A list of start nodes** runs `d` once, after all of the listed nodes have completed. If one of them never runs — for example, a conditional edge does not select its branch — `d` never runs, no error is raised, and the values written by the branches that did complete are not passed on.
...
Note that `defer=True` postpones the node until no tasks are pending anywhere in the graph, not only in the branches that feed it, and a `Send` addressed to the node still invokes it separately.
:::
