...
When a [graceful shutdown](/oss/langgraph/timeout-and-error-handling#graceful-shutdown) has been requested, `runtime.drain_requested` is `True`. Read this inside a node to skip expensive work before the next superstep boundary:
...
<Note>
Requires `langgraph>=1.2`, currently in alpha. See [Graceful shutdown](/oss/langgraph/timeout-and-error-handling#graceful-shutdown) for the full `RunControl` API.
</Note>
