...
Some updates need to replace a merging field's value entirely rather
than be combined into it — for example, clearing an error buffer
between retry attempts (see [Resetting a reducer
field](/oss/javascript/langgraph/graph-api#resetting-a-reducer-field)).
Wrap the value in `Overwrite` to bypass the reducer for that update
and write the value directly:

```typescript theme={"theme":{"light":"catppuccin-latte","dark":"catppuccin-mocha"}}
import { GraphNode, Overwrite, ReducedValue, StateSchema } from "@langchain/langgraph";
...
const clearErrors: GraphNode<typeof State> = (state) => {
  return { errors: new Overwrite([]) };
};

// node A returns { errors: ["bad sql"] }
// node B returns { errors: new Overwrite([]) }
// state.errors is now []; Overwrite bypassed the merging reducer
```

Receiving more than one `Overwrite` value for the same field in a single
super-step throws `InvalidUpdateError: Can receive only one Overwrite
value per step.`
