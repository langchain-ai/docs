...
Agent Chat UI has full support for [human-in-the-loop](/oss/human-in-the-loop) workflows. To try it out, replace the agent code in `src/agent/graph.py` (from the [deployment](/oss/langgraph/local-server) guide) with this [agent implementation](/oss/langgraph/add-human-in-the-loop#add-interrupts-to-any-tool).
