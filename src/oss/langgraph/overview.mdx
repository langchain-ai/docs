...
* **Deterministic and agentic steps in one graph**: Combine hand-coded logic with LLM-driven steps in a single workflow. Hard-code the parts that must be predictable and auditable, and use model-driven steps where flexibility is needed, without sacrificing overall control.
