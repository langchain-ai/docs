...
|------|-----------------|----------|
| [Per-invocation](#per-invocation-default) | `None` (default) | Each call starts fresh. Inherits the parent's checkpointer to support [interrupts](/oss/langgraph/interrupts) and [durable execution](/oss/langgraph/durable-execution) within a single call. |
...
- **Multiple calls (same subgraph)**: The same subgraph instance can be invoked multiple times within a single node. With per-thread persistence, these calls write to the same checkpoint namespace and conflict — use per-invocation persistence instead.
