...
<Warning>
**Requires statically discoverable subgraphs**

`get_state` with `subgraphs` only works when LangGraph can statically discover subgraphs in the graph structure via `get_subgraphs()`. This works when you:

- [Add a compiled graph directly as a node](#add-a-graph-as-a-node) (e.g., `builder.add_node("node_1", subgraph)`)
- [Invoke a subgraph from a node function](#invoke-a-graph-from-a-node) that LangGraph can analyze

It does **not** work when a subgraph is invoked in a way that LangGraph cannot statically analyze — for example, inside a [tool](/oss/langchain/tools) function, a nested helper function, or any other indirection that hides the subgraph from static discovery. In these cases, `get_subgraphs()` returns an empty result and subgraph state is not accessible. This applies to the [subagents](/oss/langchain/multi-agent/subagents) pattern, where agents are wrapped as tools. If you need to inspect subgraph state, invoke the subgraph directly from a [node function](#invoke-a-graph-from-a-node) or [add it as a node](#add-a-graph-as-a-node).
</Warning>
