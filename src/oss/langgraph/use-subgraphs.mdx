...
The fruit expert subagent is wrapped in a unique node name to guarantee namespace isolation:
...
<Tabs>
  ...
  If multiple calls to different stateful subgraphs from a single node occur, the agents accumulate state independently using [namespace isolation](#namespace-isolation):
...
If you use an agent without wrapping it in a unique node name, you must ensure that multiple subgraph calls will not execute in parallel.
