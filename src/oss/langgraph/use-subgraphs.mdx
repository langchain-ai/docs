...
<Tabs>
  ...
  If multiple calls to different stateful subgraphs from a single node occur, the agents accumulate state independently:
...
```typescript
import { createAgent, tool } from "langchain";
import { MemorySaver, Command, interrupt, StateGraph, StateSchema, MessagesValue, START } from "@langchain/langgraph";
import * as z from "zod";
...
<Note>
**Namespace isolation:** When you have multiple **different** stateful subgraphs (for example, a fruit expert and a veggie expert), each one needs its own storage space so their checkpoints don't overwrite each other. This is called namespace isolation.

If you [call subgraphs inside a node](#call-a-subgraph-inside-a-node), LangGraph assigns namespaces based on call order (first call, second call, etc.). This means reordering your calls can mix up which subgraph loads which state. To avoid this, each subgraph should be wrapped in its own `StateGraph` with a unique node name — this gives each subgraph a stable, unique namespace. Subgraphs [added as nodes](#add-a-subgraph-as-a-node) get unique namespaces automatically.
</Note>
