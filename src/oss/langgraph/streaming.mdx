...
<Note>
The `tools` stream mode is currently available in JavaScript only.
</Note>
  ...
  async function* (input) {
    ...
    for (let i = 0; i < airlines.length; i++) {
      ...
      yield {  // [!code highlight]
        message: `Searching ${airlines[i]}...`,  // [!code highlight]
        progress: (i + 1) / airlines.length,  // [!code highlight]
...
for await (const [mode, chunk] of await graph.stream(
  ...
  { streamMode: ["updates", "tools"] }  // [!code highlight]
)) {
  if (mode === "tools") {
    switch (chunk.event) {
      case "on_tool_start":
...
function Chat() {
  const stream = useStream({
    ...
    streamMode: ["values", "tools"],  // [!code highlight]
  });

  // Filter for actively running tools
  const activeTools = stream.toolProgress.filter(  // [!code highlight]
    (t) => t.state === "starting" || t.state === "running"  // [!code highlight]
  );  // [!code highlight]
