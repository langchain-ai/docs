...
<Tip>
**For new applications, use [event streaming](/oss/langgraph/event-streaming).** `graph.stream_events(..., version="v3")` is the recommended in-process streaming API: it returns a run stream with typed projections (`stream.messages`, `stream.values`, `stream.output`, `stream.subgraphs`, `stream.interrupts`, `stream.interrupted`) that you can consume independently—no branching on `stream_mode` chunks required.

The stream-mode API described on this page remains supported for cases where you need direct access to raw stream-mode output.
</Tip>
...
:::python
LangGraph exposes three in-process streaming APIs. Each uses a different output shape and reference surface:

<Tabs default="Event streaming (v3)">
<Tab title="Event streaming (v3)">

Recommended for new applications. Call @[`stream_events`][Pregel.stream_events] (or @[`astream_events`][Pregel.astream_events]) with `version="v3"`. See [Event streaming](/oss/langgraph/event-streaming) for full guidance.

```python
stream = graph.stream_events(inputs, version="v3")
```

**Output shape:** A run stream object with typed projections. Multiple projections can be read concurrently from one run (for example, `stream.messages` and `stream.values` at the same time). Await final state with `stream.output`.

| Projection | Description |
| :--------- | :---------- |
| `stream` | Iterate every protocol event. |
| `stream.messages` | Chat model messages and token deltas (see [LLM tokens](#llm-tokens)). |
| `stream.values` | State snapshots after each step (see [Graph state](#graph-state)). |
| `stream.output` | Final graph output when the run completes. |
| `stream.subgraphs` | Nested graph runs (see [Subgraph outputs](#subgraph-outputs)). |
| `stream.interrupts` | Human-in-the-loop interrupt payloads. |
| `stream.interrupted` | Whether the run paused for human input. |
| `stream.extensions` | Custom stream transformer projections. |

Underlying Pregel events still use modes such as `updates`, `values`, and `messages`; event streaming routes them through transformers so application code does not branch on raw `stream_mode` chunks.

</Tab>
<Tab title="stream() v2">

<Note>
Requires LangGraph >= 1.1. Pass `version="v2"` to @[`stream`][CompiledStateGraph.stream] or @[`astream`][CompiledStateGraph.
astream] with one or more [stream modes](#graph-state) in `stream_mode`. Supported for existing code; new applications should use **Event streaming (v3)**.
</Note>

Pass `version="v2"` with one or more [stream modes](#graph-state) in `stream_mode`. Every chunk is a unified @[`StreamPart`] dict with the same fields regardless of how many modes or subgraphs you enable:

```python
{
    "type": "values" | "updates" | "messages" | "custom" | "checkpoints" | "tasks" | "debug",
    "ns": (),    # namespace tuple; populated for subgraph events
    "data": ..., # payload; type depends on "type"
}
```

Each mode has a corresponding `TypedDict` in `langgraph.types` (@[`ValuesStreamPart`], @[`UpdatesStreamPart`], @[`MessagesStreamPart`], and so on). The union @[`StreamPart`] is a disjoint union on `part["type"]`, so filtering on `chunk["type"]` narrows `chunk["data"]` in type checkers.

The shape is always `{"type", "ns", "data"}`:
...
**Type narrowing across multiple modes:**
...
Pass one or more of the following modes as a list to @[`stream`][CompiledStateGraph.stream] or @[`astream`][CompiledStateGraph.astream]. For a single consistent chunk shape across modes, use the **stream() v2** tab or **Event streaming (v3)** instead.

**Output shape:** Chunk shape depends on your options:

- Single `stream_mode`: yields the raw payload (for example, `stream_mode="updates"` yields a `{node_name: ...}` dict).
- Multiple modes: yields `(mode, data)` tuples.
- `subgraphs=True`: yields `(namespace, data)` tuples, or `(namespace, mode, data)` with multiple modes.
...
| Mode | Chunk shape | Description |
| :--- | :------------ | :---------- |
| [values](#graph-state) | Full state dict | Full state after each step. |
| [updates](#graph-state) | `{node_name: partial_state}` dict | State updates after each step. Multiple updates in the same step are streamed separately. |
| [messages](#llm-tokens) | `(message_chunk, metadata)` tuple | LLM tokens and metadata from LLM calls. |
| [custom](#custom-data) | Arbitrary JSON-serializable value | Custom data from @[`get_stream_writer`] in nodes. |
| [checkpoints](#checkpoints) | Checkpoint dict | Checkpoint events (same format as `get_state()`). Requires a checkpointer. |
| [tasks](#tasks) | Task event dict | Task start and finish events with results and errors. Requires a checkpointer. |
| [debug](#debug) | Debug event dict | All available info — combines `checkpoints` and `tasks` with extra metadata. |
...
<Tabs>
    ...
    for chunk in graph.stream(
        ...
        print(f"topic: {chunk['topic']}, joke: {chunk['joke']}")
    ```
...
# The "messages" stream mode yields (message_chunk, metadata) tuples
...
<Tabs>
    ...
    for chunk in graph.stream(inputs, stream_mode="custom"):
        print(f"Custom event: {chunk['custom_key']}")
    ```
    </Tab>
    <Tab title="tool">
    ```python
...
:::python
To include outputs from [subgraphs](/oss/langgraph/use-subgraphs) in the streamed outputs, set `subgraphs=True` in the `.stream()` method of the parent graph. This streams outputs from both the parent graph and any subgraphs as `(namespace, data)` tuples, where `namespace` is the path to the node where a subgraph is invoked, e.g. `("parent_node:<task_id>", "child_node:<task_id>")`.
...
<Tip>
For new applications, prefer [event streaming](/oss/langgraph/event-streaming): iterate `stream.subgraphs` to observe nested graphs without parsing namespace strings.
</Tip>
:::
...
:::python
You can pass a list as the `stream_mode` parameter to stream multiple modes at once. Each iteration yields a `(mode, chunk)` tuple — branch on `mode` to handle each stream mode:
...
for chunk in graph.stream(
    ...
    # The chunk contains the custom data streamed from the llm
    print(chunk)
```
:::
