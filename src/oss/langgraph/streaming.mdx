...
The shape of each chunk yielded by `graph.stream(...)` depends on your options:

- Single stream mode: yields the raw payload (e.g. for `stream_mode="updates"`, a `{node_name: ...}` dict).
- Multiple stream modes: yields `(mode, data)` tuples.
- `subgraphs=True`: yields `(namespace, data)` tuples (or `(namespace, mode, data)` with multiple modes).

<Tip>
For applications that want a single, consistent shape across modes, prefer [event streaming](/oss/langgraph/event-streaming): `graph.stream_events(..., version="v3")` returns a run stream object with typed projections (`stream.messages`, `stream.values`, etc.) and a stable raw event protocol.
</Tip>

LangGraph also supports an opt-in unified `StreamPart` format on `stream()` by passing `version="v2"`. It is supported but is no longer the recommended path for new code; new applications should use event streaming instead.
:::
