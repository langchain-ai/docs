...
<Info>
For new applications, we recommend [event streaming](/oss/langgraph/streaming/event-streaming)—the typed-projection API introduced in LangGraph v1.2. Event streaming gives you separate iterators per projection (messages, values, subgraphs, output) so you can consume them independently instead of branching on `stream_mode` chunks. The API on this page remains fully supported and is not planned for removal.
</Info>
