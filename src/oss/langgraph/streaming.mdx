...
<Note>
Requires LangGraph >= 1.1. See the [release notes](/oss/releases/langgraph-v1.1) for full details.
</Note>
...
The v2 format also enables type narrowing — you can filter chunks by `chunk["type"]` and get the correct payload type:

```python
for chunk in graph.stream(
    inputs, stream_mode=["values", "updates"], version="v2"
):
...
With v2, subgraph events use the same `StreamPart` format. The `ns` field identifies the source graph:
...
Pass `version="v2"` to `invoke()` or `ainvoke()` to get a @[`GraphOutput`] object with `.value` and `.interrupts` attributes:
...
# Backwards-compatible dict access still works
print(result["key"])
```

This cleanly separates state from interrupt metadata. With v1, interrupts are embedded in the returned dict under `__interrupt__`:
...
The table below summarizes the format changes when migrating from v1 to v2:
