...
Use Pregel Streaming when you need direct access to graph-runtime events or advanced stream mode output. For most application and frontend streaming use cases, start with [Event Streaming](/oss/langgraph/streaming/event-streaming), which builds typed projections such as `run.messages`, `run.values`, `run.subgraphs`, and `run.output` on top of the same execution events.
