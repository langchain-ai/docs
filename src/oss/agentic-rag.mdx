...
Now we will start building components ([nodes](.././low_level#nodes) and [edges](.././low_level#edges)) for our agentic RAG graph.

:::python
Note that the components will operate on the [`MessagesState`](.././low_level#messagesstate) — graph state that contains a `messages` key with a list of [chat messages](https://python.langchain.com/docs/concepts/messages/).
:::
