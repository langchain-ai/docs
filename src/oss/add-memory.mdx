...
---
AI applications need [memory](.././memory) to share context across multiple interactions. In LangGraph, you can add two types of memory:

* [Add short-term memory](#add-short-term-memory) as a part of your agent's [state](.././low_level#state) to enable multi-turn conversations.
* [Add long-term memory](#add-long-term-memory) to store user-specific or application-level data across sessions.

## Add short-term memory

**Short-term** memory (thread-level [persistence](.././persistence)) enables agents to track multi-turn conversations. To add short-term memory:
...
If your graph contains [subgraphs](.././subgraphs), you only need to provide the checkpointer when compiling the parent graph. LangGraph will automatically propagate the checkpointer to the child subgraphs.
...
If you want the subgraph to have its own memory, you can compile it with the appropriate checkpointer option. This is useful in [multi-agent](.././multi_agent) systems, if you want agents to keep track of their internal message histories.
...
:::python
To delete messages from the graph state, you can use the `RemoveMessage`. For `RemoveMessage` to work, you need to use a state key with @[`add_messages`][add_messages] [reducer](.././low_level#reducers), like [`MessagesState`](.././low_level#messagesstate).
...
:::js
To delete messages from the graph state, you can use the `RemoveMessage`. For `RemoveMessage` to work, you need to use a state key with @[`messagesStateReducer`][messagesStateReducer] [reducer](.././low_level#reducers), like `MessagesZodState`.
...
<Tabs>
  ...
  <Tab title="In a workflow">
    :::python
    Prompting and orchestration logic can be used to summarize the message history. For example, in LangGraph you can extend the [`MessagesState`](.././low_level#working-with-messages-in-graph-state) to include a `summary` key:
    ...
    :::js
    Prompting and orchestration logic can be used to summarize the message history. For example, in LangGraph you can extend the [`MessagesZodState`](.././low_level#working-with-messages-in-graph-state) to include a `summary` key:
