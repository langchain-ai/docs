...
LangGraph provides two high-level APIs for creating a Pregel application: the [StateGraph (Graph API)](/oss/low-level) and the [Functional API](/oss/functional-api).
...
<Tabs>
  ...
  <Tab title="Functional API">
    :::python
    In the [Functional API](functional_api), you can use an @[`entrypoint`][entrypoint] to create a Pregel application. The `entrypoint` decorator allows you to define a function that takes input and returns output.
