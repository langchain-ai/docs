...
If you want the subgraph to **have its own memory**, you can compile it `with checkpointer=True`. This is useful in [multi-agent](/oss/multi-agent) systems, if you want agents to keep track of their internal message histories:
...
When you enable [persistence](/oss/persistence), you can [inspect the graph state](/oss/persistence#checkpoints) (checkpoint) via `graph.get_state(config)`. To view the subgraph state, you can use `graph.get_state(config, subgraphs=True)`.
