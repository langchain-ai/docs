...
When you enable [persistence](../concepts/persistence), you can [inspect the graph state](../concepts/persistence#checkpoints) (checkpoint) via `graph.get_state(config)`. To view the subgraph state, you can use `graph.get_state(config, subgraphs=True)`.
