...
`ChatAnthropic` sends leading system messages in the top-level `system` field, and every later @[`SystemMessage`] at its own position in the conversation. A later `SystemMessage` must follow a human or tool message, and must either be the last message or be followed by an AI message.
