...
`ChatAnthropic` sends leading system messages in the top-level `system` field, and every later @[`SystemMessage`] at its own position in the conversation. Place a later `SystemMessage` after a human or tool message, either last or followed by an AI message.

Claude Opus 4.8, Opus 5, Opus 5.5, Fable 5, Fable 5.1, Mythos 5, and Mythos 5.1 support mid-conversation system messages. Claude Sonnet 5 does not. `ChatAnthropic` does not check the model or the message's position, so the API returns a 400 error when either is invalid.
...
{/* TODO: Set the version once langchain-ai/langchainjs#11707 is released. */}
<Note>
