...
<Accordion title="Example: OpenRouter">
    For OpenRouter, use the dedicated [`ChatOpenRouter`](/oss/integrations/chat/openrouter) integration (not `ChatOpenAI` with a custom base URL). Sign up and create an [API key](https://openrouter.ai/docs/api/reference/authentication), then install and instantiate:
    ...
    See the [ChatOpenRouter integration guide](/oss/integrations/chat/openrouter) for tool calling, structured output, provider routing, and multi-model fallback.
</Accordion>
