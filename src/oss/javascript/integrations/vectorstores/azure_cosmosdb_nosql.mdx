...
const results = await store.vectorSearchWithThreshold(
...
The `AzureCosmosDBNoSQLVectorStoreRetriever` provides a flexible way to search with different search types. Use `asCosmosRetriever()` to create a retriever with Cosmos DB-specific search capabilities:
...
const docs = await retriever.invoke("What is artificial intelligence?");

// Available search types:
// - "similarity" or "vector": Standard vector similarity search
// - "vector_score_threshold": Vector search with minimum score filter
// - "mmr": Maximal Marginal Relevance search
// - "full_text_search": Full-text search (preview)
// - "full_text_ranking": Full-text search with BM25 ranking (preview)
// - "hybrid": Hybrid vector + full-text search (preview)
// - "hybrid_score_threshold": Hybrid search with score threshold (preview)
```

<Note>
Use `asCosmosRetriever()` when you need Cosmos DB-specific search types like hybrid search, full-text search, or score thresholds. The standard `asRetriever()` method inherited from `VectorStore` is also available for basic vector similarity search.
</Note>

## Full-text and hybrid search (preview)

<Warning>
Full-text and hybrid search are preview features in Azure Cosmos DB. You need to configure your container with a full-text policy to use these features. See the [Azure Cosmos DB documentation](https://learn.microsoft.com/azure/cosmos-db/nosql/full-text-search) for setup instructions.
</Warning>
...
const results = await store.fullTextSearch("artificial intelligence", 10);
```
...
const results = await store.hybridSearchWithScore("machine learning", 10);

// With score threshold
const filteredResults = await store.hybridSearchWithThreshold(
  "machine learning",
  10,
...
await store.deleteDocumentById("document-id-123");
```
