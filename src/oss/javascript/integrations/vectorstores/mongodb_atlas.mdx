...
**Note:** The cluster must be MongoDB 7.0 or higher for manual embedding mode. [Automated embedding mode](#automated-embedding) requires [MongoDB 8.2 or higher](https://www.mongodb.com/docs/atlas/atlas-vector-search/crud-embeddings/create-embeddings-automatic/?interface=driver&language=nodejs).
...
**Manual embedding** (MongoDB 7.0+): you embed documents client-side and store the vectors in a field. Use the following definition, adjusting `numDimensions` to match your embeddings model (for example, 1024 for Cohere, 1536 for OpenAI):
...
**Automated embedding** (MongoDB 8.2+): MongoDB generates embeddings server-side using [Voyate AI models](https://www.mongodb.com/products/platform/ai-search-and-retrieval/models). Use the `autoEmbed` field type and specify the model:
...
By default, the vector store expects an index named `default`, a text field named `text`, and an embedding field named `embedding`. Initialize the vector store with field names that match your index definition.
