...
<Note>
    Prompt caching reduces API costs by caching tokens, but does **not** provide conversation memory. To persist conversation history across invocations, use a [checkpointer](/oss/langgraph/persistence#checkpointer-libraries) like `MemorySaver`.
</Note>
