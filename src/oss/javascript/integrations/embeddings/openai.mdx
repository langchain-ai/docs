...
<Note>
    **`baseURL` from environment**
    ...
    `@langchain/openai` does not read a base URL env var itself. If neither `configuration.baseURL` nor a top-level base URL is provided, the underlying `openai` Node SDK falls back to the `OPENAI_BASE_URL` env var. The `OPENAI_API_BASE` env var (used by the Python package) is **not** consulted.
</Note>
