...
import McpWrapToolCallPy from '/snippets/code-samples/mcp-wrap-tool-call-py.mdx';

<Prompt
    description="Migrate a codebase from langchain-mcp-adapters to langchain.mcp."
    icon="arrow-right"
    actions={["copy"]}
>
Migrate this codebase from `langchain-mcp-adapters` to the built-in `langchain.mcp` namespace (requires `langchain[mcp]>=1.4.0`).

Key changes:

1. Uninstall `langchain-mcp-adapters` and install `langchain[mcp]`.
2. Replace `MultiServerMCPClient` with `MCPAdapter`. `MCPAdapter` is an async context manager — use it with `async with MCPAdapter(config) as adapter:` and call `adapter.list_tools()` instead of `client.get_tools()`.
3. Update the config shape from `{"server": {"transport": "...", ...}}` to the standard `MCPConfig` shape: `{"mcpServers": {"server": {"command": ..., "args": ...}}}` for stdio or `{"mcpServers": {"server": {"url": ...}}}` for HTTP. The transport is now inferred automatically.
4. Update imports: `from langchain_mcp_adapters.client import MultiServerMCPClient` → `from langchain.mcp import MCPAdapter`; `convert_mcp_tool_to_langchain_tool` is now `as_langchain_tool` (also a coroutine: `await as_langchain_tool(tool, client)`).
5. Remove `handle_tool_errors` — error handling is now fixed: `isError=True` becomes a `ToolMessage(status="error")`.
6. Replace `tool_interceptors` / `ToolCallInterceptor` with LangChain `@wrap_tool_call` middleware.
7. Elicitation is now automatic via LangGraph `interrupt` — no `Callbacks(on_elicitation=...)` needed.
8. For `on_progress` or `on_logging_message` callbacks, build a `fastmcp.Client` with `progress_handler=` or `log_handler=` and pass it to `MCPAdapter`.
9. `get_prompt` and `get_resources` have no `langchain.mcp` equivalent yet — access them directly via the FastMCP client (`client.get_prompt(...)`, `client.read_resource(...)`).

Search the codebase for all usages of `langchain_mcp_adapters`, `MultiServerMCPClient`, `StdioConnection`, `StreamableHttpConnection`, `SSEConnection`, `WebsocketConnection`, `ToolCallInterceptor`, `handle_tool_errors`, and `Callbacks`, and apply the necessary changes. Flag anything that cannot be migrated automatically.
</Prompt>
...
For the full feature documentation, see [Model Context Protocol (MCP)](/oss/langchain/mcp).
...
| `handle_tool_errors` | **Removed as a flag.** Behavior is now fixed: `isError=True` becomes a `ToolMessage(status="error")`; transport failures raise. See [Tools](/oss/langchain/mcp/tools#errors). |
...
| `SSEConnection` | `Client(SSETransport(url))`, passed to @[`MCPAdapter`]. Supported, but the transport is deprecated. See [Deprecated transports](#deprecated-transports). |
...
Elicitation moved from a callback registered on the client to a LangGraph @[`interrupt`], and it is now on by default. @[`MCPAdapter`] arms every client it builds to advertise the capability and drives the interrupt loop; answer the server's request when the run pauses, resuming with `Command(resume={"responses": {key: answer}})`.
...
See [Elicitation](/oss/langchain/mcp/tools#elicitation).
...
|---|---|
| `on_elicitation` | Handled automatically as an interrupt; no handler needed. A prebuilt client's own `elicitation_handler` is honored instead. See [Elicitation](/oss/langchain/mcp/tools#elicitation). |
...
The `handle_tool_errors` flag is gone. Behavior is now fixed: an MCP tool that reports `isError=True` reaches the model as a @[`ToolMessage`] with `status="error"` carrying the server's message, while transport failures raise. See [Tools](/oss/langchain/mcp/tools#errors).
...
| `langchain-mcp-adapters` | `langchain.mcp` |
|---|---|
| `MCPToolArtifact` (structured content) | **Kept.** Exported from `langchain.mcp`. See [structured content](/oss/langchain/mcp/tools#structured-content). |
| Multimodal content blocks | **Kept.** See [multimodal content](/oss/langchain/mcp/tools#multimodal-content). |
| Tool metadata | **Extended.** Grouped under an `mcp` namespace on the tool's metadata, with annotations and server identity. See [tool metadata](/oss/langchain/mcp/tools#tool-metadata). |
| `convert_mcp_tool_to_langchain_tool` | Renamed to @[`as_langchain_tool`], and now a coroutine: `await as_langchain_tool(tool, client)`. |
