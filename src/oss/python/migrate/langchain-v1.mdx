...
<Prompt
    description="Migrate a codebase to LangChain v1."
    icon="arrow-right"
    actions={["copy"]}
>
Migrate this codebase to LangChain v1 (requires `langchain>=1.0.0`, `langchain-core>=0.3.0`).

Key changes:

1. **Package namespace reduction** — the `langchain` namespace now contains only `langchain.agents`, `langchain.messages`, `langchain.tools`, `langchain.chat_models`, and `langchain.embeddings`. Everything else has moved to `langchain-classic`. If you import from `langchain.chains`, `langchain.retrievers`, `langchain.indexes`, `langchain.hub`, or `langchain.embeddings.*` (community embeddings), install `langchain-classic` and update those imports to `langchain_classic.*`.
2. **`create_react_agent` → `create_agent`** — replace all `from langgraph.prebuilt import create_react_agent` with `from langchain.agents import create_agent`. `create_agent(model, tools, system_prompt=...)` replaces `create_react_agent(model, tools, prompt=...)`. The graph is no longer exposed directly — use `agent.invoke({"messages": [...]})` / `agent.stream(...)`. Remove any explicit `AgentState`, `AgentStatePydantic`, `AgentStateWithStructuredResponse` imports; use `langchain.agents.AgentState` where needed.
3. **Middleware replaces graph-level customization** — replace `state_modifier=`, `pre_model_hook=`, `post_model_hook=`, `response_format=`, `ValidationNode`, and custom `StateGraph` wrappers with `create_agent` middleware: `@wrap_model_call`, `@wrap_tool_call`, `human_in_the_loop(...)`, and `StructuredOutputMiddleware`. See the "Middleware" section of this guide for mapping.
4. **Human-in-the-loop** — replace `interrupt_before=["tools"]` / `HumanInterruptConfig` / `HumanInterrupt` / `ActionRequest` with `langchain.agents.middleware.human_in_the_loop.human_in_the_loop(InterruptOnConfig(...))`. The interrupt payload type is now `HITLRequest`.
5. **`init_chat_model`** — if you use any removed model-specific imports (e.g. `from langchain_openai import ChatOpenAI`), they still work from provider packages; no change required. But prefer `init_chat_model("openai:gpt-4o")` for new code.
6. **Streaming** — `agent.stream(input, stream_mode="messages")` now yields `(message_chunk, metadata)` tuples; update any unpacking that expected plain chunks.
7. **Remove `langchain-community` re-exports** — if you relied on `langchain` re-exporting community integrations, install the provider package directly (e.g. `langchain-openai`) and import from it.

Search the codebase for all imports from `langchain`, `langgraph.prebuilt`, and `langchain_community`, and apply the necessary changes. Flag anything that cannot be migrated automatically.
</Prompt>
