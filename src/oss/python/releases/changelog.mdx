...
<Update label="Sep 1, 2026" tags={["langchain"]} rss={{ title: "Sep 1, 2026 - langchain" }}>
    ...
    - **Interrupt-driven elicitation**: `MCPAdapter(target, elicitation="interrupt")` surfaces a server's mid-call question as a LangGraph `interrupt()`, so a human answers and the run resumes. See [Elicitation](/oss/langchain/mcp/tools#elicitation).
    ...
    - **Richer tool metadata**: each tool carries its MCP provenance under an `mcp` namespace on its metadata, including annotations such as `destructive_hint` for [gating destructive tools behind approval](/oss/langchain/mcp/tools#gate-destructive-tools). See [Tools](/oss/langchain/mcp/tools#tool-metadata).
