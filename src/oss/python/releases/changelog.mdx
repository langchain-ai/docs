...
<Update label="May 12, 2026" tags={["langgraph"]} rss={{ title: "May 12, 2026 - langgraph" }}>
    ...
    - **New event streaming API (beta)**: A new content-block-centric streaming protocol that replaces dict-shaped events with typed, per-channel streams. Pass `version="v3"` to `stream_events()` / `astream_events()` to get a `GraphRunStream` (sync) or `AsyncGraphRunStream` (async) handle with typed projections—`run.values`, `run.messages`, `run.lifecycle`, `run.subgraphs`—plus opt-in transformers (`UpdatesTransformer`, `CustomTransformer`, `CheckpointsTransformer`, `TasksTransformer`, `DebugTransformer`) for additional channels. `run.messages` yields one `ChatModelStream` per LLM call, with typed sub-projections for text tokens, reasoning, tool calls, and usage—the recommended path for token-level streaming to a UI in 1.2. `version="v1"` and `version="v2"` are unchanged.
