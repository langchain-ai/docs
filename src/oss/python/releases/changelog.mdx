...
<Update label="May 12, 2026" tags={["deepagents"]} rss={{ title: "May 12, 2026 - langchain" }}>
    ...
    <Warning>
    Deep Agents v0.6.0 changes persisted message history and agent files to `DeltaChannel`. Downgrading to an earlier Deep Agents version changes these channels back to non-delta channels and can cause incomplete or incorrect state reconstruction for existing threads. Before downgrading an application with persisted v0.6.0 threads, migrate or discard the affected threads. More generally, avoid switching a persisted channel between delta and non-delta representations. See [Version compatibility and channel changes](/oss/langgraph/pregel#version-compatibility-and-channel-changes).
    </Warning>
