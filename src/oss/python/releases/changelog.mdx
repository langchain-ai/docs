...
<Update label="Mar 10, 2026" tags={["langgraph"]} rss={{ title: "Mar 10, 2026 - langgraph" }}>
    ...
    * **v2 streaming format**: Pass `version="v2"` to `stream()` / `astream()` for a unified `StreamPart` output format. Every chunk is a dict with `type`, `ns`, and `data` keys—the same shape regardless of stream mode, number of modes, or subgraph settings. No more juggling tuples vs. dicts vs. triples. See [streaming docs](/oss/langgraph/streaming#v2-streaming-format).
    * **v2 invoke format**: Pass `version="v2"` to `invoke()` / `ainvoke()` to get a `GraphOutput` object with `.value` and `.interrupts` attributes, cleanly separating state from interrupt metadata. See [v2 invoke docs](/oss/langgraph/streaming#v2-invoke-format).
    * **Pydantic and dataclass coercion**: With `version="v2"`, `values` stream mode and `invoke()` automatically coerce output to Pydantic model or dataclass instances when your state is defined as one.
    * Fully backwards compatible — `version="v2"` is opt-in, existing code continues to work unchanged.
</Update>
