...
<Update label="Mar 10, 2026" tags={["langgraph"]} rss={{ title: "Mar 10, 2026 - langgraph" }}>
    ...
    - **Type-safe streaming (`version="v2"`)**: Pass `version="v2"` to `stream()` / `astream()` for a unified `StreamPart` output — a typed dict with `type`, `ns`, and `data` keys on every chunk, regardless of stream mode, subgraph depth, or how many modes you combine. Each mode has its own `TypedDict` (`ValuesStreamPart`, `UpdatesStreamPart`, etc.), all importable from `langgraph.types` and forming a discriminated union for full editor and type-checker narrowing. See [streaming docs](/oss/langgraph/streaming#v2-streaming-format).

    - **Type-safe invoke (`version="v2"`)**: Pass `version="v2"` to `invoke()` / `ainvoke()` to get a `GraphOutput` object with `.value` (your state) and `.interrupts` (a `tuple[Interrupt, ...]`) — no more fishing `__interrupt__` out of a plain dict. See [invoke docs](/oss/langgraph/streaming#v2-invoke-format).

    - **Pydantic and dataclass coercion**: When your state schema is a Pydantic model or dataclass, `version="v2"` automatically coerces `invoke()` and `values`-mode stream output to the declared type instead of returning a plain dict.

    - **Fixed time travel with interrupts and subgraphs**: Resolved two replay/fork bugs — replays no longer silently reuse stale `RESUME` values from prior `interrupt()` calls, and subgraphs now correctly restore the checkpoint corresponding to the parent's historical state rather than always loading the latest one.

    - **Fully backwards compatible**: `version="v2"` is opt-in; existing code is unchanged. `GraphOutput` also supports deprecated dict-style access (`result["key"]`, `result["__interrupt__"]`) for gradual migration — this will be removed in v3.0.
</Update>
