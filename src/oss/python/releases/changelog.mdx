...
<Update label="May 12, 2026" tags={["deepagents"]} rss={{ title: "May 12, 2026 - langchain" }}>
    ...
    - **[Delta channels](/oss/deepagents/event-streaming)**: The `version="v3"` streaming API exposes per-subagent delta projections (`output_deltas`) on tool calls, so you can stream incremental tool output without buffering the full result.
    ...
    - **[`ContextHubBackend`](/oss/deepagents/backends#contexthubbackend)**: A new filesystem backend that stores agent files durably in a LangSmith Hub repo, without provisioning a separate LangGraph store.
</Update>
