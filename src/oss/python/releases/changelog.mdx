...
<Update label="Mar 10, 2026" tags={["langgraph"]} rss={{ title: "Mar 10, 2026 - langgraph" }}>
    ...
    * [Release notes](/oss/releases/langgraph-v1.1)
    * **v2 streaming format**: Pass `version="v2"` to `stream()` / `astream()` for a unified `StreamPart` output format. Every chunk is a dict with `type`, `ns`, and `data` keys — the same shape regardless of stream mode, number of modes, or subgraph settings. No more juggling tuples vs. dicts vs. triples.
    * **v2 invoke format**: Pass `version="v2"` to `invoke()` / `ainvoke()` to get a `GraphOutput` object with `.value` and `.interrupts` attributes, cleanly separating state from interrupt metadata.
