---
title: NeMo Switchyard middleware integration
description: Integrate with the SwitchyardRoutingMiddleware type using LangChain Python.
integration:
  name: NeMo Switchyard middleware
  featured: true
  available: Model routing across LangChain chat models
  source: "[`langchain-ai/langchain-nvidia`](https://github.com/langchain-ai/langchain-nvidia/tree/main/libs/switchyard)"
---

The experimental `SwitchyardRoutingMiddleware` routes each deep agent model call through a configured [NeMo Switchyard](https://github.com/NVIDIA-NeMo/Switchyard) `libsy` algorithm. Use it to combine LangChain chat models with different cost and performance profiles while Deep Agents continues to manage the agent loop, tools, state, and middleware composition. Learn more about [middleware](/oss/langchain/middleware/overview).
...
|------------|-------------|
| [`SwitchyardRoutingMiddleware`](https://github.com/langchain-ai/langchain-nvidia/tree/main/libs/switchyard#switchyardroutingmiddlewarealgorithm) | Replace each model call with the target selected by a configured Switchyard algorithm |
