...
> **Preview** — AgentCore Payments is currently in preview. Requires `bedrock-agentcore>=1.18.0`.
...
When these conditions are satisfied, the lenient fallback detection handles 402 responses automatically. For MCP tools with non-standard response formats (e.g., those using `structuredContent` or custom transport headers), register a [custom handler](#custom-handlers) to parse and inject payment credentials correctly.
