...
With this setup, the agent can access any paid API transparently. When a tool receives a 402 response, the middleware handles payment signing, budget enforcement, and retry, so the agent sees only the successful response content.
