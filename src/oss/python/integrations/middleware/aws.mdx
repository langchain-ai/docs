...
- MCP tools that return raw JSON with 402 status codes
...
```bash
pip install -U bedrock-agentcore langchain langgraph
```
...
With this setup, the agent can access any paid API transparently. When a tool receives a 402 response, the middleware handles payment signing, budget enforcement, and retry — the agent sees only the successful response content.
...
The error handler callback is recommended because it keeps payment lifecycle complexity out of the agent's reasoning. Without it, the agent receives error messages about expired sessions or missing instruments and must attempt to debug payment configuration — wasting tokens and often failing. With the callback, your application code resolves issues programmatically (creating sessions, refreshing instruments, increasing budgets) and the agent only ever sees successful payment results.
...
| `PaymentInstrumentNotFound` | Likely config error — PROPAGATE |
| `PaymentSessionConfigurationRequired` | Create session, or enable `auto_session=True` |
| Generic `PaymentError` | Log and PROPAGATE — usually transient |
...
- **Human-in-the-loop approval** — surface 402 details to the user and let them authorize each payment before it's signed
...
Custom handlers receive the **raw `ToolMessage.content`** — parse it yourself. Don't pass built-in handlers (like `GenericPaymentHandler`) as custom handlers directly; they expect a different normalized shape.

### MCP server tools

MCP tools connected via `langchain-mcp-adapters` work with the middleware out of the box:

```python
from langchain_mcp_adapters.client import MultiServerMCPClient

client = MultiServerMCPClient({
    "paid_api": {"transport": "stdio", "command": "python", "args": ["server.py"]}
})
...
Since MCP adapters serialize responses as raw JSON strings, the fallback detection handles the common case automatically. For non-standard formats, register a custom handler.
...
- No on-chain timing delay — fast facilitators may reject
- No budget error messages to the agent
