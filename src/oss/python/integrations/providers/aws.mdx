...
> gives agents current information from the web with a source URL on every result, authenticated with your AWS credentials rather than a search API key.
...
See a [usage example](/oss/integrations/tools/bedrock_agentcore_web_search).
