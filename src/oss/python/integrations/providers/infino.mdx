...
>SQL, full-text (BM25), vector, and hybrid (RRF) search over one copy of your
>data in Apache Parquet on object storage — no separate search cluster or
>vector store to keep in sync.

The `langchain-infino` package surfaces that whole retrieval surface, not just
`similarity_search`. Infino never embeds: you bring a LangChain `Embeddings`
object and the integration supplies the vectors.
