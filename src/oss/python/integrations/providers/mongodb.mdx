...
- [Short-term memory with MongoDB Atlas](/oss/integrations/memory/mongodb-short-term-memory) (`MongoDBSaver` checkpointer)
- [Long-term memory with MongoDB Atlas](/oss/integrations/memory/mongodb-long-term-memory) (`MongoDBStore`)
...
- **Vector store**: `MongoDBAtlasVectorSearch` stores embeddings in MongoDB and supports semantic search, filtered search, and hybrid search (vector + full-text via BM25). See the [MongoDB Atlas vector store](/oss/integrations/vectorstores/mongodb_atlas) guide.
- **Retrievers**: `MongoDBAtlasFullTextSearchRetriever` and `MongoDBAtlasHybridSearchRetriever` support full-text and combined vector + keyword retrieval. See the [MongoDB Atlas retriever](/oss/integrations/retrievers/mongodb_atlas) guide.
...
See the [MongoDB Atlas integrations](/oss/integrations/providers/mongodb_atlas) page for imports and usage examples.
