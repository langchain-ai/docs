...
The experimental [`langchain-nvidia-switchyard`](https://github.com/langchain-ai/langchain-nvidia/tree/main/libs/switchyard) package lets a deep agent route each model call across existing LangChain chat models with [Switchyard](https://github.com/NVIDIA-NeMo/Switchyard) algorithms. Use it to combine efficient and capable models behind one agent while Deep Agents continues to manage the agent loop, tools, state, and middleware.

<Warning>
  This integration is experimental. Its APIs and behavior are subject to breaking changes without notice.
</Warning>

The integration provides:

- **Model-agnostic targets**: Adapt any LangChain `BaseChatModel` for use as a Switchyard target.
- **Pluggable routing**: Supply any algorithm exposed by the installed `nemo-switchyard` Python bindings, including random routing, LLM task classification, and signal-driven Stage routing.
