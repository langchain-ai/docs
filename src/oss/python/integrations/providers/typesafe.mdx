...
<Note>
Earlier versions configured `questions` on `TypeSafeClassifier` and passed only the state to `invoke`. Pass both `state` and `questions` to `invoke` now, matching the TypeSafeAI SDK request format.
</Note>
