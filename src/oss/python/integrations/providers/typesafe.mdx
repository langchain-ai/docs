...
`TypeSafeClassifier` exposes these decisions as a LangChain `Runnable`, so you can invoke, batch, or compose them with other runnables. It accepts strings, structured JSON, and LangChain messages, and records traces and token usage in LangSmith. Use it for focused decisions such as routing a request, choosing a model, or checking whether a tool call is safe to run.

Use TypeSafe decisions in LangChain middleware to control an agent at specific points in its lifecycle. The included experimental middleware can route a run to the appropriate model or block a risky tool call. See [Agent middleware](#agent-middleware) for complete examples.
...
By default, the classifier sends requests to `https://api.typesafe.ai`. Set `TYPESAFE_BASE_URL` to use a compatible gateway, test server, or private deployment:
...
The package includes two experimental [middleware](/oss/langchain/middleware/overview) that put the classifier on an agent's decision points. Import them from `langchain_typesafe.experimental.middleware`.
...
<Warning>
Do not put secrets in tool arguments or conversation state unless sending them to TypeSafe is acceptable. This middleware refuses risky calls. It does not request approval. Pair it with [human-in-the-loop middleware](/oss/langchain/middleware/built-in) when you want a person to approve them.
</Warning>

### Custom middleware

`TypeSafeClassifier` accepts [LangChain message objects](/oss/langchain/messages) directly, so a custom middleware hook can classify an agent's conversation state without converting it first. This example classifies the conversation once at the start of each run and stores the complete `ChoiceAnswer` in agent state:
...
Use any lifecycle hook that matches your decision point. For example, use `before_model` to reclassify after each tool result, or `wrap_tool_call` to classify a proposed action. See [Custom middleware](/oss/langchain/middleware/custom) for all hooks and state patterns.
