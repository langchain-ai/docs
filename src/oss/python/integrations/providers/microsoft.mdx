...
<Note>
    For hosted graphs using models based on `BaseChatOpenAI`, use `output_version="responses/v1"` with either the Responses or Invocations hosting protocol. For model calls through the Responses API, this is the default in `langchain-openai` 1.0.0 or later. You can omit `output_version` if `LC_OUTPUT_VERSION` does not override it. This model output setting is independent of the hosting protocol.
</Note>
