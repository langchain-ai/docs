...
For more information, see [Backend integrations](/oss/integrations/backends). See the [package repository](https://github.com/langchain-ai/langchain-azure/tree/main/libs/azure-storage) for full usage details and security guidance.
...
<Note>
    When using OpenAI models through the Responses API with `ResponsesHostServer`, set `use_responses_api=True` and `output_version="responses/v1"`. Its default output converter supports this native Responses format. The legacy format `output_version="v0"` and the LangChain standard format `output_version="v1"` are not supported. The model API and output format are separate from the hosting protocol selected above.
</Note>
