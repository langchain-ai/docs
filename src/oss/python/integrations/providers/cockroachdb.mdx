...
async with AsyncCockroachDBSaver.from_conn_string(DB_URI) as checkpointer:
    ...
    # Compile a LangGraph workflow with CockroachDB persistence
    graph = workflow.compile(checkpointer=checkpointer)

    config = {"configurable": {"thread_id": "user-123"}}
    result = await graph.ainvoke({"input": "Hello"}, config)

    # State is persisted -- resume from the same thread later
    result = await graph.ainvoke({"input": "Follow up"}, config)
```
