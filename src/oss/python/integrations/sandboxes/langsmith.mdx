...
Pass the backend to @[create_deep_agent] along with a chat model such as @[ChatAnthropic], then invoke the agent. The `try`/`finally` block deletes the sandbox when the try blog finishes or an error occurs.
