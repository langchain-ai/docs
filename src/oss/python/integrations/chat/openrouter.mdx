...
By default, models may request multiple tool calls in a single response. To force the model to request only one tool call at a time, set `parallel_tool_calls=False`:

```python
model_with_tools = model.bind_tools([GetWeather], parallel_tool_calls=False)
```
