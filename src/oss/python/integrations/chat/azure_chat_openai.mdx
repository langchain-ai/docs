...
<Tip>
    **API Reference**
    ...
    For detailed documentation of all features and configuration options, head to the [AzureChatOpenAI API reference](https://reference.langchain.com/python/langchain-openai/chat_models/azure/AzureChatOpenAI). Visit the [ChatOpenAI docs](/oss/integrations/chat/openai/) for details on available features.
</Tip>
...
With Microsoft Entra ID — pass the token provider to `api_key`:
...
With Microsoft Entra ID — pass the token provider to `azure_ad_token_provider`:
...
With an API key (reads `AZURE_OPENAI_API_KEY` and `AZURE_OPENAI_ENDPOINT` from environment):
...
llm = ChatOpenAI(
    model="gpt-5-mini",  # your Azure reasoning model deployment
    base_url="https://YOUR-RESOURCE-NAME.openai.azure.com/openai/v1/",
