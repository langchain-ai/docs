...
When using `with_structured_output(method="function_calling")`, avoid passing additional tools (like `Google Search`- tools=[{"google_search": {}}]) directly in the `invoke()` call. 
...
Here is the complete Markdown snippet ready for you to copy and paste into the LangChain GitHub editor.
