...
`effort` is an alias for the standard [`reasoning_effort`](/oss/langchain/models#reasoning) parameter, at both construction and call time. If both are set, `effort` wins. `reasoning_effort` requires `langchain-anthropic>=1.5.3`:
