...
Server-side tools (e.g., [web search](#web-search-tool)) are passed to the model and executed by Anthropic. Client-side tools (e.g., [bash tool](#bash-tool)) require you to implement the callback execution logic in your application and return results to the model.
...
Using [`create_agent`][langchain.agents.create_agent] handles the tool execution loop automatically.
