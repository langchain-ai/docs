...
<Info>
    **AWS Bedrock and Gemini Enterprise Agent Platform**
    ...
    Note that certain Anthropic models can also be accessed via AWS Bedrock and Gemini Enterprise Agent Platform. See the [`ChatBedrock`](/oss/integrations/chat/bedrock/) and [`ChatVertexAI`](/oss/integrations/chat/google_vertex_ai) integrations to use Anthropic models via these services.
...
- `defer_loading` (bool): Load tool on-demand for [tool search](#tool-search) or a [mid-conversation tool change](#change-tools-mid-conversation)
...
`effort` is an alias for the standard [`reasoning_effort`](/oss/langchain/models#reasoning) parameter, at both construction and call time. If both are set, `effort` wins.
...
<Note>
    For LangChain agents, prefer [`AnthropicPromptCachingMiddleware`](/oss/integrations/middleware/anthropic#prompt-caching) when you want LangChain to optimize stable system prompt and tool content. The middleware is a LangChain agent/harness optimization and is not the same as the invocation-level `cache_control` shown below, which mirrors the Anthropic API behavior.
</Note>
...
<Note>
    Mid-conversation system messages require `langchain-anthropic>=1.7.3`.
</Note>
...
A leading @[`SystemMessage`] always goes in the top-level `system` field. `ChatAnthropic` sends a later `SystemMessage` at its own position in the conversation when both of these conditions hold:

- The model supports mid-conversation system messages. The [Claude documentation](https://platform.claude.com/docs/en/build-with-claude/mid-conversation-system-messages) lists which models do.
- The message follows a human or tool message, and is either the last message or followed by an AI message.

Otherwise, `ChatAnthropic` moves the message into the top-level `system` field with a warning, where it applies to the whole conversation. If a leading system message already fills that field, `ChatAnthropic` raises a `ValueError`.
...
response = model.invoke(
    [
        ...
        HumanMessage("Suggest a day trip from Lisbon."),
        AIMessage("Sintra is a good choice: palaces, gardens, and a short train ride."),
        HumanMessage("How do I get there?"),
        SystemMessage("The user has switched to the mobile app. Keep answers under 50 words."),  # [!code highlight]
    ]
)
```
...
{/* TODO: Set the version once langchain-ai/langchain#40758 is released. */}
<Note>
    Mid-conversation tool changes require `langchain-anthropic>=TODO`.
</Note>
...
The [placement and model rules](#mid-conversation-system-messages) above apply. When `ChatAnthropic` cannot send the message in place, it drops the tool-change blocks with a warning.

`ChatAnthropic` adds the required beta header, so you do not need to set `betas`. It uses `inline-tools-2026-09-15` when a block defines a tool inline, and `mid-conversation-tool-changes-2026-07-01` otherwise.
...
model = ChatAnthropic(model="claude-opus-5-5")
model_with_tools = model.bind_tools([get_time, get_weather])
...
response = model.invoke(
    [
        HumanMessage("How many orders did we get yesterday?"),
        SystemMessage(
            [
                {
                    "type": "tool_addition",
...
You can also write these blocks wrapped in a `non_standard` block, which is how `content_blocks` returns them. `ChatAnthropic` sends the same request for either form.
...
Specify `response_format` with [`ProviderStrategy`](/oss/langchain/structured-output) to engage Anthropic's structured output feature when generating its final response.
...
To offer a deferred tool yourself instead of through search, [change tools mid-conversation](#change-tools-mid-conversation).
