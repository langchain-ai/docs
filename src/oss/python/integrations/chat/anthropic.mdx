...
<Tabs>
    <Tab title="Anthropic type">
        ...
        # LangChain provides two public MCP servers:
        # - docs-langchain: guides, tutorials, and how-tos
        # - reference-langchain: API reference (classes, methods, parameters)
        # Adding both gives your agent the full picture.
        mcp_servers = [
            {
                "type": "url",
        ...
        mcp_tools = [ # [!code highlight]
            BetaMCPToolsetParam(type="mcp_toolset", mcp_server_name="LangChain Docs"), # [!code highlight]
            BetaMCPToolsetParam(type="mcp_toolset", mcp_server_name="LangChain Reference"), # [!code highlight]
        ] # [!code highlight]

        response = model.invoke(
            "What are LangChain content blocks?",
            tools=mcp_tools, # [!code highlight]
        )
        ```
    </Tab>
