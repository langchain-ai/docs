...
When routed through the Agent API, response objects carry richer metadata. URL citations are surfaced under `response_metadata`, and `usage_metadata` is populated from the Responses-shaped `usage` payload (`input_tokens`, `output_tokens`, `total_tokens`). Tool calls returned by the model appear in `response.tool_calls` exactly as they do for `ChatOpenAI`.
