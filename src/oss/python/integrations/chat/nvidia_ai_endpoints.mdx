...
| `priority` | `int` | `1` | Lower priority settings recieve more scheduling priority |
...
### Prioritize requests in agentic workflows

The real power of `ChatNVIDIADynamo` emerges in multi-step agentic pipelines where not all LLM calls are equally urgent.

Consider a customer support triage workflow. A short classification call (~5 output tokens) gates several longer analysis branches that run in parallel (~500 tokens each). By marking the classification as high-priority, Dynamo resolves it quickly so the longer-running decodes can start sooner. Once those parallel branches are in flight, they may share KV cache blocks with other similar concurrent requests through Dynamo's cache-aware routing, further reducing overall latency. A final short review call is also high-priority so it doesn't queue behind the still-decoding background work.

With Dynamo's priority scheduling enabled (`--enable-priority-scheduling` on the Dynamo worker), high-priority requests are scheduled ahead of lower-priority ones, even if the lower-priority requests arrived first.
...
# High-priority: critical-path calls (classification, review)
llm_high = ChatNVIDIADynamo(
    base_url=BASE_URL, model=MODEL,
    priority=0, latency_sensitivity=10.0,
)

# Low-priority: parallel background analysis
llm_low = ChatNVIDIADynamo(
    base_url=BASE_URL, model=MODEL,
    priority=01, latency_sensitivity=0,
)

# Stage 1 — Intent classification (HIGH priority, short output)
classify_chain = (
    ChatPromptTemplate.from_messages([
        ("system", "Classify the customer query into one of: billing, "
                   "technical_support, account, or general. "
...
# Stage 2 — Parallel background analysis (LOW priority, long output)
research_chain = (
    ChatPromptTemplate.from_messages([
        ("system", "Research relevant context for this customer query."),
...
sentiment_chain = (
    ChatPromptTemplate.from_messages([
        ("system", "Analyze the sentiment and emotional tone of this message."),
        ("user", "{query}"),
    ])
    | llm_low.bind(osl=500)
    | StrOutputParser()
...
# Stage 3 — Review (HIGH priority, short output)
review_chain = (
    ChatPromptTemplate.from_messages([
        ("system", "Review this draft response. Reply APPROVED or NEEDS_REVISION."),
...
async def triage(query: str) -> dict:
    # Critical path — user is waiting
    category = (await classify_chain.ainvoke({"query": query})).strip()

    # Background — parallel, latency-tolerant
    ...
    # Critical path — user is waiting
    review = await review_chain.ainvoke({"context": context, "sentiment": sentiment})
    return {"category": category, "review": review.strip()}
```
