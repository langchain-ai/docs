...
Some NVIDIA reasoning models support a configurable thinking mode. Use `with_thinking_mode` to create a reusable runnable with thinking enabled:

```python
from langchain_nvidia_ai_endpoints import ChatNVIDIA

llm = ChatNVIDIA(model="nvidia/nemotron-3-nano-30b-a3b")
thinking_llm = llm.with_thinking_mode(enabled=True)

response = thinking_llm.invoke("Solve this step by step: 17 * 23")
print(response.content)
print(response.additional_kwargs.get("reasoning_content"))
```

You can also enable thinking for a single request by passing `thinking_mode=True` to `invoke`:

```python
response = llm.invoke("Solve this step by step: 17 * 23", thinking_mode=True)
```

To list models that are known to support thinking mode, inspect `supports_thinking`:
