...
[Snowflake Cortex REST API](/oss/integrations/chat/cortex_rest_api) is fully compatible with both the OpenAI and Anthropic APIs. Use `ChatOpenAI` or `ChatAnthropic` with a custom `base_url` to access frontier models running within Snowflake's security perimeter.
