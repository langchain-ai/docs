...
> [Anthropic Claude](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-claude) models on Gemini Enterprise Agent Platform offer fully managed and serverless models as APIs. To use a Claude model on Gemini Enterprise Agent Platform, send a request directly to the Gemini Enterprise Agent Platform API endpoint. Because Anthropic Claude models use a managed API, there's no need to provision or manage infrastructure.
