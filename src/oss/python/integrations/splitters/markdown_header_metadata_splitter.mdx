...
## Troubleshooting: `chunk_overlap` seems not to apply
...
- If the header becomes a tiny first chunk, consider `strip_headers={true}` (Python: `strip_headers=True`) so the header line doesn’t become a standalone tiny chunk.
