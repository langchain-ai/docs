...
This extra installs the service-specific dependencies used by these tools, including `azure-ai-documentintelligence`, `azure-ai-vision-imageanalysis`, `azure-cognitiveservices-speech`, and `azure-ai-textanalytics`.

<Note>
    `AzureAIContentUnderstandingTool` additionally requires the `azure-ai-contentunderstanding` package, which is not included in the `tools` extra. Install it separately with `pip install azure-ai-contentunderstanding`.
</Note>
