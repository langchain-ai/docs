...
Web search needs `bedrock-agentcore` 1.23.0 or later. Below that version the import is skipped rather than raising, so if `create_web_search_toolkit` cannot be imported from `langchain_aws.tools`, check the installed version first.
...
Address the gateway by `gateway_id`, by `gateway_arn`, or by `gateway_endpoint` if you already know the MCP URL. A `gateway_arn` carries its own region, so pass `region` alongside a `gateway_id`, or to override the region in an ARN.
