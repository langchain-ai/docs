...
[Stagehand](https://stagehand.dev) is an open-source browser automation framework built on Playwright, backed by [Browserbase](https://browserbase.com) — a developer platform for running reliable, managed headless browsers. The integration ships a ready-made MCP server that exposes three tools — `run`, `snapshot`, and `screenshot` — directly to your Deep Agent, with no custom tool definitions required.
...
| `STAGEHAND_MODEL_API_KEY` | Unset | Required with `STAGEHAND_MODEL` |
