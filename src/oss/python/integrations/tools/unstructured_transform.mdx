...
{/*
  DRAFT for a PR to https://github.com/langchain-ai/docs
  Target path in that repo: src/oss/python/integrations/tools/unstructured_transform.mdx
  Do NOT commit this file to the langchain-ai/docs repo with these comments; they are
  scaffolding notes for the Unstructured team.

  Before opening the PR:
  - [ ] Publish `langchain-unstructured-transform` to PyPI so the version/downloads badges render.
  - [ ] Add the page to the docs nav (docs.json / mint.json equivalent in that repo).
  - [ ] Verify all code blocks run against a live Unstructured API key.
*/}
