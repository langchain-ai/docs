  ---                                                                                     
  title: FMP Data                                                                         
  ---                                                                                     
                                                                                          
  Access financial market data through natural language queries.                          
                                                                                          
  ## Overview                                                                             
                                                                                          
  The FMP (Financial Modeling Prep) LangChain integration provides a seamless way to      
  access financial market data through natural language queries. This integration offers  
  two main components:                                                                    
                                                                                          
  - `FMPDataToolkit`: Creates collections of tools based on natural language queries      
  - `FMPDataTool`: A single unified tool that automatically selects and uses the          
  appropriate endpoints                                                                   
                                                                                          
  The integration leverages LangChain's semantic search capabilities to match user queries
   with the most relevant FMP API endpoints, making financial data access more intuitive  
  and efficient.                                                                          
                                                                                          
  ## Setup                                                                                
                                                                                          
  ```python                                                                               
  !pip install -U langchain-fmp-data                                                      
                                                                                          
  import os                                                                               
                                                                                          
  # Replace with your actual API keys                                                     
  os.environ["FMP_API_KEY"] = "your-fmp-api-key"  # pragma: allowlist secret              
  os.environ["OPENAI_API_KEY"] = "your-openai-api-key"  # pragma: allowlist secret        
                                                                                          
  It's also helpful (but not needed) to set up https://smith.langchain.com/ for           
  best-in-class observability:                                                            
                                                                                          
  os.environ["LANGSMITH_TRACING"] = "true"                                                
  # os.environ["LANGSMITH_API_KEY"] = getpass.getpass()                                   
                                                                                          
  Instantiation                                                                           
                                                                                          
  There are two main ways to instantiate the FMP LangChain integration:                   
                                                                                          
  1. Using FMPDataToolkit                                                                 
                                                                                          
  from langchain_fmp_data import FMPDataToolkit                                           
                                                                                          
  query = "Get stock market prices and technical indicators"                              
  # Basic instantiation                                                                   
  toolkit = FMPDataToolkit(query=query)                                                   
                                                                                          
  # Instantiation with specific query focus                                               
  market_toolkit = FMPDataToolkit(                                                        
      query=query,                                                                        
      num_results=5,                                                                      
  )                                                                                       
                                                                                          
  # Instantiation with explicit API keys                                                  
  custom_toolkit = FMPDataToolkit(                                                        
      query="Financial analysis",                                                         
      num_results=3,                                                                      
      fmp_api_key="your-fmp-key",  # pragma: allowlist secret                             
      openai_api_key="your-openai-key",  # pragma: allowlist secret                       
  )                                                                                       
                                                                                          
  2. Using FMPDataTool                                                                    
                                                                                          
  from langchain_fmp_data import FMPDataTool                                              
                                                                                          
  # Basic instantiation                                                                   
  tool = FMPDataTool()                                                                    
                                                                                          
  # Advanced instantiation with custom settings                                           
  advanced_tool = FMPDataTool(                                                            
      max_iterations=50,                                                                  
      temperature=0.2,                                                                    
  )                                                                                       
                                                                                          
  Invocation                                                                              
                                                                                          
  The tools can be invoked in several ways:                                               
                                                                                          
  Direct invocation                                                                       
                                                                                          
  from langchain_fmp_data import FMPDataTool                                              
  from langchain_fmp_data.tools import ResponseFormat                                     
                                                                                          
  # Using FMPDataTool                                                                     
  tool = FMPDataTool()                                                                    
                                                                                          
  # Basic query                                                                           
  result = tool.invoke({"query": "What's Apple's current stock price?"})                  
                                                                                          
  # Advanced query with specific format                                                   
  detailed_result = tool.invoke(                                                          
      {                                                                                   
          "query": "Compare Tesla and Ford's profit margins",                             
          "response_format": ResponseFormat.BOTH,                                         
      }                                                                                   
  )                                                                                       
                                                                                          
  Using with LangChain agents                                                             
                                                                                          
  from langchain.agents import AgentExecutor, create_react_agent                          
  from langchain_openai import ChatOpenAI                                                 
  from langchain import hub                                                               
                                                                                          
  # Setup                                                                                 
  llm = ChatOpenAI(temperature=0)                                                         
  toolkit = FMPDataToolkit(                                                               
      query="Stock analysis",                                                             
      num_results=3,                                                                      
  )                                                                                       
  tools = toolkit.get_tools()                                                             
                                                                                          
  # Get prompt template for ReAct agent                                                   
  prompt = hub.pull("hwchase17/react")                                                    
                                                                                          
  # Create agent                                                                          
  agent = create_react_agent(llm, tools, prompt)                                          
  agent_executor = AgentExecutor(agent=agent, tools=tools)                                
                                                                                          
  # Run query                                                                             
  response = agent_executor.invoke({"input": "What's the PE ratio of Microsoft?"})        
                                                                                          
  Advanced usage                                                                          
                                                                                          
  You can customize the tool's behavior:                                                  
                                                                                          
  from langchain_fmp_data import FMPDataTool                                              
  from langchain_fmp_data.tools import ResponseFormat                                     
                                                                                          
  # Initialize with custom settings                                                       
  advanced_tool = FMPDataTool(                                                            
      max_iterations=50,  # Increase max iterations for complex queries (default: 30)     
      temperature=0.2,  # Adjust temperature for more/less focused responses              
  )                                                                                       
                                                                                          
  # Example of a complex multi-part analysis                                              
  query = """                                                                             
  Analyze Apple's financial health by:                                                    
  1. Examining current ratios and debt levels                                             
  2. Comparing profit margins to industry average                                         
  3. Looking at cash flow trends                                                          
  4. Assessing growth metrics                                                             
  """                                                                                     
                                                                                          
  response = advanced_tool.invoke(                                                        
      {                                                                                   
          "query": query,                                                                 
          "response_format": ResponseFormat.BOTH,                                         
      }                                                                                   
  )                                                                                       
  print("Detailed Financial Analysis:")                                                   
  print(response)                                                                         
                                                                                          
  Chaining                                                                                
                                                                                          
  You can chain the tool with other LangChain components:                                 
                                                                                          
  from langchain_core.output_parsers import StrOutputParser                               
  from langchain_openai import ChatOpenAI                                                 
  from langchain_fmp_data import FMPDataToolkit                                           
                                                                                          
  # Setup                                                                                 
  llm = ChatOpenAI(temperature=0)                                                         
  toolkit = FMPDataToolkit(query="Stock analysis", num_results=3)                         
  tools = toolkit.get_tools()                                                             
                                                                                          
  llm_with_tools = llm.bind_tools(tools)                                                  
  output_parser = StrOutputParser()                                                       
                                                                                          
  # Create chain                                                                          
  chain = llm_with_tools | output_parser                                                  
                                                                                          
  # Run chain                                                                             
  response = chain.invoke("What's the PE ratio of Microsoft?")                            
                                                                                          
  API reference                                                                           
                                                                                          
  FMPDataToolkit                                                                          
                                                                                          
  Main class for creating collections of FMP API tools:                                   
                                                                                          
  from typing import List, Optional                                                       
                                                                                          
  from langchain_core.tools import BaseTool, BaseToolkit                                  
                                                                                          
                                                                                          
  class FMPDataToolkit(BaseToolkit):                                                      
      """Creates a collection of FMP data tools based on queries."""                      
                                                                                          
      def __init__(                                                                       
          self,                                                                           
          query: str,                                                                     
          num_results: int = 3,                                                           
          fmp_api_key: Optional[str] = None,                                              
          openai_api_key: Optional[str] = None,                                           
      ) -> None: ...                                                                      
                                                                                          
      def get_tools(self) -> List[BaseTool]:                                              
          """Returns a list of relevant FMP API tools based on the query."""              
          ...                                                                             
                                                                                          
  FMPDataTool                                                                             
                                                                                          
  Unified tool that automatically selects appropriate FMP endpoints:                      
                                                                                          
  from typing import Any, Optional                                                        
                                                                                          
                                                                                          
  class FMPDataTool:                                                                      
      """Single unified tool for accessing FMP data through natural language."""          
                                                                                          
      def __init__(                                                                       
          self,                                                                           
          fmp_api_key: Optional[str] = None,                                              
          openai_api_key: Optional[str] = None,                                           
          max_iterations: int = 30,                                                       
          temperature: float = 0.0,                                                       
      ) -> None: ...                                                                      
                                                                                          
      def invoke(                                                                         
          self,                                                                           
          input: dict[str, Any],                                                          
      ) -> str | dict[str, Any]:                                                          
          """Execute a natural language query against FMP API."""                         
          ...                                                                             
                                                                                          
  ResponseFormat                                                                          
                                                                                          
  Enum for controlling response format:                                                   
                                                                                          
  from enum import Enum                                                                   
                                                                                          
                                                                                          
  class ResponseFormat(str, Enum):                                                        
      NATURAL_LANGUAGE = "natural_language"  # Human-readable text response (default)     
      DATA_STRUCTURE = "data_structure"  # Structured JSON data                           
      BOTH = "both"  # Both natural language and structured data                          
                                                                                          
  ## Key Corrections Made:                                                                
                                                                                          
  1. **ResponseFormat enum** - Fixed values from `RAW`/`ANALYSIS` to                      
  `NATURAL_LANGUAGE`/`DATA_STRUCTURE`                                                     
  2. **FMPDataToolkit** - Removed non-existent `similarity_threshold` and `cache_dir`     
  parameters                                                                              
  3. **FMPDataTool** - Fixed default `max_iterations` from 3 to 30                        
  4. **Agent example** - Changed from deprecated `create_openai_functions_agent` to       
  `create_react_agent`                                                                    
  5. **Chaining example** - Changed from deprecated `llm.bind(functions=tools)` to        
  `llm.bind_tools(tools)`                                                                 
  6. **Removed `# fmt: off/on`** comments that aren't needed in documentation             
