...
One of the most powerful applications enabled by LLMs is sophisticated question-answering (Q\&A) chatbots. These are applications that can answer questions about specific source information. These applications use a technique known as Retrieval Augmented Generation, or [RAG](/oss/concepts/rag).

This is a multi-part tutorial:

* [Part 1](/oss/tutorials/rag) (this guide) introduces RAG and walks through a minimal implementation.
* [Part 2](/oss/tutorials/qa_chat_history) extends the implementation to accommodate conversation-style interactions and multi-step retrieval processes.
...
If you're already familiar with basic retrieval, you might also be interested in this [high-level overview of different retrieval techniques](/oss/concepts/retrieval).

**Note**: Here we focus on Q\&A for unstructured data. If you are interested for RAG over structured data, check out our tutorial on doing [question/answering over SQL data](/oss/tutorials/sql_qa).
...
Note: the indexing portion of this tutorial will largely follow the [semantic search tutorial](/oss/tutorials/retrievers).

The most common full sequence from raw data to answer looks like:

### Indexing

1. **Load**: First we need to load our data. This is done with [Document Loaders](/oss/concepts/document_loaders).
2. **Split**: [Text splitters](/oss/concepts/text_splitters) break large `Documents` into smaller chunks. This is useful both for indexing data and passing it into a model, as large chunks are harder to search over and won't fit in a model's finite context window.
3. **Store**: We need somewhere to store and index our splits, so that they can be searched over later. This is often done using a [VectorStore](/oss/concepts/vectorstores) and [Embeddings](/oss/concepts/embedding_models) model.

![index\_diagram](/images/oss/tutorials/rag/assets/images/rag_indexing-8160f90a90a33253d0154659cf7d453f.png)

### Retrieval and generation

4. **Retrieve**: Given a user input, relevant splits are retrieved from storage using a [Retriever](/oss/concepts/retrievers).
5. **Generate**: A [ChatModel](/oss/concepts/chat_models) / [LLM](/oss/concepts/text_llms) produces an answer using a prompt that includes both the question with the retrieved data
...
<Note>
  This section is an abbreviated version of the content in the [semantic search tutorial](/oss/tutorials/retrievers). If you're comfortable with [document loaders](/oss/concepts/document_loaders), [embeddings](/oss/concepts/embedding_models), and [vector stores](/oss/concepts/vectorstores), feel free to skip to the next section on [retrieval and generation](/oss/tutorials/rag/#orchestration).
</Note>

### Loading documents

We need to first load the blog post contents. We can use [DocumentLoaders](/oss/concepts/document_loaders) for this, which are objects that load in data from a source and return a list of [Document](https://python.langchain.com/api_reference/core/documents/langchain_core.documents.base.Document.html) objects.
...
* [Docs](/oss/how_to/#document-loaders): Detailed documentation on how to use `DocumentLoaders`.
...
As in the [semantic search tutorial](/oss/tutorials/retrievers), we use a [RecursiveCharacterTextSplitter](/oss/how_to/recursive_text_splitter), which will recursively split the document using common separators like new lines until each chunk is the appropriate size. This is the recommended text splitter for generic text use cases.
...
* Learn more about splitting text using different methods by reading the [how-to docs](/oss/how_to/#text-splitters)
* [Code (py or js)](/oss/integrations/document_loaders/source_code)
* [Scientific papers](/oss/integrations/document_loaders/grobid)
* [Interface](https://python.langchain.com/api_reference/text_splitters/base/langchain_text_splitters.base.TextSplitter.html): API reference for the base interface.

`DocumentTransformer`: Object that performs a transformation on a list of `Document` objects.

* [Docs](/oss/how_to/#text-splitters): Detailed documentation on how to use `DocumentTransformers`
* [Integrations](/oss/integrations/document_transformers)
...
Now we need to index our 66 text chunks so that we can search over them at runtime. Following the [semantic search tutorial](/oss/tutorials/retrievers), our approach is to [embed](/oss/concepts/embedding_models) the contents of each document split and insert these embeddings into a [vector store](/oss/concepts/vectorstores). Given an input query, we can then use vector search to retrieve relevant documents.

We can embed and store all of our document splits in a single command using the vector store and embeddings model selected at the [start of the tutorial](/oss/tutorials/rag/#components).
...
* [Docs](/oss/how_to/embed_text): Detailed documentation on how to use embeddings.
...
* [Docs](/oss/how_to/vectorstores): Detailed documentation on how to use vector stores.
...
For generation, we will use the chat model selected at the [start of the tutorial](/oss/tutorials/rag/#components).
...
<Accordion title="Do I need to use LangGraph?">
  ...
  * Support for persistence, human-in-the-loop, and other features.
  ...
  Many use-cases demand RAG in a conversational experience, such that a user can receive context-informed answers via a stateful conversation. As we will see in [Part 2](/oss/tutorials/qa_chat_history) of the tutorial, LangGraph's management and persistence of state simplifies these applications enormously.
</Accordion>
...
Stream [tokens](/oss/concepts/tokens):
...
Note that by storing the retrieved context in the state of the graph, we recover sources for the model's generated answer in the `"context"` field of the state. See [this guide](/oss/how_to/qa_sources) on returning sources for more detail.

#### Go deeper

[Chat models](/oss/concepts/chat_models) take in a sequence of messages and return a message.

* [Docs](/oss/how_to/#chat-models)
* [Integrations](/oss/integrations/chat): 25+ integrations to choose from.
...
[Query analysis](/oss/concepts/retrieval/#query-analysis) employs models to transform or construct optimized search queries from raw user input. We can easily incorporate a query analysis step into our application. For illustrative purposes, let's add some metadata to the documents in our vector store. We will add some (contrived) sections to the document which we can filter on later.
...
Let's next define a schema for our search query. We will use [structured output](/oss/concepts/structured_outputs) for this purpose. Here we define a query as containing a string query and a document section (either "beginning", "middle", or "end"), but this can be defined however you like.
...
Query Analysis is a rich problem with a wide range of approaches. Refer to the [how-to guides](/oss/how_to/#query-analysis) for more examples.

## Next steps

We've covered the steps to build a basic Q\&A app over data:

* Loading data with a [Document Loader](/oss/concepts/document_loaders)
* Chunking the indexed data with a [Text Splitter](/oss/concepts/text_splitters) to make it more easily usable by a model
* [Embedding the data](/oss/concepts/embedding_models) and storing the data in a [vectorstore](/oss/how_to/vectorstores)
* [Retrieving](/oss/concepts/retrievers) the previously stored chunks in response to incoming questions
* Generating an answer using the retrieved chunks as context.

In [Part 2](/oss/tutorials/qa_chat_history) of the tutorial, we will extend the implementation here to accommodate conversation-style interactions and multi-step retrieval processes.

Further reading:

* [Return sources](/oss/how_to/qa_sources): Learn how to return source documents
* [Streaming](/oss/how_to/streaming): Learn how to stream outputs and intermediate steps
* [Add chat history](/oss/how_to/message_history): Learn how to add chat history to your app
* [Retrieval conceptual guide](/oss/concepts/retrieval): A high-level overview of specific retrieval techniques
