...
<Info>
  ...
  * [LangGraph](/oss/concepts/architecture/#langgraph)
</Info>

Enabling a LLM system to query structured data can be qualitatively different from unstructured text data. Whereas in the latter it is common to generate text that can be searched against a vector database, the approach for structured data is often for the LLM to write and execute queries in a DSL, such as SQL. In this guide we'll go over the basic ways to create a Q\&A system over tabular data in databases. We will cover implementations using both [chains](/oss/tutorials/sql_qa/#chains) and [agents](/oss/tutorials/sql_qa/#agents). These systems will allow us to ask a question about the data in a database and get back a natural language answer. The main difference between the two is that our agent can query the database in a loop as many times as it needs to answer the question.
...
Note that querying data in CSVs can follow a similar approach. See our [how-to guide](/oss/how_to/sql_csv) on question-answering over CSV data for more detail.

![sql\_usecase.png](/images/oss/tutorials/sql_qa/assets/images/sql_usecase-d432701261f05ab69b38576093718cf3.png)
...
Chains are compositions of predictable steps. In [LangGraph](/oss/concepts/architecture/#langgraph), we can represent a chain via simple sequence of nodes. Let's create a sequence of steps that, given a question, does the following:
...
The first step is to take the user input and convert it to a SQL query. To reliably obtain SQL queries (absent markdown formatting and explanations or clarifications), we will make use of LangChain's [structured output](/oss/concepts/structured_outputs) abstraction.
...
To execute the query, we will load a tool from [langchain-community](/oss/concepts/architecture/#langchain-community). Our `execute_query` node will just wrap this tool:
...
* [Prompting strategies](/oss/how_to/sql_prompting): Advanced prompt engineering techniques.
* [Query checking](/oss/how_to/sql_query_checking): Add query validation and error handling.
* [Large databases](/oss/how_to/sql_large_db): Techniques for working with large databases.
...
We will use a prebuilt [LangGraph](/oss/concepts/architecture/#langgraph) agent to build our agent
...
Let's select an [embeddings model](/oss/integrations/text_embedding) and [vector store](/oss/integrations/vectorstores) for this step:
