...
In this tutorial, we will use [tool-calling](/oss/concepts/tool_calling) features of [chat models](/oss/concepts/chat_models) to extract structured information from unstructured text. We will also demonstrate how to use [few-shot prompting](/oss/concepts/few_shot_prompting) in this context to improve performance.
...
Please review [the documentation](/oss/concepts/tool_calling) for all models that can be used with this API.
...
<Info>
  Extraction results might not be perfect here. Read on to see how to use **Reference Examples** to improve the quality of extraction, and check out our extraction [how-to](/oss/how_to/#extraction) guides for more detail.
</Info>
...
The behavior of LLM applications can be steered using [few-shot prompting](/oss/concepts/few_shot_prompting). For [chat models](/oss/concepts/chat_models), this can take the form of a sequence of pairs of input and response messages demonstrating desired behaviors.

For example, we can convey the meaning of a symbol with alternating `user` and `assistant` [messages](/oss/concepts/messages/#role):
...
[Structured output](/oss/concepts/structured_outputs) often uses [tool calling](/oss/concepts/tool_calling) under-the-hood. This typically involves the generation of [AI messages](/oss/concepts/messages/#aimessage) containing tool calls, as well as [tool messages](/oss/concepts/messages/#toolmessage) containing the results of tool calls. What should a sequence of messages look like in this case?

Different [chat model providers](/oss/integrations/chat) impose different requirements for valid message sequences. Some will accept a (repeating) message sequence of the form:
...
See [this guide](/oss/how_to/extraction_examples) for more detail on extraction workflows with reference examples, including how to incorporate prompt templates and customize the generation of example messages.

## Next steps

Now that you understand the basics of extraction with LangChain, you're ready to proceed to the rest of the how-to guides:

* [Add Examples](/oss/how_to/extraction_examples): More detail on using **reference examples** to improve performance.
* [Handle Long Text](/oss/how_to/extraction_long_text): What should you do if the text does not fit into the context window of the LLM?
* [Use a Parsing Approach](/oss/how_to/extraction_parse): Use a prompt based approach to extract with models that do not support **tool/function calling**.
