...
* `function`: Like [extraction](/oss/tutorials/extraction), tagging uses [functions](https://openai.com/blog/function-calling-and-other-api-updates) to specify how the model should tag a document
* `schema`: defines how we want to tag the document

## Quickstart

Let's see a very straightforward example of how we can use OpenAI tool calling for tagging in LangChain. We'll use the [`with_structured_output`](/oss/how_to/structured_output) method supported by OpenAI models.

```
pip install --upgrade --quiet langchain-core
```

We'll need to load a [chat model](/oss/integrations/chat):
...
* You can use the [metadata tagger](/oss/integrations/document_transformers/openai_metadata_tagger) document transformer to extract metadata from a LangChain `Document`.
