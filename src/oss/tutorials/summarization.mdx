...
<Info>
  ...
  A [previous version](https://python.langchain.com/v0.1/oss/use_cases/summarization/) of this page showcased the legacy chains [StuffDocumentsChain](/oss/versions/migrating_chains/stuff_docs_chain), [MapReduceDocumentsChain](/oss/versions/migrating_chains/map_reduce_chain), and [RefineDocumentsChain](https://python.langchain.com/oss/versions/migrating_chains/refine_docs_chain/). See [here](/oss/versions/migrating_chains) for information on using those abstractions and a comparison with the methods demonstrated in this tutorial.
</Info>
...
In the context of [retrieval-augmented generation](/oss/tutorials/rag), summarizing text can help distill the information in a large number of retrieved documents to provide context for a LLM.
...
* Using [document loaders](/oss/concepts/document_loaders), specifically the [WebBaseLoader](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.web_base.WebBaseLoader.html) to load content from an HTML webpage.

* Two ways to summarize or otherwise combine documents.

  1. [Stuff](/oss/tutorials/summarization/#stuff), which simply concatenates documents into a prompt;
  2. [Map-reduce](/oss/tutorials/summarization/#map-reduce), for larger sets of documents. This splits documents into batches, summarizes those, and then summarizes the summaries.

Shorter, targeted guides on these strategies and others, including [iterative refinement](/oss/how_to/summarize_refine), can be found in the [how-to guides](/oss/how_to/#summarization).
...
1. `Stuff`: Simply "stuff" all your documents into a single prompt. This is the simplest approach (see [here](/oss/how_to/summarize_stuff) for more on the `create_stuff_documents_chain` constructor, which is used for this method).
...
* You can easily try different LLMs, (e.g., [Claude](/oss/integrations/chat/anthropic)) via the `llm` parameter.
...
We encourage you to check out the [how-to guides](/oss/how_to) for more detail on:

* Other summarization strategies, such as [iterative refinement](/oss/how_to/summarize_refine)
* Built-in [document loaders](/oss/how_to/#document-loaders) and [text-splitters](/oss/how_to/#text-splitters)
* Integrating various combine-document chains into a [RAG application](/oss/tutorials/rag)
* Incorporating retrieval into a [chatbot](/oss/how_to/chatbots_retrieval)
