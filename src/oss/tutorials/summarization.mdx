...
<Info>
  ...
  A [previous version](https://python.langchain.com/v0.1/oss/use_cases/summarization/) of this page showcased the legacy chains [StuffDocumentsChain](/src/oss/versions/migrating_chains/stuff_docs_chain), [MapReduceDocumentsChain](/src/oss/versions/migrating_chains/map_reduce_chain), and [RefineDocumentsChain](https://python.langchain.com/oss/versions/migrating_chains/refine_docs_chain/). See [here](/src/oss/versions/migrating_chains) for information on using those abstractions and a comparison with the methods demonstrated in this tutorial.
</Info>
...
In the context of [retrieval-augmented generation](/src/oss/tutorials/rag), summarizing text can help distill the information in a large number of retrieved documents to provide context for a LLM.

In this walkthrough we'll go over how to summarize content from multiple documents using LLMs.

![Image description](/images/oss/tutorials/summarization/assets/images/summarization_use_case_1-874f7b2c94f64216f1f967fb5aca7bc1.png)
...
* Using [document loaders](/src/oss/concepts/document_loaders), specifically the [WebBaseLoader](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.web_base.WebBaseLoader.html) to load content from an HTML webpage.

* Two ways to summarize or otherwise combine documents.

  1. [Stuff](/src/oss/tutorials/summarization/#stuff), which simply concatenates documents into a prompt;
  2. [Map-reduce](/src/oss/tutorials/summarization/#map-reduce), for larger sets of documents. This splits documents into batches, summarizes those, and then summarizes the summaries.

Shorter, targeted guides on these strategies and others, including [iterative refinement](/src/oss/how_to/summarize_refine), can be found in the [how-to guides](/src/oss/how_to/#summarization).
...
1. `Stuff`: Simply "stuff" all your documents into a single prompt. This is the simplest approach (see [here](/src/oss/how_to/summarize_stuff) for more on the `create_stuff_documents_chain` constructor, which is used for this method).
...
Note that map-reduce is especially effective when understanding of a sub-document does not rely on preceding context. For example, when summarizing a corpus of many, shorter documents. In other cases, such as summarizing a novel or body of text with an inherent sequence, [iterative refinement](/src/oss/how_to/summarize_refine) may be more effective.

![Image description](/images/oss/tutorials/summarization/assets/images/summarization_use_case_2-f2a4d5d60980a79140085fb7f8043217.png)
...
* You can easily try different LLMs, (e.g., [Claude](/src/oss/integrations/chat/anthropic)) via the `llm` parameter.
...
![Image description](/images/oss/tutorials/summarization/assets/images/summarization_use_case_3-896f435bc48194ddaead73043027e16f.png)

## Next steps

We encourage you to check out the [how-to guides](/src/oss/how_to) for more detail on:

* Other summarization strategies, such as [iterative refinement](/src/oss/how_to/summarize_refine)
* Built-in [document loaders](/src/oss/how_to/#document-loaders) and [text-splitters](/src/oss/how_to/#text-splitters)
* Integrating various combine-document chains into a [RAG application](/src/oss/tutorials/rag)
* Incorporating retrieval into a [chatbot](/src/oss/how_to/chatbots_retrieval)
