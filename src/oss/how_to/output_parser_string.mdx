...
LangChain [message](/src/oss/concepts/messages) objects support content in a [variety of formats](/src/oss/concepts/messages/#content), including text, [multimodal data](/src/oss/concepts/multimodality), and a list of [content block](/src/oss/concepts/messages/#aimessage) dicts.

The format of [Chat model](/src/oss/concepts/chat_models) response content may depend on the provider. For example, the chat model for [Anthropic](/src/oss/integrations/chat/anthropic) will return string content for typical string input:
