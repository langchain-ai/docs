...
<Info>
  ...
  * [Chat Messages](/src/oss/concepts/messages)
...
Adding memory to a [chat model](/src/oss/concepts/chat_models) provides a simple example. Chat models accept a list of messages as input and output a message. LangGraph includes a built-in `MessagesState` that we can use for this purpose.
