...
This guide shows you how to use the [`XMLOutputParser`](https://python.langchain.com/api_reference/core/output_parsers/langchain_core.output_parsers.xml.XMLOutputParser.html) to prompt models for XML output, then and [parse](/src/oss/concepts/output_parsers) that output into a usable format.
...
You've now learned how to prompt a model to return XML. Next, check out the [broader guide on obtaining structured output](/src/oss/how_to/structured_output) for other related techniques.
