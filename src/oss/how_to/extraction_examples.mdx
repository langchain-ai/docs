...
Data extraction attempts to generate [structured representations](/src/oss/concepts/structured_outputs) of information found in text and other unstructured or semi-structured formats. [Tool-calling](/src/oss/concepts/tool_calling) LLM features are often used in this context. This guide demonstrates how to build few-shot examples of tool calls to help steer the behavior of extraction and similar applications.
...
LangChain implements a [tool-call attribute](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.ai.AIMessage.html#langchain_core.messages.ai.AIMessage.tool_calls) on messages from LLMs that include tool calls. See our [how-to guide on tool calling](/src/oss/how_to/tool_calling) for more detail. To build reference examples for data extraction, we build a chat history containing a sequence of:
...
Let's re-use the person schema from the [extraction tutorial](/src/oss/tutorials/extraction).
...
Let's select an LLM. Because we are using tool-calling, we will need a model that supports a tool-calling feature. See [this table](/src/oss/integrations/chat) for available LLMs.
...
Following the [extraction tutorial](/src/oss/tutorials/extraction), we use the `.with_structured_output` method to structure model outputs according to the desired schema:
