...
In some situations you may want to implement a custom [parser](/src/oss/concepts/output_parsers) to structure the model output into a custom format.

There are two ways to implement a custom parser:

1. Using `RunnableLambda` or `RunnableGenerator` in [LCEL](/src/oss/concepts/lcel) -- we strongly recommend this for most use cases
