...
In many [Q\&A applications](/oss/concepts/rag) we want to allow the user to have a back-and-forth conversation, meaning the application needs some sort of "memory" of past questions and answers, and some logic for incorporating those into its current thinking.
...
1. [Chains](/oss/how_to/qa_chat_history_how_to/#chains), in which we always execute a retrieval step;
2. [Agents](/oss/how_to/qa_chat_history_how_to/#agents), in which we give an LLM discretion over whether and how to execute a retrieval step (or multiple steps).
...
We'll use OpenAI embeddings and an InMemory vector store in this walkthrough, but everything shown here works with any [Embeddings](/oss/concepts/embedding_models), and [VectorStore](/oss/concepts/vectorstores) or [Retriever](/oss/concepts/retrievers).
...
A [chat model](/oss/integrations/chat):
...
An [embedding model](/oss/integrations/text_embedding):
...
And a [vector store](/oss/integrations/vectorstores):
...
The [RAG Tutorial](/oss/tutorials/rag) indexes an [LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/) blog post by Lilian Weng. We will repeat that here. Below we load the content of the page, split it into sub-documents, and embed the documents into our [vector store](/oss/concepts/vectorstores):
...
As detailed in [Part 2](/oss/tutorials/qa_chat_history) of the RAG tutorial, we can naturally support a conversational experience by representing the flow of the RAG application as a sequence of [messages](/oss/concepts/messages):
...
We will use [tool-calling](/oss/concepts/tool_calling) to facilitate this, which additionally allows the query to be generated by the LLM. We can build a [tool](/oss/concepts/tools) to execute the retrieval step:
...
For a detailed walkthrough of LangChain's conversation memory abstractions, visit the [How to add message history (memory)](/oss/how_to/message_history) LCEL page.

To learn more about agents, head to the [Agents Modules](/oss/tutorials/agents).
