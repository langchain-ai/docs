...
When splitting documents for [retrieval](/src/oss/concepts/retrieval), there are often conflicting desires:
...
In this mode, we want to retrieve the full documents. Therefore, we only specify a child [splitter](/src/oss/concepts/text_splitters).
