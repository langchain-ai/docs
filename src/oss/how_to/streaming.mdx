...
<Info>
  ...
  * [Output parsers](/src/oss/concepts/output_parsers)
</Info>

Streaming is critical in making applications based on LLMs feel responsive to end-users.

Important LangChain primitives like [chat models](/src/oss/concepts/chat_models), [output parsers](/src/oss/concepts/output_parsers), [prompts](/src/oss/concepts/prompt_templates), [retrievers](/src/oss/concepts/retrievers), and [agents](/src/oss/concepts/agents) implement the LangChain [Runnable Interface](/src/oss/concepts/runnables).
...
<Info>
  For a higher-level overview of streaming techniques in LangChain, see [this section of the conceptual guide](/src/oss/concepts/streaming).
</Info>
...
Note that we're getting streaming output even though we're using `parser` at the end of the chain above. The `parser` operates on each streaming chunk individidually. Many of the [LCEL primitives](/src/oss/how_to/#langchain-expression-language-lcel) also support this kind of transform-style passthrough streaming, which can be very convenient when constructing apps.

Custom functions can be [designed to return generators](/src/oss/how_to/functions/#streaming), which are able to operate on streams.

Certain runnables, like [prompt templates](/src/oss/how_to/#prompt-templates) and [chat models](/src/oss/how_to/#chat-models), cannot process individual chunks and instead aggregate all previous steps. Such runnables can interrupt the streaming process.
...
To learn more, check out the other how-to guides in this section, or the [conceptual guide on Langchain Expression Language](/src/oss/concepts/lcel).
