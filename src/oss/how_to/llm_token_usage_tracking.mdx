...
<Warning>
  The callback handler does not currently support streaming token counts for legacy language models (e.g., `langchain_openai.OpenAI`). For support in a streaming context, refer to the corresponding guide for chat models [here](/src/oss/how_to/chat_token_usage_tracking).
</Warning>
...
<Warning>
  ...
  * Use chat models as described in [this guide](/src/oss/how_to/chat_token_usage_tracking);
  * Implement a [custom callback handler](/src/oss/how_to/custom_callbacks) that uses appropriate tokenizers to count the tokens;
