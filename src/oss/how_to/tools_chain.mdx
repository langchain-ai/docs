...
In this guide, we will go over the basic ways to create Chains and Agents that call [Tools](/src/oss/concepts/tools). Tools can be just about anything — APIs, functions, databases, etc. Tools allow us to extend the capabilities of a model beyond just outputting text/messages. The key to using models with tools is correctly prompting a model and parsing its response so that it chooses the right tools and provides the right inputs for them.
...
First, we need to create a tool to call. For this example, we will create a custom tool from a function. For more information on creating custom tools, please see [this guide](/src/oss/how_to/custom_tools).
...
![chain](/images/oss/how_to/tools_chain/assets/images/tool_chain-3571e7fbc481d648aff93a2630f812ab.svg)

### Tool/function calling

One of the most reliable ways to use tools with LLMs is with [tool calling](/src/oss/concepts/tool_calling) APIs (also sometimes called function calling). This only works with models that explicitly support tool calling. You can see which models support tool calling [here](/src/oss/integrations/chat), and learn more about how to use tool calling in [this guide](/src/oss/how_to/function_calling).
...
Chains are great when we know the specific sequence of tool usage needed for any user input. But for certain use cases, how many times we use tools depends on the input. In these cases, we want to let the model itself decide how many times to use tools and in what order. [Agents](/src/oss/concepts/agents) let us do just this.

We'll demonstrate a simple example using a LangGraph agent. See [this tutorial](/src/oss/tutorials/agents) for more detail.

![agent](/images/oss/how_to/tools_chain/assets/images/tool_agent-d25fafc271da3ee950ac1fba59cdf490.svg)
