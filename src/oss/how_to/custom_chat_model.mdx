...
In this guide, we'll learn how to create a custom [chat model](/src/oss/concepts/chat_models) using LangChain abstractions.
...
As an bonus, your LLM will automatically become a LangChain [Runnable](/src/oss/concepts/runnables) and will benefit from some optimizations out of the box (e.g., batch via a threadpool), async support, the `astream_events` API, etc.

## Inputs and outputs

First, we need to talk about **[messages](/src/oss/concepts/messages)**, which are the inputs and outputs of chat models.

### Messages

Chat models take messages as inputs and return a message as output.

LangChain has a few [built-in message types](/src/oss/concepts/messages):
...
Next, check out the other how-to guides chat models in this section, like [how to get a model to return structured output](/src/oss/how_to/structured_output) or [how to track chat model token usage](/src/oss/how_to/chat_token_usage_tracking).
