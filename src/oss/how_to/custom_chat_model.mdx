...
As an bonus, your LLM will automatically become a LangChain [Runnable](/oss/concepts/runnables) and will benefit from some optimizations out of the box (e.g., batch via a threadpool), async support, the `astream_events` API, etc.
...
Next, check out the other how-to guides chat models in this section, like [how to get a model to return structured output](/oss/how_to/structured_output) or [how to track chat model token usage](/oss/how_to/chat_token_usage_tracking).
