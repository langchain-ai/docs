...
We can now load a [retriever](/src/oss/concepts/retrievers) and construct our [prompt](/src/oss/concepts/prompt_templates):
...
Now that we've got a [model](/src/oss/concepts/chat_models), [retriever](/src/oss/concepts/retrievers) and [prompt](/src/oss/concepts/prompt_templates), let's chain them all together. Following the how-to guide on [adding citations](/src/oss/how_to/qa_citations) to a RAG application, we'll make it so our chain returns both the answer and the retrieved Documents. This uses the same [LangGraph](/src/oss/concepts/architecture/#langgraph) implementation as in the [RAG Tutorial](/src/oss/tutorials/rag).
...
This approach effectively updates our `retrieve` step to compress the documents. Let's first select an [embedding model](/src/oss/integrations/text_embedding):
