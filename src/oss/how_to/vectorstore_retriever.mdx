...
A vector store retriever is a [retriever](/src/oss/concepts/retrievers) that uses a [vector store](/src/oss/concepts/vectorstores) to retrieve documents. It is a lightweight wrapper around the vector store class to make it conform to the retriever [interface](/src/oss/concepts/runnables). It uses the search methods implemented by a vector store, like similarity search and MMR, to query the texts in the vector store.
