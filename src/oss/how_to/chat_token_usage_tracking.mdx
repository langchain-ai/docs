...
You can also enable streaming token usage by setting `stream_usage` when instantiating the chat model. This can be useful when incorporating chat models into LangChain [chains](/oss/concepts/lcel): usage metadata can be monitored when [streaming intermediate steps](/oss/how_to/streaming/#using-stream-events) or using tracing software such as [LangSmith](https://docs.smith.langchain.com/).
...
Next, check out the other how-to guides chat models in this section, like [how to get a model to return structured output](/oss/how_to/structured_output) or [how to add caching to your chat models](/oss/how_to/chat_model_caching).
