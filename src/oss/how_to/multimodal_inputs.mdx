...
Here we demonstrate how to pass [multimodal](/src/oss/concepts/multimodality) input directly to models.
...
Below, we demonstrate the cross-provider standard. See [chat model integrations](/src/oss/integrations/chat) for detail on native formats for specific providers.

<Note>
  Most chat models that support multimodal **image** inputs also accept those values in OpenAI's [Chat Completions format](https://platform.openai.com/oss/guides/images?api-mode=chat):
...
Some providers (including [OpenAI](/src/oss/integrations/chat/openai), [Anthropic](/src/oss/integrations/chat/anthropic), and [Google Gemini](/src/oss/integrations/chat/google_generative_ai)) will also accept images from URLs directly.
...
Some providers (including [OpenAI](/src/oss/integrations/chat/openai), [Anthropic](/src/oss/integrations/chat/anthropic), and [Google Gemini](/src/oss/integrations/chat/google_generative_ai)) will accept PDF documents.
...
Some providers (specifically [Anthropic](/src/oss/integrations/chat/anthropic)) will also accept documents from URLs directly.
...
Some providers (including [OpenAI](/src/oss/integrations/chat/openai) and [Google Gemini](/src/oss/integrations/chat/google_generative_ai)) will accept audio inputs.
...
Some providers will support or require additional fields on content blocks containing multimodal data. For example, Anthropic lets you specify [caching](/src/oss/integrations/chat/anthropic/#prompt-caching) of specific content to reduce token consumption.

To use these fields, you can:

1. Store them on directly on the content block; or
2. Use the native format supported by each provider (see [chat model integrations](/src/oss/integrations/chat) for detail).
