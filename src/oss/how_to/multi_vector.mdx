...
Below we walk through an example. First we instantiate some documents. We will index them in an (in-memory) [Chroma](/oss/integrations/providers/chroma) vector store using [OpenAI](https://python.langchain.com/oss/integrations/text_embedding/openai/) embeddings, but any LangChain vector store or embeddings model will suffice.
...
We construct a simple [chain](/oss/how_to/sequence) that will receive an input [Document](https://python.langchain.com/api_reference/core/documents/langchain_core.documents.base.Document.html) object and generate a summary using a LLM.
...
Below, we use the [with\_structured\_output](/oss/how_to/structured_output) method to structure the LLM output into a list of strings.
