...
<Info>
  ...
  * [LangChain Tools](/src/oss/concepts/tools)
  * [Custom tools](/src/oss/how_to/custom_tools)
  * [Using stream events](/src/oss/how_to/streaming/#using-stream-events)
  * [Accessing RunnableConfig within a custom tool](/src/oss/how_to/tool_configure)
</Info>

If you have [tools](/src/oss/concepts/tools) that call [chat models](/src/oss/concepts/chat_models), [retrievers](/src/oss/concepts/retrievers), or other [runnables](/src/oss/concepts/runnables), you may want to access [internal events](https://python.langchain.com/oss/how_to/streaming/#event-reference) from those runnables or configure them with additional properties. This guide shows you how to manually pass parameters properly so that you can do this using the `astream_events()` method.
...
But if you wanted to access the raw output from the chat model rather than the full tool, you might try to use the [`astream_events()`](/src/oss/how_to/streaming/#using-stream-events) method and look for an `on_chat_model_end` event. Here's what happens:
...
* Pass [runtime values to tools](/src/oss/how_to/tool_runtime)
* Pass [tool results back to a model](/src/oss/how_to/tool_results_pass_to_model)
* [Dispatch custom callback events](/src/oss/how_to/callbacks_custom_events)

You can also check out some more specific uses of tool calling:

* Building [tool-using chains and agents](/src/oss/how_to/#tools)
* Getting [structured outputs](/src/oss/how_to/structured_output) from models
