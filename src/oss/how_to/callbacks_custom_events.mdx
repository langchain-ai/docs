...
<Info>
  ...
  * [Astream Events API](/src/oss/concepts/streaming/#astream_events) the `astream_events` method will surface custom callback events.
</Info>

In some situations, you may want to dispatch a custom callback event from within a [Runnable](/src/oss/concepts/runnables) so it can be surfaced in a custom callback handler or via the [Astream Events API](/src/oss/concepts/streaming/#astream_events).
...
The most useful way to consume custom events is via the [Astream Events API](/src/oss/concepts/streaming/#astream_events).
...
You've seen how to emit custom events, you can check out the more in depth guide for [astream events](/src/oss/how_to/streaming/#using-stream-events) which is the easiest way to leverage custom events.
