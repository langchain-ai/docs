...
* Using [language models](/src/oss/concepts/chat_models), in particular their tool calling ability
* Creating a [Retriever](/src/oss/concepts/retrievers) to expose specific information to our agent
* Using a Search [Tool](/src/oss/concepts/tools) to look up things online
* [`Chat History`](/src/oss/concepts/chat_history), which allows a chatbot to "remember" past interactions and take them into account when responding to follow-up questions.
...
We first need to create the tools we want to use. We will use two tools: [Tavily](/src/oss/integrations/tools/tavily_search) (to search online) and then a retriever over a local index we will create
...
We will also create a retriever over some data of our own. For a deeper explanation of each step here, see [this tutorial](/src/oss/tutorials/rag).
...
Now that we have defined the tools and the LLM, we can create the agent. We will be using a tool calling agent - for more information on this type of agent, as well as other options, see [this guide](/src/oss/concepts/agents).
...
<Info>
  This section covered building with LangChain Agents. They are fine for getting started, but past a certain point you will likely want flexibility and control which they do not offer. To develop more advanced agents, we recommend checking out [LangGraph](/src/oss/concepts/architecture/#langgraph)
</Info>

If you want to continue using LangChain agents, some good advanced guides are:

* [How to use LangGraph's built-in versions of `AgentExecutor`](/src/oss/how_to/migrate_agent)
* [How to create a custom agent](https://python.langchain.com/v0.1/oss/modules/agents/how_to/custom_agent/)
* [How to stream responses from an agent](https://python.langchain.com/v0.1/oss/modules/agents/how_to/streaming/)
* [How to return structured output from an agent](https://python.langchain.com/v0.1/oss/modules/agents/how_to/agent_structured/)
