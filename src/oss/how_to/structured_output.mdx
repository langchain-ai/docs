...
For more on few shot prompting when using tool calling, see [here](/oss/how_to/tools_few_shot).
...
For a deeper dive into using output parsers with prompting techniques for structured output, see [this guide](/oss/how_to/output_parser_structured).
