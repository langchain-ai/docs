...
<Info>
  ...
  * [Passing data through](/src/oss/how_to/passthrough)
</Info>

An alternate way of [passing data through](/src/oss/how_to/passthrough) steps of a chain is to leave the current values of the chain state unchanged while assigning a new value under a given key. The [`RunnablePassthrough.assign()`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.passthrough.RunnablePassthrough.html#langchain_core.runnables.passthrough.RunnablePassthrough.assign) static method takes an input value and adds the extra arguments passed to the assign function.

This is useful in the common [LangChain Expression Language](/src/oss/concepts/lcel) pattern of additively creating a dictionary to use as input to a later step.
