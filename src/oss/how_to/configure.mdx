...
<Info>
  ...
  * [The Runnable interface](/src/oss/concepts/runnables)
  * [Chaining runnables](/src/oss/how_to/sequence)
  * [Binding runtime arguments](/src/oss/how_to/binding)
</Info>
...
* A `configurable_fields` method. This lets you configure particular fields of a runnable.
  * This is related to the [`.bind`](/src/oss/how_to/binding) method on runnables, but allows you to specify parameters for a given step in a chain at runtime rather than specifying them beforehand.
...
If using [init\_chat\_model](/src/oss/how_to/chat_models_universal_init) to create a chat model, you can specify configurable fields in the constructor:
...
This method is applicable when [binding tools](/src/oss/concepts/tool_calling) as well:
...
In addition to `.with_config`, we can now include the parameter when passing a configuration directly. See example below, where we allow the underlying model temperature to be configurable inside of a [langgraph agent](/src/oss/tutorials/agents):
...
You can also use the `.configurable_fields` method on arbitrary [Runnables](/src/oss/concepts/runnables), as shown below:
...
* Using [.bind()](/src/oss/how_to/binding) as a simpler way to set a runnable's runtime parameters
