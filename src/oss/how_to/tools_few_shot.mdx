...
For more complex tool use it's very useful to add [few-shot examples](/src/oss/concepts/few_shot_prompting) to the prompt. We can do this by adding `AIMessage`s with `ToolCall`s and corresponding `ToolMessage`s to our prompt.
