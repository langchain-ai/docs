...
- We don't think reading agent state inside a namespace factory is a good pattern, so there's no direct replacement for `ctx.state`. Namespace info should be read-only and stable for the lifetime of a run, whereas state is mutable and changes step-to-step — deriving a namespace from it risks data ending up under inconsistent keys. If you have a use case that requires it, please [open an issue](https://github.com/langchain-ai/deepagents/issues).
