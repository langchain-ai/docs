...
- Whether files survive beyond a thread depends on your checkpointer: an in-memory checkpointer loses files when the process exits, while a durable checkpointer (such as Postgres or the one used by [LangSmith Deployment](/langsmith/deployment)) retains them as long as the thread exists.
