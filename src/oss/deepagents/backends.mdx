...
Deep agents expose a filesystem surface to the agent using tools like `ls`, `read_file`, `write_file`, `edit_file`, `glob`, and `grep`. These tools operate through a pluggable backend.
...
Here are a few pre-built filesystem backends that you can quickly use with your deep agent:
 ...
 |---|---|
 | [`StateBackend`](#statebackend-ephemeral) | The default, ephemeral filesystem backend for an agent which is stored in `langgraph` state. This filesystem only persists _for a single thread_. |
 | [`FilesystemBackend`](#filesystembackend-local-disk) | Local filesystem persistence which gives the deep agent access to your local machine's filesystem. You can specify the root directory that the agent has access to. Any provided `root_dir` must be an absolute path. |
 | [`CompositeBackend`](#storebackend-langgraph-store) | Durable LangGraph store which gives the agent access to long-term storage that is _persisted across threads_. This is great for storing longer term memories or instructions that are applicable to the agent over multiple executions. |
 | [CompositeBackend](#compositebackend-router) | Composite filesystem backend which comprises ephemeral storage and persisted storage at `/memories/`. The Composite backend is maximally flexible. You can specify different routes in the filesystem to point towards different backends. |

<Warning>
    If your deep agent can write to your filesystem's disk, we strongly recommend you use [human-in-the-loop workflows](/oss/deepagents/human-in-the-loop) to require human approval before execution of sensitive tool operations.
</Warning>
...
**How it works:**
- Reads/writes real files in a configurable `root_dir`.
...
- When you have multiple sources of information that you want to provide to your agent as part of a single filesystem.
    - Example: You have long-term memories stored under /memories/ in one Store and you also have a custom backend that has documentation accessible at /docs/.

## Specify a backend

- Pass a backend to `create_deep_agent(backend=...)`. The filesystem middleware will then use it for all tooling.
- You can pass either:
...
Build a custom backend to project a remote or database filesystem (for example with S3 or Postgres) into the tools namespace.
