...
agent = create_deep_agent(
    backend=CompositeBackend(
        default=StateBackend(),
        routes={"/memories/": StoreBackend(namespace=lambda rt: (rt.context.user_id,))},
    ),
)
```
:::
...
In `deepagents>=0.5.2` (Python) and `deepagents>=1.9.1` (TypeScript), namespace factories receive a LangGraph @[`Runtime`] directly instead of a `BackendContext` wrapper. The old `BackendContext` form still works but emits a deprecation warning and will be removed in `deepagents>=0.7`.

**What changed:**

- The factory argument is now a `Runtime`, not a `BackendContext`.
- Drop the `.runtime` accessor — for example, `ctx.runtime.context.user_id` becomes `rt.context.user_id`.
- `ctx.state` is no longer available on the factory argument. If you need state, read it from the store or pass it through `Runtime.context`.
...
# After
StoreBackend(
    namespace=lambda rt: (rt.context.user_id,),  # [!code ++]
)
```
:::
...
// After
new StoreBackend({
  namespace: (rt) => [rt.context.userId],  // [!code ++]
});
```
:::
