...
Implement a custom backend to connect Deep Agents to any storage system — databases, object stores, remote filesystems, and more. See [community-built backends](/oss/python/integrations/backends) for examples.

### Use a virtual filesystem

Project a remote or database filesystem (e.g., S3 or Postgres) into the agent's tools namespace by implementing the backend protocol.
