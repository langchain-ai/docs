...
```mermaid
graph TB
    Tools[Filesystem Tools] --> Protocol[Backend Protocol]

    Protocol --> State[StateBackend]
    Protocol --> Disk[FilesystemBackend]
    Protocol --> Store[StoreBackend]
    Protocol --> Composite[CompositeBackend]
    Protocol --> Custom[Custom Backend]

    State --> |ephemeral| StateStore[(Agent State)]
    Disk --> |persistent| LocalFS[(Local Disk)]
    Store --> |cross-thread| LGStore[(LangGraph Store)]

    Composite --> Router{Path Router}
    Router --> |/memories/*| Store
    Router --> |/tmp/*| State
    Router --> |other| Disk

    Custom --> |S3, DB, etc| VirtualFS[(Virtual FS)]

    style Protocol fill:#e1f5ff
    style Composite fill:#fff4e1
