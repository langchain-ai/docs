...
| **`subagents/`** | Specialized [subagents](/oss/deepagents/subagents) the main agent can delegate to. Each subdirectory contains its own `deepagents.toml` and `AGENTS.md`. See [subagents](#subagents). |
...
| `subagents/` | [Subagents](#subagents) the main agent can delegate to. Each subdirectory must contain a `deepagents.toml` and `AGENTS.md`. Auto-discovered at bundle time. | No |
...
Skills, user memories, subagents, MCP servers, and model dependencies are auto-detected from the project layout — you don't declare them in `deepagents.toml`:
...
A subagent's `deepagents.toml` only supports the `[agent]` section. The `[sandbox]` section is not allowed — subagents share the main agent's sandbox.
...
<ResponseField name="response_format" type="string">
    JSON Schema string to constrain the subagent's output to structured JSON. The main agent receives the result as parsed JSON instead of free-form text.
</ResponseField>
...
Use `response_format` to constrain a subagent's output to a JSON schema. The main agent receives the result as structured JSON instead of free-form text:

```toml subagents/researcher/deepagents.toml
[agent]
name = "researcher"
description = "Researches market trends and returns structured findings"
response_format = """
{
  "type": "object",
  "properties": {
  ...
  "required": ["executive_summary", "key_competitors"]
}
"""
```
...
The bundler validates subagents at bundle time and rejects invalid configurations:

- Missing `deepagents.toml` or `AGENTS.md`
- Empty `description`
- Disallowed sections (`[sandbox]`, `[[async_subagents]]`)
- Nested `subagents/` directories inside a subagent
- Invalid `response_format` JSON
- Duplicate subagent names
- MCP configs using `stdio` transport
...
- **Subagent memory is isolated.** Subagents cannot read the main agent's `AGENTS.md`, skills, or user memory. Use the subagent's own `AGENTS.md` and `skills/` directory for its context.
