...
| **`auth`** | Optional authentication configuration. Controls who can access the deployed agent — choose between Clerk, Supabase, or anonymous providers. Required when `[frontend].enabled = true`; otherwise LangSmith Deployment's default `x-api-key` requirement applies. See [`[auth]`](#auth). |
