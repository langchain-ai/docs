...
| `../.env` | Environment variables (API keys, secrets). Placed in the **parent** directory of the project root so secrets stay outside the agent source tree. | No |
...
The `.env` file lives one level above the project root so it is never bundled as a seed file. The deploy command automatically picks it up if present.
...
When omitted or set to `provider = "none"`, the runtime falls back to an in-process backend and tools like `execute` become no-ops.
...
| --- | --- | --- |
| None | `"none"` | No sandbox. The runtime uses an in-process backend and `execute` becomes a no-op. |
