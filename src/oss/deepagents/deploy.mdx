...
`deepagents deploy` takes your agent configuration and files and deploys them together as a [LangSmith Deployment](/langsmith/deployment). It's the fastest path to production for agents whose runtime needs (memory, sandboxes, MCP tools, auth, frontend) can be expressed in a single `deepagents.toml`. For the conceptual overview of what production deployment entails—multi-tenancy, durability, guardrails, and frontend integration—see [Going to production](/oss/deepagents/going-to-production).
...
For a deeper discussion of sandbox lifecycle in multi-tenant deployments, including TTL, snapshots, and cleanup, see [Going to production: Lifecycle](/oss/deepagents/going-to-production#lifecycle).
...
Every frontend uses one of three authentication providers — Clerk, Supabase, or anonymous (see [`[auth]`](#auth)). For UI patterns specific to deep agents (subagent cards, todo lists, custom state rendering) and for connecting your own React/Vue/Svelte/Angular frontend instead of the bundled UI, see [Going to production: Frontend](/oss/deepagents/going-to-production#frontend).
...
User memory gives each user their own writable `AGENTS.md` that persists across conversations. This is the CLI's built-in equivalent of the user-scoped memory pattern described in [Going to production: Memory](/oss/deepagents/going-to-production#memory); the CLI wires up a per-user `StoreBackend` namespace for you. To enable it, create a `user/` directory at your project root:
