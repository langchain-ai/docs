...
<Tabs>
    ...
    <Tab title="You.com">
        Install the `@youdotcom-oss/langchain` package and set `YDC_API_KEY`. Get an API key at [you.com/platform](https://you.com/platform).
        ...
        const internetSearch = tool(
          async ({ query, count = 5 }: { query: string; count?: number }) => {
            const youTool = youSearch({ apiKey: process.env.YDC_API_KEY });
            return await youTool.invoke({ query, count });
          },
          {
