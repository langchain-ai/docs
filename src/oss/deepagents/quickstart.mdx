...
<Tabs>
    <Tab title="Google">
        ...
        # Or, if you use You.com for search:
...
<Tabs>
    ...
    <Tab title="You.com">
        Install the `langchain-youdotcom` package and set `YDC_API_KEY`. Get an API key at [you.com/platform](https://you.com/platform).
        ...
        `YouSearchTool` reads `YDC_API_KEY` from the environment. Use `count` to cap results, `freshness` to limit results by recency, and `livecrawl` to fetch full page content.
    </Tab>
</Tabs>
:::
...
<Tabs>
    ...
    <Tab title="You.com">
        Install the `@youdotcom-oss/langchain` package and set `YDC_API_KEY`. Get an API key at [you.com/platform/api-keys](https://you.com/platform/api-keys).

        ```ts
        import { createYouClient } from "@youdotcom-oss/langchain";

        const youClient = await createYouClient();
        const [internetSearch] = (await youClient.getTools()).filter(
          (t) => t.name === "you-search",
        );
        ```

        `createYouClient()` reads `YDC_API_KEY` from the environment and connects to the hosted You.com MCP server. The `you-search` tool accepts a `query`, optional `count`, `freshness`, `country`, and `livecrawl` arguments.
    </Tab>
</Tabs>
:::
