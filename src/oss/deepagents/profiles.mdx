...
import ProfilesRegistrationPy from '/snippets/code-samples/profiles-registration-py.mdx';
import ProfilesRegistrationJs from '/snippets/code-samples/profiles-registration-js.mdx';
import ProfilesReregisterHarnessPy from '/snippets/code-samples/profiles-reregister-harness-py.mdx';
import ProfilesReregisterHarnessJs from '/snippets/code-samples/profiles-reregister-harness-js.mdx';
import ProfilesReregisterProviderPy from '/snippets/code-samples/profiles-reregister-provider-py.mdx';
...
Deep Agents ships **built-in profiles**: default settings included with the SDK and registered automatically. Registering under the same key customizes those settings using the [merge rules](#merge-semantics) below.
...
Register profiles before creating an agent. Pass a model string or a model object you construct yourself; see [Configure model parameters](/oss/deepagents/models#configure-model-parameters) for examples. Harness profiles apply in both cases.
...
:::python
Only the first colon separates the provider from the model identifier. For the hypothetical key `my_provider:my-model:tag`, the provider is `my_provider` and the complete model identifier is `my-model:tag`. Additional colons belong to the identifier, including consecutive colons in a Bedrock ARN.
:::
...
Constructing `my_provider:my-model` uses `temperature=0` and inherits `timeout=30`. Other models from `my_provider` use `temperature=0.7` and `timeout=30`.
