...
Deep Agents ships **built-in profiles**: default settings registered automatically for selected providers and models. Registering under the same key customizes those settings using the [merge rules](#merge-semantics) below.
...
:::python
Register profiles before calling `create_deep_agent`. You can pass either a model string or a model object you construct yourself; see [Configure model parameters](/oss/deepagents/models#configure-model-parameters) for examples. Harness profiles apply in both cases. Provider profiles supply construction settings only when you pass a string.
:::
...
An agent using `my_provider:my-model:tag` excludes `execute` and receives the 100-word suffix. Other models from `my_provider` exclude `execute` and receive the 500-word suffix.
:::
