...
<ResponseField name="base_system_prompt" type="string">
    Replace the base Deep Agents system prompt (`CUSTOM` in the [prompt-assembly cheat sheet](/oss/deepagents/customization#prompt-assembly)).
</ResponseField>

<ResponseField name="system_prompt_suffix" type="string">
    Append text to the assembled base prompt (`SUFFIX` in the [prompt-assembly cheat sheet](/oss/deepagents/customization#prompt-assembly)); applied to the main agent, declarative subagents, and the auto-added general-purpose subagent.
</ResponseField>
