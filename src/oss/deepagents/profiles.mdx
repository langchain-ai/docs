...
<Warning>
    ...
    To run an agent without the `task` tool, see [Default subagent](/oss/deepagents/subagents#default-subagent) — set `general_purpose_subagent=GeneralPurposeSubagentProfile(enabled=False)` and pass no synchronous subagents via `subagents=`. With nothing for it to back, `SubAgentMiddleware` isn't added and the scaffolding guard never fires. Async subagents are unaffected.
</Warning>
