...
:::js
<Note>
    Harness profiles are currently a Python-only feature of Deep Agents. They are not yet available in the TypeScript SDK. To tune model behavior in TypeScript, pass `systemPrompt` or `middleware` directly to `createDeepAgent`.
</Note>
:::
