...
- `base_system_prompt` — replace the base Deep Agents system prompt (`CUSTOM` in the [prompt-assembly cheat sheet](/oss/deepagents/customization#prompt-assembly))
- `system_prompt_suffix` — append text to the assembled base prompt (`SUFFIX` in the cheat sheet); applied to the main agent, declarative subagents, and the auto-added general-purpose subagent
...
- `general_purpose_subagent` — disable, rename, or re-prompt the general-purpose subagent. When this field's `system_prompt` is set alongside `base_system_prompt`, the general-purpose-specific subagent prompt wins — see [General-purpose subagent prompt](/oss/deepagents/customization#general-purpose-subagent-prompt)
