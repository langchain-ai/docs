...
| `edit_file` | Make targeted edits to existing files | required |
| `glob` | Find files matching a pattern (e.g., `**/*.py`) | - |
| `grep` | Search for text patterns across files | - |
| `shell` | Execute shell commands (local mode) | required |
| `execute` | Execute commands in remote sandbox (sandbox mode) | required |
| `web_search` | Search the web using Tavily API | required |
| `fetch_url` | Fetch and convert web pages to markdown | required |
| `task` | Delegate work to subagents for parallel execution | required |
...
To skip prompts you can toggle auto-approve or start the deep agent with the `auto-approve` option:
