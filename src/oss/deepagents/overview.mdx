...
When using **Deep Agents SDK**:
- **Specify where memory will be located**: Control where your deep agent can write to in the filesystem by configuring the deep agent's [memory](/oss/deepagents/long-term-memory)
- **Prevent prompt injection**: If you use filesystem [backend](/oss/deepagents/backends), we strongly recommend you enable human-in-the-loop workflows](/oss/deepagents/human-in-the-loop) to require human approval before execution of sensitive tool operations.

When using **Deep Agents CLI**:
- Use [sandboxes for remote code execution](/oss/deepagents/cli#use-remote-sandboxes) to avoid any side-effects on your system.
