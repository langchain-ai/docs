...
Deep Agents support streaming updates from both the coordinator and every delegated subagent. Use `stream_events` (or `streamEvents` in JS) with the v3 API to get typed projections—separate iterators for subagents, messages, tool calls, and values—so you can consume each independently.

<Tip>
For complete streaming documentation including LLM token streaming, tool-call streaming, custom update events, and concurrent consumption, see [Event streaming](/oss/deepagents/event-streaming).
</Tip>
...
agent = create_deep_agent(
    model="google_genai:gemini-3.5-flash",
    system_prompt="You are a project coordinator. Delegate research tasks to your subagent.",
    subagents=[
        {
            "name": "research-agent",
...
# Iterate top-level coordinator messages
for message in stream.messages:
    print("[coordinator]", message.text)

# Iterate each delegated subagent task
for subagent in stream.subagents:
    print(f"[{subagent.name}] started")
    for message in subagent.messages:
        print(f"[{subagent.name}]", message.text)
    print(f"[{subagent.name}] status: {subagent.status}")
```
:::
...
const agent = createDeepAgent({
  systemPrompt: "You are a project coordinator. Delegate research tasks to your subagent.",
  subagents: [
    {
      name: "research-agent",
      description: "Used to research more in depth questions",
...
2. Switch the view to **Runs** (instead of Traces or Threads) to see individual spans.
