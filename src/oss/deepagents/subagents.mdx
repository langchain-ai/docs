...
<Warning>
    **The word "workflow" is a trigger.** The built-in interpreter system prompt treats a "workflow" as a signal to organize work through the interpreter—dispatching subagents with `task()` from code. Phrasing a request as a "workflow" strongly nudges the agent toward dynamic orchestration, so use it deliberately: include it when you want fan-out from code, and avoid it when you want a single, direct delegation.
</Warning>
