...
Subagents **must** be created with either `create_agent` or with a custom LangGraph graph built with the [Graph API](/oss/langgraph/graph-api).
The type of the subagent must be `graph: Pregel`.
