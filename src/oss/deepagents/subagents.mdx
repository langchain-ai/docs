...
<ParamField body="model" type="str | BaseChatModel">
    Override the main agent's model. You can pass either a model identifier string like `'openai:gpt-5'` (using the `'provider:model'` format) or a LangChain chat model object, matching the options in the main `model` configuration.
    ...
    { model: (await initChatModel("gpt-5")), ... }
    ```
