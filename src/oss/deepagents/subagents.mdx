...
<Tip>
    **CLI users:** You can also define subagents as `AGENTS.md` files on disk instead of in code. The `name`, `description`, and `model` fields map to YAML frontmatter, and the markdown body becomes the `system_prompt`. See [Use subagents in the CLI](/oss/deepagents/cli/subagents) for the file format.
