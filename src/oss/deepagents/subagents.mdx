...
```mermaid
graph TB
    Main[Main Agent<br/>Clean Context] --> |task tool| Decision{Which<br/>subagent?}

    Decision --> |"research"| Research[Research Subagent]
    Decision --> |"code"| Code[Code Subagent]
    Decision --> |"general"| General[General Subagent]

    Research --> |web search<br/>file reads<br/>analysis| RWork[Isolated<br/>Work]
    Code --> |write code<br/>run tests<br/>debug| CWork[Isolated<br/>Work]
    General --> |multi-step<br/>tasks| GWork[Isolated<br/>Work]

    RWork --> |final result only| Main
    CWork --> |final result only| Main
    GWork --> |final result only| Main
    ...
    style RWork fill:#ffebee
    style CWork fill:#ffebee
    style GWork fill:#ffebee
```
