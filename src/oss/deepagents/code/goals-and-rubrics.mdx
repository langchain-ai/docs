...
How the draft criteria are handled depends on your [approval mode](/oss/deepagents/code/approval-modes):

- **Manual**: The inline review always opens. You can accept the proposal, edit the criteria, request another revision, or cancel it.
- **Auto**: The inline review opens by default. Set `goals.auto_accept_criteria = true` in `~/.deepagents/config.toml` to apply generated criteria without review. The first time you create or amend a goal in Auto mode, a one-time prompt asks which behavior you prefer and saves your choice; an explicit config value suppresses the prompt.
- **YOLO**: Generated criteria are always applied without review.

<Note>
    `goals.auto_accept_criteria` requires `deepagents-code>=0.1.46`.
</Note>
