...
Define custom synchronous [subagents](/oss/deepagents/subagents) as markdown files so Deep Agents Code can delegate specialized tasks to them.

<Note>
    Async subagents are functional in Deep Agents Code but not yet documented — setup requires independently building, deploying, and maintaining an Agent Protocol-compliant server (a LangSmith Deployment or self-hosted equivalent) and pointing `dcode` at it via `config.toml`. There is no guided setup or TUI surfacing for this flow yet. Full documentation is planned once the setup experience improves.
</Note>
...
Project subagents override user subagents with the same name (see [precedence rules](/oss/deepagents/code/data-locations#subagents)).

The frontmatter requires `name` and `description` (same as the [`SubAgent` dictionary spec](/oss/deepagents/subagents#subagent-dictionary-based)). The markdown body becomes the subagent's `system_prompt`. In addition to the base spec, `AGENTS.md` files support an optional `model` frontmatter field that overrides the main agent's model for this subagent. Uses the `provider:model-name` format (e.g., `anthropic:claude-opus-4-8`, `openai:gpt-5.5`). Omit to inherit the main agent's model.
