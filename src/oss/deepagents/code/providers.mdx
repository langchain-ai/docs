...
Other cloud providers with open weights models include **Fireworks** (`langchain-fireworks`), **Together** (`langchain-together`), **OpenRouter** (`langchain-openrouter`), and **Hugging Face** (`langchain-huggingface`). See the [Provider reference](#provider-reference) for credentials and package names.
