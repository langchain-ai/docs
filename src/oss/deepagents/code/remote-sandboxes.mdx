...
description: Run Deep Agents Code tool execution in LangSmith, Daytona, Modal, Runloop, Vercel, AgentCore, or E2B sandboxes. Install provider dependencies, set credentials, and use flags and setup scripts.
---
...
<Note>
    Using E2B requires dcode 0.1.19 or newer and `langchain-e2b>=0.0.4`.
</Note>

<Steps>
    <Step title="Install provider dependency" icon="download">
        Some providers ship as optional extras. E2B ships as a third-party package. Install a provider from within a session with `/install`, or from the shell with `dcode --install`:
        ...
        <Tabs>
            ...
            <Tab title="E2B">
                ...
                This installs `langchain-e2b` into the same environment that runs `dcode`, so Deep Agents Code can discover the `e2b` sandbox provider entry point.
            </Tab>
        </Tabs>

        To install support for every built-in sandbox provider at once, use the `all-sandboxes` extra: `/install all-sandboxes` in a session, or `dcode --install all-sandboxes` from the shell. E2B is provided by the separate `langchain-e2b` package.
    </Step>
    ...
    <Step title="Set provider credentials" icon="key">
        <Tabs>
            ...
            <Tab title="E2B">
                ...
                To scope the key to Deep Agents Code only, set `DEEPAGENTS_CODE_E2B_API_KEY`. When both variables are set, `DEEPAGENTS_CODE_E2B_API_KEY` takes precedence.
            </Tab>
        </Tabs>
    </Step>
...
| Flag | Description |
|------|-------------|
| `--sandbox TYPE` | Sandbox provider to use. Built-ins: `langsmith`, `agentcore`, `modal`, `daytona`, `runloop`, `vercel` (default: `none`). [Third-party](#third-party-providers) providers such as `e2b` and [config-declared](#config-declared-providers) providers are also accepted. Pass `--sandbox` with no value to use `[sandboxes].default` from your config |
...
# Install and use the E2B provider package
dcode --install langchain-e2b --package
export E2B_API_KEY="your-key"
dcode --sandbox e2b
```
...
The built-in providers above are not the only options. Deep Agents Code discovers sandbox providers from three sources, so you can use providers shipped by other packages or declare your own without changing Deep Agents Code:
...
The `langchain-e2b` package registers the `e2b` provider in that entry-point group (see [sandbox integrations](/oss/integrations/sandboxes)). After you install the package into the `dcode` environment with `dcode --install langchain-e2b --package`, Deep Agents Code discovers it automatically and accepts `dcode --sandbox e2b`.
