...
The `langchain-baponi` package publishes a `baponi` provider the same way. Install it, set your API key, then select it:
...
Baponi supports reattaching to a sandbox by id with `--sandbox-id`; its working directory is `/home/baponi`. To use a self-hosted Baponi deployment, set `BAPONI_BASE_URL` to your cluster endpoint before running `dcode`.
