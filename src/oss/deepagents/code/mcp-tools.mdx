    ...
    <Accordion title="stdio (default)">
        **Required:** `command`. **Optional:** `args`, `env`, plus the shared [tool-filter fields](#tool-filtering).
        ...
        <ResponseField name="command" type="string" required>
            The executable to run. Supports `${VAR}` and `${VAR:-default}` references to parent-shell environment variables. See [Environment variable interpolation](#environment-variable-interpolation).
        </ResponseField>

        <ResponseField name="args" type="string[]">
            Arguments passed to the command. Entries support `${VAR}` and `${VAR:-default}` references to parent-shell environment variables. See [Environment variable interpolation](#environment-variable-interpolation).
        </ResponseField>

        <ResponseField name="env" type="object">
            Environment variables set for the subprocess. Use this to pass API keys and other credentials without exposing them in shell history. Values support `${VAR}` and `${VAR:-default}` references to parent-shell environment variables. See [Environment variable interpolation](#environment-variable-interpolation).
        </ResponseField>
    </Accordion>
...
String values in the `command`, `args`, `env`, `url`, and `headers` fields support `${VAR}` references to parent-shell environment variables, resolved at server activation rather than at config load. One unset variable only fails the server that needs it; the rest still come up.

<Note>
    Environment variable expansion in MCP configs requires `deepagents-code>=0.1.37`.
</Note>
...
- Only the braced forms expand. A bare `$VAR` and a literal `$` pass through untouched.
- `${VAR:-default}` uses `default` when `VAR` is unset or set to an empty value (POSIX `:-` semantics). An empty default (`${VAR:-}`) resolves to the empty string.
- An unset `${VAR}` with no default fails that server with an error naming the field and the variable. Set the variable in the parent shell or in `~/.deepagents/.env`, or provide a default.
- Malformed references such as `${VAR-default}`, `${VAR:default}`, or an unterminated `${VAR` are rejected at config load, so a typo cannot inject a garbage value into a command, URL, or header.

#### Interpolation in stdio configs

Interpolation also works in stdio server definitions, which lets you keep machine-specific paths and per-user values out of the config file:
...
<Warning>
    Project-level `.mcp.json` files are parsed before trust is granted: a malicious config can name any environment variable and exfiltrate its value through a header or a spawned stdio command. Review `${VAR}` references in project configs before approving them. See [Project-level trust](#project-level-trust).
</Warning>
...
<Note>
    Re-authenticating from the `/mcp` viewer requires `deepagents-code>=0.1.58`.
</Note>

You can also re-authenticate an OAuth-backed remote server directly from the `/mcp` viewer: select the server header and press `Enter` (or click the selected header again). The footer shows `Enter re-auth` when the action is available. The viewer runs the same OAuth login flow in the app, always driving a fresh browser authorization rather than reusing a stored token. The action is only offered where it can take effect:

- Servers behind a static `Authorization` header do not offer re-auth, because that header takes precedence over stored OAuth tokens.
- Public servers with no OAuth flow, stdio servers, and disabled servers do not offer it either.
- Selecting an `error` server header opens its detail modal instead.
...
<AccordionGroup>
    ...
    <Accordion title="`${VAR}` references fail">
        Interpolation runs at activation time, so an unset variable only fails the server that needs it. Export the variable in the parent shell, add it to `~/.deepagents/.env`, or give the reference a `${VAR:-default}` fallback. To debug, set `DEEPAGENTS_CODE_DEBUG=1` and inspect the per-session log path printed to stderr on shutdown.
    </Accordion>
</AccordionGroup>
