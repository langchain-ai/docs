...
## LangSmith Gateway credentials

If your organization routes model traffic through the [LangSmith LLM Gateway](/langsmith/llm-gateway-coding-agents), Deep Agents Code recognizes the gateway environment variables as credentials in place of provider-native keys. When both `LANGSMITH_GATEWAY` (set to `true` or a gateway URL; `false`, `0`, and `no` disable it) and a non-empty `LANGSMITH_GATEWAY_API_KEY` are present, Anthropic, Baseten, Fireworks, Google GenAI, and OpenAI models pass credential preflight without their own `*_API_KEY`. Other providers, and providers overridden with `class_path`, still require their own key. See [Direct model access](/langsmith/llm-gateway-direct-model-access) for how to enable the gateway.

<Note>
    LangSmith Gateway credential recognition requires `deepagents-code>=0.1.49`.
</Note>

The gateway counts as a credential source, not an endpoint override. Endpoint and key still resolve as a pair per [Endpoints, keys, and gateways](/oss/deepagents/code/config-file#endpoints-keys-and-gateways): storing your own key with `/auth` or setting a `DEEPAGENTS_CODE_`-prefixed key and endpoint overrides the gateway pair, and a stored key with a blank base URL routes to the provider's default endpoint rather than the gateway.
