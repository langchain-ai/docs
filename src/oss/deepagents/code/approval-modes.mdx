...
Cycle between modes at any time during a session with `Shift+Tab`. By default the cycle runs Manual → Auto → YOLO → Manual.
...
A session launched in YOLO exits to Manual when you press `Shift+Tab`. From there the default cycle continues through Auto and back into YOLO unless `startup.yolo_switcher = false` restricts the cycle to Manual/Auto.
...
| `Shift+Tab` | toggle | Cycles Manual → Auto → YOLO (set `startup.yolo_switcher = false` to omit YOLO) |
