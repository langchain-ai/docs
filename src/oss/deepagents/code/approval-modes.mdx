...
Cycle between modes at any time during a session with `Shift+Tab` or `Ctrl+T`. By default the cycle runs Manual → Auto → YOLO → Manual. Set `startup.yolo_switcher = false` in `~/.deepagents/config.toml` to keep the cycle Manual/Auto only; launching with `--yolo` still works, and `Shift+Tab` always exits YOLO back to Manual.

<Note>
    YOLO in the `Shift+Tab` cycle requires `deepagents-code>=0.1.47`. In earlier versions the toggle switches between Manual and Auto only.
</Note>
...
Auto is generally available in `deepagents-code>=0.1.46` and no longer requires an experimental opt-in. It is eligible only in interactive, unsandboxed sessions.
...
<Steps>
    ...
    <Step title="Launch with Auto" icon="terminal">
        ...
        You can also cycle to Auto mid-session with `Shift+Tab` or `Ctrl+T`. The first time you enable Auto, a one-time education notice explains what the mode does before it activates.
    </Step>
</Steps>

If Auto is requested in a sandboxed session, it falls back to Manual with a warning.

In `deepagents-code<0.1.46`, Auto is gated behind an experimental opt-in. Set `DEEPAGENTS_CODE_EXPERIMENTAL=1` in your shell or `~/.deepagents/.env` before choosing Auto; without it, Auto falls back to Manual with a warning.
...
A session launched in YOLO exits to Manual when you press `Shift+Tab` or `Ctrl+T`. From there the default cycle continues through Auto and back into YOLO unless `startup.yolo_switcher = false` restricts the cycle to Manual/Auto.
...
In non-interactive mode (`-n` or piped stdin), passing `-y`/`--auto-approve` or `--yolo` prints a warning and the flag is ignored; the run proceeds headlessly. In `deepagents-code<0.1.61`, these flags are rejected with an error instead. ACP server mode accepts both flags: Auto installs its classifier middleware (`--auto-classifier-model` requires Auto there), and YOLO requires that you have already acknowledged the risk in the interactive TUI.

### Remember the last mode across sessions

When neither a flag nor `[startup].mode` selects a mode, the launch restores the app-managed `[startup].recent` value: the last Manual or Auto mode you selected is written to `~/.deepagents/config.toml` and applied to the next bare launch. YOLO is never stored as a recent mode; it must be explicitly configured each time (`--yolo` or `[startup].mode = "yolo"`). A restored Auto mode applies only after you have seen the current Auto education notice; otherwise the launch falls back to Manual and the TUI explains how to re-enable Auto.

<Note>
    Restoring the last Manual or Auto mode requires `deepagents-code>=0.1.60`.
</Note>
...
`--yolo` takes priority over `-y`/`--auto-approve`, which takes priority over `[startup].mode`. When none of them set a mode, `[startup].recent` restores the last Manual or Auto selection.

| Source | Value | Selects |
|---|---|---|
| `--yolo` | flag | YOLO (interactive only, after acknowledgement) |
| `-y`, `--auto-approve` | flag | Auto |
| `[startup].mode` | `"manual"` | Manual |
| `[startup].mode` | `"auto"` | Auto |
| `[startup].mode` | `"yolo"` | YOLO |
| `[startup].recent` | `"manual"` or `"auto"` | Last selected Manual or Auto (app-managed) |
| `Shift+Tab`, `Ctrl+T` | toggle | Cycles Manual → Auto → YOLO (set `startup.yolo_switcher = false` to omit YOLO) |
