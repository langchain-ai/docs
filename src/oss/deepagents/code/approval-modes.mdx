...
The classifier review has a configurable time budget. If a batch is not reviewed within the deadline, it is denied as `classifier_unavailable` and the agent can revise its plan. After repeated misses, Auto falls back to the normal approval UI. To raise the limit for a slow or heavily loaded classifier model, set `DEEPAGENTS_CODE_AUTO_CLASSIFIER_TIMEOUT` or `[models].auto_classifier_timeout` in `config.toml`. See [Auto classifier timeout](/oss/deepagents/code/config-file#auto-classifier-timeout) for details.
