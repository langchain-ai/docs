...
| `-y`, `--auto-approve` | Enable beta classifier-backed [Auto](/oss/deepagents/code/approval-modes) mode. Requires `DEEPAGENTS_CODE_EXPERIMENTAL=1` and an interactive local session. Toggle with `Shift+Tab` during an interactive session |
