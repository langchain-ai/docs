...
`/btw` answers a question about the current conversation without interrupting the main task. Use it to check context while the agent keeps working. In an interactive session, type `/btw <question>`, or enter `/btw` to open a question field. The answer appears in a dismissible dialog, not in the main conversation. Press `Esc` to dismiss it without stopping the main run.

Side answers use the latest checkpointed conversation. Output not yet saved in a checkpoint is unavailable. The side request uses the server-resolved model and instructions without tools or an agent loop. Model changes made only in the client take effect after the next main turn. Side questions time out after 120 seconds. Their cost is tracked separately from the main run and included in the displayed session total.
