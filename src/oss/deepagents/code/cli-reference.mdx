...
Deep Agents Code exposes three read-only slash commands for inspecting what a session loads and spends: `/tools` lists the active tool set, `/context-doctor` audits the context injected before the conversation starts, and `/cost` reports the thread's estimated spend. Use them to confirm that a tool-shaping flag took effect or to find the source of unexpected context growth. None of these commands modify the session; all three queue behind active work.

### List available tools

<Note>
    `/tools` requires `deepagents-code>=0.1.37`.
</Note>
...
MCP servers that are disabled, need login, or failed to load appear in a separate table instead of disappearing from the list, so you can tell a missing tool apart from one that never loaded. When a custom or remote agent cannot be inspected, `/tools` explains the limitation rather than reporting that no tools are available. MCP tool descriptions are available in `/mcp`.

Use `/tools` to verify tool-shaping flags: the list reflects `--allow-fs-tools`, `--no-mcp`, `--mcp-config`, and `--trust-project-mcp`. For the non-interactive equivalent, see [Restrict filesystem tools](#restrict-filesystem-tools) and [`dcode tools list`](#cli-commands).

### Audit injected context

<Note>
    `/context-doctor` requires `deepagents-code>=0.1.62`.
</Note>
...
When the provider reports context usage, the report reconciles the injected estimate and conversation history against the provider total and shows the difference as an unattributed delta. A large delta or an unexpectedly large injected total points at the component to trim, such as an oversized skill description or an MCP server with many tools. Trim the component, then run `/context-doctor` again to confirm. For custom or remote agents that Deep Agents Code cannot introspect, the affected rows are marked unavailable instead of guessed.
...
<Note>
    `/cost` requires `deepagents-code>=0.1.51`.
</Note>
...
The total covers every model call the thread makes: assistant turns, subagents, offload and summarization, the Auto mode classifier, and rubric grading. It persists in the thread's checkpoint, so it is restored when you resume a thread with `-r` or switch threads, and the same figure appears beside token usage in the status bar. Per-type and per-model breakdowns cover the period since the thread was loaded; restored usage is included only in the total.

Estimates come from the [genai-prices](https://github.com/pydantic/genai-prices) catalog, with cache reads and writes priced separately. When the catalog has no rates for a model, that request is left out of the total and `/cost` reports how many of the recorded requests are included in the figure. Estimates are display-only: nothing caps spend or gates execution on them.

Use `/tokens` for the token counts behind the estimate, or add your own rates for uncovered models in `~/.deepagents/prices.json`.
