...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/plugins`: Manage [plugins and marketplaces](/oss/deepagents/code/plugins). Experimental; requires `DEEPAGENTS_CODE_EXPERIMENTAL=1`.
        - `/notifications`: Configure startup warning preferences.
        - `/reload`: Re-read `.env` files, refresh configuration, and re-discover skills without restarting. When experimental plugins are enabled, also reloads plugin skills and plugin MCP servers. Conversation state is preserved. See [`DEEPAGENTS_CODE_` prefix](/oss/deepagents/code/configuration#deepagents_code_-prefix) for override behavior.
