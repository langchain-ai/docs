...
<Steps>
    ...
    <Step title="Add provider credentials" icon="key">
        ...
        <Note>
            Web search uses [Tavily](https://tavily.com). Add a key from `/auth` or set `TAVILY_API_KEY`. See [Enable web search](/oss/deepagents/code/credentials#enable-web-search-with-tavily).
        </Note>
    </Step>
...
<<<<<<< Updated upstream
    <Step title="Enable tracing" icon="chart-dots">
        To log agent operations, tool calls, and decisions in LangSmith, optionally add the following to `~/.deepagents/.env` or export the variables in your shell:
=======
