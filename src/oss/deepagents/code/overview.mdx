...
<AccordionGroup>
    ...
    <Accordion title="Cap wall-clock time with `--timeout`" icon="clock">
        ...
        `SECONDS` must be a positive integer. On expiry the agent is cancelled and the process exits with code 124 (matching GNU `timeout(1)`), the same code used by `--max-turns`, so CI can treat both budget hits uniformly. Requires `-n` or piped stdin; otherwise exits with code 2.
    </Accordion>
