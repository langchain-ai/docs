...
Deep Agents Code is an open source coding agent built on the [Deep Agents SDK](/oss/deepagents/quickstart).
Deep Agents Code works with any LLM that supports tool calling and allows you to switch LLMs between inputs.
...
<AccordionGroup>
    ...
    <Accordion title="CLI commands" icon="terminal">
        ...
        | `deepagents update`                  | Check for and install Deep Agents Code updates |
        ...
        | `deepagents mcp login NAME [--config PATH]` | Run the OAuth login flow for an MCP server marked `auth: "oauth"`. See [MCP tools](/oss/deepagents/code/mcp-tools#oauth-login) |
