...
Every entry must start with `./` and remain inside the plugin root. After changing an installed plugin extension, run `/restart`. For the setup function, supported registrations, trust model, and security behavior, see [Python extensions](/oss/deepagents/code/extensions).
