...
description: Install plugins from marketplaces or package skills, MCP servers, hooks, and Python extensions for Deep Agents Code
---
...
4. Run `/reload` to activate newly installed plugin skills, MCP servers, and hooks without restarting the session. Plugins with [Python extensions](/oss/deepagents/code/extensions) require `/restart`.

The plugin manager also lets you enable, disable, and uninstall installed plugins. Disabling a plugin keeps it installed but excludes its skills, MCP servers, hooks, and Python extensions after you run `/reload` or start a new session. Run `/restart` when a plugin includes a Python extension.
...
<Note>
    Python extensions require `deepagents-code>=0.1.64` and `DEEPAGENTS_CODE_EXPERIMENTAL=1`.
</Note>
...
The plugin must declare a non-empty `version`. Every entry must start with `./` and remain inside the plugin root. After changing an installed plugin extension, run `/restart` instead of `/reload`. For the setup function, supported registrations, trust model, and security behavior, see [Python extensions](/oss/deepagents/code/extensions).
