...
Plugins extend Deep Agents Code with reusable [skills](/oss/deepagents/code/memory-and-skills) and [MCP servers](/oss/deepagents/code/mcp-tools). Marketplaces provide catalogs for discovering and installing plugins across projects or teams.
...
5. Run `/reload-plugins` to activate newly installed plugin skills and MCP servers without restarting the session.
...
`plugin list` and `plugin marketplace list` accept `--json`. After installing a plugin, run `/reload-plugins` in an active interactive session or start a new session.
...
Test a local marketplace by adding its directory, installing a plugin, and starting a new session or running `/reload-plugins`:
