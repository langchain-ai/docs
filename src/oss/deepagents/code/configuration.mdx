...
<Warning>
    Running `dcode` inside an untrusted project directory exposes you to project-controlled files. A malicious `.env`, `Makefile`, or build script in that directory can influence the agent's process environment and what it runs. Because `dcode` discovers and loads the nearest project `.env` automatically, treat any directory you would not run arbitrary scripts in as untrusted, and use a [remote sandbox](/oss/deepagents/code/remote-sandboxes) for untrusted repositories. Shell startup-hook keys (such as `BASH_ENV` and `ENV`) are ignored when loading `.env` files so a project cannot use them to run code, but the broader trust boundary remains: when you run `dcode` in a directory, that directory's files can shape execution.
</Warning>
