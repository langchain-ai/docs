...
<ResponseField name="DEEPAGENTS_CODE_RECENT_THREADS" type="integer" default="20" post={["optional"]}>
    Maximum number of recent threads to load and display in thread lists and selectors. Values below `1` are clamped to `1`, and non-integer values use the default. `-n` or `--limit` overrides this value for `dcode threads list`. The former `DA_CLI_RECENT_THREADS` name remains available as a fallback when this variable is unset.

    <Note>
        `DEEPAGENTS_CODE_RECENT_THREADS` requires `deepagents-code>=0.1.68`. Earlier versions only support `DA_CLI_RECENT_THREADS`.
    </Note>
</ResponseField>
