...
<ResponseField name="DEEPAGENTS_CODE_EXPERIMENTAL" type="string" post={["optional"]}>
    Opt into experimental, unstable Deep Agents Code behavior. Set to `1` (or any truthy value) to enable the classifier-backed [Auto approval mode](/oss/deepagents/code/approval-modes). Auto falls back to Manual when this is unset.
</ResponseField>
