...
<ResponseField name="DEEPAGENTS_CODE_ONBOARDING" type="string" post={["optional"]}>
    Control the first-run onboarding flow. Set to a truthy value (`1`, `true`, `yes`, or `on`) to force the onboarding flow open on every interactive startup — useful when testing or debugging the onboarding experience. Set to a falsy value (`0`, `false`, `no`, or `off`) to suppress the flow entirely — useful for fresh containers, CI pipelines, and provisioned machines where first-run onboarding is not needed. Leave unset to use the default behavior: the flow runs the first time only, and a completion marker file records that it has been seen. Suppressing the flow does not write the completion marker, so removing the variable restores first-run behavior rather than silently marking onboarding as complete. This variable replaces the former `DEEPAGENTS_CODE_DEBUG_ONBOARDING`, which is no longer recognized.
</ResponseField>
