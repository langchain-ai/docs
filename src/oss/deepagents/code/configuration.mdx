...
If you installed with the install script, Deep Agents Code is installed as a `uv` tool. To remove the `dcode` and `deepagents-code` binaries and the isolated tool environment, run:
...
Optionally clear `uv`'s shared tool cache only if no other `uv` tools rely on it. The cache is usually `~/.cache/uv` on Linux and `~/Library/Caches/uv` on macOS.
