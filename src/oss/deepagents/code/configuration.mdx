...
<ResponseField name="DEEPAGENTS_CODE_EXPERIMENTAL" type="string" post={["optional"]}>
    Opt into experimental, unstable Deep Agents Code behavior. Set to `1` (or any truthy value) to enable experimental features, including [Python extensions](/oss/deepagents/code/extensions).
</ResponseField>

<ResponseField name="DEEPAGENTS_CODE_EXTENSIONS" type="string" default="true" post={["optional"]}>
    Enable or disable [Python extension](/oss/deepagents/code/extensions) discovery. Overrides `[extensions].enabled` in `config.toml`. `DEEPAGENTS_CODE_EXPERIMENTAL=1` is still required.
</ResponseField>
