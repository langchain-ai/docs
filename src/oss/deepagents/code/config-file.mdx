...
<Note>
    The cache-expiry workflow and `warnings.cache_prompt` setting described here are unreleased and depend on [the cache prompt update](https://github.com/langchain-ai/deepagents/pull/6477). Released versions retain the send-time cost warning.
</Note>
