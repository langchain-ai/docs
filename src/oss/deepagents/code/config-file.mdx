...
<Note>
    This recursion-limit behavior requires `deepagents-code>=0.1.64`.
</Note>

Deep Agents Code does not set a default. When no Deep Agents source sets a valid value, it inherits `LANGGRAPH_DEFAULT_RECURSION_LIMIT` if that variable is set. Otherwise, the bundled LangGraph server default applies, currently `10011`.

Values from managed configuration, `DEEPAGENTS_CODE_RECURSION_LIMIT`, and `config.toml` must be integers from `25` to `100000` (inclusive). Invalid values log a warning and resolution continues to the next source. This fallback can raise the effective budget. For example, a `config.toml` value of `20` is rejected and falls through to the server default of `10011`. The `--recursion-limit` flag is exempt from the floor and accepts any integer greater than or equal to `1`.
...
6. LangGraph server default (`10011`)

Use `dcode config get runtime.recursion_limit` to see the effective value and its source. The command reports `LANGGRAPH_DEFAULT_RECURSION_LIMIT` when set. An unset result leaves the limit to the LangGraph server.
