...
Deep Agents Code does not set a default. When no Deep Agents source sets a valid value, the LangGraph server default applies.

Values from managed configuration, `DEEPAGENTS_CODE_RECURSION_LIMIT`, and `config.toml` must be integers from `25` to `100000` (inclusive). Invalid values log a warning and resolution continues to the next source. The `--recursion-limit` flag is exempt from the floor and accepts any integer greater than or equal to `1`.
