...
:::
  ...
  For the full human-in-the-loop guide, see [Human-in-the-loop](/oss/deepagents/human-in-the-loop).
...
:::python
For the full configuration, see [Rate limiting](/oss/langchain/models#rate-limiting).
:::
...
:::python
<Note>
Major integration packages raise standard exception types (@[ModelAuthenticationError], @[ModelRateLimitError], @[ModelTimeoutError], and others) that carry an `is_retryable` flag the retry middleware respects by default. See [Model exceptions](/oss/langchain/models#model-exceptions) for the full list.
</Note>
:::
