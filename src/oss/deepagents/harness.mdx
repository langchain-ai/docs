...
<Note>
    **A `general-purpose` synchronous subagent is enabled by default.** Every deep agent created with `create_deep_agent` ships with a `general-purpose` subagent and the `task` tool wired up — you do not need to pass anything to `subagents=` to get one. Anything you add via `subagents=` is _in addition to_ this default. To replace, rename, or disable it, see [Default subagent](/oss/deepagents/subagents#default-subagent).
</Note>
...
- Can be replaced, renamed, re-prompted, or disabled via the [harness profile](#harness-profiles) — see [Default subagent](/oss/deepagents/subagents#default-subagent)
