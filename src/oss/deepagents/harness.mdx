...
sidebarTitle: Harness capabilities
---

 Harness = built-in-subagents + skills/tools/mcp +  runtime execution + context management + hooks + prompts
...
Alongside these capabilities, deep agents also make use of provided context and instructions using:
...
The agent's behavior, capabilities, and knowledge base is made up of its [prompt](#prompt) and its [long-term memory](#long-term-memory).
...
1. **The inputs for a tool cross 20,000 tokens** (configurable via `tool_token_limit_before_evict`): File write and edit operations leave behind tool calls containing the complete file content in the agent's conversation history.
...
2. **The results of tool calls exceed **: When the results exceed the defined threshold, the deep agent offloads the response to the configured backend and substitutes it with a file path reference and a preview of the first 10 lines. Agents can then re-read or search the content as needed.
