...
- Appending to the base `deepagents` system prompt
- Overriding individual tool descriptions
- Excluding specific harness-level tools
- Adding extra middleware for specific models or providers
- Disabling, renaming, or re-prompting the general purpose subagent

Register a profile under a provider name like `"openai"` for provider-wide defaults, or under a fully qualified `provider:model` key like `"openai:gpt-5.4"` for per-model overrides. Registrations are additive: re-registering under an existing key merges on top of the prior registration (chaining `pre_init`, unioning `excluded_tools`, merging middleware by type, and preserving unset fields).
...
# and skips the auto-added `general-purpose` subagent (which also drops the
# `task` tool when no other subagents are configured).
register_harness_profile(
    "openai:gpt-5.4",
    HarnessProfile(
