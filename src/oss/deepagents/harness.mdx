...
| `execute` |  Run shell commands in a sandboxed environment. Only available if the backend implements `SandboxBackendProtocol`. |
...
3. **StoreBackend** - Persistent cross-conversation storage
   - Uses [LangGraph's BaseStore](/oss/integrations/stores/index#interface) for durability
...
- Customizable with additional tools/middleware
...
- Preserves recent context while compressing older history
