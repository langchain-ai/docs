...
Some tools are excluded from the large result eviction logic:

- `"ls"`, `"glob"`, and `"grep"` are excluded because they implement their own truncation logic.
- `"read_file"` is excluded to avoid failure modes on single long lines where the agent would see truncation and may then request to read the file again and receive the same truncated output.
- `"edit_file"` and `"write_file"` which only produce short confirmation messages.
