...
```mermaid
graph TB
    User[User Query] --> Agent[Deep Agent]
    Agent --> FS[File System Tools]
    Agent --> Todo[To-Do List]
    Agent --> Sub[Subagents]

    FS --> |6 tools| FSTools["ls, read, write
    edit, glob, grep"]
    FSTools --> Backend[Storage Backend]
    ...
    Sub --> |delegate| SubExec["Isolated Context
    Execution"]
    SubExec --> |result only| Agent

    Agent --> Evict["Large Result
    Eviction"]
    Evict --> |"20k+ tokens"| Backend
