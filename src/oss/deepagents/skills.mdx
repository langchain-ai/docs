...
For centrally managed skill libraries, route `/skills/` to a shared `StoreBackend` and deny write operations on that path. This lets the agent read approved skills from a store-backed knowledge base without letting it create or edit those skill files through `write_file` or `edit_file`. See [read-only skills from a shared store](/oss/deepagents/backends#add-read-only-skills-from-a-shared-store) for a complete example.

Use this when application code or an admin workflow owns skill updates, and agents should only consume the curated source of truth.
