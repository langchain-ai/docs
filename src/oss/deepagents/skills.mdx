...
The middleware's `beforeAgent` hook runs before each agent invocation, reading skill files from the store and uploading them into the sandbox filesystem. Once synced, the agent can execute scripts with the `execute` tool just like any other file in the sandbox.
:::
