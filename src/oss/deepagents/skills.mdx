...
Add the skills when creating your deep agent:
...
<ParamField body="skills" type="list[str]" optional>
    ...
    - With `FilesystemBackend` and `StoreBackend`, create the backend, call `backend.upload_files()` to add skill files, then pass the backend to `create_deep_agent`. For `FilesystemBackend`, use `virtual_mode=True` to sandbox paths under `root_dir`.
...
<ParamField body="skills" type="list[str]" optional>
    ...
    - With `FilesystemBackend` and StoreBackend, create the backend, call `backend.upload_files()` to add skill files, then pass the backend to `createDeepAgent`. For `FilesystemBackend`, use `virtualMode: true` to sandbox paths under `rootDir`.
