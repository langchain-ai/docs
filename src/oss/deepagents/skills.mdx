...
Functional skills are skills that expose code modules to an [interpreter](/oss/deepagents/interpreters). When configured with interpreter middleware, the agent can import these modules from code, for example with `await import("@/skills/<name>")`, and use them for deterministic helper logic.
...
Functional skills are useful when the agent needs reusable helpers for structured data workflows, such as sorting, grouping, scoring, parsing, validating, or aggregating data. For setup details, see [Interpreters](/oss/deepagents/interpreters).
