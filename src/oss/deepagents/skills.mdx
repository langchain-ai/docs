...
:::js
Reset the stored metadata to load every source again on the next model call.
:::
...
:::js
<Note>Reloading skills requires `deepagents>=1.14.0`.</Note>
:::
...
:::js
Loading happens before each model call, so a reset from a hook takes effect on the next model call, including partway through a run.
:::
