...
Skills load once per thread and stay in agent state, so every later model call on that thread reuses the same list. Skills added, edited, or deleted since then never reach the model. Reset the stored metadata to load every source again on the next model call. This applies when a checkpointer is configured. Without one, state does not survive between runs, so every run loads skills again.
...
Middleware can reset the value as well, which lets your application decide when a thread is stale. Loading happens before each model call, so a reset from a hook takes effect on the next model call, including partway through a run.
