...
import SkillsComposeSourcesPy from '/snippets/code-samples/skills-compose-sources-py.mdx';
import SkillsComposeSourcesJs from '/snippets/code-samples/skills-compose-sources-js.mdx';
...
import SkillsWritablePy from '/snippets/code-samples/skills-writable-py.mdx';
import SkillsWritableJs from '/snippets/code-samples/skills-writable-js.mdx';
import SkillsReloadInvokePy from '/snippets/code-samples/skills-reload-invoke-py.mdx';
import SkillsReloadInvokeJs from '/snippets/code-samples/skills-reload-invoke-js.mdx';
import SkillsReloadUpdateStatePy from '/snippets/code-samples/skills-reload-update-state-py.mdx';
import SkillsReloadUpdateStateJs from '/snippets/code-samples/skills-reload-update-state-js.mdx';
import SkillsReloadMiddlewarePy from '/snippets/code-samples/skills-reload-middleware-py.mdx';
import SkillsReloadMiddlewareJs from '/snippets/code-samples/skills-reload-middleware-js.mdx';
...
The following example uses @[StoreBackend], but the same pattern applies to any backend that can resolve files based on runtime context:
...
:::python
<SkillsComposeSourcesPy />
:::

:::js
<SkillsComposeSourcesJs />
:::
...
Skills load once per thread and stay in agent state, so a thread uses the skill list from its first run for every run after it. Skills added, edited, or deleted since then never reach the model. Reset the stored metadata to load every source again on the next run. This applies when a checkpointer is configured. Without one, state does not survive between runs, so every run loads skills again.

<Note>
Local runs often have no checkpointer, so skill edits take effect immediately and skills appear to reload on their own. [LangSmith Deployments](/langsmith/deployment) configure a persistent checkpointer automatically, so the same agent holds its skills for the life of a thread once deployed. See [Going to production](/oss/deepagents/going-to-production).
</Note>

:::python
<Note>Reloading skills requires `deepagents>=TODO`.</Note>
:::
...
:::python
<SkillsReloadInvokePy />
:::

:::js
<SkillsReloadInvokeJs />
:::

Update state to reset a thread between runs:

:::python
<SkillsReloadUpdateStatePy />
:::

:::js
<SkillsReloadUpdateStateJs />
:::
...
:::js
Use `null`, not an empty list. An empty list means the sources loaded and held no skills, so the agent sees no skills for the rest of the thread.
:::
...
:::python
Any hook can return the reset as a state update, but the reload always happens in the `before_agent` hook of the next run. The hook you pick decides when your application makes the decision, not when the reload happens. `after_agent` runs once at the end of a run, so the check below sees everything the agent did. Do not reset in `before_model`. The skill list is written into the system prompt on every model call, so clearing it partway through leaves the agent with no skills for the rest of the run.

<SkillsReloadMiddlewarePy />
:::

:::js
Any hook can return the reset as a state update, but the reload always happens in the `beforeAgent` hook of the next run. The hook you pick decides when your application makes the decision, not when the reload happens. `afterAgent` runs once at the end of a run, so the check below sees everything the agent did. Do not reset in `beforeModel`. The skill list is written into the system prompt on every model call, so clearing it partway through leaves the agent with no skills for the rest of the run.

<SkillsReloadMiddlewareJs />
:::
...
Skill availability controls which skill files the agent can see. Skill permissions control whether the agent can read those files, write to them, or pause for human approval before writing. Use [filesystem permissions](/oss/deepagents/permissions) for path-based read and write rules, and use [`interrupt_on`](/oss/deepagents/human-in-the-loop) or permission rules with `mode="interrupt"` when writes need approval.
...
3. Add `deny` rules for any paths that should stay read-only. Place more specific rules before broader deny rules if paths overlap ([rule ordering](/oss/deepagents/permissions#rule-ordering)).
...
The agent uses `write_file` and `edit_file` to create or update `SKILL.md` and supporting files under writable paths. To capture general learnings outside the skills format, route a separate path such as `/memories/` to another writable backend. See [Backends](/oss/deepagents/backends) for routing and store setup.
...
1. **Reset the stored metadata.** Skills load once per thread, so a thread that has already run does not see the change. See [Reload skills](#reload-skills).

2. **Confirm whether a checkpointer is configured.** Without one, every run loads skills again and edits take effect immediately. With one, such as in a [LangSmith Deployment](/langsmith/deployment), skills stay fixed for the life of the thread.
