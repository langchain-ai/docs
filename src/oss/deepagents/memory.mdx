...
1. **Point the agent at memory files.** Pass file paths to `memory=` when creating the agent. A [backend](/oss/deepagents/backends) controls where those files are stored and who can access them.
...
Give the agent a shared memory file that all users read from and write to. The agent improves over time as it accumulates knowledge across every conversation. It can also learn and update [skills](/oss/deepagents/skills) when it has write access. See [skills as procedural memory](#skills-as-procedural-memory).
...
- **Read-write** (agent-learned): The agent creates and updates skills based on experience. When the agent has write access to memory, it can also write to its skills directory. Use [policy hooks](/oss/deepagents/backends#add-policy-hooks) to control which paths are writable.
...
Both approaches deploy a **consolidation agent** alongside your main agent: a deep agent that reads conversation history, extracts key facts, and merges them into the memory store. There are two ways to trigger it:

- **[Cron](#cron-recommended)** (recommended): Runs on a fixed schedule, can synthesize across many conversations at once.
- **[Scheduled run](#scheduled-run)**: Fires after a conversation ends, only runs when users are active.
...
async def search_recent_conversations(query: str) -> str:
    """Search all conversations updated in the last 6 hours."""
...
const searchRecentConversations = tool(
  ...
  },
  {
    ...
    description: "Search all conversations updated in the last 6 hours.",
  }
);

const agent = createDeepAgent({
  model: "claude-sonnet-4-6",
  systemPrompt: `Review recent conversations and update the memory file.
Merge new facts, remove outdated information, and keep it concise.`,
...
Register the consolidation agent alongside your main agent and schedule it with a cron job:
...
The consolidation agent is similar to the cron version, but scoped per user and searching only that user's conversations:
...
Schedule the run after a conversation completes. The delay gives the conversation time to finish (the user may send follow-up messages) so the consolidation agent sees the full exchange:
...
# After a conversation completes, schedule consolidation in 30 minutes
...
<Tip>
You can trigger the scheduled run from a [webhook](/langsmith/use-webhooks) on the main agent's run, so consolidation is automatically queued after every conversation.
</Tip>
