...
<Note>
Accessing `ctx.runtime.server_info` (Python) / `ctx.runtime.serverInfo` (JS/TS) requires `deepagents>=0.5.0`. On older versions, read the assistant ID from `get_config()["metadata"]["assistant_id"]` instead.
</Note>
...
const searchPastConversations = tool(
  async ({ query }: { query: string }, runtime) => {
