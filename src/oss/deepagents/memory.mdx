...
You can extend deep agents with **memory** by using a **CompositeBackend** that routes specific paths to persistent storage. This enables hybrid storage where some files persist across threads while others remain ephemeral.
...
Configure memory by using a `CompositeBackend` that routes the `/memories/` path to a `StoreBackend`:
