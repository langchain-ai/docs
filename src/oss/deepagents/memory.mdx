...
3. **Agent updates memory during the conversation.** As the agent learns new information, it uses its built-in `edit_file` tool to update memory files. Changes are persisted and available in the next conversation. Some memory is read-only: for example, developer-defined [skills](/oss/deepagents/skills) and [organization policies](#organization-level-memory) are typically not writable by the agent. See [read-only vs writable memory](#read-only-vs-writable-memory) for details.
