...
Memory lets your agent learn and improve across conversations. Deep Agents makes memory first class with a filesystem-based system: the agent reads and writes memory as files, and you control where those files are stored using [backends](/oss/deepagents/backends).
...
3. **The agent writes to update.** During the conversation, the agent uses its built-in `edit_file` tool to update memory files. Changes are persisted by the [backend](/oss/deepagents/backends).
...
To persist memory across conversations, route `/memories/` to a persistent backend using a @[`CompositeBackend`]. How you scope the backend controls who can see the memory: [agent-scoped](#agent-scoped-memory) (shared across all users) or [user-scoped](#user-scoped-memory) (isolated per user).
...
In this pattern:
- `AGENTS.md` and [skills](/oss/deepagents/skills) live in the filesystem and are editable by the agent
...
- [Skills](/oss/deepagents/skills) are not editable by the agent
...
`StoreBackend` works with any LangGraph @[`BaseStore`] implementation. The store is LangGraph's cross-thread persistence layer: it lets you save and retrieve data scoped to any custom namespace, not just a single thread. When deploying to [LangSmith Deployment](/langsmith/deployment), the platform provisions a store automatically.

For local development, @[`InMemoryStore`] is sufficient. It stores data in an in-memory dictionary and supports basic operations (`put`, `search`, `get`) as well as [semantic search](/oss/langgraph/persistence#semantic-search) with embeddings.

For production, use a persistent store:

| Store | Use case |
|---|---|
| @[`PostgresStore`] | Recommended for most production deployments. Supports async via `AsyncPostgresStore`. |
| @[`RedisStore`] | Low-latency access patterns. Supports async via `AsyncRedisStore`. |
| Custom | Implement the @[`BaseStore`] interface to back memory with a vector database, custom API, or cloud storage. See the [backend protocol](/oss/deepagents/backends#protocol-reference). |
...
For more on store operations, semantic search, and using stores inside LangGraph nodes, see the [persistence guide](/oss/langgraph/persistence#memory-store).
...
|---|---|---|
| **Duration** | How long does it last? | [Short-term](/oss/deepagents/context-engineering) (single conversation) or [long-term](#how-it-works) (across conversations) |
| **Information type** | What kind of information is it? | [Episodic](/oss/concepts/memory#episodic-memory) (experiences), [procedural](/oss/concepts/memory#procedural-memory) (instructions and [skills](/oss/deepagents/skills)), or [semantic](/oss/concepts/memory#semantic-memory) (facts) |
...
| **Retrieval** | How are memories read? | Loaded into prompt (default), on demand (e.g., [episodic memory](#episodic-memory), [skills](/oss/deepagents/skills)), or via [custom middleware](#custom-memory-loading) |
...
Episodic memory lets the agent reference past conversations to learn from what worked and what didn't. Deep Agents get this for free through [checkpointers](/oss/langgraph/persistence#checkpoints): every conversation is persisted as a checkpointed thread.
...
Load organization memory into the prompt alongside user memory by adding it to the `memory` list. Use [policy hooks](/oss/deepagents/backends#add-policy-hooks) to enforce that org-level memory is read-only.
...
To enforce read-only memory, use [policy hooks](/oss/deepagents/backends#add-policy-hooks) on the backend that reject write operations to specific paths:
    ...
    backend=CompositeBackend(
        ...
        routes={
            ...
            "/policies/": StoreBackend(
                namespace=lambda ctx: (ctx.runtime.context.org_id, "policies"),
                read_only=True,
            ),
        },
    ),
...
By default, `MemoryMiddleware` loads memory files into the system prompt at the start of each conversation. For more control over how memories are injected, use [custom middleware](/oss/langchain/middleware/custom) to load memories yourself.
...
def load_memories(
    ...
    """Load memories into the system prompt on the first model call."""
    runtime = request.runtime
    store = runtime.store
    namespace = (runtime.context.user_id,)

    # Read the preferences file from the store
    ...
    if item and item.value:
        ...
        new_content = list(request.system_message.content_blocks) + [
            {
                "type": "text",
                "text": f"""Memory file: /memories/preferences.md
<user_memories>
{memories}
</user_memories>""",
            }
...
When an agent runs inside a [sandbox](/oss/deepagents/sandboxes), it operates in an isolated container that can't directly access the memory store. To bridge this gap, seed memories into the sandbox before the agent runs and sync updated memories back after it finishes. Use [custom middleware](/oss/langchain/middleware/custom) with `before_agent` and `after_agent` hooks.

For implementation details and a full code example, see the [file transfers](/oss/deepagents/going-to-production#file-transfers) guide.

### Skills as memory

[Skills](/oss/deepagents/skills) are reusable agent capabilities defined as `SKILL.md` files. They function as a form of long-term memory: persistent instructions that shape agent behavior across conversations. The agent can also create and refine its own skills at a writable path like `/memories/skills/`.

For details on configuring skills, see the [skills guide](/oss/deepagents/skills).
...
- Use [policy hooks](/oss/deepagents/backends#add-policy-hooks) to block unauthorized writes
