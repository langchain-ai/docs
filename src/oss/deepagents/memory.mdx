...
| **[Webhook](#webhook)** | Automatically on run completion | Event-driven consolidation with no client-side code, most production setups |
...
A [webhook](/langsmith/use-webhooks) fires consolidation automatically when a run on your main agent completes. No client-side scheduling code is needed: you pass the `webhook` parameter when creating the main agent's run, and the platform sends a POST request to your endpoint on completion.

```mermaid
graph LR
    Store[(Memory store)] -.->|reads| Conv[Conversation]
    Conv -->|run completes| Webhook[Webhook endpoint]
    Webhook -->|triggers| Agent[Consolidation agent]
    Agent -->|writes| Store
    ...
    class Agent process
    class Store output
    class Webhook schedule
```

Your webhook endpoint receives the completed run payload and triggers the consolidation agent. The `thread_id` from the payload gives the consolidation agent access to the conversation context:
...
:::js
```typescript src/webhook-server.ts
...
Then pass the webhook URL when creating runs on your main agent:
...
async for chunk in client.runs.stream(
    ...
    webhook="https://my-server.app/consolidate",
):
...
See [Use webhooks](/langsmith/use-webhooks) for details on webhook payloads, security, and configuration.
