...
Agent-scoped memory is shared across all users. The agent has a memory file that it reads at the start of every conversation and updates when it learns something new. Over time, the agent may also learn new [skills](/oss/deepagents/skills) as a form of procedural memory.
