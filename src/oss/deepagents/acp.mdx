...
import AcpStartServerJs from '/snippets/code-samples/acp-start-server-js.mdx';
import AcpZedCustomServerJs from '/snippets/code-samples/acp-zed-custom-server-js.mdx';
...
import AcpSkillsMemoryJs from '/snippets/code-samples/acp-skills-memory-js.mdx';
...
Then start the server with `await startServer(serverOptions)`.
...
| `middleware`    | `AgentMiddleware[]`                            | Custom middleware appended to the [default stack](/oss/deepagents/customization#default-stack-main-agent) |
