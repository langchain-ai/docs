...
Deep Agents can be exposed over multiple protocols depending on what you are integrating with:

- **ACP**: connect an agent to an IDE or editor (this page).
- **MCP**: let an agent call tools hosted by external servers. See [MCP](/oss/langchain/mcp/).
- **A2A**: orchestrate agent-to-agent interactions (for example, calling an agent as another agent’s skill). If you are deploying via LangSmith, see [A2A server](/langsmith/server-a2a).
