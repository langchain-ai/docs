...
Control which files and directories an agent can read or write using declarative permission rules. Pass a list of rules to `permissions=` and the agent's built-in filesystem tools respect them automatically, without writing custom [backend policy hooks](/oss/deepagents/backends#add-policy-hooks).
...
<Warning>
Permissions only apply to the built-in filesystem tools (`ls`, `read_file`, `glob`, `grep`, `write_file`, `edit_file`). Custom tools and MCP tools that access the filesystem are not covered.
</Warning>

<Warning>
Sandbox backends support arbitrary command execution, which can bypass path-based restrictions. Using permissions with a sandbox backend raises `NotImplementedError` unless all permission paths are scoped to [composite backend](/oss/deepagents/backends) routes.
</Warning>
