...
:::python
Use [Agent Skills](https://agentskills.io/) to provide your deep agent with new capabilities and expertise.

The `SkillsMiddleware` loads skills from all directories provided in `sources`.
...
skills_middleware = SkillsMiddleware(
    ...
    sources=["/Users/user/{project}/skills/"]
)
...
<ParamField body="backend" type="BackendProtocol | BackendFactory" required>
    Backend instance or factory function that takes runtime and returns a backend.
</ParamField>

<ParamField body="sources" type="list[str]" required>
    Paths to the skills directories. Later sources override earlier ones for skills with the same name (last one wins).
</ParamField>
...
The `createSkillsMiddleware` loads user-level skills and project-level skills.
Optionally, it can also load project-level skills.
