...
```mermaid
graph LR
    Agent[create_deep_agent] --> Stack[Middleware Stack]

    Stack --> Todo[TodoListMiddleware]
    Stack --> FS[FilesystemMiddleware]
    Stack --> Sub[SubAgentMiddleware]

    Todo --> |write_todos tool| Planning[Planning &<br/>Task Tracking]
    FS --> |6 filesystem tools| Files[File Operations]
    Sub --> |task tool| Delegation[Subagent<br/>Delegation]

    Planning --> Tools[Available Tools]
    Files --> Tools
    Delegation --> Tools
