...
Deep agents are built with a modular middleware architecture.
Each feature is implemented as separate middleware.
When you create a deep agent with `create_deep_agent`, the resulting deep agents automatically gets access to the following middleware:

- [`TodoListMiddleware`](#to-do-list-middleware): Plan tasks and track progress
- [`FilesystemMiddleware`](#filesystem-middleware): Perform file operations and context offloading (auto-saves large results)
- [`SubAgentMiddleware`](#subagent-middleware): Delegate of tasks to isolated sub-agents
- [`MemoryMiddleware`](#memory-middleware): Load memory from configured sources and inject it into system prompt
- `SummarizationMiddleware`: Auto-summarize context that exceeds 170k tokens
- `AnthropicPromptCachingMiddleware`: Cache system prompts to reduce costs (Anthropic only)
- `PatchToolCallsMiddleware`: Fix dangling tool calls from interruptions
- `HumanInTheLoopMiddleware`: Pause execution for human approval (requires `interrupt_on` config)
...
- [**ls**](/oss/deepagents/harness#file-system-access): List the files in the filesystem
- [**read_file**](/oss/deepagents/harness#file-system-access): Read an entire file or a certain number of lines from a file
- [**write_file**](/oss/deepagents/harness#file-system-access): Write a new file to the filesystem
- [**edit_file**](/oss/deepagents/harness#file-system-access): Edit an existing file in the filesystem
- [**glob**](/oss/deepagents/harness#file-system-access)
- [**grep**](/oss/deepagents/harness#file-system-access)
- [**execute**\](/oss/deepagents/harness#file-system-access)
...
A [subagent](/oss/deepagents/subagents#subagent-dictionary-based) is defined with a **name**, **description**, **system prompt**, and **tools**. You can also provide a subagent with a custom **model**, or with additional **middleware**. This can be particularly useful when you want to give the subagent an additional state key to share with the main agent.
...
In addition to any user-defined subagents, the main agent has access to a [`general-purpose` subagent](/oss/deepagents/subagents#the-general-purpose-subagent) at all times. This subagent has the same instructions as the main agent and all the tools it has access to. The primary purpose of the `general-purpose` subagent is context isolation—the main agent can delegate a complex task to this subagent and get a concise answer back without bloat from intermediate tool calls.
