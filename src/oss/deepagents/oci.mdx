---
title: Deep Agents on OCI
sidebarTitle: OCI
description: Build Deep Agents with OCI Generative AI, Oracle Database, and OpenSearch retrieval
---

:::python

For retrieval with Oracle Autonomous Database and OpenSearch, see
[OCI retrieval](/oss/deepagents/oci-retrieval). For Oracle Database-backed
checkpoints, storage, and files, see
[OCI persistence](/oss/deepagents/oci-persistence). For model cleanup and
troubleshooting, see [OCI operations](/oss/deepagents/oci-operations).

For generic Deep Agents features, see
[Customization](/oss/deepagents/customization),
[Subagents](/oss/deepagents/subagents),
[Skills](/oss/deepagents/skills),
[Memory](/oss/deepagents/memory), and
[Human-in-the-loop](/oss/deepagents/human-in-the-loop).
...
| Honors `middleware` | ✅ on the new `create_agent` API only | ✅ (inserted mid-stack, see [Capabilities](/oss/deepagents/customization)) |
...
| `embedding_model` | LangChain embeddings = `None` | Used for routing *and* store search. Default: `OCIGenAIEmbeddings` (see [Embeddings](/oss/deepagents/oci-retrieval#embeddings)). |
...
| Parameter | Type / default | Behavior |
|---|---|---|
| `model` | `BaseChatModel` = `None` | **Bring-your-own model.** Used as-is; `model_id` + all OCI inference auth options are ignored (see [Operations](/oss/deepagents/oci-operations#bring-your-own-model)). |
...
| Parameter | Type / default | Behavior (details in [Capabilities](/oss/deepagents/customization)) |
|---|---|---|
...
|---|---|---|
| `checkpointer` | both | Thread-level state persistence — e.g. `OracleSaver` (see [Persistence](/oss/deepagents/oci-persistence)). |
