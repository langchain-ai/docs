...
<Note>
Requires LangGraph >= 1.1. See the [LangGraph v1.1 release notes](/oss/releases/langgraph-v1.1) for full details.
</Note>

Pass `version="v2"` to `stream()` or `astream()` to get a unified output format. Every chunk is a `StreamPart` dict with `type`, `ns`, and `data` keys — the same shape regardless of stream mode, number of modes, or subgraph settings.

This is especially useful for deep agents, where you're typically combining multiple modes with `subgraphs=True`:

<CodeGroup>
```python v2 (new)
# Unified format — no more nested tuple unpacking
