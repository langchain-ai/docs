...
<Note>
Async subagents require [LangSmith Deployments](/langsmith/agent-deployment/overview). Each background task runs as a separate LangGraph run on its own thread, managed through the LangGraph SDK.
</Note>
...
|---|---|---|
| **Execution model** | Supervisor blocks until subagent completes | Returns task ID immediately; supervisor continues |
...
| **Cancellation** | Not possible | Cancel running tasks via `cancel` |
