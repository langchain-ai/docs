...
| `graph_id` | `str` | Required. The graph or agent ID. For LangGraph-based deployments, this must match a graph registered in `langgraph.json`. |
...
| `graphId` | `string` | Required. The graph or agent ID. For LangGraph-based deployments, this must match a graph registered in `langgraph.json`. |
