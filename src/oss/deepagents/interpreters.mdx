...
<CardGroup cols={2}>
  ...
  <Card title="Subagent orchestration" icon="arrows-split" href="/oss/deepagents/programmatic-subagents">
...
| Many independent units of work, multiple perspectives, or recursive analysis over large inputs | Interpreter with [subagent orchestration](/oss/deepagents/programmatic-subagents) |
...
- **Subagents**, through [subagent orchestration](/oss/deepagents/programmatic-subagents). Dispatch configured subagents from code and orchestrate them in plain JavaScript.
...
For configuration, examples, orchestration patterns, and safety notes, see [Subagent orchestration](/oss/deepagents/programmatic-subagents).
...
| `subagents` | `True` | Expose the built-in `task()` global for [subagent orchestration](/oss/deepagents/programmatic-subagents). Set to `False` to require subagent dispatch through the normal `task` tool path. |
