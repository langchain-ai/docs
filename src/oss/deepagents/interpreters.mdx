...
Where [sandboxes](/oss/deepagents/sandboxes) are a code-first way for acting on an environment (such as running commands, installing dependencies, and editing files), interpreters are a code-first way for composing tools, preserving state, and deciding what information should return to the model.
...
Use [sandboxes](/oss/deepagents/sandboxes) for code against an environment: shell commands, package installs, tests, filesystem edits, and OS-level execution.
...
- **Subagents**, through [dynamic subagents](/oss/deepagents/dynamic-subagents). When the agent has subagents configured, the interpreter exposes a `task()` global for dispatching them from code.
...
The following overview below covers when to use dynamic subagents and a minimal `task()` pattern. For configuration, orchestration examples, workflow triggers, and safety notes, see [Dynamic subagents](/oss/deepagents/dynamic-subagents).
...
Because interpreter snapshots are stored in graph state, a [checkpointer](/oss/langgraph/checkpointers) also captures them in checkpoint history. Add one when you need durable threads or [time travel](/oss/langgraph/use-time-travel):
...
| Filesystem access | No | Add the [built-in filesystem tools](/oss/deepagents/overview#virtual-filesystem-access) via the PTC allowlist |
...
<Note>
    **How code execution works**
    ...
    The Python interpreter bundles a [WebAssembly](https://webassembly.org/docs/security/) binary that calls into QuickJS. On each thread, a fresh WASM object is initialized, so interpreter state does not share memory with the host process and corrupted state on one thread cannot affect other threads.

    The `JS_ReadObject` serialization path is still present; it is not eliminated. Any bad instructions that reach QuickJS are compartmentalized to the initiating thread. Side effects from a compromised interpreter environment are limited to the capabilities explicitly bridged into it, primarily the PTC allowlist, which is why keeping that allowlist narrow matters.

    Treat interpreters as a capability-scoped execution layer with per-thread memory isolation. For workloads where thread-level isolation is not sufficient, run agents in isolated worker processes or containers.
</Note>
:::
