...
Programmatic subagents let the interpreter dispatch configured subagents from code using the built-in `task()` global. Use them for fan-out, verification, and recursive workflows over large inputs.
