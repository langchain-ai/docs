...
This page covers:
- **[Managed Deep Agents](#managed-deep-agents)**: private preview managed hosting for Deep Agents
...
The fastest way to deploy a deep agent is [`deepagents deploy`](/oss/deepagents/deploy), which packages your agent configuration and creates or updates a Managed Deep Agent. Managed Deep Agents is in private preview and exposes managed agent and thread APIs under `/v1/deepagents`.

For the CLI-based approach, see [Deploy with Managed Deep Agents](/oss/deepagents/deploy). For the full LangSmith guide, see [Deploy a Managed Deep Agent](/langsmith/deploy-managed-deep-agent). For manual LangGraph deployment setup, see the [LangSmith Deployments quickstart](/langsmith/deployment-quickstart).
...
In Deep Agents, memory is stored as files in a virtual filesystem. By default, files are scoped to a single thread (conversation) and not shared across threads. To share memory across threads, route a path like `/memories/` to a @[StoreBackend] that writes to the LangGraph [Store](/langsmith/custom-store). Use a @[CompositeBackend] to give the agent both thread-scoped scratch space and cross-thread [long-term memory](/oss/deepagents/memory).
