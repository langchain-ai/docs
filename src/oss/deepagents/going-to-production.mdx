...
| Path | Use when |
| --- | --- |
| [Managed Deep Agents](/oss/deepagents/deploy) | You want the fastest managed path for deploying a Deep Agents project with the `deepagents deploy` CLI and the Managed Deep Agents API. |
...
For the CLI-based Managed Deep Agents path, see [Deploy with Managed Deep Agents](/oss/deepagents/deploy). For the full LangSmith guide, see [Deploy a Managed Deep Agent](/langsmith/deploy-managed-deep-agent). For standard LangSmith Deployment setup, see the [LangSmith Deployments quickstart](/langsmith/deployment-quickstart).
