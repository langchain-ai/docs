...
**Credential injection for sandboxes.** If your agent runs code inside a [sandbox](#sandboxes) that calls external APIs, use the [sandbox auth proxy](/langsmith/sandbox-auth-proxy) to inject credentials into outbound requests automatically. The proxy intercepts requests to configured hosts and adds authentication headers using secrets stored in your LangSmith workspace, so sandbox code never sees raw API keys.
...
**Workspace secrets.** For API keys shared across all users (your organization's LLM provider keys, search API keys), store them as [workspace secrets](/langsmith/setup) in LangSmith. These are available to all agents in the workspace at runtime via environment variables.
