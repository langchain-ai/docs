...
A [`ProviderProfile`](/oss/deepagents/profiles#provider-profiles) packages initialization parameters that apply whenever the harness turns a `provider:model` string into a chat model. It is not consulted when you pass a preconfigured `BaseChatModel`instance.
