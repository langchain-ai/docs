...
When you pass an `openai:...` **string** as the model, Deep Agents configures the OpenAI integration to use the [Responses API](/oss/integrations/chat/openai#responses-api) by default. That path supports OpenAI conversation state and related features; see the [OpenAI provider](/oss/integrations/providers/openai) and [chat model](/oss/integrations/chat/openai) docs for current API options.
