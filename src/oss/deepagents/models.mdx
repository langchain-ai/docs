...
The model identifier must match the format expected by the provider. Some providers use simple names like `gpt-5.4`; others use namespaced IDs or deployment paths like `zai-org/GLM-5`, so the full Deep Agents string would be `baseten:zai-org/GLM-5`. Check the provider's model catalog or integration docs for the current identifiers.
