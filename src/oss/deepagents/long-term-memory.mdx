...
Long-term memory lets your agent learn and improve across conversations. Deep Agents makes long-term memory first class with a filesystem-based memory system: the agent reads and writes memory as files, and you control where those files are stored using [backends](/oss/deepagents/backends).
...
To persist memory across conversations, route `/memories/` to a @[`StoreBackend`] using a @[`CompositeBackend`]. The `namespace` on `StoreBackend` controls who can see the memory: [agent-scoped](#agent-scoped-memory) (shared across all users) or [user-scoped](#user-scoped-memory) (isolated per user).
...
| **Retrieval** | How are memories read? | Loaded into prompt (default) or on demand (e.g., [episodic memory](#episodic-memory), [skills](/oss/deepagents/skills)) |
