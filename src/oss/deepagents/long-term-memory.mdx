...
Deep Agents use their filesystem to store and retrieve information. How long that information persists depends on which [backend](/oss/deepagents/backends) you configure.

## Types of memory

Before configuring memory, it helps to understand the different kinds of information an agent can remember. For a full conceptual overview, see the [memory concepts guide](/oss/concepts/memory).

| Memory type | What is stored | Agent example |
|---|---|---|
| [Semantic](/oss/concepts/memory#semantic-memory) | Facts and knowledge | User preferences, project context |
| [Episodic](/oss/concepts/memory#episodic-memory) | Past experiences | Successful tool-call sequences, few-shot examples |
| [Procedural](/oss/concepts/memory#procedural-memory) | Instructions and rules | Self-improving system prompts, [persistent skills](#persistent-skills) |

Semantic memories can be stored as a single continuously updated [profile](/oss/concepts/memory#profile) (higher coherence, harder to maintain) or as a [collection](/oss/concepts/memory#collection) of individual documents (higher recall, easier to extend). See the [concepts guide](/oss/concepts/memory#semantic-memory) for tradeoffs.
...
To persist information across conversations, use a @[`CompositeBackend`] that routes `/memories/` to a @[`StoreBackend`]. The agent's scratch files stay ephemeral while anything written to `/memories/` persists in a LangGraph [Store](/langsmith/custom-store).
...
Then use [middleware](/oss/langchain/middleware/custom) or your system prompt to load this data into the agent's context at the start of each conversation. The agent can read and follow the policies but has no mechanism to alter them.
...
[Skills](/oss/deepagents/skills) are reusable agent capabilities defined as `SKILL.md` files. When stored in `/memories/`, they become a form of [procedural memory](/oss/concepts/memory#procedural-memory) — the agent can create, refine, and reuse its own workflows across conversations.
...
const agent = createDeepAgent({
  skills: ["/memories/skills/"],
  backend: makeBackend,  // CompositeBackend with /memories/ route
  store,
});
```
:::
...
Memories can be updated [in the hot path](/oss/concepts/memory#in-the-hot-path) (during the conversation, with immediate availability but added latency) or [in the background](/oss/concepts/memory#in-the-background) (asynchronously, with no latency impact but delayed availability). For most Deep Agents, hot-path writing via the filesystem tools is the simplest approach.
