...
- **Multi-tenant**: Set a [namespace factory](/oss/deepagents/backends#namespace-factories) on `StoreBackend` to isolate data per user (for example, `namespace=lambda ctx: (ctx.runtime.server_info.user.identity,)`)
