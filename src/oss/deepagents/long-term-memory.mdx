...
Deep Agents use the filesystem as the interface for memory. The agent reads and writes files, and the [backend](/oss/deepagents/backends) you configure determines how long that information persists.
...
| [**Information type**](#information-type) | [Semantic](/oss/concepts/memory#semantic-memory) (facts), [episodic](/oss/concepts/memory#episodic-memory) (experiences), or [procedural](/oss/concepts/memory#procedural-memory) (instructions) | What kind of information is it? |
...
|---|---|---|
| [Semantic](/oss/concepts/memory#semantic-memory) | Facts and knowledge | [User preferences](#user-preferences), [knowledge base](#knowledge-base) |
| [Episodic](/oss/concepts/memory#episodic-memory) | Past experiences | [Episodic log](#episodic-log) |
| [Procedural](/oss/concepts/memory#procedural-memory) | Instructions and rules | [agents.md](#the-agentsmd-pattern), [persistent skills](#persistent-skills) |

Semantic memories can be stored as a single continuously updated [profile](/oss/concepts/memory#profile) or as a [collection](/oss/concepts/memory#collection) of individual documents, each with different tradeoffs. For a deeper conceptual overview, see the [memory concepts guide](/oss/concepts/memory).
...
As conversations grow, the full message history may not fit in the LLM's context window, or may degrade response quality. Deep Agents handle this automatically through [context engineering](/oss/deepagents/context-engineering), but you can also manage short-term memory manually. For techniques like summarization, trimming, and filtering, see the [manage short-term memory](/oss/langgraph/add-memory#manage-short-term-memory) guide.
...
To enable long-term memory, use a @[`CompositeBackend`] that routes `/memories/` to persistent storage. The recommended approach is a @[`StoreBackend`] backed by a LangGraph [Store](/langsmith/custom-store), but because backends are pluggable, you can persist memories to any external data source (a vector database, a custom API, cloud storage) by implementing the [backend protocol](/oss/deepagents/backends#protocol-reference).
...
<Note>
`CompositeBackend` strips the route prefix before storing. `/memories/agents.md` is stored as `/agents.md` in the `StoreBackend`. The agent always uses the full path. See [CompositeBackend](/oss/deepagents/backends#compositebackend-router) for details.
</Note>
...
- **Separate read-only and read-write paths.** For data that the agent should use but not modify (org policies, curated skills), populate it from your application code using the [Store API](/langsmith/custom-store) and load it into the system prompt or seed it as files. Don't route it to a writable backend.
...
- For most agents, write memories [in the hot path](/oss/concepts/memory#in-the-hot-path) (during the conversation). It's the simplest approach.
