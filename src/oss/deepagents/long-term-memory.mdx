...
| [**Information type**](#memory-patterns) | What kind of information is it? | [Semantic](/oss/concepts/memory#semantic-memory) (facts), [episodic](/oss/concepts/memory#episodic-memory) (experiences), or [procedural](/oss/concepts/memory#procedural-memory) (instructions) |
| [**Scope**](#scoping-and-access-control) | Who can see and modify it? | [User, agent, or global](#scoping-and-access-control) |
| [**Update strategy**](#update-strategies) | When are memories written? | [Hot path](#hot-path) (during conversation) or [background](#background-consolidation) (between conversations) |
| [**Retrieval**](#retrieval-strategies) | When and how are memories read? | [Upfront](#upfront-retrieval) (loaded into prompt), [progressive](#progressive-retrieval) (read on-demand), or [environment seeding](#environment-seeding) (synced into sandbox) |
...
Use a @[`CompositeBackend`] that routes `/memories/` to persistent storage:
...
def load_memories(
    ...
    """Load memories into the system prompt on each model call."""
    runtime = request.runtime
    store = runtime.store
    namespace = (runtime.context.user_id,)

    item = store.get(namespace, "/preferences.md")
    if item and item.value:
        memories = "\n".join(item.value.get("content", []))
        new_content = list(request.system_message.content_blocks) + [
            {"type": "text", "text": f"User memories:\n{memories}"}
        ]
...
<Note>
`CompositeBackend` strips the route prefix before storing. A file written to `/memories/preferences.md` by an agent with namespace `(user_id,)` is stored at key `/preferences.md` under the `("user-123",)` namespace in the `StoreBackend`. The agent always uses the full path. See [CompositeBackend](/oss/deepagents/backends#compositebackend-router) for details.
</Note>
...
- **[Procedural](/oss/concepts/memory#procedural-memory)** — instructions and rules that shape how the agent behaves (agents.md, user preferences, skills)
- **[Semantic](/oss/concepts/memory#semantic-memory)** — facts and knowledge the agent can reference (knowledge bases, project context)
- **[Episodic](/oss/concepts/memory#episodic-memory)** — past experiences the agent can learn from (conversation history, debugging sessions)
...
<Tip>
For simpler use cases, a single profile file may be enough. As the number of facts grows, a directory of individual documents offers higher recall at the cost of more complex updates. See the [profile vs collection](/oss/concepts/memory#semantic-memory) tradeoffs in the memory concepts guide.
</Tip>
...
Deep Agents get episodic memory for free through [checkpointers](/oss/langgraph/persistence#checkpoints). Every conversation is already persisted as a checkpointed thread, so the agent's full history of past interactions is available without any additional configuration. You can search and retrieve past threads using the [LangGraph SDK](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/python_sdk_ref/#langgraph_sdk.client.LangGraphClient) to surface relevant past experiences.
...
[Skills](/oss/deepagents/skills) are reusable agent capabilities defined as `SKILL.md` files. Skills are a form of procedural memory: they differ from other memory files in structure (frontmatter + steps) and mutability, but serve the same purpose of shaping agent behavior across conversations.
...
Because skills use [progressive disclosure](/oss/deepagents/skills#how-skills-work), only the frontmatter is read at startup. The full skill content is loaded only when the agent determines it's relevant, keeping token usage low even as the skill library grows.
...
For the full namespace factory API, see [namespace factories](/oss/deepagents/backends#namespace-factories).
...
For most applications, start with the hot path. Add background consolidation when you need to reduce latency or improve memory quality across many conversations. See the [memory concepts guide](/oss/concepts/memory#writing-memories) for more on these tradeoffs.
...
Load memories into the system prompt at conversation start using a `wrap_model_call` [middleware](/oss/langchain/middleware/custom). This guarantees the agent always has context, at the cost of tokens on every model call. The [Quick start](#quick-start) example demonstrates this approach.
...
[Skills](/oss/deepagents/skills) are a good example of progressive retrieval: only the frontmatter is loaded at startup, and the full skill content is read only when the agent determines it's relevant.
...
When an agent runs inside a [sandbox](/oss/deepagents/sandboxes), it operates in an isolated container that cannot directly access the memory store. Neither upfront retrieval (middleware injecting into the prompt) nor progressive retrieval (filesystem reads from `/memories/`) works out of the box because the sandbox filesystem is separate from the store.

To bridge this gap, seed memories into the sandbox before the agent runs and sync updated memories back after it finishes. Use [custom middleware](/oss/langchain/middleware/custom) with `before_agent` and `after_agent` hooks to transfer files across the sandbox boundary:
...
- The agent runs in a [sandbox](/oss/deepagents/sandboxes) with an isolated filesystem
- Memories or [skills](/oss/deepagents/skills) need to be executable inside the container (e.g., skill scripts the agent runs via `execute`)
...
For implementation details and a full code example, see [file transfers](/oss/deepagents/going-to-production#file-transfers) and the [syncing skills and memories](/oss/deepagents/going-to-production#file-transfers) example in the going to production guide.
...
- Use [skills](/oss/deepagents/skills) for reusable workflows. They scale better than a monolithic instructions file because they use [progressive disclosure](/oss/deepagents/skills#how-skills-work).
