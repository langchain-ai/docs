...
```mermaid
graph TB
    Agent[Deep Agent] --> Route{Path<br/>Router}

    Route --> |/memories/*| Store[StoreBackend]
    Route --> |other paths| State[StateBackend]

    Store --> Persist[(LangGraph Store<br/>Persistent)]
    State --> Ephemeral[(Agent State<br/>Ephemeral)]

    Persist --> |across threads| Thread1[Thread 1]
    Persist --> |across threads| Thread2[Thread 2]
    Persist --> |across threads| Thread3[Thread 3]

    Ephemeral --> |lost after| Thread1

    style Store fill:#e8f5e9
    style Persist fill:#c8e6c9
    style State fill:#fff4e1
    style Ephemeral fill:#ffe0b2
```
