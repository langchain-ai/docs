...
:::python
For model-specific system prompt customization, such as replacing the base prompt or appending a suffix for a particular provider, use a [harness profile](/oss/deepagents/profiles#harness-profiles): set `base_system_prompt` to replace the built-in base, or `system_prompt_suffix` to append text after it.
:::
