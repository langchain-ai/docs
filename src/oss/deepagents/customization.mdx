...
Deep Agents assemble a fixed [middleware](/oss/langchain/middleware/overview) stack in `create_deep_agent` / `createDeepAgent`. **Order matters** when you add custom middleware, debug Anthropic prompt caching, or reason about which tools exist at each step.
...
2. @[`SkillsMiddleware`]: Only when you pass `skills` / `Skills`. Injected **immediately after** the todo middleware and **before** filesystem middleware so skill metadata is available before file tools run.
3. @[`FilesystemMiddleware`]: Handles file system operations such as reading, writing, and navigating directories.
4. @[`SubAgentMiddleware`]: Spawns and coordinates subagents for delegating tasks to specialized agents.
5. @[`SummarizationMiddleware`]: Condenses message history to stay within context limits when conversations grow long (via @[create_summarization_middleware] / @[createSummarizationMiddleware]).
...
7. @[`AsyncSubAgentMiddleware`]: Only when you configure async subagents (`graph_id` specs).
...
12. @[`MemoryMiddleware`]: Only when you pass `memory` / `Memory`.
...
13. `HumanInTheLoopMiddleware`: Only when you pass `interrupt_on` / `interruptOn`. Pauses for human approval or input at configured tool calls.
14. **Filesystem permissions enforcement**: Only when you pass `permissions` / `permissions`. Always **last** on the stack so it can evaluate every tool the agent might call.

### Default stack (synchronous subagents)

The built-in **general-purpose** subagent and each declarative synchronous `SubAgent` graph use a stack that `create_deep_agent` / `createDeepAgent` builds in code. It matches the main agent in broad shape (todo list, filesystem, summarization, Patch, profile extras, Anthropic caching, optional permissions) but differs in two ways readers should remember:

- **Skills run after** @[`PatchToolCallsMiddleware`] on these inner agents (on the main agent, skills run **before** filesystem middleware when `skills` / `Skills` is set).
- There is **no** @[`SubAgentMiddleware`] inside a subagent graph (only the parent agent exposes the `task` tool).

When a declarative subagent sets `interrupt_on` / `interruptOn`, `HumanInTheLoopMiddleware` is appended for that subagent. See [Human-in-the-loop](/oss/deepagents/human-in-the-loop) for configuration. `CompiledSubAgent` runnables use whatever middleware you defined on the compiled graph.
