...
<AccordionGroup>
    ...
    <Accordion title="Restrict the enabled filesystem tools" icon="filter">
        ...
        Override @[`FilesystemMiddleware`] with a `tools` allowlist to expose only a subset of the filesystem tools to the model, instead of the full default set. Any built-in filesystem tool left out of the list is removed from the model's tool list.
