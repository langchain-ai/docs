...
Deep Agents ship with a built-in base system prompt that teaches the agent how to use the harness scaffolding (filesystem tools, subagents). Pass `system_prompt=` to prepend your own instructions before that base prompt:
...
3. @[`SubAgentMiddleware`]: Spawns and coordinates subagents for delegating tasks to specialized agents.
