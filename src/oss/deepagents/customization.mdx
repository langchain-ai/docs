...
6. @[`PatchToolCallsMiddleware`]: Repairs dangling tool calls in message history when a run resumes after an interruption or receives malformed tool-call arguments. See [Patch tool-call history](#patch-tool-call-history). Runs **before** Anthropic prompt caching and the tail stack below.
...
9. **Harness profile extras**: Provider-specific middleware from the resolved model profile, if any.
10. **Excluded-tool filtering**: When the harness profile lists excluded tools, middleware removes those tools from the agent.
11. @[`AnthropicPromptCachingMiddleware`]: Automatically added when you are using an Anthropic mode. Runs **after** Patch and after your middleware so the cached prefix matches what is actually sent to the model.
12. @[`MemoryMiddleware`]: Only when you pass `memory`.
...
### Patch tool-call history

`PatchToolCallsMiddleware` is an internal Deep Agents middleware that keeps message history valid across interruptions and retries. It runs before the next agent step, scans prior `AIMessage` tool calls, and appends a synthetic `ToolMessage` for any tool call ID that does not already have a matching result.

This repair is most useful when a run is interrupted, canceled, or resumed after the model emitted a tool call but before the tool returned. It also handles malformed or truncated tool-call arguments by adding an explanatory tool message instead of leaving an unanswered tool call in the history. The middleware does not execute tools or change tool outputs. It only patches the transcript so the next model call satisfies provider message-ordering requirements.

You may see `PatchToolCallsMiddleware` in every Deep Agents trace because it is part of the default harness stack. Seeing it in a trace is expected and usually does not indicate an error. If you build a custom harness outside `create_deep_agent` or `createDeepAgent`, include equivalent handling when your application supports interrupts, cancellation, or replay from checkpoints.
