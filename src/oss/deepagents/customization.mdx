...
12. @[`BedrockPromptCachingMiddleware`]: Added after Anthropic caching when the model is an AWS Bedrock model; it no-ops on Bedrock models without cache support (`unsupported_model_behavior="ignore"`). Runs **after** Patch and after your middleware so the cached prefix matches what is actually sent to the model.
13. @[`MemoryMiddleware`]: Only when you pass `memory`.
...
12. @[`BedrockPromptCachingMiddleware`]: Automatically added when you are using an Amazon Bedrock Converse model. Runs **after** Patch and after your middleware so the cached prefix matches what is actually sent to the model.
13. @[`MemoryMiddleware`]: Only when you pass `memory`.
