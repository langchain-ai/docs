...
<AccordionGroup>
    ...
    <Accordion title="Restrict the enabled filesystem tools" icon="filter">
        ...
        Override @[`FilesystemMiddleware`] with a `tools` allowlist to expose only a subset of the filesystem tools to the model, instead of the full default set. Any built-in filesystem tool left out of the list is removed from both the model's tool list and the middleware's dynamic system prompt section.
        ...
        `read_file` must always be included in the list—omitting it raises `ValueError` when the agent is created. As with any @[`FilesystemMiddleware`] override, pass the same `backend` (and `permissions`, if applicable) used elsewhere, since the override is not merged with the default. See [Restricting filesystem tools](/oss/deepagents/overview#virtual-filesystem-access) for more details, including subagent behavior.
    </Accordion>
</AccordionGroup>
:::
