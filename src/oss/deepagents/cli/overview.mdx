...
<Steps>
    ...
    <Step title="Add provider credentials" icon="key">
        The CLI works with any LLM that supports tool calling. Use `/auth` to paste an API key for common providers — see [Provider credentials](/oss/deepagents/cli/configuration#provider-credentials) for the full flow and storage details.
