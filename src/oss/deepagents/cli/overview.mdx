...
<AccordionGroup>
    <Accordion title="Command-line options" icon="flag">
        ...
        | `-n`, `--non-interactive TEXT` | Run a single task non-interactively and exit. Shell is disabled unless `--shell-allow-list` is set. Use `--auto-approve` to also skip HITL prompts |
...
```bash
# Allow specific commands (gated by HITL)
...
`--auto-approve` is orthogonal — it disables all HITL prompts (shell and non-shell). Combine it with `--shell-allow-list` for fully autonomous execution:

```bash
deepagents -n "Run tests and fix failures" --shell-allow-list all --auto-approve
```

<Warning>
    `--shell-allow-list all --auto-approve` lets the agent execute arbitrary shell commands with no human confirmation. Use with caution.
</Warning>
