...
Use `--startup-cmd` to run a shell command once before the session accepts its first prompt. The output is shown to you at the top of the session — handy for checking `git status`, listing files, or verifying environment setup before typing the first prompt.
