...
* <Icon icon="search" size={16} /> **Web search** - search the web for up-to-date information and documentation. Requires a Tavily API key in [`TAVILY_API_KEY`](/oss/deepagents/cli/configuration#enable-web-search-with-tavily).
...
<Accordion title="Full list of built-in tools">
    ...
    | `web_search` | Search the web using Tavily (requires `TAVILY_API_KEY` — see [Enable web search](/oss/deepagents/cli/configuration#enable-web-search-with-tavily)) | Required<sup>1</sup> |
...
<AccordionGroup>
    ...
    <Accordion title="CLI commands" icon="terminal">
        ...
        | `deepagents deploy`                  | Deprecated. For new deployments, use the [Managed Deep Agents API private preview](/langsmith/managed-deep-agents-api). See [Deploy with Managed Deep Agents](/oss/deepagents/deploy) for the deprecated CLI flow |
