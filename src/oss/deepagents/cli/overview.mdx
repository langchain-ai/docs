...
<AccordionGroup>
    <Accordion title="Command-line options" icon="flag">
        ...
        | `-n`, `--non-interactive TEXT` | Run a single task non-interactively and exit. Shell is disabled unless `--shell-allow-list` or `--auto-approve` is set |
...
By default, shell execution is disabled in non-interactive mode. There are two ways to enable it:

1. **`--shell-allow-list`** — enables shell but gates every command against the allow-list via human-in-the-loop:
...
2. **`--auto-approve`** — enables shell **and** auto-approves all tool calls (including shell commands) with no prompts:

```bash
deepagents -n "Run the tests and fix failures" --auto-approve
```

<Warning>
    `--auto-approve` bypasses all human-in-the-loop gating. The agent can execute arbitrary shell commands without confirmation. If both `--auto-approve` and `--shell-allow-list` are set, `--auto-approve` takes precedence and the allow-list is ignored.
</Warning>
