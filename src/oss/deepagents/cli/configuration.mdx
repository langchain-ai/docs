...
A stored key always wins over an env-var key for the same provider—pasting a key into `/auth` is the deliberate, recent action, so it overrides whatever the shell happens to export.
...
Explicit `-a`/`--agent` always overrides both, and `-r`/`--resume` bypasses both so the thread's original agent is restored. See [Switch agents](/oss/deepagents/cli/overview#switch-agents) for the full precedence chain.
