...
<Note>
    `/auth` only manages **LLM provider** credentials. Tool credentials such as `TAVILY_API_KEY` (web search) and `LANGSMITH_API_KEY` (tracing) are read from the environment instead — set them in `~/.deepagents/.env` or your shell. See [Enable web search with Tavily](#enable-web-search-with-tavily) and [Trace with LangSmith](/oss/deepagents/cli/overview#trace-with-langsmith).
</Note>
...
Explicit `-a`/`--agent` always overrides both, and `-r`/`--resume` bypasses both so the thread's original agent is restored. See [Command reference](/oss/deepagents/cli/overview#command-reference) for related flags.
...
<ResponseField name="DEEPAGENTS_CLI_LANGSMITH_PROJECT" type="string" post={["optional"]}>
    Override the LangSmith project name for agent traces. See [Trace with LangSmith](/oss/deepagents/cli/overview#trace-with-langsmith).
</ResponseField>
