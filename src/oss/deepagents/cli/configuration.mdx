...
description: Configure the Deep Agents CLI with config.toml and hooks
---

The CLI stores its configuration in the `~/.deepagents/` directory. The two main config files are:

| File | Format | Purpose |
|------|--------|---------|
| `config.toml` | [TOML](https://toml.io/) | Model defaults, provider settings, constructor params, profile overrides |
