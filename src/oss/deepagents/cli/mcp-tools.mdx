...
<ResponseField name="auth" type='"oauth"'>
    Set to `"oauth"` to drive an OAuth login flow with `deepagents mcp login` instead of supplying an `Authorization` header. Only valid for `http`/`sse` transports. Cannot be combined with an `Authorization` header. See [OAuth login](#oauth-login).
</ResponseField>
...
<Note>
    The `type` field can also be written as `transport` for compatibility with other MCP clients. `streamable_http` and `streamable-http` are accepted as aliases for `http` so configs copied from upstream MCP examples work without edits.
</Note>
