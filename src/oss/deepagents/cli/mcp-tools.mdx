...
There is no dedicated `mcp add` command. Instead, MCP servers are added by editing a `.mcp.json` config file. The CLI auto-discovers configs in three locations, which lets you scope servers either to a single project or to every project on your machine (similar to Claude Code's `.mcp.json` and Codex/Cursor's `mcpServers` block).
...
<Steps>
    <Step title="Choose a scope" icon="layer-group">
        Pick the location that matches how broadly you want the server to be available. Use the user-level path for servers you want everywhere (the equivalent of Claude Code's `claude mcp add --scope user` or Cursor/Codex global config); use a project-level path for servers tied to one repo.

        | Scope | Path | When to use |
        |-------|------|-------------|
        | **User (recommended for general tools)** | `~/.deepagents/.mcp.json` | Available in every project on this machine |
        | Project | `<project>/.mcp.json` | Shared with collaborators via the repo (Claude Code compatible) |
        | Project (hidden) | `<project>/.deepagents/.mcp.json` | Project-specific but kept out of the repo root |

        See [Discovery locations](#discovery-locations) for full precedence rules.
    </Step>

    <Step title="Create the config file" icon="file">
        Create the file at the chosen path. The format follows the [Claude Desktop convention](https://modelcontextprotocol.io/quickstart/user), so the same `mcpServers` block works across clients.

        ```bash
        mkdir -p ~/.deepagents
        # Then create ~/.deepagents/.mcp.json with the content below
        ```
        ...
        To add more servers, add more entries to `mcpServers`. See [Configuration format](#configuration-format) for stdio, SSE, and HTTP server fields, environment variables, and headers.
    </Step>
...
<Tip>
    For remote servers that require OAuth (Linear, Notion, Slack, GitHub, and similar hosted endpoints), set `"auth": "oauth"` on the server entry and then run `deepagents mcp login <server>` once. See [OAuth login](#oauth-login).
</Tip>
