...
- **Add guardrails before launch**. Configure
  [rate limits](/oss/deepagents/fault-tolerance#rate-limiting),
  [error handling](/oss/deepagents/fault-tolerance#error-handling), and
...
<CardGroup cols={2}>
  <Card title="Tool calling" icon="hammer" href="/oss/langchain/frontend/tool-calling">
    Render file and execution tool calls as purpose-built UI cards with type-safe results.
  </Card>
  <Card title="Headless tools" icon="cpu" href="/oss/langchain/frontend/headless-tools">
    Run browser and device APIs on the client side—useful for embedding live preview or terminal output in the IDE panel.
  </Card>
  <Card title="Open-ended generative UI" icon="layout-grid" href="/oss/langchain/frontend/open-ended-generative-ui">
