...
thread's metadata and resolved at runtime via a [graph factory](/langsmith/graph-rebuild) that reads `thread_id` from the runtime.
...
:::python
<Note>
Accessing `runtime.server_info` requires `deepagents>=0.5.0`.
</Note>
:::

:::js
<Note>
Accessing `runtime.serverInfo` requires `deepagents>=1.9.0`.
</Note>
:::
...
async def agent(runtime: Runtime):
    ...
    sandbox = await get_or_create_sandbox_for_assistant(assistant_id)
    return create_deep_agent(backend=sandbox)
```
...
export async function agent(runtime: Runtime) {
  ...
  const sandbox = await getOrCreateSandboxForAssistant(assistantId);
  return createDeepAgent({ backend: sandbox });
}
```
...
async def agent(runtime: Runtime):
    ...
    sandbox = await get_or_create_sandbox_for_user(user_id)
    return create_deep_agent(backend=sandbox)
```
...
export async function agent(runtime: Runtime) {
  ...
  const sandbox = await getOrCreateSandboxForUser(userId);
  return createDeepAgent({ backend: sandbox });
}
```
...
all threads and may expire), resolve the sandbox per-thread at runtime using a
[graph factory](/langsmith/graph-rebuild). The factory receives a `Runtime`
on each invocation, so it can read `thread_id` from `runtime.execution_info`
and build the agent with the correct sandbox before the run starts:
...
async function getOrCreateSandboxForThread(threadId: string): Promise<LangSmithSandbox> {
  ...
  const sandboxId = thread.metadata?.sandbox_id as string | undefined;
...
```python
from deepagents import create_deep_agent
from deepagents.sandbox import LangSmithSandbox, SandboxClient
from langgraph.runtime import Runtime
from langgraph_sdk import get_client


async def get_or_create_sandbox_for_thread(thread_id: str) -> LangSmithSandbox:
    # Check thread metadata for an existing sandbox_id
    client = get_client(url="http://localhost:2024")
    thread = await client.threads.get(thread_id)
    sandbox_id = (thread.get("metadata") or {}).get("sandbox_id")

    if sandbox_id:
        # Reconnect to existing sandbox
        sandbox = await SandboxClient().get_sandbox(sandbox_id)
        return LangSmithSandbox(sandbox=sandbox)

    # Create a new sandbox and store its ID in thread metadata
    sandbox = await LangSmithSandbox.create(template_name="my-template")
    await seed_sandbox(sandbox)
    await client.threads.update(thread_id, metadata={"sandbox_id": sandbox.id})
    return sandbox
...
Wire the factory into `langgraph.json` as the graph entry point so the server calls it on each run.
