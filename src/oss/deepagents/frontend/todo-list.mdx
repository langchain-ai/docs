...
<Note>
Task planning is opt-in. Without @[`TodoListMiddleware`], `stream.values.todos` is not present. See [Task planning](/oss/deepagents/overview#task-planning).
</Note>
...
<CardGroup cols={2}>
  <Card title="Structured output" icon="table" href="/oss/langchain/frontend/structured-output">
    Render any structured agent state—not just todos—with custom UI components instead of plain text.
  </Card>
  <Card title="Join & rejoin streams" icon="arrows-merge" href="/oss/langchain/frontend/join-rejoin">
    Reconnect to a running plan after a page reload or tab switch without losing progress.
  </Card>
  <Card title="Message queues" icon="list" href="/oss/langchain/frontend/message-queues">
    Queue follow-up tasks while the agent is still working through its current plan.
  </Card>
  <Card title="Human-in-the-Loop" icon="hand" href="/oss/langchain/frontend/human-in-the-loop">
