...
<Info>
**Already using `agent.stream(stream_mode=...)`?** That API still works—Event streaming is the new typed-projection layer on top. Existing `.stream()` code continues to function unchanged. The stream modes (`updates`, `values`, `messages`, `custom`, ...) and the existing Deep Agents streaming guide are at the [`.stream()` API guide](/oss/deepagents/streaming).
</Info>
