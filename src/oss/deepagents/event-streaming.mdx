...
<Info>
Deep Agents v0.6 introduced **Event streaming**, a typed-projection API on top of `.stream()`. Each projection (`run.subagents`, `run.messages`, `run.tool_calls`, `run.values`) is its own iterator, so you can read them independently instead of branching on `stream_mode` chunks. For docs on the existing `.stream()` API, see the [`.stream()` API guide](/oss/deepagents/streaming).
</Info>
