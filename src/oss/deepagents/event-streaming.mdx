...
<Info>
Deep Agents v0.6 introduced **event streaming**, a typed-projection API on top of `.stream()`. Each projection is its own iterator, so you can consume them independently instead of branching on stream-mode chunks. See the [streaming](/oss/deepagents/streaming) docs for the existing API.
</Info>
