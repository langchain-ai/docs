...
Each thread should have a unique sandbox associated with it.

For convenience, we recommend using the sandbox ID assigned by the provider when the sandbox is created as the `thread_id`.
If your sandbox ID and `thread_id` differ, store the mapping in your application, or with the sandbox if the sandbox provider allows attaching metadata to the sandbox.
...
If you provision sandboxes in [middleware](/oss/deepagents/middleware), follow these guidelines to manage lifecycle:

1. **Set a TTL** for the sandbox—most providers support this.
    End-users may at any point stop chatting with an agent, so it is best practice to deleter or archive sandboxes aftere some amount of inactivity to avoid ongoing costs.
2. **Wrap `agent.invoke()` in try/finally** to ensure cleanup runs whether the invocation succeeds or raises:
...
try:
    ...
    # cleanup sandbox (delete, archive, etc.)
    ...
```
