...
<CardGroup cols={2}>
    ...
    <Card title="Local shell" href="/oss/deepagents/backends">
        Run commands directly on your host machine with `LocalShellBackend` (development/testing only).
    </Card>
</CardGroup>
:::
...
These examples assume you have already created a sandbox/devbox using the provider's SDK and have credentials set up. For signup, authentication, and provider-specific lifecycle details, see [Available providers](#available-providers). If you use `LocalShellBackend`, no provider setup is required, but commands run directly on your host machine.
...
<Warning>
`LocalShellBackend` provides unrestricted filesystem and shell access on your host machine. Use only in controlled development and testing environments.
</Warning>
