...
<Note>
    **Looking for LangSmith Sandboxes?** LangSmith provides first-party managed sandboxes you can use directly from the LangSmith UI or SDK — no third-party account required. See [LangSmith Sandboxes](/langsmith/sandboxes) for managed sandbox resources, snapshots, service URLs, and the auth proxy.
</Note>
