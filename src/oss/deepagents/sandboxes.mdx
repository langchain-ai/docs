...
The agent runs on your machine or server. When it needs to execute code, it calls a remote sandbox via the provider's API.
...
For a deeper dive with trade-offs and implementation details, see [The two patterns by which agents connect sandboxes](https://blog.langchain.com/the-two-patterns-by-which-agents-connect-sandboxes/).
