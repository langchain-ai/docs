...
3. **Require concise subagent outputs in configuration** – The main agent delegates via the `task` tool, but you control each subagent's `system_prompt`. Use it to ask for short summaries and synthesized findings instead of pasting raw tool output back to the parent.
