...
| **[Context compression](#context-compression)** | Configure built-in offloading and summarization to keep context within window limits | Automatic, when limits approached |
...
For some use cases you may want a dynamic prompt: for example, to tell the model "You have admin access" vs "You have read-only access," or to inject user preferences like "User prefers concise responses" from the store.
If your prompt depends on context or the store, use `@dynamic_prompt` to build context-aware instructions. Your middleware can read `request.runtime.context` and `request.runtime.store`. See [Customization](/oss/deepagents/customization#middleware) for adding [custom middleware](/oss/langchain/middleware) and the [LangChain context engineering](/oss/langchain/context-engineering#system-prompt) guide for examples.

You do **not** need middleware when tools alone use context or store; tools receive `config.context` and `runtime.store` directly. Add middleware only when the system prompt itself must vary per request.
:::

:::js
The `systemPrompt` parameter is static which means it does not change per invocation.
For some use cases you may want a dynamic prompt: for example, to tell the model "You have admin access" vs "You have read-only access," or to inject user preferences like "User prefers concise responses" from the store.
If your prompt depends on context or the store, use `dynamicSystemPromptMiddleware` to build context-aware instructions. Your middleware can read `request.runtime.context` and `request.runtime.store`. See [Customization](/oss/deepagents/customization#middleware) for adding [custom middleware](/oss/langchain/middleware) and the [LangChain context engineering](/oss/langchain/context-engineering#system-prompt) guide for examples.
...
Content offloading happens when tool call inputs or results exceed a token threshold (default 20,000, configurable via `tool_token_limit_before_evict`):
...
Deep agents can use long-term memorey for storing user preferences, accumulated knowledge, research progress, or any information that should persist beyond a single session.
...
For example, you may prompt the agent to store preeferences in `/memories/preferences.txt`.
