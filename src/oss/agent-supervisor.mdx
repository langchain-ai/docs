...
For math agent tools we will use [vanilla Python functions](.././tool-calling#define-a-tool):
...
Putting this all together, let's create a graph for our overall multi-agent system. We will add the supervisor and the individual agents as subgraph [nodes](.././low_level#nodes).
...
Notice that we've added explicit [edges](.././low_level#edges) from worker agents back to the supervisor — this means that they are guaranteed to return control back to the supervisor. If you want the agents to respond directly to the user (i.e., turn the system into a router, you can remove these edges).
