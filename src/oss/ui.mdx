...
First, set up LangGraph API server [locally](./langgraph-platform/local-server) or deploy your agent on [LangGraph Platform](/langgraph-platform/quick-start-studio).
...
Agent Chat UI has full support for [human-in-the-loop](./human_in_the_loop) workflows. To try it out, replace the agent code in `src/agent/graph.py` (from the [deployment](./langgraph-platform/local-server) guide) with this [agent implementation](./human_in_the_loop/add-human-in-the-loop#add-interrupts-to-any-tool):
