...
Here we show how to define and update [state](/oss/low-level#state) in LangGraph. We will demonstrate:

1. How to use state to define a graph's [schema](/oss/low-level#schema)
2. How to use [reducers](/oss/low-level#reducers) to control how state updates are processed.

### Define state

[State](/oss/low-level#state) in LangGraph can be a `TypedDict`, `Pydantic` model, or dataclass. Below we will use `TypedDict`. See [this section](#use-pydantic-models-for-graph-state) for detail on using Pydantic.
...
:::python
Let's consider a simple example using [messages](/oss/low-level#messagesstate). This represents a versatile formulation of state for many LLM applications. See our [concepts page](/oss/low-level#working-with-messages-in-graph-state) for more detail.
:::
...
Let's build an example graph with a single node. Our [node](/oss/low-level#nodes) is just a Python function that reads our graph's state and makes updates to it. The first argument to this function will always be the state:
...
Let's next define a simple graph containing this node. We use [StateGraph](/oss/low-level#stategraph) to define a graph that operates on this state. We then use [add_node](/oss/low-level#nodes) populate our graph.
...
Each key in the state can have its own independent [reducer](/oss/low-level#reducers) function, which controls how updates from nodes are applied. If no reducer function is explicitly specified then it is assumed that all updates to the key should override it.
...
* We may want to accept short-hands for [message formats](/oss/low-level#using-messages-in-your-graph), such as [OpenAI format](https://python.langchain.com/docs/concepts/messages/#openai-format).
...
To add a sequence of nodes, we use the `.add_node` and `.add_edge` methods of our [graph](/oss/low-level#stategraph):
...
and debugged using [LangGraph Studio](../../langgraph-platform/studio).
...
Let's first define our [state](/oss/low-level#state). This governs the [schema of the graph](/oss/low-level#schema), and can also specify how to apply updates. See [this section](#process-state-updates-with-reducers) for more detail.
...
Our [nodes](/oss/low-level#nodes) are just Python functions that read our graph's state and make updates to it. The first argument to this function will always be the state:
...
<Note>
  ...
  By default, this will **overwrite** the value of the corresponding key. You can also use [reducers](/oss/low-level#reducers) to control how updates are processed— for example, you can append successive updates to a key instead. See [this section](#process-state-updates-with-reducers) for more detail.
</Note>

Finally, we define the graph. We use [StateGraph](/oss/low-level#stategraph) to define a graph that operates on this state.

:::python
We will then use [add_node](/oss/low-level#messagesstate) and [add_edge](/oss/low-level#edges) to populate our graph and define its control flow.
:::
...
* We must specify the entry point of the graph. For this we add an edge with the [START node](/oss/low-level#start-node).
* The graph halts when there are no more nodes to execute.

We next [compile](/oss/low-level#compiling-your-graph) our graph. This provides a few basic checks on the structure of the graph (e.g., identifying orphaned nodes). If we were adding persistence to our application via a [checkpointer](/oss/persistence), it would also be passed in here.
...
In this example, we fan out from `Node A` to `B and C` and then fan in to `D`. With our state, [we specify the reducer add operation](/oss/low-level#reducers). This will combine or accumulate values for the specific key in the State, rather than simply overwriting the existing value. For lists, this means concatenating the new list with the existing list. See the above section on [state reducers](#process-state-updates-with-reducers) for more detail on updating state with reducers.
...
<Note>
  In the above example, nodes `"b"` and `"c"` are executed concurrently in the same [superstep](/oss/low-level#graphs). Because they are in the same step, node `"d"` executes after both `"b"` and `"c"` are finished.
  
  Importantly, updates from a parallel superstep may not be ordered consistently. If you need a consistent, predetermined ordering of updates from a parallel superstep, you should write the outputs to a separate field in the state together with a value with which to order them.
</Note>

<Accordion title="Exception handling?">
  LangGraph executes nodes within [supersteps](/oss/low-level#graphs), meaning that while parallel branches are executed in parallel, the entire superstep is **transactional**. If any of these branches raises an exception, **none** of the updates are applied to the state (the entire superstep errors).
  ...
  2. You can set a **[retry_policy](../reference/types#langgraph.types.RetryPolicy)** to direct the graph to retry nodes that raise certain types of exceptions. Only failing branches are retried, so you needn't worry about performing redundant work.
...
When creating a graph with a loop, we require a mechanism for terminating execution. This is most commonly done by adding a [conditional edge](/oss/low-level#conditional-edges) that routes to the [END](/oss/low-level#end-node) node once we reach some termination condition.

You can also set the graph recursion limit when invoking or streaming the graph. The recursion limit sets the number of [supersteps](/oss/low-level#graphs) that the graph is allowed to execute before it raises an error. Read more about the concept of recursion limits [here](/oss/low-level#recursion-limit).
...
In some applications, we may not have a guarantee that we will reach a given termination condition. In these cases, we can set the graph's [recursion limit](/oss/low-level#recursion-limit). This will raise a `GraphRecursionError` after a given number of [supersteps](/oss/low-level#graphs). We can then catch and handle this exception:
...
<Accordion title="Extended example: loops with branches">
  ...
  This graph looks complex, but can be conceptualized as loop of [supersteps](/oss/low-level#graphs):
...
We can now create the `StateGraph` with the above nodes. Notice that the graph doesn't have [conditional edges](/oss/low-level#conditional-edges) for routing! This is because control flow is defined with `Command` inside `node_a`.
...
<Warning>
  **State updates with `Command.PARENT`**
  When you send updates from a subgraph node to a parent graph node for a key that's shared by both parent and subgraph [state schemas](/oss/low-level#schema), you **must** define a [reducer](/oss/low-level#reducers) for the key you're updating in the parent graph state. See the example below.
</Warning>
