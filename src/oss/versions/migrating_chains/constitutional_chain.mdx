...
* Delegation of instructions to [message roles](/oss/concepts/messages) (e.g., chat models can understand what a `ToolMessage` represents without the need for additional prompting);
...
<Accordion title="Details">
  Below, we use the [.with\_structured\_output](/oss/how_to/structured_output) method to simultaneously generate (1) a judgment of whether a critique is needed, and (2) the critique. We surface all prompts involved for clarity and ease of customizability.
...
See guides for generating structured output [here](/oss/how_to/structured_output).
