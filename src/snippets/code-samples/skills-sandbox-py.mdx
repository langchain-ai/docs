...
class SkillSandboxSyncMiddleware(AgentMiddleware[AgentState, AppContext, Any]):
    ...
    async def abefore_agent(self, state: AgentState, runtime: Runtime[AppContext]) -> None:
    ...
    backend = CompositeBackend(
        ...
        routes={
            f"{SKILLS_ROOT}/": StoreBackend(
                store=store,
                namespace=lambda rt: ("skills", rt.server_info.user.identity if rt.server_info else rt.context.user_id),
            ),
        },
    )
