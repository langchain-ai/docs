...
Method names and endpoints are unchanged. Only the session (project) ID requirement changes. `session_id` is now required on every feedback creation path: the SDK methods, `POST /api/v1/feedback`, and the `feedback.<run_id>` parts of `POST /runs/multipart`.
...
<Tabs>
  ...
  <Tab title="cURL">
    <Warning>
    Both feedback creation paths now require `session_id`, the UUID of the project (session) that owns the run. It was previously optional.
    </Warning>

    | Operation | Before | After | Notes |
    |---|---|---|---|
    | `POST /api/v1/feedback` | `session_id` (optional) | `session_id` (**required**) | Send it as a field in the JSON request body |
    | `POST /runs/multipart` | `session_id` (optional) | `session_id` (**required**) | Send it in each `feedback.<run_id>` part's JSON payload |

    Resolve the project UUID with `GET /api/v1/sessions` if you do not already have it.
  </Tab>
</Tabs>
