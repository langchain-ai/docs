...
- [Remote MCP servers](/langsmith/fleet/remote-mcp-servers) now accept LangSmith API keys through `X-Api-Key`, so programmatic clients can connect without OAuth.
