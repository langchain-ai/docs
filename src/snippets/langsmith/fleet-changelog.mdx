...
- [Agent identity](/langsmith/fleet/agent-identity) error messages now refer to agent identity instead of `agent_type`, so the immutability failure is easier to understand.
