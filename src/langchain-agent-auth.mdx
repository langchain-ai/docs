...
The agent then resumes execution from the point it left off at, and the token can be used for any API calls. We store and refresh OAuth tokens on our side so that future uses of the service by either the user or agent don't require an OAuth flow.
...
Grab the `auth_url` and provide it to the user for out-of-band OAuth flows.
