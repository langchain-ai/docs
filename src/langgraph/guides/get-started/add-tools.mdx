import ChatModel from '/snippets/chat_model.mdx';
...
!!! note
    ...
    This tutorial builds on [Build a basic chatbot](build-a-basic-chatbot.mdx).
...
For the `StateGraph` you created in the [first tutorial](build-a-basic-chatbot.mdx), add `bind_tools` on the LLM. This lets the LLM know the correct JSON format to use if it wants to use the search engine.
...
![chatbot-with-tools-diagram](/images/chatbot-with-tools.png)
...
The chatbot cannot remember past interactions on its own, which limits its ability to have coherent, multi-turn conversations. In the next part, you will [add **memory**](../tutorials/get-started/3-add-memory.md) to address this.
