...
<Info>
  **Prerequisites**
  * [LangGraph Studio Overview](langgraph_studio)
</Info>

LangGraph Studio supports connecting to two types of graphs:

* Graphs deployed on [LangGraph Platform](deployment_quickstart).
...
For applications that are [deployed](deployment_quickstart) on LangGraph Platform, you can access Studio as part of that deployment. To do so, navigate to the deployment in LangGraph Platform within the LangSmith UI and click the "LangGraph Studio" button.
...
Next, install the [LangGraph CLI](langgraph_cli):
