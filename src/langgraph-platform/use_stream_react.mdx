...
The [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) React hook provides a seamless way to integrate LangGraph into your React applications. It handles all the complexities of streaming, state management, and branching logic, letting you focus on building great chat experiences.
...
Let's explore how to use [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) in your React application.

The [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) provides a solid foundation for creating bespoke chat experiences. For pre-built chat components and interfaces, we also recommend checking out [CopilotKit](https://docs.copilotkit.ai/coagents/quickstart/langgraph) and [assistant-ui](https://www.assistant-ui.com/docs/runtimes/langgraph).
...
The [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) hook takes care of all the complex state management behind the scenes, providing you with simple interfaces to build your UI. Here's what you get out of the box:
...
The [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) hook can automatically resume an ongoing run upon mounting by setting `reconnectOnMount: true`. This is useful for continuing a stream after a page refresh, ensuring no messages and events generated during the downtime are lost.
...
The [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) hook will keep track of the message chunks received from the server and concatenate them together to form a complete message. The completed message chunks can be retrieved via the `messages` property.
...
Under the hood, the [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) hook will use the `streamMode: "messages-tuple"` to receive a stream of messages (i.e. individual LLM tokens) from any LangChain chat model invocations inside your graph nodes. Learn more about messages streaming in the [streaming](streaming#messages) guide.

### Interrupts

The [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) hook exposes the `interrupt` property, which will be filled with the last interrupt from the thread. You can use interrupts to:
...
The [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) hook is friendly for apps written in TypeScript and you can specify types for the state to get better type safety and IDE support.
...
The [`useStream()`](https://langchain-ai.github.io/langgraph/cloud/reference/sdk/js_ts_sdk_ref) hook provides several callback options to help you respond to different events:
