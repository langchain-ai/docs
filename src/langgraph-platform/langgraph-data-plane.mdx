...
The term "data plane" is used broadly to refer to [LangGraph Servers](langgraph-server) (deployments), the corresponding infrastructure for each server, and the "listener" application that continuously polls for updates from the [LangGraph Control Plane](langgraph-control-plane).
...
The data plane "listener" application periodically calls [control plane APIs](langgraph-control-plane#control-plane-api) to:
...
<Info>
  **Only for Cloud SaaS**
  Data regions are only applicable for [Cloud SaaS](langgraph-cloud) deployments.
</Info>
...
[`Production` type](langgraph-control-plane#deployment-types) deployments automatically scale up to 10 containers. Scaling is based on 3 metrics:
...
<Info>
  **Only for Cloud SaaS**
  Static IP addresses are only available for [Cloud SaaS](langgraph-cloud) deployments.
</Info>
...
<Info>
  Custom Postgres instances are only available for [Self-Hosted Data Plane](langgraph-self-hosted-data-plane) and [Self-Hosted Control Plane](langgraph-self-hosted-control-plane) deployments.
</Info>

A custom Postgres instance can be used instead of the [one automatically created by the control plane](langgraph-control-plane#database-provisioning). Specify the [`POSTGRES_URI_CUSTOM`](env-var#postgres_uri_custom) environment variable to use a custom Postgres instance.
...
<Info>
  Custom Redis instances are only available for [Self-Hosted Data Plane](langgraph-self-hosted-control-plane) and [Self-Hosted Control Plane](langgraph-self-hosted-control-plane) deployments.
</Info>
