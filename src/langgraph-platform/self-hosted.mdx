...
2. **[Standalone Container](#standalone-container)**: Deploy standalone instances of a LangGraph Server without the control plane UI
...
The Standalone Container deployment model is the least restrictive option for deployment. There is no [control plane](/langgraph-platform/control-plane). [Data plane](/langgraph-platform/data-plane) infrastructure is managed by you.
...
The Standalone Container deployment model supports deploying data plane infrastructure to a Kubernetes cluster.

#### Docker

The Standalone Container deployment model supports deploying data plane infrastructure to any Docker-supported compute platform.
