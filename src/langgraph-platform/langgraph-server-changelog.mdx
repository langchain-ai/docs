...
<a id="2025-07-30"></a>
## v0.2.113
- Improved thread search pagination by adjusting response headers to better indicate remaining pages and next offset.

<a id="2025-07-30"></a>
## v0.2.112
- Ensured proper asynchronous handling by awaiting the sync logging method and added a linter to prevent future issues.
- Fixed an issue where JS tasks were not populated correctly in task results.
...
<a id="2025-07-29"></a>
## v0.2.110
- Added interrupts as default values for all outputs except streams, preserving behavior for join operations.

<a id="2025-07-28"></a>
## v0.2.109
- Fixed a bug causing missing configuration schema when `config_type` was not set.

<a id="2025-07-28"></a>
## v0.2.108
- Added compatibility with langgraph v0.6, including support for the new context API and improved runtime handling.

<a id="2025-07-27"></a>
## v0.2.107
- Cached authentication data to improve retrieval speed and efficiency.
- Merged count and select queries to optimize database performance.

<a id="2025-07-27"></a>
## v0.2.106
- Made log stream operations resumable to enhance reliability and user experience.

<a id="2025-07-27"></a>
## v0.2.105
- Added a heapdump endpoint to write memory heap to a file for better debugging.

<a id="2025-07-25"></a>
## v0.2.103
- Corrected the metadata endpoint usage to ensure accurate data retrieval.

<a id="2025-07-24"></a>
## v0.2.102
- Captured interrupt events in the wait method to restore previous behavior in streaming values.
- Added support for SDK structlog in the JavaScript implementation.

<a id="2025-07-24"></a>
## v0.2.101
- Corrected the metadata endpoint used for self-hosted deployments.

<a id="2025-07-22"></a>
## v0.2.99
- Improved license check by adding in-memory caching and better error handling for Redis connection issues.
- Ensured only manually created assistants persist in memory, discarding removed graph entries from the config.
- Ensured the UI namespace for generated UI is a valid JavaScript property name.
- Raised a 422 error for invalid input data to improve error handling.

<a id="2025-07-19"></a>
## v0.2.98
- Added langgraph node context to improve log filtering and enhance trace visibility.

<a id="2025-07-19"></a>
## v0.2.97
- Fixed an issue with task scheduling in the main loop for better compatibility with the ckpt ingestion worker.
- Ensured the queue worker starts only after migrations have completed to prevent deployment issues.
- Enhanced thread state error messages and ensured proper HTTP 201 response when thread state update issues occur during creation.
- Exposed interrupt ID when retrieving thread state for improved API clarity.
