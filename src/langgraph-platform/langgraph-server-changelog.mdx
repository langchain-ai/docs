...
<a id="2025-07-17"></a>
## v0.2.113 (2025-07-30)
- Improved thread search pagination by updating response headers for more accurate total and next offset information.

## v0.2.112 (2025-07-30)
- Ensured async logging methods are properly awaited and added a linter to prevent future issues.
- Fixed an issue where JS tasks were not being populated correctly in JS graphs.

## v0.2.111 (2025-07-29)
- Started the heartbeat immediately upon connection to prevent JS graph streaming failures during long startups.

## v0.2.110 (2025-07-29)
- Added interrupts as default values for all operations except streams, ensuring consistent behavior.

## v0.2.109 (2025-07-28)
- Fixed a long-standing bug by returning the config schema even when `config_type` is not set.

## v0.2.108 (2025-07-28)
- Added compatibility for langgraph v0.6, including new context API support and improved context handling.

## v0.2.107 (2025-07-27)
- Implemented caching for authentication processes to enhance performance.
- Merged count and select queries for improved database performance and efficiency.

## v0.2.106 (2025-07-27)
- Implemented resumable functionality for log streams to enhance reliability and user experience.

## v0.2.105 (2025-07-27)
- Added a new endpoint to generate and save heap dumps to a file for diagnostic purposes.

## v0.2.103 (2025-07-25)
- Used the correct metadata endpoint to resolve connectivity issues.

## v0.2.102 (2025-07-24)
- Captured interrupt events in the wait method to restore previous behavior of streaming values.
- Added support for SDK structlog in the JavaScript module.

## v0.2.101 (2025-07-24)
- Corrected the metadata endpoint for self-hosted environments to ensure proper functionality.

## v0.2.99 (2025-07-22)
- Added an in-memory cache for licenses and improved handling of Redis connection errors during license validation.
- Ensured that only manually created assistants persist in memory, discarding those removed from the configuration.
- Ensured the UI namespace for gen UI is a valid JavaScript property name to prevent errors.
- Enhanced error handling by raising a 422 status code for unprocessable entity responses.

## v0.2.98 (2025-07-19)
- Added langgraph node context to improve log filtering and tracing capabilities.

## v0.2.97 (2025-07-19)
- Resolved an issue with task scheduling for the checkpoint ingestion worker when running in isolated background loops.
- Delayed the start of the queue worker until all migrations have finished to prevent deployment issues.
- Enhanced error messages for thread state issues by adding specific metadata fields and improved status codes for better clarity.
- Exposed interrupt ID when retrieving thread state for better API clarity.
