...
title: Streaming API
---
[LangGraph SDK](../cloud/reference/sdk/python_sdk_ref/) allows you to [stream outputs](https://langchain-ai.github.io/langgraph/concepts/streaming/) from the LangGraph API server.

<Note>
  LangGraph SDK and LangGraph Server are a part of [LangGraph Platform](langgraph_platform).
</Note>
...
LangGraph Platform allows you to join an active [background run](background_run) and stream outputs from it. To do so, you can use [LangGraph SDK's](../cloud/reference/sdk/python_sdk_ref/) `client.runs.join_stream` method:
