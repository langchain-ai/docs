...
The term "data plane" is used broadly to refer to [LangGraph Servers](langgraph_server) (deployments), the corresponding infrastructure for each server, and the "listener" application that continuously polls for updates from the [LangGraph Control Plane](langgraph_control_plane).

## Server Infrastructure

In addition to the [LangGraph Server](langgraph_server) itself, the following infrastructure components for each server are also included in the broad definition of "data plane":
...
The data plane "listener" application periodically calls [control plane APIs](langgraph_control_plane#control-plane-api) to:
...
<Info>
  **Only for Cloud SaaS**
  Data regions are only applicable for [Cloud SaaS](langgraph_cloud) deployments.
</Info>
...
[`Production` type](langgraph_control_plane#deployment-types) deployments automatically scale up to 10 containers. Scaling is based on 3 metrics:
...
<Info>
  **Only for Cloud SaaS**
  Static IP addresses are only available for [Cloud SaaS](langgraph_cloud) deployments.
</Info>
...
<Info>
  Custom Postgres instances are only available for [Self-Hosted Data Plane](langgraph_self_hosted_data_plane) and [Self-Hosted Control Plane](langgraph_self_hosted_control_plane) deployments.
</Info>

A custom Postgres instance can be used instead of the [one automatically created by the control plane](langgraph_control_plane#database-provisioning). Specify the [`POSTGRES_URI_CUSTOM`](env_var#postgres_uri_custom) environment variable to use a custom Postgres instance.
...
<Info>
  Custom Redis instances are only available for [Self-Hosted Data Plane](langgraph_self_hosted_control_plane) and [Self-Hosted Control Plane](langgraph_self_hosted_control_plane) deployments.
</Info>

A custom Redis instance can be used instead of the one automatically created by the control plane. Specify the [REDIS_URI_CUSTOM](env_var#redis_uri_custom) environment variable to use a custom Redis instance.
