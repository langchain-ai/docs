...
public class TraceablePipeline {
  ...
  private static final Function<String, List<ChatCompletionMessageParam>> formatPrompt =
  ...
  private static final Function<List<ChatCompletionMessageParam>, ChatCompletion> invokeLlm =
      Tracing.traceFunction(
          messages ->
              openai.chat()
                  .completions()
  ...
  private static final Function<ChatCompletion, String> parseOutput =
      Tracing.traceFunction(
          response -> response.choices().get(0).message().content().orElse(""),
          TraceConfig.builder().name("parse_output").build());

  private static final Function<String, String> runPipeline =
      Tracing.traceFunction(
