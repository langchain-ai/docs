...
title: Quickstart
description: Set up a local documentation preview, route a change to its authored owner or generator input, and run the smallest relevant validation for Mintlify routes and generated surfaces.
...
Read `AGENTS.md` before editing. It is the repository-wide authoring guide; `CLAUDE.md` directs OpenWiki agents there. Task-specific procedures live in `.agents/skills/`. Claude Code reads the linked `.claude/skills/` tree, so run `make skills` after adding or renaming a skill.
...
| Change | Edit this owner | Inspect after a build |
| --- | --- | --- |
...
| Managed Deep Agents | `src/langsmith/managed-deep-agents*.mdx` | Python and JavaScript `/langsmith/...` variants. Unversioned Managed Deep Agents URLs redirect to Python routes. |
...
The site navigation has two products. **AGENT DEVELOPMENT LIFECYCLE** contains Home, Build, Test, Deploy, and Monitor. **PRODUCTS AND SETUP** contains LangSmith setup, LLM Gateway, No-code agents, Engine, and Deep Agents Code. Labels do not establish source ownership: Build combines OSS and LangSmith route families, and **No-code agents** is the navigation label for `src/langsmith/fleet/`. Find a neighboring route in the intended `docs.json` array and mirror its shape deliberately.

For source-to-route details and redirects, see [Source Directory Map](/openwiki/architecture/source-map.md). For language variants and conditional content, see [Versioned Documentation and Routes](/openwiki/concepts/versioning.md). For adding, moving, or retiring a page, see [Adding and Maintaining Documentation Pages](/openwiki/operations/adding-pages.md).

## Change derived content at its input

Do not use generated output as an alternate source:
...
- Inspect every output variant required by the source family, especially both Managed Deep Agents variants.
