...
Documentation changes follow a PR workflow where all tests must pass before merging. Publishing is handled by the [publish workflow](https://github.com/langchain-ai/docs/actions/workflows/publish.yml) (requires authorization). Preview branches with build artifacts are automatically created via the [Build Preview workflow](https://github.com/langchain-ai/docs/actions/workflows/build-preview.yml). See steps below on how to preview locally and begin contributing.
...
When you create or update a PR, the [Build Preview workflow](https://github.com/langchain-ai/docs/actions/workflows/build-preview.yml) automatically:
- Runs preprocessing to generate build artifacts
- Creates or updates a preview branch (e.g., `preview-pr-123`) with the build artifacts
- Posts/updates a comment on your PR with the preview branch name and timestamp
- Triggers Mintlify's automatic deployment of the preview branch

**Your PR branch stays clean** - build artifacts are only committed to the preview branch.

To view the preview:

1. Wait for the Build Preview workflow to complete (you'll see a PR comment).
2. Mintlify will automatically detect and deploy the preview branch.
3. Visit the [Mintlify Previews Dashboard](https://dashboard.mintlify.com/langchain-5e9cc07a/langchain-5e9cc07a?section=previews).
4. Look for your preview branch (e.g., `preview-pr-123`) and click **Visit** to view it.

**Updates:** When you push new changes to your PR:
- The same preview branch is updated (force-pushed) with fresh build artifacts
- The PR comment is updated with a new timestamp
- Mintlify automatically redeploys the updated preview

**Cleanup:** When your PR is merged or closed, the [Cleanup Preview workflow](https://github.com/langchain-ai/docs/actions/workflows/cleanup-preview.yml) automatically deletes the preview branch and removes the Mintlify deployment.
