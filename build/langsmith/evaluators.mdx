...
An evaluator attaches to an [environment](/langsmith/agent-environments#select-an-environment) of an [agent](/langsmith/agents). Attaching one to a single environment is how to score production traffic without also scoring local runs.
