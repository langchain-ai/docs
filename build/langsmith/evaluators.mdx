...
import AgentBasedNote from '/snippets/python/langsmith/agent-based-note.mdx';
import ProjectBasedNote from '/snippets/python/langsmith/project-based-note.mdx';
...
What an evaluator attaches to depends on how your workspace is organized, and the sidebar tells you which one you are on.

<AgentBasedNote />

An evaluator attaches to a tracing project, which in an agent-based workspace is one [environment](/langsmith/agent-environments#the-four-environments) of an [agent](/langsmith/agents). Attaching one to a single environment is how to score production traffic without also scoring local runs.
...
1. Open the **Projects & Datasets** tab. For each attachment listed there, select **Detach** in the **Actions** menu at the right of the row.
