...
This runs your agent in LangSmith Studio so you can test it before deploying. Optionally compile [evals](/langsmith/managed-deep-agents-evals) with `mda evals compile` and run them with Harbor.
...
On success, the CLI prints a URL to your deployment dashboard in LangSmith where you can inspect build status, traces, and runtime state.
