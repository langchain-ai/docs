...
In an [agent-based workspace](/langsmith/agents), the project that `LANGSMITH_PROJECT` names becomes an [environment](/langsmith/agent-environments) of an agent. To address the agent directly, see [Log traces to an agent](/langsmith/log-traces-to-agent).
