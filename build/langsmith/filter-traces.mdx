...
import FilteringExperienceNote from '/snippets/python/langsmith/filtering-experience-note.mdx';
...
To query runs programmatically instead, refer to [Query traces using the SDK](/langsmith/export-traces). The SDK and REST API use a separate structured query language, documented in [Trace query syntax](/langsmith/trace-query-syntax). For a mapping between the UI operator names and their API equivalents, refer to the [UI-to-API operator map](/langsmith/trace-query-syntax#ui-to-api-operator-map).
