...
- **Retry on failure.** Configurable [retry policies](/oss/python/langgraph/graph-api-nodes#add-retry-policies) control backoff, max attempts, and which exceptions trigger retries on a per-node basis. Runs survive process restarts, infrastructure failures, and code revisions mid-execution.
