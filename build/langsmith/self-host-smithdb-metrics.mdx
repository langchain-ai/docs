...
Kubernetes signals such as OOM kills, container restarts, CPU and memory against limits, and SSD node disk usage are worth alerting on but are not SmithDB metrics. See [Troubleshoot supporting infrastructure](/langsmith/self-host-smithdb-infrastructure#troubleshoot-supporting-infrastructure).
