...
The `langsmith:` provider prefix routes through the gateway's unified `/v1` endpoint. Set `LANGSMITH_GATEWAY="true"` so LangChain picks up your `LANGSMITH_API_KEY` automatically:
...
The `langsmith:` prefix also gives you access to [Gateway Credits](/langsmith/llm-gateway-credits) models (for example, `langsmith:moonshotai/kimi-k3`).
