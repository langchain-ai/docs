...
<Tabs>
  <Tab title="LangSmith UI">
    ...
    1. Select **Submit**. LangSmith queues the deployment for provisioning and creates a tracing project with the same name. In an agent-based workspace the deployment is bound to an [agent](/langsmith/agents) and reports into one of its [environments](/langsmith/agent-environments).
