...
1. If the deployment is pinned to a Git tag, set **Preview base branch** to the branch whose pull requests should create previews.
...
Preview builds match pull requests targeting the deployment's Git branch by default. For a deployment pinned to a Git tag, set **Preview base branch** to identify the pull requests that create previews. The main deployment remains pinned to the tag, while preview deployments use the pull request commits.
...
### Deletion
Preview deployments are tied to their parent deployment. Deleting the parent deployment triggers a cascaded deletion of its preview deployments. 

LangSmith will automatically delete a preview deployment when its configured TTL expires. You can also delete a preview deployment manually at any time.
