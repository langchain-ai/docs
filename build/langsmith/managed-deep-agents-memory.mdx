...
Memory is not your system prompt. Define behavior in `instructions.md` and `skills/**`; use memory for durable knowledge the agent learns while it runs.

## Enable memory

Add an optional `memory.py` or `memory.ts` file at the project root. Export memory with the `"agent"` scope:
...
Remove the memory declaration to turn durable memory off. You can also use `scope="none"` in Python or `scope: "none"` in TypeScript.
