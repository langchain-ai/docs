...
Middleware adds behavior around model calls, tool calls, and the agent lifecycle. Like [custom tools](/langsmith/python/managed-deep-agents-tools), MDA does not discover middleware automatically. Import it into the agent entry and pass it to the agent definition.
