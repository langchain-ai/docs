...
On later deploys, the CLI compares Context Hub instructions with the previous sync. If the local `instructions.md` does not match a Hub edit, an interactive deploy asks which version to keep; unattended deploys require an explicit conflict strategy. See [Resolve Context Hub edits](/langsmith/python/managed-deep-agents-deploy#resolve-context-hub-edits).
