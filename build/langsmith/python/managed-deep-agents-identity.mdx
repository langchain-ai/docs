...
OIDC principal authentication binds an end user's verified identity to their [user-owned connections](/langsmith/python/managed-deep-agents-connections#choose-a-credential-owner). It lets a custom application identify callers without sending a user ID that the application chose.
