...
On later deploys, the CLI compares Context Hub instructions and skills with the previous sync. If a local version does not match a Hub edit, you choose which version to keep. Context Hub also backs optional [durable memory](/langsmith/python/managed-deep-agents-memory), which deployments never overwrite.

Developers can keep context in code while domain experts edit it in LangSmith. Learn more about [instructions](/langsmith/python/managed-deep-agents-instructions), [skills](/langsmith/python/managed-deep-agents-skills), [deploy conflict handling](/langsmith/python/managed-deep-agents-deploy#resolve-context-hub-edits), and [Context Hub](/langsmith/use-the-context-hub).
