...
If an MCP server requires an API key or OAuth token, use a connection to resolve the credential at runtime. See [Manage connections](/langsmith/python/managed-deep-agents-connections).
