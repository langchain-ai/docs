...
`define_deep_agent` does not compile an agent. It returns a definition that `mda deploy` hands to the managed runtime, which supplies the backend, store, checkpointer, memory, skills, and system prompt, then compiles the agent with [create_deep_agent](https://reference.langchain.com/python/deepagents/graph/create_deep_agent). The parameters below are that same surface minus the managed ones. For how the two functions relate, see [Relationship to Deep Agents](/langsmith/python/managed-deep-agents-overview#relationship-to-deep-agents).
