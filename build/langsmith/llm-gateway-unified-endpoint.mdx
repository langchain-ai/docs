...
Your response contains one entry per model from every provider reachable for your workspace. A provider whose [Provider Secret](/langsmith/llm-gateway-admin-setup#1-add-provider-secrets) isn't configured is skipped rather than failing the request.
