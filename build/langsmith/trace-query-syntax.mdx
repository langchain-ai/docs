...
| `like`     | Pattern match (case-sensitive); `%` matches any sequence | `like(metadata_value, "%prod%")` |
| `notlike`  | Inverse pattern match; `%` matches any sequence          | `notlike(metadata_value, "%staging%")` |
...
`like` and `notlike` match a field value against a pattern. Use `%` as a wildcard that matches any sequence of characters, including an empty sequence.

- `like(metadata_value, "%prod%")` matches any value that contains `prod`, such as `production` or `eu-prod-1`.
- `notlike(metadata_value, "%staging%")` matches any value that does not contain `staging`.

Patterns are case-sensitive. Wrap the pattern in double quotes and place `%` inside the quotes where you want the wildcard, for example `"%staging%"`. Without `%`, `like` behaves like `eq`.

`like` and `notlike` are the API equivalents of the UI's **contains** and **does not contain** operators. Refer to the [UI-to-API operator map](#ui-to-api-operator-map) for a full mapping.
...
## UI-to-API operator map

The LangSmith UI offers named operators in its filter builder. The table below shows the API comparator that corresponds to each UI operator.

| UI operator | API comparator | Notes |
| ----------- | -------------- | ----- |
| is | `eq` | Exact match. |
| is not | `neq` | Inverts `eq`. |
| contains | `like` | Use `%` as a wildcard: `like(metadata_value, "%staging%")`. |
| does not contain | `notlike` | Use `%` as a wildcard: `notlike(metadata_value, "%staging%")`. |
| greater than | `gt` | Numeric and timestamp fields. |
| greater than or equal | `gte` | Numeric and timestamp fields. |
| less than | `lt` | Numeric and timestamp fields. |
| less than or equal | `lte` | Numeric and timestamp fields. |
| has tag | `has` | `has(tags, "value")`. |
| in list | `in` | `in(metadata_key, ["a", "b"])`. |

Refer to [Filter traces](/langsmith/filter-traces) for the full UI filter reference.

## Alert filters

Alert rules in LangSmith accept the same filter query language in their `filter` and `denominator_filter` fields. A few behaviors are specific to alerts:

- **Runs without the metadata key** match `neq` and `notlike` conditions. A run with no `env` metadata key satisfies `notlike(metadata_value, "%staging%")`.
- **Error, inputs, and outputs** are matched token-by-token, not as a full phrase. Use `like` with `%` to match a substring reliably.
- The `filter` field narrows which runs count toward the alert numerator. The `denominator_filter` field narrows the baseline population.

Example alert filter that excludes staging runs:

```text
and(eq(metadata_key, "env"), notlike(metadata_value, "%staging%"))
```
...
# Runs whose "env" metadata value contains "prod" (like: contains)
and(eq(metadata_key, "env"), like(metadata_value, "%prod%"))

# Runs whose "env" metadata value does not contain "staging" (notlike: does not contain)
and(eq(metadata_key, "env"), notlike(metadata_value, "%staging%"))
```
