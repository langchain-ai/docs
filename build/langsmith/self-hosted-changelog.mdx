...
- When a Slack channel event failed before reaching your Managed Deep Agent — a dropped delivery, a missing bot token, an interrupt that couldn't be shown, or an ambiguous route — it now surfaced in `mda logs` and the deployment logs UI instead of failing silently.
