...
Put each channel in a separate module under `channels/`. Export a named `channel` in TypeScript or a module-level `channel` in Python:

```text
my-agent/
  agent.py | agent.ts
  channels/
    support.py | support.ts
```

The file name becomes the configured channel name. It identifies the channel at runtime and forms part of its inbound route. For example, a declaration in `channels/support.ts` receives events at:

```text
POST /channels/support/events
```

Do not name a declaration `channels/channel.py` or `channels/channel.ts`. Channel names must be unique within a project.

The provider factory creates the declaration. For example, `channels.slack()` creates a Slack channel. See the provider guide for its complete declaration and setup procedure.
