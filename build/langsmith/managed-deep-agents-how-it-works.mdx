...
A sandbox gives the agent an isolated environment for code execution and filesystem work. Configure one by exporting `sandbox` from `sandbox/index.ts` or `sandbox/__init__.py`. Sandboxes default to one per thread; set `scope` to `agent` to share one across the agent process. For configuration options and examples, see [Sandboxes](/langsmith/managed-deep-agents-sandboxes).
