...
How LangSmith groups traces depends on which information architecture your workspace uses. The sidebar tells you which one you are on.

#### Agents and environments

<AgentBasedNote />

An _agent_ collects the traces of a single application, and it is the unit LangSmith organizes everything else around. Each agent has four _environments_: Local, Development, Staging, and Production. Traces land in one environment of one agent, so a query about production behavior never has to exclude test traffic by hand.
