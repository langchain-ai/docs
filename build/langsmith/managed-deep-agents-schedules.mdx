...
Set `deliver_to` / `deliverTo` to post the final response through a configured [Slack channel](/langsmith/managed-deep-agents-channels-slack). Use a Slack channel ID because scheduled runs have no originating thread.
