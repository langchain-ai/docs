...
Each mount has an `id`, a `type`, and a `mount_path` / `mountPath`. Bucket and Git mount paths must be absolute paths under `/mnt/mounts`. Context Hub mount paths can be any absolute path outside the system directories, so an agent can read its context from the location it already expects.
...
Identify the repo as `owner/name`. Use `-` as the owner for a repo in the current workspace, such as `-/my-agent`. The caller's API key must have read access to the repo: LangSmith rejects sandbox creation for a repo that is private to another workspace, and reports missing and inaccessible repos identically. Context Hub mounts need no AWS or GCP auth.
...
The mount contains the flattened file tree of the repo's latest commit. Files that the repo links from another agent or skill repo appear at the path where the parent repo references them, so a mounted agent also carries the skills it composes. For more information on composing repos, see [Manage contexts with the SDK](/langsmith/manage-contexts-sdk).

### Read a repo as it changes

Context Hub mounts are read-only, and the sync is one-way. Files written under the mount path inside the sandbox have nowhere to go, and the next refresh overwrites them. Write sandbox output somewhere else, and push it with the Context Hub SDK when it belongs in the repo.

LangSmith keeps the mount in sync for the sandbox's lifetime. New commits reach a running sandbox within roughly 30 seconds. Treat that cadence as best effort rather than a freshness guarantee, because a refresh replaces the whole tree at once: a reader sees either the previous commit or the new one, never a mix.
...
- **A rejected request**: reads fail with `EIO`. Revoking the caller's access to the repo after the sandbox starts rejects later refreshes, because LangSmith re-checks access on every one.
- **A repo that exceeds the sync limits**: the mount serves no tree. A synced commit can hold at most 2500 files and 25 MiB of file content, counting everything the repo links.
...
- Context Hub mounts accept agent and skill repos. Other repo types, and a repo with no commits, are rejected.
- A Context Hub mount path cannot be the filesystem root, or at or under a system directory such as `/etc`, `/usr`, or `/var`.
