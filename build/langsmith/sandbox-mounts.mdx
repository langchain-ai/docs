...
The mount-specific `mount_config.auth.aws` API accepts either `role_arn` or the static `access_key_id` and `secret_access_key` pair, not both. Mount-specific roles are restricted to the configured S3 buckets, prefixes, and read-only settings. Mount authentication cannot be combined with an AWS proxy rule; it is separate from the shared role setup described here.
