...
A trajectory evaluator scores how an agent reached its answer, not just what it said. It reads the [trajectory](/langsmith/observability-concepts#trajectories) of a conversation: the flat, ordered list of messages from start to finish, including tool calls and their results. Use one to score whether the agent picked the right tools, followed its plan, and got there without wasted steps.

Trajectory evaluators require a tracing project. They differ from thread evaluators in what the judge receives:

- **Thread evaluators** receive the conversation assembled from the top-level `messages` key of each trace, through the `all_messages`, `human_ai_pairs`, or `first_human_last_ai` variable. These views can omit the agent's actual work: tool calls, tool results, and system messages.
- **Trajectory evaluators** receive the resolved trajectory through the `trajectory` variable, built from the model and tool calls inside each trace. The tool calls and tool results that the thread views omit are preserved.
...
<Warning>
A trajectory is larger than any thread view, because it includes the tool calls and reasoning that `all_messages` omits. Choose a model with a high context window, as described in step 6 of [Configuration](#configuration), and expect higher token usage per evaluation.
</Warning>
...
1. Under **Source**, select **Threads**.
2. Reference `{{trajectory}}` in your prompt and map that variable to the `trajectory` source. The mapping is what makes the evaluator a trajectory evaluator, and the variable must be named `trajectory`: a differently named variable pointed at that source is rejected. The seeded default prompt and mapping already do this, so a new evaluator needs no changes here.
...
`{{trajectory}}` cannot appear alongside `{{all_messages}}`, `{{human_ai_pairs}}`, or `{{first_human_last_ai}}`. Once you map the trajectory source, the thread sources are disabled in the variable source dropdown, and once you map a thread source, the trajectory source is disabled. Either way the dropdown explains that trajectory variables and non-trajectory variables cannot be used together. Saving an evaluator that mixes them fails the same way.

A trajectory evaluator runs on an [automation rule](/langsmith/rules) scoped to a tracing project. When it completes, LangSmith writes feedback to the most recent root run in the thread, under the feedback key you configured, and associates that feedback with the thread. To find it later, see [View feedback](/langsmith/threads#view-feedback).
