...
<Note>
These are integration requirements and practical recommendations, not a prescribed cloud architecture. Provider examples cover Amazon EKS and Google GKE. Azure is not documented yet.
</Note>
...
SmithDB requires PostgreSQL 18 or later on a service reachable from the cluster. Common choices include Amazon RDS for PostgreSQL, Aurora PostgreSQL, AlloyDB, and Cloud SQL for PostgreSQL.
...
Configure access so SmithDB can list the bucket and read, write, and delete objects. Prefer IRSA on EKS or Workload Identity on GKE over static credentials. Block public access and require encrypted transport.
