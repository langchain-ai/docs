...
On later deploys, the CLI compares Context Hub skills with the previous sync. If the local `skills/` directory does not match a Hub edit, an interactive deploy asks which version to keep; unattended deploys require an explicit conflict strategy. The choice applies to the entire `skills/` directory. See [Resolve Context Hub edits](/langsmith/javascript/managed-deep-agents-deploy#resolve-context-hub-edits).
