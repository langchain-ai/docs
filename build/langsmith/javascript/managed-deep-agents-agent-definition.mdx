...
`defineDeepAgent` does not compile an agent. It returns a definition that `mda deploy` hands to the managed runtime, which supplies the backend, store, checkpointer, memory, skills, and system prompt, then compiles the agent with [createDeepAgent](https://reference.langchain.com/javascript/deepagents/agent/createDeepAgent). The options below are that same surface minus the managed ones. For how the two functions relate, see [Relationship to Deep Agents](/langsmith/javascript/managed-deep-agents-overview#relationship-to-deep-agents).
