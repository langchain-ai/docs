...
| **Filesystem and shell** | Any [backend](/oss/javascript/deepagents/backends): agent state, local disk, a store, or a [sandbox](/oss/javascript/deepagents/sandboxes) you provision with a provider. | One managed [sandbox](/langsmith/javascript/managed-deep-agents-sandboxes), declared in a file. LangSmith creates it, snapshots it, and stops it when idle. |
| **Memory across conversations** | You run the storage and wire it up yourself. | One shared [memory](/langsmith/javascript/managed-deep-agents-memory) tree, enabled by adding a file. |
| **Third-party access for end users** | You provision and store credentials for each user. | [Connections](/langsmith/javascript/managed-deep-agents-connections) hold workspace secrets and run OAuth, so end users authorize services themselves. |
| **Chat surfaces** | You write the integration. | [Channels](/langsmith/javascript/managed-deep-agents-channels) connect Slack and other messaging services. |
| **Scheduled runs** | You run the scheduler. | [Schedules](/langsmith/javascript/managed-deep-agents-schedules) run managed cron jobs. |
...
The harness is the same either way, so building on Deep Agents first and moving when the agent is ready for production is a normal path. See [Going to production](/oss/javascript/deepagents/going-to-production).
...
| --- | --- | --- |
| `systemPrompt` | `instructions.md` | Move the text into the file, then remove the option. See [Instructions](/langsmith/javascript/managed-deep-agents-instructions). |
...
- **Custom backends**: A hand-written `BackendProtocol` implementation has no equivalent. The runtime owns the backend, and the [managed sandbox](/langsmith/javascript/managed-deep-agents-sandboxes) is the only choice for giving the agent a filesystem and shell.
