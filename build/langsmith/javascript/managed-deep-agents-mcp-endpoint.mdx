...
- **Tool name**: The agent `name` set in the [agent definition](/langsmith/javascript/managed-deep-agents-agent-definition#name).
...
Each `/mcp` request is stateless and independent. The endpoint does not support sessions, so separate calls do not share thread state. To retain knowledge across calls, enable [durable memory](/langsmith/javascript/managed-deep-agents-memory).

## Distinguish the endpoint from other capabilities

- **MCP endpoint** exposes the deployed agent as a tool to MCP clients.
- **[MCP connectors](/langsmith/javascript/managed-deep-agents-mcp-connectors)** add tools hosted by remote MCP servers to the agent.
- **[Channels](/langsmith/javascript/managed-deep-agents-channels)** receive messages from an external messaging service that start agent runs and deliver responses.
...
<CardGroup cols={2}>
  ...
  <Card title="Memory" icon="database" href="/langsmith/javascript/managed-deep-agents-memory">
