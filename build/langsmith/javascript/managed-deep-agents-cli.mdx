...
A connection links a managed deep agent to an external service. A connection can store a fixed secret, such as an API key for a custom [Tavily](https://www.tavily.com/) tool, or use OAuth to access a service such as Google on behalf of a user. Use `mda connections` to create and manage this authentication in the current LangSmith workspace.
...
The OAuth catalog lists services with preconfigured settings. When you pass a listed service to `--oauth`, the CLI supplies its authorization URL, token URL, token endpoint authentication method, authorization parameters, and default scopes. The catalog includes services such as Notion, Google, and GitHub:
