...
A connection links a managed deep agent to an external service. A connection can store a fixed secret, such as an API key for a custom [Tavily](https://www.tavily.com/) tool, or user-specific OAuth grants for services such as Google. Use `mda connections` to create and manage this authentication in the current LangSmith workspace. Connection values do not appear in command output.
...
List the OAuth services built into the CLI. The catalog includes services such as Notion, Google, and GitHub:
...
Each connection is tied to the current LangSmith workspace. A secret connection value is shared by all callers of the agent. An OAuth connection stores a separate grant for each authenticated user.
