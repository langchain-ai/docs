...
A connection links a managed deep agent to an external service. A connection can have the type `secret`, such as Tavily, or `oauth2`, such as Google. Use `mda connections` to create and manage the authentication for these services in the current LangSmith workspace. Connection values do not appear in command output.

| Command | Use |
| --- | --- |
| `mda connections catalog` | List the OAuth services built into the CLI. |
| `mda connections create <slug>` | Create an agent-owned API key or user-owned OAuth connection. |
| `mda connections list` | List connection metadata for the workspace. |
| `mda connections get <slug>` | Show metadata for one connection. |
| `mda connections check` | Verify that all connections declared by MCP connectors exist and have the correct owner. |
...
| `--oauth SERVICE` | Use a provider from `mda connections catalog`. |
...
For runtime and authorization examples, see [Manage connections](/langsmith/javascript/managed-deep-agents-connections).
