...
| `mda connection …` | Create and inspect workspace connections used by tools and MCP connectors. |
...
A connection links the agent to an external service. A connection can have the type `secret`, such as Tavily, or `oauth2`, such as Google or Notion. Use `mda connection` to create and manage connections in the current LangSmith workspace. Connection values do not appear in command output.

| Command | Use |
| --- | --- |
| `mda connection catalog` | List the OAuth services built into the CLI. |
| `mda connection create <slug>` | Create an agent-owned API key or user-owned OAuth connection. |
| `mda connection list` | List connection metadata for the workspace. |
| `mda connection get <slug>` | Show metadata for one connection. |
| `mda connection check` | Verify that all connections declared by MCP connectors exist and have the correct owner. |
| `mda connection delete <slug>` | Delete a connection and its stored material. |

Create an agent-owned API key from an environment variable:
...
Create a user-owned OAuth connection from the built-in service catalog:
...
<CodeGroup>
    ...
    pnpm exec mda connection create notion --oauth notion
    ```

    ```bash bun
    bunx mda connection catalog
    bunx mda connection create notion --oauth notion
    ```
</CodeGroup>


Each connection is tied to the current LangSmith workspace. A secret connection value is shared by all callers of the agent. An OAuth connection stores a separate grant for each authenticated user. If an MCP connector declaration uses the slug, the CLI verifies that its owner matches the declaration.

The following flags control connection creation:

| Flag | Use |
| --- | --- |
| `--from-env VAR` | Read a fixed value or OAuth client secret from the shell or project `.env`. |
| `--from-file PATH` | Read a fixed value or OAuth client secret from a file. |
| `--oauth SERVICE` | Use a provider from `mda connection catalog`. |
| `--app APP` | Create or reuse a named workspace OAuth application. |
| `--client-id ID` | Set the OAuth client ID. |
| `--client-secret-env VAR` | Read the OAuth client secret from the named environment variable. |
...
| `--authorize-url URL` | Set a custom OAuth authorization endpoint. Requires `--token-url` and `--app`. |
| `--token-url URL` | Set a custom OAuth token endpoint. Requires `--authorize-url` and `--app`. |

Use `--json` with `catalog`, `list`, `get`, or `check` for machine-readable output. Use `--yes` with `delete` to skip the confirmation prompt.

For runtime examples and authorization interrupt handling, see [Manage connections](/langsmith/javascript/managed-deep-agents-connections).
...
A project with a Slack channel cannot use `--no-wait` because Slack provisioning requires the deployed Agent Server URL. For the complete workflow, see [Connect a Managed Deep Agent to Slack](/langsmith/javascript/managed-deep-agents-channels-slack).
