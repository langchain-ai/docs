...
A connection links a managed deep agent to an external service. Tools and MCP connectors resolve it at runtime. Create connections in one of three modes: opaque secret (fixed API key), general OAuth (BYOT app from the catalog or custom endpoints), or MCP OAuth (discover and register from an MCP server URL). Use `mda connections` to manage these credentials in the current workspace.
...
The OAuth catalog lists services with preconfigured settings. When you pass a listed service to `--oauth`, the CLI supplies its authorization URL, token URL, token endpoint authentication method, authorization parameters, and default scopes. Catalog names include `github`, `google`, and `notion-api`:
...
For create modes, owners, and runtime examples, see [Manage connections](/langsmith/javascript/managed-deep-agents-connections).
