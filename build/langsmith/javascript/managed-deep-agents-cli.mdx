...
Use `mda connection` to manage API keys and OAuth connection definitions in the current LangSmith workspace. Connection values do not appear in command output.

The command requires `LANGSMITH_WORKSPACE_ID` or `--workspace-id`, except for `mda connection catalog`. Run connection commands from the project root, or pass `--project PATH`.
...
The CLI infers the owner from an MCP connector declaration when the project declares the slug. Without a declaration, a fixed value creates an agent-owned connection, and `--oauth` creates a user-owned connection.
