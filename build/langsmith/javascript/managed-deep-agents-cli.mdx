...
| `--context-strategy overwrite\|keep-hub` | Resolve Context Hub changes without prompting. `overwrite` applies local files; `keep-hub` preserves Hub-edited sections. Requires `managed-deepagents>=0.5.3`. |
...
5. Inspect and sync deploy-owned context to Context Hub, prompting if Hub and local edits conflict.
...
| Deploy reports that Context Hub instructions or skills changed since the last sync | Run interactively to choose which version to keep, or pass `--context-strategy overwrite` or `--context-strategy keep-hub`. |
| Deploy reports that the Context Hub repo changed during deploy | Re-run `mda deploy` against the latest Context Hub commit. |
