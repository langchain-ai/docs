...
The only required file is `agent.ts` or `agent.tsx` at the project root containing the [agent definition](/langsmith/python/managed-deep-agents-agent-definition) as a named `agent`. It must export a named `agent` created with `defineDeepAgent`. Use only one agent entry in a project.
