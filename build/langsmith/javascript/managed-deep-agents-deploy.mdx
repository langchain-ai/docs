...
## Resolve Context Hub edits

<Note>
Context Hub change detection requires `managed-deepagents>=0.5.3`.
</Note>

Each Context Hub sync records a baseline for `instructions.md` and `skills/`. On a later deploy, the CLI detects when a section changed in Context Hub and the local version does not match that change. An interactive deploy asks whether to overwrite the Hub-edited section with the local version. Answer **No** to preserve the Hub version or **Yes** to apply the local version.

For CI and other unattended deploys, choose a strategy explicitly:

```bash
# Preserve sections edited in Context Hub.
mda deploy . --context-strategy keep-hub

# Replace Hub-edited sections with the local project copies.
mda deploy . --context-strategy overwrite
```

The strategy applies separately to `instructions.md` and the entire `skills/` directory. With `keep-hub`, the deploy skips local changes for each conflicting section. With `overwrite`, the deploy replaces the section with the local version and removes Hub skill files that do not exist locally.

If a section has not changed in Context Hub since the previous sync, the deploy applies local changes automatically. The CLI evaluates instructions and skills separately, so preserving one section does not block the other from syncing.

Conflict detection starts after the CLI records a baseline in Context Hub. Managed Deep Agents treats durable memory under `memories/` as Hub-owned, so neither strategy overwrites it. If the Context Hub repo changes while the deploy is running, the deploy stops; rerun `mda deploy` against the latest version.
