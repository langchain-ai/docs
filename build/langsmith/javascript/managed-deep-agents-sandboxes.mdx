...
Keep the agent entry point at the project root and the sandbox declaration under `sandbox/`. Add `sandbox/setup.sh` only if you want to provision a recipe snapshot:
...
| --- | --- | --- |
| `idleTtlSeconds` | `600` | Seconds of inactivity before the sandbox can be reclaimed. |
| `defaultTimeout` | `600` | Seconds allowed for each command. |


## Provision a snapshot

If `sandbox/setup.sh` exists, `mda deploy` and `mda dev` bake it once into a recipe snapshot. New threads clone that snapshot instead of running `setup.sh`.

The script runs with `bash -e`. A non-zero exit fails the bake and the deploy or `mda dev` session. The previous deployment pointer stays in place.
...
Project `.env` values that deploy forwards are available as environment variables on the builder, for example a token used to clone a private repo. Thread sandboxes that clone the snapshot do not inherit those variables. Do not write secrets onto the filesystem during bake; anything on disk is part of every thread's image.

Editing `setup.sh` and redeploying does not wipe `/workspace` on live threads. Those boxes keep the files they already have. A new thread clones the new recipe.
