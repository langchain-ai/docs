...
A channel receives messages that start agent runs and delivers responses. A [connector](/langsmith/javascript/managed-deep-agents-connectors) gives the agent tools from an external service, such as a remote MCP server. A project can use either or both.
...
- [Connectors](/langsmith/javascript/managed-deep-agents-connectors): attach remote MCP servers and load their tools.
