...
Managed Deep Agents runs the open source [Deep Agents](/oss/javascript/deepagents/overview) harness rather than a second framework. The agent loop, filesystem tools, subagents, and skills are the same. What differs is who compiles the agent.
