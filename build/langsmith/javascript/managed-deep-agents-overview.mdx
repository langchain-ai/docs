...
On later deploys, the CLI compares Context Hub instructions and skills with the previous sync. If a local version does not match a Hub edit, you choose which version to keep. Context Hub also backs optional [durable memory](/langsmith/javascript/managed-deep-agents-memory), which deployments never overwrite.

Developers can keep context in code while domain experts edit it in LangSmith. Learn more about [instructions](/langsmith/javascript/managed-deep-agents-instructions), [skills](/langsmith/javascript/managed-deep-agents-skills), [deploy conflict handling](/langsmith/javascript/managed-deep-agents-deploy#resolve-context-hub-edits), and [Context Hub](/langsmith/use-the-context-hub).
