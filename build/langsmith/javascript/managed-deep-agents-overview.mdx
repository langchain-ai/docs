...
- **A managed runtime**: Every deployment runs on [LangSmith Agent Server](/langsmith/agent-server-overview). You get the Agent Server API, threads, runs, streaming, and the [MCP endpoint](/langsmith/javascript/managed-deep-agents-mcp-endpoint) without operating the server yourself.
...
Managed Deep Agents runs the open source [Deep Agents](/oss/javascript/deepagents/overview) harness. It is not a second agent framework, and the agent loop, filesystem tools, subagents, and skills are the same ones Deep Agents provides. The two differ in who compiles and runs the agent.



- [createDeepAgent](https://reference.langchain.com/javascript/deepagents/agent/createDeepAgent) returns a compiled agent. You run it in your own process, and you own the backend, store, checkpointer, and server around it.
...
Use Deep Agents directly when you run the agent yourself: in your own process or image, behind your own server, or embedded in a larger LangGraph application. Choose it as well when the agent needs a custom [backend](/oss/javascript/deepagents/backends), store, or checkpointer, or when the deployment needs custom application code and routes. See [Going to production](/oss/javascript/deepagents/going-to-production).

Use Managed Deep Agents when you want LangSmith to run the agent and the surrounding infrastructure. To convert an agent you already built, see [Move from Deep Agents](/langsmith/javascript/managed-deep-agents-migrate).
