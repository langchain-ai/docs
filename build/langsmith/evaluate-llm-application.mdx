...
In Java/Kotlin use `evaluateAsync()` for non-blocking evaluation. Requires `com.langchain.smith:langsmith-java:0.1.0-beta.11` or later.
...
// Optionally wrap with traceable to record inputs and outputs in LangSmith.
val toxicityClassifier = traceable("toxicity_classifier") { inputs: Map<String, Any?> ->
    // Call your LLM here and return a map your evaluators can read.
    mapOf("class" to "Not toxic")
}
```
...
val labeledTexts =
    listOf(
        mapOf("text" to "Shut up, idiot") to mapOf("label" to "Toxic"),
        mapOf("text" to "You're a wonderful person") to mapOf("label" to "Not toxic"),
        mapOf("text" to "This is the worst thing ever") to mapOf("label" to "Toxic"),
        mapOf("text" to "I had a great day today") to mapOf("label" to "Not toxic"),
        mapOf("text" to "Nobody likes you") to mapOf("label" to "Toxic"),
        mapOf("text" to "This is unacceptable. I want to speak to the manager.") to
            mapOf("label" to "Not toxic"),
    )

for ((inputs, outputs) in labeledTexts) {
    client.examples().create(
        ExampleCreateParams.builder()
            .datasetId(dataset.id())
            .inputs(
                ExampleCreateParams.Inputs.builder()
                    .putAdditionalProperty("text", JsonValue.from(inputs["text"]))
                    .build()
            )
            .outputs(
                ExampleCreateParams.Outputs.builder()
                    .putAdditionalProperty("label", JsonValue.from(outputs["label"]))
                    .build()
            )
            .build()
    )
...
- Java/Kotlin: Requires `com.langchain.smith:langsmith-java:0.1.0-beta.11`
...
val correct = runEvaluator { outputs, reference ->
    val score = if (outputs["class"] == reference["label"]) 1.0 else 0.0
...
val params =
    EvaluateParams.builder()
        ...
        .metadata(
            mapOf(
                "models" to listOf("openai:gpt-5.4-mini"),
                "prompts" to listOf("my-org/my-eval-prompt:abc12345"),
            )
        )
        .addEvaluator(correct)
        .build()

val results = evaluate(client, toxicityClassifier, params)
```
...
Use `evaluateAsync()` when you do not want to wait for every row to finish:

```kotlin
val future = evaluateAsync(client, toxicityClassifier, params)
...
evaluateComparative(
    ...
    EvaluateComparativeParams.builder()
        .addEvaluator(myComparativeEvaluator)
        .build(),
)
```
