...
To evaluate your agent's performance you can use `LangSmith` [evaluations](/langsmith/evaluation-techniques). You would need to first define an evaluator function to judge the results from an agent, such as final outputs or trajectory. Depending on your evaluation technique, this may or may not involve a reference output:
...
To run an evaluator, you will first need to create a [LangSmith dataset](/langsmith/evaluation-overview). To use the prebuilt AgentEvals evaluators, you will need a dataset with the following schema:
