...
for example, if you want to [extract output matching some schema](/oss/javascript/tutorials/extraction) 
...
response. LangChain includes a suite of [built-in tools](/oss/javascript/integrations/tools/) 
and supports several methods for defining your own [custom tools](/oss/javascript/how-to/custom_tools). 
...
[]
```
Note that adding message chunks will merge their corresponding tool call chunks. This is the principle by which LangChain's various [tool output parsers](/oss/javascript/how-to/output_parser_structured) support streaming.
...
- **Output parsing**: See [OpenAI Tools output
    ...
    various formats.
- **Structured output chains**: [Some models have constructors](/oss/javascript/how-to/structured_output) that
