...
In some situations you may want to implement a custom [parser](/oss/javascript/concepts/output_parsers/) to structure the model output into a custom format.

There are two ways to implement a custom parser:

1. Using `RunnableLambda` or `RunnableGenerator` in [LCEL](/oss/javascript/concepts/lcel/) -- we strongly recommend this for most use cases
