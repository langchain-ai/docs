...
- [The Runnable interface](/oss/javascript/concepts/runnables/)
- [Chaining runnables](/oss/javascript/how-to/sequence/)
- [Binding runtime arguments](/oss/javascript/how-to/binding/)
...
- A `configurable_fields` method. This lets you configure particular fields of a runnable.
  - This is related to the [`.bind`](/oss/javascript/how-to/binding) method on runnables, but allows you to specify parameters for a given step in a chain at runtime rather than specifying them beforehand.
...
If using [init_chat_model](/oss/javascript/how-to/chat_models_universal_init/) to create a chat model, you can specify configurable fields in the constructor:
...
This method is applicable when [binding tools](/oss/javascript/concepts/tool_calling/) as well:
...
In addition to `.with_config`, we can now include the parameter when passing a configuration directly. See example below, where we allow the underlying model temperature to be configurable inside of a [langgraph agent](/oss/javascript/tutorials/agents/):
...
You can also use the `.configurable_fields` method on arbitrary [Runnables](/oss/javascript/concepts/runnables/), as shown below:
...
- Using [.bind()](/oss/javascript/how-to/binding) as a simpler way to set a runnable's runtime parameters
