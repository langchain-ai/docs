...
Tracking [token](/oss/javascript/concepts/tokens/) usage to calculate cost is an important part of putting your app in production. This guide goes over how to obtain this information from your LangChain model calls.
...
```
You can also enable streaming token usage by setting `stream_usage` when instantiating the chat model. This can be useful when incorporating chat models into LangChain [chains](/oss/javascript/concepts/lcel): usage metadata can be monitored when [streaming intermediate steps](/oss/javascript/how-to/streaming#using-stream-events) or using tracing software such as [LangSmith](https://docs.smith.langchain.com/).
...
Next, check out the other how-to guides chat models in this section, like [how to get a model to return structured output](/oss/javascript/how-to/structured_output) or [how to add caching to your chat models](/oss/javascript/how-to/chat_model_caching).
