...
* **RECOMMENDED**: Load the CSV(s) into a SQL database, and use the approaches outlined in the [SQL tutorial](/oss/javascript/tutorials/sql_qa).
...
Most SQL databases make it easy to load a CSV file in as a table ([DuckDB](https://duckdb.org/docs/data/csv/overview.html), [SQLite](https://www.sqlite.org/csv.html), etc.). Once you've done this you can use all of the chain and agent-creating techniques outlined in the [SQL tutorial](/oss/javascript/tutorials/sql_qa). Here's a quick example of how we might do this with SQLite:
...
```
And create a [SQL agent](/oss/javascript/tutorials/sql_qa) to interact with it:
...
This approach easily generalizes to multiple CSVs, since we can just load each of them into our database as its own table. See the [Multiple CSVs](/oss/javascript/how-to/sql_csv#multiple-csvs) section below.
...
To help enforce proper use of our Python tool, we'll using [tool calling](/oss/javascript/how-to/tool_calling):
...
There are a number of tools like [E2B](/oss/javascript/integrations/tools/e2b_data_analysis) and [Bearly](/oss/javascript/integrations/tools/bearly) that provide sandboxed environments for Python code execution, to allow for safer code-executing chains and agents.

## Next steps

For more advanced data analysis applications we recommend checking out:

* [SQL tutorial](/oss/javascript/tutorials/sql_qa): Many of the challenges of working with SQL db's and CSV's are generic to any structured data type, so it's useful to read the SQL techniques even if you're using Pandas for CSV data analysis.
* [Tool use](/oss/javascript/how-to/tool_calling): Guides on general best practices when working with chains and agents that invoke tools
* [Agents](/oss/javascript/tutorials/agents): Understand the fundamentals of building LLM agents.
* Integrations: Sandboxed envs like [E2B](/oss/javascript/integrations/tools/e2b_data_analysis) and [Bearly](/oss/javascript/integrations/tools/bearly), utilities like [SQLDatabase](https://python.langchain.com/api_reference/community/utilities/langchain_community.utilities.sql_database.SQLDatabase.html#langchain_community.utilities.sql_database.SQLDatabase), related agents like [Spark DataFrame agent](/oss/javascript/integrations/tools/spark_sql).
