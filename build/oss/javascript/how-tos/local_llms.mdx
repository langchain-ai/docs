...
Some providers have [chat model](/oss/javascript/concepts/chat_models) wrappers that takes care of formatting your input prompt for the specific local model you're using. However, if you are prompting local models with a [text-in/text-out LLM](/oss/javascript/concepts/text_llms) wrapper, you may need to use a prompt tailored for your specific model.
...
See the [`llama.cpp`](/oss/javascript/integrations/llms/llamacpp) setup [here](https://github.com/abetlen/llama-cpp-python/blob/main/docs/install/macos.md) to enable this.
...
We can use model weights downloaded from [GPT4All](/oss/javascript/integrations/llms/gpt4all) model explorer.
...
For example, you can implement a [RAG application](/oss/javascript/tutorials/rag) using the chat models demonstrated here.
