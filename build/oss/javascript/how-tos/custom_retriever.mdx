...
Many LLM applications involve retrieving information from external data sources using a [Retriever](/oss/javascript/concepts/retrievers/). 
...
<Tip>
**By inherting from `BaseRetriever`, your retriever automatically becomes a LangChain [Runnable](/oss/javascript/concepts/runnables) and will gain the standard `Runnable` functionality out of the box!**
...
The main benefit of implementing a retriever as a `BaseRetriever` vs. a `RunnableLambda` (a custom [runnable function](/oss/javascript/how-to/functions)) is that a `BaseRetriever` is a well
