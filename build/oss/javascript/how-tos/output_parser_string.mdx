...
LangChain [message](/oss/javascript/concepts/messages/) objects support content in a [variety of formats](/oss/javascript/concepts/messages/#content), including text, [multimodal data](/oss/javascript/concepts/multimodality/), and a list of [content block](/oss/javascript/concepts/messages/#aimessage) dicts.

The format of [Chat model](/oss/javascript/concepts/chat_models/) response content may depend on the provider. For example, the chat model for [Anthropic](/oss/javascript/integrations/chat/anthropic/) will return string content for typical string input:
