...
- [LangChain Expression Language](/oss/javascript/concepts/lcel)
- [Chains](/oss/javascript/how-to/sequence/)
- [Streaming](/oss/javascript/how-to/streaming/)
...
[LangChain Expression Language (LCEL)](/oss/javascript/concepts/lcel) supports aborting runnables that are in-progress via a runtime [signal](https://developer.mozilla.org/en-US/docs/Web/API/AbortController/signal) option.
...
Built-in signal support requires `@langchain/core>=0.2.20`. Please see here for a [guide on upgrading](/oss/javascript/how-to/installation/#installing-integration-packages).
...
To see how this works, construct a chain such as the one below that performs [retrieval-augmented generation](/oss/javascript/tutorials/rag). It answers questions by first searching the web using [Tavily](/oss/javascript/integrations/retrievers/tavily), then passing the results to a chat model to generate a final answer:
...
- [Pass through arguments from one step to the next](/oss/javascript/how-to/passthrough)
- [Dispatching custom events](/oss/javascript/how-to/callbacks_custom_events)
