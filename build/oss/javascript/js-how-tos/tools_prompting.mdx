...
- [Tool calling](/oss/javascript/how-to/tool_calling/)

</Info>

In this guide we'll build a Chain that does not rely on any special model APIs (like tool calling, which we showed in the [Quickstart](/oss/javascript/how-to/tool_calling)) and instead just prompts the model directly to invoke tools.
...
First, we need to create a tool to call. For this example, we will create a custom tool from a function. For more information on all details related to creating custom tools, please see [this guide](/oss/javascript/how-to/custom_tools).
