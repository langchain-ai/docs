...
- [Custom callback handlers](/oss/javascript/how-to/custom_callbacks)
- [Stream Events API](/oss/javascript/concepts/streaming#streamevents)

</Info>

In some situations, you may want to dipsatch a custom callback event from within a [Runnable](/oss/javascript/concepts/#runnable-interface) so it can be surfaced
in a custom callback handler or via the [Stream Events API](/oss/javascript/concepts/streaming#streamevents).
...
The most useful way to consume custom events is via the [`.streamEvents()`](/oss/javascript/concepts/streaming#streamevents) method.
...
Dispatching custom callback events requires `@langchain/core>=0.2.16`. See [this guide](/oss/javascript/how-to/installation/#installing-integration-packages) for some considerations to take when upgrading `@langchain/core`.
...
You can check out the more in depth guide for [stream events](/oss/javascript/how-to/streaming/#using-stream-events) for more ways to parse and receive intermediate steps from your chains.
