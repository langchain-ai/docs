...
[`TypeSafeClassifier`](https://github.com/langchain-ai/langchainjs/tree/main/libs/providers/langchain-typesafe) exposes these decisions as a LangChain `Runnable`, so you can invoke, batch, or compose them with other runnables. It accepts strings, structured JSON, and [LangChain message objects](/oss/javascript/langchain/messages). Use it for focused decisions such as routing a request, choosing a model, or checking whether a tool call is safe to run.
...
Pass standard runnable configuration, such as `signal`, `tags`, and `metadata`, as the second argument to `invoke`. Questions and the model are constructor settings, not per-call overrides.
