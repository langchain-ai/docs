...
`ChatAnthropic` sends leading system messages in the top-level `system` field, and every later [`SystemMessage`](https://reference.langchain.com/javascript/langchain-core/messages/SystemMessage) at its own position in the conversation. Place a later `SystemMessage` after a human or tool message, either last or followed by an AI message.
