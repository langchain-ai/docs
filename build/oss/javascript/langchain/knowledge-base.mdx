...
Build a semantic search engine over a PDF with LangChain [embeddings](/oss/javascript/integrations/embeddings) and [vector stores](/oss/javascript/integrations/vectorstores). Use it to retrieve passages similar to a query, then plug the retriever into [retrieval-augmented generation (RAG)](/oss/javascript/langchain/retrieval) or other LLM workflows.
...
- [Retrieval overview](/oss/javascript/langchain/retrieval)
