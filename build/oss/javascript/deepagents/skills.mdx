...
import SkillsComposeSourcesPy from '/snippets/javascript/code-samples/skills-compose-sources-py.mdx';
import SkillsComposeSourcesJs from '/snippets/javascript/code-samples/skills-compose-sources-js.mdx';
...
import SkillsWritablePy from '/snippets/javascript/code-samples/skills-writable-py.mdx';
import SkillsWritableJs from '/snippets/javascript/code-samples/skills-writable-js.mdx';
import SkillsReloadInvokePy from '/snippets/javascript/code-samples/skills-reload-invoke-py.mdx';
import SkillsReloadInvokeJs from '/snippets/javascript/code-samples/skills-reload-invoke-js.mdx';
import SkillsReloadUpdateStatePy from '/snippets/javascript/code-samples/skills-reload-update-state-py.mdx';
import SkillsReloadUpdateStateJs from '/snippets/javascript/code-samples/skills-reload-update-state-js.mdx';
import SkillsReloadMiddlewarePy from '/snippets/javascript/code-samples/skills-reload-middleware-py.mdx';
import SkillsReloadMiddlewareJs from '/snippets/javascript/code-samples/skills-reload-middleware-js.mdx';
...
<Note>
Local runs often have no checkpointer, so skill edits take effect immediately and skills appear to reload on their own. [LangSmith Deployments](/langsmith/deployment) configure a persistent checkpointer automatically, so the same agent holds its skills for the life of a thread once deployed. See [Going to production](/oss/javascript/deepagents/going-to-production).
</Note>
...
Skill availability controls which skill files the agent can see. Skill permissions control whether the agent can read those files, write to them, or pause for human approval before writing. Use [filesystem permissions](/oss/javascript/deepagents/permissions) for path-based read and write rules, and use [`interrupt_on`](/oss/javascript/deepagents/human-in-the-loop) or permission rules with `mode="interrupt"` when writes need approval.
...
3. Add `deny` rules for any paths that should stay read-only. Place more specific rules before broader deny rules if paths overlap ([rule ordering](/oss/javascript/deepagents/permissions#rule-ordering)).
