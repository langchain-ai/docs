...
Deep Agents runs in two modes, and your tools, middleware, and subagents carry over unchanged between them:

- **Managed:** create, run, and operate deep agents with [Managed Deep Agents](/langsmith/javascript/managed-deep-agents-overview) in LangSmith. The managed runtime owns the backend, store, and checkpointer, so you declare the agent for the runtime instead of compiling it yourself, and the system prompt, skills, and memory move into project files. See [Relationship to Deep Agents](/langsmith/javascript/managed-deep-agents-overview#relationship-to-deep-agents).
- **Self-hosted:** run [`langgraph build`](/langsmith/cli#build) to produce a [standalone Docker image](/langsmith/deploy-standalone-server) you can deploy anywhere, with no changes to the agent.
...
- **Choose Deep Agents** if you want model and infrastructure flexibility, built-in multi-tenant deployment, and the option to run managed or self-hosted with the same agent code.
