...
For a side-by-side decision guide, see [Choosing between Graph and Functional APIs](/oss/python/langgraph/choosing-apis). Both APIs share the same underlying runtime, so you can use them together in the same application.

High-level differences:

* **Control flow**: The Functional API uses standard language constructs. The [Graph API](/oss/python/langgraph/graph-api) uses nodes and edges.
* **State**: The Graph API requires declaring [state](/oss/python/langgraph/graph-api#state) and often [reducers](/oss/python/langgraph/graph-api#reducers). Entrypoints and tasks keep state scoped to the function.
* **Checkpointing**: The Graph API checkpoints after each [super-step](/oss/python/langgraph/graph-api). The Functional API saves task results to the entrypoint checkpoint.
