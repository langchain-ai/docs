...
Build a semantic search engine over a PDF with LangChain [embeddings](/oss/python/integrations/embeddings) and [vector stores](/oss/python/integrations/vectorstores). Use it to retrieve passages similar to a query, then plug the retriever into [retrieval-augmented generation (RAG)](/oss/python/langchain/retrieval) or other LLM workflows.
...
You can use retrievers in more complex apps such as [retrieval-augmented generation (RAG)](/oss/python/langchain/retrieval), which combine a question with retrieved context in a prompt for an LLM. To learn more about building such an application, check out the [RAG tutorial](/oss/python/deepagents/rag) tutorial.
...
- [Retrieval overview](/oss/python/langchain/retrieval)
