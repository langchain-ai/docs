...
MCP servers can attach standard `ToolAnnotations` when advertising a tool. Deep Agents Code lets a tool bypass classifier review in [Auto approval mode](/oss/python/deepagents/code/approval-modes) only when all of the following are true:
