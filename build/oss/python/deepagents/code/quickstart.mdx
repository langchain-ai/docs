...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/plugins`: Manage [plugins and marketplaces](/oss/python/deepagents/code/plugins).
        - `/notifications`: Configure startup warning preferences.
        - `/reload`: Re-read `.env` files, refresh configuration, and re-discover skills without restarting. This also reloads plugin skills and MCP configuration. Conversation state is preserved. See [`DEEPAGENTS_CODE_` prefix](/oss/python/deepagents/code/configuration#deepagents_code_-prefix) for override behavior.
    ...
    <Accordion title="Keyboard shortcuts" icon="keyboard">
        **General**
        ...
        | `Shift+Tab` or `Ctrl+T` | Toggle between Manual and Auto [approval mode](/oss/python/deepagents/code/approval-modes) |
