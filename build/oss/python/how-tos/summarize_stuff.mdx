...
Let's first load a [chat model](/oss/python/concepts/chat_models/):
...
Next, we need some documents to summarize. Below, we generate some toy documents for illustrative purposes. See the document loader [how-to guides](/oss/python/how-to/#document-loaders) and [integration pages](/oss/python/integrations/document_loaders/) for additional sources of data. The [summarization tutorial](/oss/python/tutorials/summarization) also includes an example summarizing a blog post.
...
Because the chain is a [Runnable](/oss/python/concepts/runnables), it implements the usual methods for invocation:
...
See the summarization [how-to guides](/oss/python/how-to/#summarization) for additional summarization strategies, including those designed for larger volumes of text.

See also [this tutorial](/oss/python/tutorials/summarization) for more detail on summarization.
