...
A vector store retriever is a [retriever](/oss/python/concepts/retrievers/) that uses a [vector store](/oss/python/concepts/vectorstores/) to retrieve documents. It is a lightweight wrapper around the vector store class to make it conform to the retriever [interface](/oss/python/concepts/runnables/).
