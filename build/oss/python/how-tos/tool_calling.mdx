...
- [Chat models](/oss/python/concepts/chat_models)
- [Tool calling](/oss/python/concepts/tool_calling)
- [Tools](/oss/python/concepts/tools)
- [Output parsers](/oss/python/concepts/output_parsers)
</Info>

[Tool calling](/oss/python/concepts/tool_calling) allows a chat model to respond to a given prompt by "calling a tool".

Remember, while the name "tool calling" implies that the model is directly performing some action, this is actually not the case! The model only generates the arguments to a tool, and actually running the tool (or not) is up to the user.

Tool calling is a general technique that generates structured output from a model, and you can use it even when you don't intend to invoke any tools. An example use-case of that is [extraction from unstructured text](/oss/python/tutorials/extraction/).

![Diagram of calling a tool](/img/tool_call.png)

If you want to see how to use the model-generated tool call to actually run a tool [check out this guide](/oss/python/how-to/tool_results_pass_to_model/).

<Note>
**Supported models**


Tool calling is not universal, but is supported by many popular LLM providers. You can find a [list of all models that support tool calling here](/oss/python/integrations/chat/).
...
LangChain also implements a `@tool` decorator that allows for further control of the tool schema, such as tool names and argument descriptions. See the how-to guide [here](/oss/python/how-to/custom_tools/#creating-tools-from-functions) for details.
...
As we can see our LLM generated arguments to a tool! You can look at the docs for [bind_tools()](https://python.langchain.com/api_reference/openai/chat_models/langchain_openai.chat_models.base.BaseChatOpenAI.html#langchain_openai.chat_models.base.BaseChatOpenAI.bind_tools) to learn about all the ways to customize how your LLM selects tools, as well as [this guide on how to force the LLM to call a tool](/oss/python/how-to/tool_choice/) rather than letting it decide.
...
- Pass [tool results back to model](/oss/python/how-to/tool_results_pass_to_model)

You can also check out some more specific uses of tool calling:

- Getting [structured outputs](/oss/python/how-to/structured_output/) from models
- Few shot prompting [with tools](/oss/python/how-to/tools_few_shot/)
- Stream [tool calls](/oss/python/how-to/tool_streaming/)
- Pass [runtime values to tools](/oss/python/how-to/tool_runtime)
