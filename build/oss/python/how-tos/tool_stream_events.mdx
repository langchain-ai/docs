...
- [Custom tools](/oss/python/how-to/custom_tools)
- [Using stream events](/oss/python/how-to/streaming/#using-stream-events)
- [Accessing RunnableConfig within a custom tool](/oss/python/how-to/tool_configure/)

</Info>

If you have [tools](/oss/python/concepts/tools/) that call [chat models](/oss/python/concepts/chat_models/), [retrievers](/oss/python/concepts/retrievers/), or other [runnables](/oss/python/concepts/runnables/), you may want to access [internal events](https://python.langchain.com/docs/how_to/streaming/#event-reference) from those runnables or configure them with additional properties. This guide shows you how to manually pass parameters properly so that you can do this using the `astream_events()` method.
...
But if you wanted to access the raw output from the chat model rather than the full tool, you might try to use the [`astream_events()`](/oss/python/how-to/streaming/#using-stream-events) method and look for an `on_chat_model_end` event. Here's what happens:
...
- Pass [tool results back to a model](/oss/python/how-to/tool_results_pass_to_model)
- [Dispatch custom callback events](/oss/python/how-to/callbacks_custom_events)
