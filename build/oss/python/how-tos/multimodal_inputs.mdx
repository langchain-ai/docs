...
Here we demonstrate how to pass [multimodal](/oss/python/concepts/multimodality/) input directly to models.
...
Below, we demonstrate the cross-provider standard. See [chat model integrations](/oss/python/integrations/chat/) for detail
...
Some providers (including [OpenAI](/oss/python/integrations/chat/openai/),
[Anthropic](/oss/python/integrations/chat/anthropic/), and
[Google Gemini](/oss/python/integrations/chat/google_generative_ai/)) will also accept images from URLs directly.
...
Some providers (including [OpenAI](/oss/python/integrations/chat/openai/),
[Anthropic](/oss/python/integrations/chat/anthropic/), and
[Google Gemini](/oss/python/integrations/chat/google_generative_ai/)) will accept PDF documents.
...
Some providers (specifically [Anthropic](/oss/python/integrations/chat/anthropic/))
...
Some providers (including [OpenAI](/oss/python/integrations/chat/openai/) and
[Google Gemini](/oss/python/integrations/chat/google_generative_ai/)) will accept audio inputs.
...
For example, Anthropic lets you specify [caching](/oss/python/integrations/chat/anthropic/#prompt-caching) of
specific content to reduce token consumption.

To use these fields, you can:

1. Store them on directly on the content block; or
2. Use the native format supported by each provider (see [chat model integrations](/oss/python/integrations/chat/) for detail).
...
Some multimodal models support [tool calling](/oss/python/concepts/tool_calling) features as well. To call tools using such models, simply bind tools to them in the [usual way](/oss/python/how-to/tool_calling), and invoke the model using content blocks of the desired type (e.g., containing image data).
