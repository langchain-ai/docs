...
Some providers have [chat model](/oss/python/concepts/chat_models) wrappers that takes care of formatting your input prompt for the specific local model you're using. However, if you are prompting local models with a [text-in/text-out LLM](/oss/python/concepts/text_llms) wrapper, you may need to use a prompt tailored for your specific model.
...
See the [`llama.cpp`](/oss/python/integrations/llms/llamacpp) setup [here](https://github.com/abetlen/llama-cpp-python/blob/main/docs/install/macos.md) to enable this.
...
We can use model weights downloaded from [GPT4All](/oss/python/integrations/llms/gpt4all) model explorer.
...
For example, you can implement a [RAG application](/oss/python/tutorials/rag) using the chat models demonstrated here.
