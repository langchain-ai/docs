...
for example, if you want to [extract output matching some schema](/oss/python/tutorials/extraction) 
...
response. LangChain includes a suite of [built-in tools](/oss/python/integrations/tools/) 
and supports several methods for defining your own [custom tools](/oss/python/how-to/custom_tools). 
...
[]
```
Note that adding message chunks will merge their corresponding tool call chunks. This is the principle by which LangChain's various [tool output parsers](/oss/python/how-to/output_parser_structured) support streaming.
...
- **Output parsing**: See [OpenAI Tools output
    ...
    various formats.
- **Structured output chains**: [Some models have constructors](/oss/python/how-to/structured_output) that
