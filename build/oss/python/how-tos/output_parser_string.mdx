...
LangChain [message](/oss/python/concepts/messages/) objects support content in a [variety of formats](/oss/python/concepts/messages/#content), including text, [multimodal data](/oss/python/concepts/multimodality/), and a list of [content block](/oss/python/concepts/messages/#aimessage) dicts.

The format of [Chat model](/oss/python/concepts/chat_models/) response content may depend on the provider. For example, the chat model for [Anthropic](/oss/python/integrations/chat/anthropic/) will return string content for typical string input:
