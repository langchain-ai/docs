...
This guide demonstrates how to configure runtime properties of a retrieval chain. An example application is to limit the documents available to a [retriever](/oss/python/concepts/retrievers/) based on the user.

When building a [retrieval app](/oss/python/concepts/rag/), you often have to build it with multiple users in mind. This means that you may be storing data not just for one user, but for many different users, and they should not be able to see eachother's data. This means that you need to be able to configure your retrieval chain to only retrieve certain information. This generally involves two steps.
...
This will let you easily call the chain and configure any relevant flags at runtime. See [this documentation](/oss/python/how-to/configure) for more information on configuration.
...
This will follow the basic implementation from the [RAG tutorial](/oss/python/tutorials/rag), but we will allow the retrieval step to be configurable.
...
For details operating your specific vector store, see the [integration pages](/oss/python/integrations/vectorstores/).
