...
- Using [language models](/oss/python/concepts/chat_models), in particular their tool calling ability
- Creating a [Retriever](/oss/python/concepts/retrievers) to expose specific information to our agent
- Using a Search [Tool](/oss/python/concepts/tools) to look up things online
- [`Chat History`](/oss/python/concepts/chat_history), which allows a chatbot to "remember" past interactions and take them into account when responding to follow-up questions. 
...
For more details, see our [Installation guide](/oss/python/how-to/installation).
...
We first need to create the tools we want to use. We will use two tools: [Tavily](/oss/python/integrations/tools/tavily_search) (to search online) and then a retriever over a local index we will create

### [Tavily](/oss/python/integrations/tools/tavily_search)
...
We will also create a retriever over some data of our own. For a deeper explanation of each step here, see [this tutorial](/oss/python/tutorials/rag).
...
Now that we have defined the tools and the LLM, we can create the agent. We will be using a tool calling agent - for more information on this type of agent, as well as other options, see [this guide](/oss/python/concepts/agents/).
...
Now, we can initialize the agent with the LLM, the prompt, and the tools. The agent is responsible for taking in input and deciding what actions to take. Crucially, the Agent does not execute those actions - that is done by the AgentExecutor (next step). For more information about how to think about these components, see our [conceptual guide](/oss/python/concepts/agents).
...
If we want to keep track of these messages automatically, we can wrap this in a RunnableWithMessageHistory. For more information on how to use this, see [this guide](/oss/python/how-to/message_history). 
...
This section covered building with LangChain Agents. They are fine for getting started, but past a certain point you will likely want flexibility and control which they do not offer. To develop more advanced agents, we recommend checking out [LangGraph](/oss/python/concepts/architecture/#langgraph)
:::

If you want to continue using LangChain agents, some good advanced guides are:

- [How to use LangGraph's built-in versions of `AgentExecutor`](/oss/python/how-to/migrate_agent)
