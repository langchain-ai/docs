...
This retriever lives in the `langchain-elasticsearch` package. For demonstration purposes, we will also install `langchain-community` to generate text [embeddings](/oss/python/concepts/embedding_models).
...
We can also incorporate retrievers into [chains](/oss/python/how-to/sequence/) to build larger applications, such as a simple [RAG](/oss/python/tutorials/rag/) application. For demonstration purposes, we instantiate an OpenAI chat model as well.
