...
`ChatAnthropicTools` also implements the [`with_structured_output` spec](/oss/python/how-to/structured_output) for extracting values. Note: this may not be as stable as with models that explicitly offer tool calling.
