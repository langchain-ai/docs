...
There are several strategies that models can use under the hood. For some of the most popular model providers, including [Anthropic](/oss/python/integrations/platforms/anthropic/), [Google VertexAI](/oss/python/integrations/platforms/google/), [Mistral](/oss/python/integrations/chat/mistral/), and [OpenAI](/oss/python/integrations/platforms/openai/) LangChain implements a common interface that abstracts away these strategies called `.withStructuredOutput`.
...
You can also use tool calling directly to allow the model to choose between options, if your chosen model supports it. This involves a bit more parsing and setup. See [this how-to guide](/oss/python/how-to/tool_calling/) for more details.
