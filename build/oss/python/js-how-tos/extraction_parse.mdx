...
- [Extraction](/oss/python/tutorials/extraction)
...
Here, we'll use Claude which is great at following instructions! See [here for more about Anthropic models](/oss/python/integrations/chat/anthropic).
...
The following example uses the built-in [`StructuredOutputParser`](/oss/python/how-to/output_parser_structured/) to parse the output of a chat model. We use the built-in prompt formatting instructions contained in the parser.
...
Next, check out some of the other guides in this section, such as [some tips on how to improve extraction quality with examples](/oss/python/how-to/extraction_examples).
