...
Language models output text. But there are times where you want to get more structured information than just text back. While some model providers support [built-in ways to return structured output](/oss/python/how-to/structured_output), not all do. For these providers, you must use prompting to encourage the model to return structured data in the desired format.

LangChain has [output parsers](/oss/python/concepts/output_parsers) which can help parse model outputs into usable objects. We'll go over a few examples below.
...
```
Output parsers implement the [Runnable interface](/oss/python/how-to/#langchain-expression-language-lcel), the basic building block of [LangChain Expression Language (LCEL)](/oss/python/how-to/#langchain-expression-language-lcel). This means they support `invoke`, `stream`, `batch`, `streamLog` calls.
...
Next, check out the [guide on tool calling](/oss/python/how-to/tool_calling), a more built-in way of obtaining structured output that some model providers support, or read more about output parsers for other types of structured data like [XML](/oss/python/how-to/output_parser_xml).
