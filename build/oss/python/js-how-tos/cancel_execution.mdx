...
- [LangChain Expression Language](/oss/python/concepts/lcel)
- [Chains](/oss/python/how-to/sequence/)
- [Streaming](/oss/python/how-to/streaming/)
...
[LangChain Expression Language (LCEL)](/oss/python/concepts/lcel) supports aborting runnables that are in-progress via a runtime [signal](https://developer.mozilla.org/en-US/docs/Web/API/AbortController/signal) option.
...
Built-in signal support requires `@langchain/core>=0.2.20`. Please see here for a [guide on upgrading](/oss/python/how-to/installation/#installing-integration-packages).
...
To see how this works, construct a chain such as the one below that performs [retrieval-augmented generation](/oss/python/tutorials/rag). It answers questions by first searching the web using [Tavily](/oss/python/integrations/retrievers/tavily), then passing the results to a chat model to generate a final answer:
...
- [Pass through arguments from one step to the next](/oss/python/how-to/passthrough)
- [Dispatching custom events](/oss/python/how-to/callbacks_custom_events)
