...
- [`Chat History`](/oss/python/concepts/chat_history), which allows a chatbot to "remember" past interactions and take them into account when responding to followup questions. 
- Debugging and tracing your application using [LangSmith](/oss/python/concepts/#langsmith)
...
```
Now, we can initalize the agent with the LLM, the prompt, and the tools. The agent is responsible for taking in input and deciding what actions to take. Crucially, the Agent does not execute those actions - that is done by the AgentExecutor (next step). For more information about how to think about these components, see our [conceptual guide](/oss/python/concepts/agents).
...
For more information on how to use this, see [this guide](/oss/python/how-to/message_history). 
...
You can also see [this guide to help migrate to LangGraph](/oss/python/how-to/migrate_agent).
:::
