...
To illustrate this, we'll create a [custom callback handler](/oss/python/how-to/custom_callbacks) that takes some time to resolve, and show the timing with and without `LANGCHAIN_CALLBACKS_BACKGROUND` set to `"false"`. Here it is without the variable set along with the `awaitAllCallbacks` global:
...
Next, check out the other how-to guides in this section, such as [how to create custom callback handlers](/oss/python/how-to/custom_callbacks).
