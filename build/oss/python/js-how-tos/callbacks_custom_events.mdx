...
- [Callbacks](/oss/python/concepts/callbacks)
- [Custom callback handlers](/oss/python/how-to/custom_callbacks)
- [Stream Events API](/oss/python/concepts/streaming#streamevents)

</Info>

In some situations, you may want to dipsatch a custom callback event from within a [Runnable](/oss/python/concepts/#runnable-interface) so it can be surfaced
in a custom callback handler or via the [Stream Events API](/oss/python/concepts/streaming#streamevents).
...
The most useful way to consume custom events is via the [`.streamEvents()`](/oss/python/concepts/streaming#streamevents) method.
...
Dispatching custom callback events requires `@langchain/core>=0.2.16`. See [this guide](/oss/python/how-to/installation/#installing-integration-packages) for some considerations to take when upgrading `@langchain/core`.
...
You can check out the more in depth guide for [stream events](/oss/python/how-to/streaming/#using-stream-events) for more ways to parse and receive intermediate steps from your chains.
