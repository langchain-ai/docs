...
title: Use subgraphs
---
This guide explains the mechanics of using [subgraphs](./subgraphs). A common application of subgraphs is to build [multi-agent](./multi_agent) systems.

When adding subgraphs, you need to define how the parent graph and the subgraph communicate:

* [Shared state schemas](#shared-state-schemas) — parent and subgraph have **shared state keys** in their state [schemas](./low_level#state)
* [Different state schemas](#different-state-schemas) — **no shared state keys** in parent and subgraph [schemas](./low_level#state)
...
A common case is for the parent graph and subgraph to communicate over a shared state key (channel) in the [schema](./low_level#state). For example, in [multi-agent](./multi_agent) systems, the agents often communicate over a shared [messages](/oss/low-level#why-use-messages) key.
...
For more complex systems you might want to define subgraphs that have a **completely different schema** from the parent graph (no shared keys). For example, you might want to keep a private message history for each of the agents in a [multi-agent](./multi_agent) system.
...
If you want the subgraph to **have its own memory**, you can compile it `with checkpointer=True`. This is useful in [multi-agent](./multi_agent) systems, if you want agents to keep track of their internal message histories:
...
When you enable [persistence](./persistence), you can [inspect the graph state](./persistence#checkpoints) (checkpoint) via `graph.get_state(config)`. To view the subgraph state, you can use `graph.get_state(config, subgraphs=True)`.
