...
- Stop propagating run configurable values to the thread configuration, as this can cause issues on subsequent runs if you are specifying a checkpoint_id. This is a **slight breaking change** in behavior, since the thread value now will no longer automaticlaly reflect the unioned configuration of the most recent run. We believe this behavior is more intuitive, however.
