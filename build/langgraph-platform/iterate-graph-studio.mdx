...
Define your [configuration](/oss/python/use-graph-api#add-runtime-configuration) to specify prompt fields and their associated nodes using `langgraph_nodes` and `langgraph_type` keys.
...
The [LangSmith Playground](/langsmith/manage-prompts) interface allows testing individual LLM calls without running the full graph:
